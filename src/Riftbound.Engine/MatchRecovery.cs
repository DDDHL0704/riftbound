...
public static class MatchRecoveryValidator
{
        ...
        List<string> errors)
    {
        ...
        if (view.Snapshot.Timing is not null)
        {
            ...
            ValidateSnapshotTimingObjectPayloadPropertyNames(
                ...
                "battle",
                errors);
            ValidateSnapshotTimingBattlePayloadValues(view, errors);
            ...
            ValidateSnapshotTimingPendingTaskQueuePayloadShape(view, errors);
    ...
    private static void ValidateSnapshotLaneBattlefieldStandbySlotItemPayloadPropertyNames(
        RecoveredPlayerView view,
        List<string> errors)
    {
        if (!TryReadObjectList(view.Snapshot.Lanes, "battlefields", out var items))
    ...
    private static void ValidateSnapshotTimingBattlePayloadValues(
        RecoveredPlayerView view,
        List<string> errors)
    {
        if (view.Snapshot.Timing is null
        ...
        List<string> errors)
    {
        ...
        ValidateSnapshotPayloadRequiredBoolValue(
            ...
            "pending flag",
            errors);
        ValidateBattleDamageAssignmentMapPayloadShapes(
            damageAssignmentPayload,
            payloadLabel,
            errors);
    ...
    private static void ValidateSnapshotTimingPendingTaskQueuePayloadShape(
        RecoveredPlayerView view,
        List<string> errors)
    {
        if (view.Snapshot.Timing is null
        ...
        {
            errors.Add($"snapshot for {view.PlayerId} timing pending task queue payload is required");
        }
    }
    ...
    private static void ValidateSnapshotTimingPendingTaskQueueMetadataPayloadShape(
        RecoveredPlayerView view,
        List<string> errors)
    {
        if (view.Snapshot.Timing is null
        ...
        {
            errors.Add($"snapshot for {view.PlayerId} timing pending task queue metadata payload is required");
        }
    }
        ...
        List<string> errors)
    {
        ...
        ValidateBattlefieldTaskKindSet(
            ...
            errors);
        ValidateSnapshotBattlefieldTaskActiveSpellDuelLifecycleConsistency(
            view,
            taskPayloads,
            errors);
    ...
    private static void ValidateSnapshotBattlefieldTaskActiveSpellDuelLifecycleConsistency(
        RecoveredPlayerView view,
        IReadOnlyList<object?> taskPayloads,
        List<string> errors)
    {
        if (view.Snapshot.Timing is null
            || !TryReadObjectValue(view.Snapshot.Timing, "spellDuel", out var spellDuelPayload)
            || !IsSnapshotPlayerPayloadObject(spellDuelPayload)
            || !TryReadObjectBool(spellDuelPayload, "isActive", out var isActive)
            || !isActive
        ...
        var hasFocusPlayerId = TryReadObjectOptionalString(spellDuelPayload, "focusPlayerId", out var focusPlayerId)
            && !string.IsNullOrWhiteSpace(focusPlayerId);
        if (hasFocusPlayerId)
        {
            focusPlayerId = focusPlayerId.Trim();
        }

        var hasStackItemIds = TryReadObjectStringList(spellDuelPayload, "stackItemIds", out var spellDuelStackItemIds);
        ...
        foreach (var taskPayload in taskPayloads)
        {
            if (!IsSnapshotPlayerPayloadObject(taskPayload)
                || !TryReadObjectString(taskPayload, "kind", out var kind)
                || string.IsNullOrWhiteSpace(kind)
                || !string.Equals(kind.Trim(), "START_SPELL_DUEL", StringComparison.Ordinal)
                || !TryReadObjectString(taskPayload, "battlefieldObjectId", out var battlefieldObjectId)
                || string.IsNullOrWhiteSpace(battlefieldObjectId)
            ...
            if (hasStackItemIds
                && TryReadObjectStringList(taskPayload, "stackItemIds", out var stackItemIds)
        ...
        List<string> errors)
    {
        ...
        foreach (var taskPayload in taskPayloads)
        {
            ...
            ValidateBattlefieldTaskParticipantObjectBattlefieldMembership(
                ...
                objectLocations,
                errors);
            ValidateBattlefieldTaskParticipantObjectUnitMembership(
                taskPayload,
                $"snapshot for {view.PlayerId} timing {payloadLabel} participant object id",
                objectTags,
                errors);
        ...
        List<string> errors)
    {
        ...
        if (TryReadObjectList(view.Snapshot.Timing, "battlefieldResolutions", out var battlefieldResolutionPayloads))
        {
            ...
            foreach (var resolutionPayload in battlefieldResolutionPayloads)
            {
                ...
                var resolutionId = ValidateBattlefieldResolutionPayloadValues(
                    ...
                    resolutionLabel,
                    errors);
                ValidateBattlefieldResolutionObjectReferences(
                    resolutionPayload,
                    resolutionLabel,
                    knownObjectIds,
                    "objects",
        ...
        foreach (var resolutionPayload in battleResolutionPayloads)
        {
            ...
            var resolutionId = ValidateBattleResolutionPayloadValues(
                ...
                resolutionLabel,
                errors);
            ValidateBattleResolutionObjectReferences(
                resolutionPayload,
                resolutionLabel,
                knownObjectIds,
                "objects",
        ...
        List<string> errors)
    {
        ...
        if (TryReadObjectList(view.Snapshot.Timing, "battlefieldResolutions", out var battlefieldResolutionPayloads))
        {
            foreach (var resolutionPayload in battlefieldResolutionPayloads)
            {
                ...
                ValidateSnapshotPayloadRequiredStringListPayloadShape(
                    ...
                    "related event kind",
                    errors);
                var relatedEventKinds = ValidateSnapshotPayloadStringListValues(
                    resolutionPayload,
                    "relatedEventKinds",
                    $"snapshot for {view.PlayerId} timing {payloadLabel}",
                    "related event kind",
                ...
                ValidateBattlefieldResolutionRelatedEventKindCompatibility(
                    ReadNormalizedPayloadString(resolutionPayload, "kind"),
                    relatedEventKinds,
                    $"snapshot for {view.PlayerId} timing {payloadLabel}",
                    errors);
        ...
        foreach (var resolutionPayload in battleResolutionPayloads)
        {
            ...
            ValidateSnapshotPayloadRequiredStringListPayloadShape(
                ...
                "attacker object id",
                errors);
            var attackerObjectIds = ValidateSnapshotPayloadStringListValues(
                resolutionPayload,
                "attackerObjectIds",
                $"snapshot for {view.PlayerId} timing {payloadLabel}",
                "attacker object id",
            ...
            ValidateSnapshotPayloadRequiredStringListPayloadShape(
                ...
                errors);
            var survivingAttackerObjectIds = ValidateSnapshotPayloadStringListValues(
                resolutionPayload,
                "survivingAttackerObjectIds",
                $"snapshot for {view.PlayerId} timing {payloadLabel}",
                "surviving attacker object id",
            ...
            ValidateSnapshotPayloadRequiredStringListPayloadShape(
                ...
                errors);
            var survivingDefenderObjectIds = ValidateSnapshotPayloadStringListValues(
                resolutionPayload,
                "survivingDefenderObjectIds",
                $"snapshot for {view.PlayerId} timing {payloadLabel}",
                "surviving defender object id",
            ...
            ValidateBattleResolutionRelatedEventKindCompatibility(
                ReadNormalizedPayloadString(resolutionPayload, "kind"),
                ReadNormalizedPayloadString(resolutionPayload, "reason"),
                relatedEventKinds,
                $"snapshot for {view.PlayerId} timing {payloadLabel}",
    ...
    private static void ValidateBattlefieldResolutionRelatedEventKindCompatibility(
        string? kind,
    ...
    {
        if (kind is null
            || relatedEventKinds is null
            || !IsKnownBattlefieldResolutionKind(kind)
            || relatedEventKinds.Any(relatedEventKind => !IsKnownBattlefieldResolutionRelatedEventKind(relatedEventKind)))
        {
            return;
        }

        var expectedRelatedEventKind = kind switch
        {
            "HELD" => "BATTLEFIELD_HELD",
            "CONQUERED" => "BATTLEFIELD_CONQUERED",
            "CONTROL_RESOLVED" => "BATTLEFIELD_CONTROL_RESOLVED",
            _ => string.Empty
        };
        if (string.IsNullOrEmpty(expectedRelatedEventKind))
        {
            return;
        }

        foreach (var relatedEventKind in relatedEventKinds)
        {
            if (!string.Equals(relatedEventKind, expectedRelatedEventKind, StringComparison.Ordinal))
            {
                errors.Add($"{payloadLabel} related event kind {relatedEventKind} is invalid for kind {kind}");
            }
        }

        if (!relatedEventKinds.Contains(expectedRelatedEventKind, StringComparer.Ordinal))
        {
            errors.Add($"{payloadLabel} related event kinds must include {expectedRelatedEventKind} for kind {kind}");
        }
    }
    ...
    private static void ValidateBattlefieldResolutionControlPreviousControllerCompatibility(
        string? kind,
        string? reason,
        string? previousControllerId,
        string? controllerId,
        ...
        List<string> errors)
    {
        ...
        var normalizedPreviousControllerId = string.IsNullOrWhiteSpace(previousControllerId)
            ? null
            : previousControllerId.Trim();
        var normalizedControllerId = string.IsNullOrWhiteSpace(controllerId) ? null : controllerId.Trim();
        if (string.Equals(reason, "CONTROL_CONFIRMED", StringComparison.Ordinal))
        {
            if (normalizedPreviousControllerId is null)
            {
                errors.Add($"{payloadLabel} {previousControllerLabel} is required for reason {reason}");
            }

            if (normalizedPreviousControllerId is not null
                && normalizedControllerId is not null
                && !string.Equals(normalizedPreviousControllerId, normalizedControllerId, StringComparison.Ordinal))
            {
                errors.Add(
                    $"{payloadLabel} {previousControllerLabel} {normalizedPreviousControllerId} must match {controllerLabel} {normalizedControllerId} for reason {reason}");
            }
        ...
        if (string.Equals(reason, "CONTROL_CHANGED", StringComparison.Ordinal)
            && normalizedPreviousControllerId is not null
            && normalizedControllerId is not null
            && string.Equals(normalizedPreviousControllerId, normalizedControllerId, StringComparison.Ordinal))
        {
            errors.Add(
                $"{payloadLabel} {previousControllerLabel} {normalizedPreviousControllerId} must differ from {controllerLabel} {normalizedControllerId} for reason {reason}");
        }
    }
    ...
    private static void ValidateBattleResolutionRelatedEventKindCompatibility(
        string? kind,
        string? reason,
        IReadOnlyList<string>? relatedEventKinds,
        string payloadLabel,
        List<string> errors)
    {
        if (kind is null
            ...
            || relatedEventKinds.Any(relatedEventKind => !IsKnownBattleResolutionRelatedEventKind(relatedEventKind)))
        {
            return;
        }

        var expectedResultEventKind = string.Equals(kind, "NO_RESULT", StringComparison.Ordinal)
            ? "BATTLE_NO_RESULT"
            : "BATTLE_CLOSED";
        if (!relatedEventKinds.Contains(expectedResultEventKind, StringComparer.Ordinal))
        {
        ...
        {
            errors.Add($"{payloadLabel} related event kind BATTLE_NO_RESULT is invalid for kind {kind} reason {reason}");
        }
    }
        ...
        List<string> errors)
    {
        ...
        foreach (var survivingAttackerObjectId in survivingAttackerObjectIds)
        {
            ...
            {
                errors.Add($"{payloadLabel} surviving attacker object id {survivingAttackerObjectId} is missing from attacker object ids");
            }
        }
    ...
    {
        var objectTags = new Dictionary<string, IReadOnlySet<string>>(StringComparer.Ordinal);
        if (snapshot.Players is null)
        {
            return objectTags;
        }
        ...
        foreach (var playerPayload in snapshot.Players.Values)
        {
            ...
            foreach (var (objectId, objectPayload) in objectPayloads)
            {
                if (string.IsNullOrWhiteSpace(objectId)
                    || !IsSnapshotPlayerPayloadObject(objectPayload)
                    || !TryReadObjectStringList(objectPayload, "tags", out var tags))
                {
                    continue;
                }

                objectTags[objectId.Trim()] = tags
        ...
        List<string> errors)
    {
        ...
        foreach (var (snapshotPlayerId, playerPayload) in view.Snapshot.Players)
        {
            ...
            ValidateSnapshotPayloadObjectPropertyNames(
                ...
                errors);
            ValidateSnapshotPlayerRunePoolPayloadShape(
                view,
                snapshotPlayerId,
                playerPayload,
                errors);
            ...
            ValidateSnapshotPlayerRunePoolPowerTraitPayloadPropertyNames(
                ...
                playerPayload,
                errors);
            ValidateSnapshotPlayerRunePoolPayloadValues(
                view,
                snapshotPlayerId,
                playerPayload,
                errors);
            ValidateSnapshotPlayerZonesPayloadShape(
                view,
                snapshotPlayerId,
                playerPayload,
                errors);
            ValidateSnapshotPlayerZonesPayloadPropertyNames(
                ...
                playerPayload,
                errors);
            ValidateSnapshotPlayerZonesPayloadValues(
                view,
                snapshotPlayerId,
                playerPayload,
                errors);
            ValidateSnapshotPlayerObjectsPayloadShape(
                view,
                snapshotPlayerId,
                playerPayload,
                errors);
    ...
    private static void ValidateSnapshotPlayerRunePoolPayloadShape(
        RecoveredPlayerView view,
        ...
        List<string> errors)
    {
        ...
        {
            errors.Add($"snapshot for {view.PlayerId} player {snapshotPlayerId} rune pool payload is required");
        }
    }
    ...
    private static void ValidateSnapshotPlayerRunePoolPayloadValues(
        RecoveredPlayerView view,
        string snapshotPlayerId,
        object? playerPayload,
        List<string> errors)
    {
        ...
        var payloadLabel = $"snapshot for {view.PlayerId} player {snapshotPlayerId} rune pool";
    ...
    private static void ValidateSnapshotPlayerZonesPayloadShape(
        RecoveredPlayerView view,
        string snapshotPlayerId,
        object? playerPayload,
        List<string> errors)
    {
        if (!TryReadObjectValue(playerPayload, "zones", out var zonesPayload)
            || IsNullSnapshotPayloadValue(zonesPayload))
        {
            return;
        }

        if (!IsSnapshotPlayerPayloadObject(zonesPayload))
        {
            errors.Add($"snapshot for {view.PlayerId} player {snapshotPlayerId} zones payload is required");
        }
    }
    ...
    private static void ValidateSnapshotPlayerZonesPayloadValues(
        RecoveredPlayerView view,
        string snapshotPlayerId,
        object? playerPayload,
        List<string> errors)
    {
        ...
        var payloadLabel = $"snapshot for {view.PlayerId} player {snapshotPlayerId} zones";
        ValidateSnapshotPayloadRequiredNonNegativeIntValue(
            zonesPayload,
            "mainDeckCount",
            payloadLabel,
    ...
    private static void ValidateSnapshotPlayerObjectsPayloadShape(
        RecoveredPlayerView view,
        string snapshotPlayerId,
        object? playerPayload,
        List<string> errors)
    {
        ...
        {
            errors.Add($"snapshot for {view.PlayerId} player {snapshotPlayerId} objects payload is required");
        }
    }
    ...
    private static void ValidateSnapshotPlayerObjectLocationPayloadPropertyNames(
        RecoveredPlayerView view,
        string snapshotPlayerId,
        object? playerPayload,
        List<string> errors)
    ...
    private static IReadOnlyList<string>? ValidateSnapshotPayloadStringListValues(
        object? payload,
        string key,
        string payloadLabel,
        string itemLabel,
    ...
    private static IReadOnlyList<string>? ValidateSnapshotPayloadRequiredStringListValues(
        object? payload,
        string key,
        string payloadLabel,
        string itemLabel,
    ...
    private static IReadOnlyList<string> ValidateSnapshotStringListValues(
    ...
    private static string? ReadNormalizedPayloadString(object? payload, string key)
    {
        return TryReadObjectString(payload, key, out var value) && !string.IsNullOrWhiteSpace(value)
            ? value.Trim()
            : null;
    }
        ...
        List<string> errors)
    {
        ...
        foreach (var (playerId, playerPayload) in snapshot.Players)
        {
            ...
            ValidateSpectatorSnapshotPlayerScalarValues(playerId, playerPayload, zones, authoritativeState, errors);
    ...
    private static void ValidateSpectatorSnapshotPlayerScalarValues(
        string playerId,
        object? playerPayload,
        PlayerZones zones,
        MatchState authoritativeState,
        List<string> errors)
    {
        var payloadLabel = $"spectator replay frame snapshot player {playerId}";
        var payloadId = ValidateSnapshotPayloadRequiredStringValue(
            playerPayload,
            "id",
            payloadLabel,
            "payload id",
        ...
        var expectedScore = authoritativeState.PlayerScores.TryGetValue(playerId, out var score)
            ? score
            : 0;
        var payloadScore = ValidateSnapshotPayloadRequiredNonNegativeIntValue(
            playerPayload,
            "score",
            payloadLabel,
            "score",
        ...
        var expectedExperience = authoritativeState.PlayerExperience.TryGetValue(playerId, out var experience)
            ? experience
            : 0;
        var payloadExperience = ValidateSnapshotPayloadRequiredNonNegativeIntValue(
            playerPayload,
            "experience",
            payloadLabel,
            "experience",
        ...
        var expectedCardsPlayedThisTurn = authoritativeState.PlayerCardsPlayedThisTurn.TryGetValue(
            ...
            ? cardsPlayedThisTurn
            : 0;
        var payloadCardsPlayedThisTurn = ValidateSnapshotPayloadRequiredNonNegativeIntValue(
            playerPayload,
            "cardsPlayedThisTurn",
            payloadLabel,
            "cards played this turn",
        ...
        List<string> errors)
    {
        ...
        ValidateSpectatorSnapshotExtraPlayerObjectPayloadPropertyNames(
            ...
            expectedObjectIdSet,
            errors);
        ValidateSpectatorSnapshotExtraPlayerObjectPayloadValues(
            playerId,
            objectPayloads,
            expectedObjectIdSet,
            errors);
    ...
    private static void ValidateSpectatorSnapshotExtraPlayerObjectPayloadValues(
        string playerId,
        IReadOnlyDictionary<string, object?> objectPayloads,
        IReadOnlySet<string> expectedObjectIds,
        List<string> errors)
        ...
        List<string> errors)
    {
        ...
        foreach (var participantObjectId in participantObjectIds)
        {
            ...
            if (!tags.Contains(CardObjectTags.UnitCard))
            {
                errors.Add($"{participantObjectLabel} {normalizedParticipantObjectId} is not a unit card");
            }
        }
    }
        ...
        List<string> errors)
    {
        ...
        ValidateContinuousEffectPowerModifierNonFoundationModifierScalarAbsence(
            ...
            layerEngineStatus,
            errors);
        ValidateContinuousEffectPowerModifierNonFoundationPowerScalarConsistency(
            effectPayload,
            effectLabel,
            scope,
            layer,
    ...
    private static void ValidateContinuousEffectNonStaticAuraDependencyListAbsence(
        object? effectPayload,
        string effectLabel,
        string? layer,
        List<string> errors)
    {
        ...
        ValidateContinuousEffectReadableStringListAbsence(
            effectPayload,
            "sourceDependencyObjectIds",
            $"{effectLabel} {layer} source dependency object id list must be absent",
            errors);
        ValidateContinuousEffectReadableStringListAbsence(
            effectPayload,
            "targetDependencyObjectIds",
            $"{effectLabel} {layer} target dependency object id list must be absent",
            errors);
    }
        ...
        List<string> errors)
    {
        ...
        ValidateContinuousEffectStaticAuraRequiredListPresence(
            ...
            errors);
        ValidateContinuousEffectStaticAuraParticipantObjectListPresence(
            effectPayload,
            effectLabel,
            errors);
        ...
        ValidateContinuousEffectStaticAuraDependencyListMembership(
            ...
            "sourceObjectId",
            "sourceDependencyObjectIds",
            $"{effectLabel} static aura source dependency object id list must include source object id",
            errors);
        ValidateContinuousEffectStaticAuraDependencyListMembership(
            effectPayload,
            "targetObjectId",
    ...
    private static void ValidateContinuousEffectStaticAuraRequiredSourceOrderPresence(
        object? effectPayload,
        string diagnostic,
        List<string> errors)
    {
        if (!TryReadObjectValue(effectPayload, "sourceOrder", out var sourceOrderPayload)
            || IsNullSnapshotPayloadValue(sourceOrderPayload))
        {
            errors.Add(diagnostic);
        }
    }
    ...
    private static void ValidateContinuousEffectStaticAuraDependencyListMembership(
        object? effectPayload,
        string objectIdKey,
        string dependencyListKey,
        string diagnosticPrefix,
        List<string> errors)
    {
        if (!TryReadObjectString(effectPayload, objectIdKey, out var objectId)
            || !TryReadObjectStringList(effectPayload, dependencyListKey, out var dependencyObjectIds)
            || dependencyObjectIds.Count == 0
            || dependencyObjectIds.Contains(objectId, StringComparer.Ordinal))
        {
            return;
        }
    ...
    private static void ValidateContinuousEffectStaticAuraParticipantObjectListPresence(
        object? effectPayload,
        string effectLabel,
        List<string> errors)
    {
        if (!TryReadObjectStringList(effectPayload, "participantDependencyObjectIds", out var participantDependencyObjectIds)
        ...
        {
            errors.Add(
                $"{effectLabel} static aura participant object id list is required when participant dependency object id list is present");
        }
    }
    ...
    private static void ValidateContinuousEffectPowerModifierFoundationModifierScalarCompleteness(
        object? effectPayload,
        string effectLabel,
        string? scope,
        string? layer,
    ...
    private static void ValidateContinuousEffectPowerModifierNonFoundationPowerScalarConsistency(
        object? effectPayload,
        string effectLabel,
        string? scope,
        string? layer,
        ...
        List<string> errors)
    {
        ...
        var requiredEffectivePower = basePower.Value + powerDelta.Value;
        if (effectivePower.Value == requiredEffectivePower)
        {
            return;
        }

        errors.Add(
            $"{effectLabel} POWER_MODIFIER non-foundation effective power {effectivePower.Value} must equal base power {basePower.Value} plus power delta {powerDelta.Value}");
    }
    ...
    private static void ValidateSpectatorTriggerQueueKeyedAuthoritativeValues(
        IReadOnlyList<object?> triggerPayloads,
        IReadOnlyDictionary<string, TriggerQueueItemState> authoritativeTriggersById,
        MatchState authoritativeState,
        List<string> errors)
    {
        foreach (var triggerPayload in triggerPayloads)
        {
            ...
            var normalizedTriggerId = triggerId.Trim();
            if (!authoritativeTriggersById.TryGetValue(normalizedTriggerId, out var authoritativeTrigger))
            {
                continue;
            }
        ...
        List<string> errors)
    {
        foreach (var resourcePayload in resourcePayloads)
        {
            ...
            ValidateSpectatorTemporaryPaymentResourceKeyedRequiredStringValue(
                resourcePayload,
                "ownerPlayerId",
                "owner player id",
                authoritativeResource.OwnerPlayerId,
                normalizedResourceId,
                errors);
            ValidateSpectatorTemporaryPaymentResourceKeyedOptionalStringValue(
                resourcePayload,
                "sourceObjectId",
                "source object id",
                authoritativeResource.SourceObjectId,
                normalizedResourceId,
                errors);
            ValidateSpectatorTemporaryPaymentResourceKeyedOptionalStringValue(
                resourcePayload,
                "abilityId",
                "ability id",
                authoritativeResource.AbilityId,
                normalizedResourceId,
                errors);
            ValidateSpectatorTemporaryPaymentResourceKeyedOptionalStringValue(
                resourcePayload,
                "paymentWindow",
                "payment window",
                authoritativeResource.PaymentWindow,
                normalizedResourceId,
                errors);
            ValidateSpectatorTemporaryPaymentResourceKeyedRequiredIntValue(
                resourcePayload,
                "generatedPower",
                "generated power",
                authoritativeResource.GeneratedPower,
                normalizedResourceId,
                errors);
            ValidateSpectatorTemporaryPaymentResourceKeyedRequiredIntValue(
                resourcePayload,
                "remainingPower",
                "remaining power",
                authoritativeResource.RemainingPower,
                normalizedResourceId,
                errors);
            ValidateSpectatorTemporaryPaymentResourceKeyedRequiredIntDictionaryValue(
                resourcePayload,
                "generatedPowerByTrait",
                "generated power traits",
                authoritativeResource.GeneratedPowerByTrait,
                normalizedResourceId,
                errors);
            ValidateSpectatorTemporaryPaymentResourceKeyedRequiredIntDictionaryValue(
                resourcePayload,
                "remainingPowerByTrait",
                "remaining power traits",
                authoritativeResource.RemainingPowerByTrait,
                normalizedResourceId,
                errors);
            ValidateSpectatorTemporaryPaymentResourceKeyedRequiredStringListValue(
                resourcePayload,
                "allowedPaymentKinds",
                "allowed payment kinds",
                authoritativeResource.AllowedPaymentKinds,
                normalizedResourceId,
                errors);
            ValidateSpectatorTemporaryPaymentResourceKeyedRequiredTrueBoolValue(
                resourcePayload,
                "paymentOnly",
                "payment-only flag",
                normalizedResourceId,
            ...
            ValidateSpectatorTemporaryPaymentResourceKeyedRequiredStringValue(
                ...
                normalizedResourceId,
                errors);
            ValidateSpectatorTemporaryPaymentResourceKeyedRequiredLongValue(
                resourcePayload,
                "createdTick",
                "created tick",
                authoritativeResource.CreatedTick,
                normalizedResourceId,
                errors);
        }
    }

    private static void ValidateSpectatorTemporaryPaymentResourceKeyedRequiredStringValue(
        object? resourcePayload,
        string key,
        string fieldLabel,
        string expected,
    ...
    private static void ValidateSpectatorTemporaryPaymentResourceKeyedOptionalStringValue(
        object? resourcePayload,
        string key,
        string fieldLabel,
        string expected,
    ...
    private static void ValidateSpectatorTemporaryPaymentResourceKeyedRequiredIntValue(
        object? resourcePayload,
        string key,
        string fieldLabel,
        int expected,
    ...
    private static void ValidateSpectatorTemporaryPaymentResourceKeyedRequiredIntDictionaryValue(
        object? resourcePayload,
        string key,
        string fieldLabel,
    ...
    private static void ValidateSpectatorTemporaryPaymentResourceKeyedRequiredStringListValue(
        object? resourcePayload,
        string key,
        string fieldLabel,
        IReadOnlyList<string> expected,
    ...
    private static void ValidateSpectatorTemporaryPaymentResourceKeyedRequiredTrueBoolValue(
        object? resourcePayload,
        string key,
        string fieldLabel,
        string resourceId,
    ...
    private static void ValidateSpectatorTemporaryPaymentResourceKeyedRequiredLongValue(
        object? resourcePayload,
        string key,
        string fieldLabel,
        long expected,
    ...
    private static void AddSpectatorTemporaryPaymentResourceKeyedValueMismatch(
        string fieldLabel,
        string? actual,
        string expected,
        string resourceId,
        ...
        List<string> errors)
    {
            ...
            StringComparer.Ordinal))
        {
            ...
            ValidateAuthoritativeStateResolutionText(
                ...
                errors);
            var attackerObjectIds = ValidateAuthoritativeStateResolutionTextList(
                "battle resolution",
                resolutionId,
                "attacker object",
            ...
            var survivingAttackerObjectIds = ValidateAuthoritativeStateResolutionTextList(
                ...
                errors,
                requireList: false);
            var survivingDefenderObjectIds = ValidateAuthoritativeStateResolutionTextList(
                "battle resolution",
                resolutionId,
    ...
    private static IReadOnlyList<string>? ValidateAuthoritativeStateResolutionTextList(
        string resolutionLabel,
        string resolutionId,
        string valueLabel,
        IReadOnlyList<string>? values,
    ...
    private static IReadOnlyDictionary<string, IReadOnlySet<string>> BuildAuthoritativeStateObjectTagIndex(MatchState authoritativeState)
    {
        var objectTags = new Dictionary<string, IReadOnlySet<string>>(StringComparer.Ordinal);
        foreach (var (objectId, cardObject) in authoritativeState.CardObjects)
        {
            ...
            objectTags[objectId.Trim()] = cardObject.Tags
                .Where(tag => !string.IsNullOrWhiteSpace(tag))
                .Select(tag => tag.Trim())
                .ToHashSet(StringComparer.Ordinal);
        }
        ...
        List<string> errors)
    {
        ...
        ValidateBattlefieldResolutionCombatPlayerAvailability(
            ...
            "player id",
            errors);
        var previousControllerId = ValidateSnapshotPayloadOptionalStringValue(
            resolutionPayload,
            "previousControllerId",
            payloadLabel,
            "previous controller id",
    ...
    private static void ValidateBattlefieldResolutionObjectReferences(
        object? resolutionPayload,
        string payloadLabel,
        IReadOnlySet<string>? knownObjectIds,
        string knownObjectLabel,
    ...
    private static void ValidateBattleResolutionObjectReferences(
        object? resolutionPayload,
        string payloadLabel,
        IReadOnlySet<string>? knownObjectIds,
        string knownObjectLabel,
    ...
    private static void ValidateBattleDamageAssignmentMapPayloadShapes(
        object? damageAssignmentPayload,
        string payloadLabel,
        List<string> errors)
    {
        ValidateBattleDamageAssignmentMapPayloadShape(
            damageAssignmentPayload,
            "damagePool",
            payloadLabel,
            "damage pool",
    ...
    private static void ValidateBattleDamageAssignmentMapPayloadShape(
        object? damageAssignmentPayload,
        string payloadKey,
        string payloadLabel,
        string mapLabel,
        ...
        List<string> errors)
    {
        ...
        {
            errors.Add($"{payloadLabel} {mapLabel} map payload is required");
        }
    }
