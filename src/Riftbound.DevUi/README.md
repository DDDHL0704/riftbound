...
- 桌面首屏在公共战场与玩家公开区之间保持平衡；战场约占有效牌桌高度的 50%，并为传奇、英雄、基地、牌库和符文保留更大的上下边缘区。
- 本玩家手牌位于底部扇形区，静止高度不超过视口的 18%；符文在左下，主牌堆与相关牌堆在右下。
- 两张战场牌位于公共战场左右两侧，双方基地、英雄、传奇、分数与牌堆沿上下边缘排列。
- 直接点击服务器标记为合法的来源、位置和目标；上下文托盘只提交服务器候选命令。按 `Esc` 清除本地选择草稿。
- 起手调度、支付费用、分配战斗伤害、触发排序和手牌选择使用居中复杂提示。
- 连接、日志和规则细节默认收进右侧“连接与规则诊断”抽屉。
- 对手手牌与暗牌只显示卡背及数量，永不使用正面图或身份作为背景。
