...
function CandidateComposer({
  ...
  const [state, setState] = useState<CandidateComposerState>(() => initialComposerState(candidate, model));

  useEffect(() => {
    setState(initialComposerState(candidate, model));
  }, [candidate.action, candidate.label, model.resetKey, candidate]);

  const requirement = selectedRequirement(model, state.sourceId);
  const controls = composerControls(candidate, model, requirement);
...
function canComposeCandidate(candidate: ActionPromptCandidateDto): boolean {
...
function composerControls(
  ...
  const sources = sourceChoicesForCandidate(candidate, model);
...
function sourceChoicesForCandidate(
  ...
  return uniqueChoices([...(candidate.sources ?? []), ...requirementChoices]);
}
