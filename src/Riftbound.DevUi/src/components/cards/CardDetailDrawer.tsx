...
type PlayCardSourceRequirement = {
  ...
  nextSpellCostReductionMana: number;
  battlefieldSpellCostReductionMana: number;
...
type HideCardSourceRequirement = {
  sourceObjectId: string;
  cardNo: string;
  displayName: string;
  destinationChoices: ActionPromptChoiceDto[];
  ...
  return (
    <article className="play-card-composer">
      ...
      <div className="composer-meta">
        ...
        {selectedRequirement.nextSpellCostReductionMana > 0 && (
          <span>下一法术减费 -{selectedRequirement.nextSpellCostReductionMana}</span>
        )}
        {selectedRequirement.battlefieldSpellCostReductionMana > 0 && (
          <span>战场法术减费 -{selectedRequirement.battlefieldSpellCostReductionMana}</span>
        )}
...
function HideCardComposer({
  ...
  const requirements = useMemo(
    () => hideCardRequirementsFor(candidate, sourceObjectId),
    [candidate, sourceObjectId]
  );
  ...
  const selectedCostMana = optionalCost === "STANDBY_FREE" ? 0 : selectedRequirement.manaCost;
  const canSubmit = Boolean(
    candidate.enabled
    && selectedRequirement.composable
    && destination
...
function hideCardRequirementsFor(candidate: ActionPromptCandidateDto, sourceObjectId?: string): HideCardSourceRequirement[] {
  if (!sourceObjectId) {
    return [];
  }
  ...
  return rawRequirements
    .map(parseHideCardRequirement)
    .filter((requirement): requirement is HideCardSourceRequirement =>
      Boolean(requirement && requirement.sourceObjectId === sourceObjectId));
}
...
function parseHideCardRequirement(value: unknown): HideCardSourceRequirement | undefined {
  const record = asRecord(value);
  if (!record) {
    return undefined;
  }
...
function parsePlayCardRequirement(value: unknown): PlayCardSourceRequirement | undefined {
  ...
  return {
    ...
    nextSpellCostReductionMana: numberField(record, "nextSpellCostReductionMana"),
    battlefieldSpellCostReductionMana: numberField(record, "battlefieldSpellCostReductionMana"),
