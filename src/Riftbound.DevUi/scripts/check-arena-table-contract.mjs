...
assert(arena.battlefieldMinHeightRatio >= 0.33, "compact battlefield ratio must be at least 0.33");
assert(arena.battlefieldMinHeightRatio <= 0.35, "battlefield must leave enough room for base and resource zones");
assert(arena.handMaxViewportRatio <= 0.15, "hand ratio must be at most 0.15");
assert.deepEqual(
...
assert.equal(arena.opponent.runes, "topRight", "opponent runes must rotate 180 degrees from the local resource cluster");
assert.equal(arena.opponent.home, "topCenter", "opponent public identity cards must occupy the top center edge");
assert.equal(arena.opponent.mainDeck, "topLeft", "opponent main deck must rotate 180 degrees from the local pile cluster");
