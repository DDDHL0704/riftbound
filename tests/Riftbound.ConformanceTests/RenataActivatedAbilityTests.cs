...
public sealed class RenataActivatedAbilityTests
{
    ...
    [Fact]
    public async Task RenataDrawStalePromptReplayUsesRejectedCacheWithoutMutation()
    {
        var journal = new RecordingMatchJournal();
        var state = BuildRenataState(P4ActivatedAbilityCatalog.RenataGlascCardNo, new RunePool(1, 0, new Dictionary<string, int>(StringComparer.Ordinal)
        {
            [RuneTrait.Blue] = 1
        }));
        ...
        const string acceptedClientIntentId = "intent-renata-draw-before-stale-prompt-replay";
        const string staleClientIntentId = "intent-renata-draw-stale-prompt-replay";
        ...
        Assert.False(accepted.State.CardObjects[RenataObjectId].IsExhausted);
        ...
        Assert.Equal(P4ActivatedAbilityCatalog.RenataGlascDrawAbilityEffectKind, acceptedStackItem.EffectKind);
        Assert.Equal(P4ActivatedAbilityCatalog.RenataGlascCardNo, acceptedStackItem.CardNo);
        ...
        Assert.False(replay.State.CardObjects[RenataObjectId].IsExhausted);
        ...
        Assert.False(duplicateReplay.State.CardObjects[RenataObjectId].IsExhausted);
        ...
        Assert.False(conflict.State.CardObjects[RenataObjectId].IsExhausted);
