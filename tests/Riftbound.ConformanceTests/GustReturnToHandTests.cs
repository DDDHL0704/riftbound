...
{
    private const string GustObjectId = "P1-SPELL-GUST";
    private const string GustCardNo = "OGN·169/298";
    private const string GustTargetObjectId = "P2-BATTLEFIELD-UNIT";
    ...
    [Fact]
    public async Task GustPlayCardStalePromptReplayAfterStackPriorityStartsUsesRejectedCacheWithoutMutation()
    {
        ...
        const string acceptedClientIntentId = "intent-gust-before-stale-prompt-replay";
        const string staleClientIntentId = "intent-gust-stale-prompt-replay";
        ...
        var acceptedStackItem = AssertGustStackPriorityState(accepted);
        ...
        AssertGustStackPriorityState(replay, acceptedStackItem);
        ...
        AssertGustStackPriorityState(reorderedDuplicateRejected, acceptedStackItem);
        Assert.Equal(p1PromptAfterAccepted, MatchStateHasher.HashValue(session.PromptFor("P1")));
        Assert.Equal(p2PromptAfterAccepted, MatchStateHasher.HashValue(session.PromptFor("P2")));
        Assert.Equal(p1SnapshotAfterAccepted, MatchStateHasher.HashValue(session.SnapshotFor("P1")));
        Assert.Equal(p2SnapshotAfterAccepted, MatchStateHasher.HashValue(session.SnapshotFor("P2")));
        ...
        AssertGustStackPriorityState(duplicateRejected, acceptedStackItem);
        Assert.Equal(p1PromptAfterAccepted, MatchStateHasher.HashValue(session.PromptFor("P1")));
        Assert.Equal(p2PromptAfterAccepted, MatchStateHasher.HashValue(session.PromptFor("P2")));
        Assert.Equal(p1SnapshotAfterAccepted, MatchStateHasher.HashValue(session.SnapshotFor("P1")));
        Assert.Equal(p2SnapshotAfterAccepted, MatchStateHasher.HashValue(session.SnapshotFor("P2")));
        ...
        AssertGustStackPriorityState(conflict, acceptedStackItem);
        Assert.Equal(p1PromptAfterAccepted, MatchStateHasher.HashValue(session.PromptFor("P1")));
        Assert.Equal(p2PromptAfterAccepted, MatchStateHasher.HashValue(session.PromptFor("P2")));
        Assert.Equal(p1SnapshotAfterAccepted, MatchStateHasher.HashValue(session.SnapshotFor("P1")));
        Assert.Equal(p2SnapshotAfterAccepted, MatchStateHasher.HashValue(session.SnapshotFor("P2")));
        ...
        ActionPromptDto prompt)
    {
        ...
        Assert.Equal(GustObjectId, rawCommand.GetProperty("cardObjectId").GetString());
        Assert.Equal(GustCardNo, rawCommand.GetProperty("cardNo").GetString());
        Assert.Equal(
            [GustTargetObjectId],
    ...
    private static StackItemState AssertGustStackPriorityState(
        ...
        StackItemState? expectedStackItem = null)
    {
        ...
        Assert.True(result.State.CardObjects.ContainsKey(GustObjectId));
        Assert.True(result.State.CardObjects.ContainsKey(GustTargetObjectId));
        Assert.Equal(GustCardNo, result.State.CardObjects[GustObjectId].CardNo);
        Assert.Equal(1, result.State.CardObjects[GustObjectId].ManaCost);
        Assert.Contains(CardObjectTags.SpellCard, result.State.CardObjects[GustObjectId].Tags);
        Assert.Equal(1, result.State.CardObjects[GustTargetObjectId].Damage);
        Assert.Equal(3, result.State.CardObjects[GustTargetObjectId].Power);
        Assert.Contains(CardObjectTags.UnitCard, result.State.CardObjects[GustTargetObjectId].Tags);
        Assert.Equal("P2", result.State.CardObjects[GustTargetObjectId].OwnerId);
        Assert.Equal("P2", result.State.CardObjects[GustTargetObjectId].ControllerId);
        Assert.Equal("P1", result.State.ObjectLocations[GustObjectId].PlayerId);
        Assert.Equal("STACK", result.State.ObjectLocations[GustObjectId].Zone);
        Assert.Equal("P2", result.State.ObjectLocations[GustTargetObjectId].PlayerId);
        Assert.Equal("BATTLEFIELD", result.State.ObjectLocations[GustTargetObjectId].Zone);

        var stackItem = Assert.Single(result.State.StackItems);
        Assert.Equal(GustObjectId, stackItem.SourceObjectId);
        Assert.Equal(GustCardNo, stackItem.CardNo);
        Assert.Equal([GustTargetObjectId], stackItem.TargetObjectIds);
        Assert.Empty(stackItem.OptionalCosts);
        Assert.Equal("GUST_RETURN_BATTLEFIELD_UNIT_POWER_3_OR_LESS_TO_HAND", stackItem.EffectKind);
