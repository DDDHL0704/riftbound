...
{
    private const string TimeGateObjectId = "P1-EQUIPMENT-TIME-GATE";
    private const string TimeGateCardNo = "SFD·078/221";
    private const string TimeGateEffectKind = "TIME_GATE_PLAY_EQUIPMENT";
    ...
    public async Task TimeGatePlayCardStalePromptReplayAfterStackPriorityStartsUsesRejectedCacheWithoutMutation()
    {
        ...
        const string acceptedClientIntentId = "intent-time-gate-before-stale-prompt-replay";
        const string staleClientIntentId = "intent-time-gate-stale-prompt-replay";
        ...
        var acceptedStackItem = AssertTimeGateStackPriorityState(accepted);
        ...
        AssertTimeGateStackPriorityState(replay, acceptedStackItem);
        ...
        AssertTimeGateStackPriorityState(reorderedReplay, acceptedStackItem);
        Assert.Equal(p1PromptAfterAccepted, MatchStateHasher.HashValue(session.PromptFor("P1")));
        Assert.Equal(p2PromptAfterAccepted, MatchStateHasher.HashValue(session.PromptFor("P2")));
        Assert.Equal(p1SnapshotAfterAccepted, MatchStateHasher.HashValue(session.SnapshotFor("P1")));
        Assert.Equal(p2SnapshotAfterAccepted, MatchStateHasher.HashValue(session.SnapshotFor("P2")));
        ...
        AssertTimeGateStackPriorityState(duplicateReplay, acceptedStackItem);
        Assert.Equal(p1PromptAfterAccepted, MatchStateHasher.HashValue(session.PromptFor("P1")));
        Assert.Equal(p2PromptAfterAccepted, MatchStateHasher.HashValue(session.PromptFor("P2")));
        Assert.Equal(p1SnapshotAfterAccepted, MatchStateHasher.HashValue(session.SnapshotFor("P1")));
        Assert.Equal(p2SnapshotAfterAccepted, MatchStateHasher.HashValue(session.SnapshotFor("P2")));
        ...
        AssertTimeGateStackPriorityState(conflict, acceptedStackItem);
        Assert.Equal(p1PromptAfterAccepted, MatchStateHasher.HashValue(session.PromptFor("P1")));
        Assert.Equal(p2PromptAfterAccepted, MatchStateHasher.HashValue(session.PromptFor("P2")));
        Assert.Equal(p1SnapshotAfterAccepted, MatchStateHasher.HashValue(session.SnapshotFor("P1")));
        Assert.Equal(p2SnapshotAfterAccepted, MatchStateHasher.HashValue(session.SnapshotFor("P2")));
        ...
        ActionPromptDto prompt)
    {
        ...
        Assert.Equal(TimeGateObjectId, rawCommand.GetProperty("cardObjectId").GetString());
        Assert.Equal(TimeGateCardNo, rawCommand.GetProperty("cardNo").GetString());
    ...
    private static StackItemState AssertTimeGateStackPriorityState(
        ...
        StackItemState? expectedStackItem = null)
    {
        ...
        Assert.Equal(
            ["P1-TARGET-UNIT", "P1-BASE-TIME-GATE", "P1-FACE-DOWN-STANDBY-TIME-GATE"],
            result.State.PlayerZones["P1"].Base);
        Assert.Null(result.State.PendingPayment);
        Assert.Equal("STACK", result.State.ObjectLocations[TimeGateObjectId].Zone);

        var equipment = result.State.CardObjects[TimeGateObjectId];
        Assert.Equal(TimeGateCardNo, equipment.CardNo);
        Assert.Equal("P1", equipment.OwnerId);
        Assert.Equal("P1", equipment.ControllerId);
        Assert.Equal([CardObjectTags.EquipmentCard], equipment.Tags);
        Assert.False(equipment.IsExhausted);
        ...
        Assert.Equal(TimeGateObjectId, stackItem.SourceObjectId);
        Assert.Equal(TimeGateCardNo, stackItem.CardNo);
        Assert.Equal(TimeGateEffectKind, stackItem.EffectKind);
