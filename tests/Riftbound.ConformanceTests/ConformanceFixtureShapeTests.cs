...
public sealed class ConformanceFixtureShapeTests
{
    ...
    [Fact]
    public void GameCommandMapperRecycleRuneUsesCommandFieldsOverVisibleSourceMetadata()
    {
        const string VisibleRecycleRuneMetadataAliases = """
              "sourceRequirements": [
                {
                  "sourceObjectId": "P1-VISIBLE-RECYCLE-RUNE",
                  "sourceChoices": [{ "id": "P1-VISIBLE-RECYCLE-RUNE", "label": "Visible source choice" }],
                  "runeChoices": [{ "id": "P1-VISIBLE-RECYCLE-RUNE", "label": "Visible rune choice" }],
                  "paymentResourceChoices": [{ "id": "RECYCLE_RUNE:P1-VISIBLE-RECYCLE-RUNE" }]
                }
              ],
              "sources": [{ "id": "P1-VISIBLE-RECYCLE-RUNE", "label": "Visible source" }],
              "sourceChoices": [{ "id": "P1-VISIBLE-RECYCLE-RUNE", "label": "Visible top-level source choice" }],
              "runeChoices": [{ "id": "P1-VISIBLE-RECYCLE-RUNE", "label": "Visible top-level rune choice" }],
              "paymentResourceChoices": [{ "id": "RECYCLE_RUNE:P1-VISIBLE-RECYCLE-RUNE" }]
            """;
        ...
        var command = Map($$"""
            {
              ...
              "sourceObjectId": "P1-CURRENT-RECYCLE-RUNE",
              {{VisibleRecycleRuneMetadataAliases}}
            }
            """);
        var malformedCurrent = Map($$"""
            {
              "cmdType": "RECYCLE_RUNE",
              "sourceObjectId": { "id": "P1-CURRENT-RECYCLE-RUNE" },
              {{VisibleRecycleRuneMetadataAliases}}
            }
            """);
        var aliasOnly = Map($$"""
        ...
        Assert.Equal("P1-CURRENT-RECYCLE-RUNE", command.SourceObjectId);
        Assert.Equal(string.Empty, malformedCurrent.SourceObjectId);
        Assert.Equal(string.Empty, aliasOnly.SourceObjectId);

        static RecycleRuneCommand Map(string json)
        {
            return Assert.IsType<RecycleRuneCommand>(
                GameCommandJsonMapper.Map(JsonDocument.Parse(json).RootElement));
        }
    }
    ...
    {
        const string acceptedClientIntentId = "intent-apnap-order-before-stale-prompt-replay";
        const string staleClientIntentId = "intent-apnap-order-stale-prompt-replay";
        var journal = new RecordingMatchJournal();
        var state = BuildP0ContractBattleInitialApnapTriggerQueueState();
        var session = new MatchSession(state, new CoreRuleEngine(), journal);
        session.EnsurePlayer("P1");
        ...
        var postAcceptedP1SessionSnapshotHash = MatchStateHasher.HashValue(session.SnapshotFor("P1"));
        var postAcceptedP2SessionSnapshotHash = MatchStateHasher.HashValue(session.SnapshotFor("P2"));
        ...
        Assert.Equal(postAcceptedP1SessionSnapshotHash, MatchStateHasher.HashValue(session.SnapshotFor("P1")));
        Assert.Equal(postAcceptedP2SessionSnapshotHash, MatchStateHasher.HashValue(session.SnapshotFor("P2")));
        Assert.Empty(replay.State.TriggerQueue);
        Assert.Equal(
            ["ordered-TRIGGER-BATTLE-ATTACKER", "ordered-TRIGGER-BATTLE-DEFENDER"],
            replay.State.StackItems.Select(item => item.StackItemId).ToArray());
        ...
        var orderTriggerEntries = journal.Entries
            ...
            .ToArray();
        Assert.Equal(2, journal.Entries.Count);
        Assert.Equal(2, orderTriggerEntries.Length);
        var acceptedEntry = Assert.Single(orderTriggerEntries, entry => entry.Accepted);
        var rejectedEntry = Assert.Single(journal.Entries, entry => !entry.Accepted);

        Assert.Equal(state.RoomId, acceptedEntry.RoomId);
        ...
        Assert.Equal(replay.ErrorMessage, cachedReplay.ErrorMessage);
    ...
    [Fact]
    public async Task OrderTriggersStaleRawPromptAfterStackPriorityStartsRecordsRejectedJournalWithoutMutation()
    {
        const string acceptedClientIntentId = "intent-apnap-order-stale-raw-first";
        const string replayClientIntentId = "intent-apnap-order-stale-raw-replay";
        var journal = new RecordingMatchJournal();
        var state = BuildP0ContractBattleInitialApnapTriggerQueueState();
        var session = new MatchSession(state, new CoreRuleEngine(), journal);
        session.EnsurePlayer("P1");
        ...
        Assert.Equal(2, orderTriggerEntries.Length);
        var acceptedEntry = Assert.Single(orderTriggerEntries, entry => entry.Accepted);
        var rejectedEntry = Assert.Single(orderTriggerEntries, entry => !entry.Accepted);
    ...
    [Fact]
    public void ActionPromptRevealCardMetadataExposesBattlefieldStandbyReactionTargets()
    {
        const string battlefieldObjectId = "P1-BATTLEFIELD-BANDLE-TREE";
        const string sourceObjectId = "P1-FACE-DOWN-BATTLEFIELD-TEEMO";
        var state = new MatchState(
            "prompt-reveal-card-target-room",
            19,
            4,
            "P1",
            new Dictionary<string, string>(StringComparer.Ordinal)
