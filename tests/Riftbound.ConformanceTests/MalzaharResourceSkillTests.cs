...
public sealed class MalzaharResourceSkillTests
{
    ...
    [Fact]
    public async Task MalzaharResourceSkillStalePromptReplayUsesRejectedCacheWithoutMutation()
    {
        ...
        AssertPromptExposesMalzaharActivation(prompt);
        var command = new ActivateAbilityCommand(
            MalzaharObjectId,
            P4ActivatedAbilityCatalog.MalzaharResourceAbilityId,
            [FriendlyUnitObjectId]);
        ...
        var changedStaleRawCommand = PromptScopedActivateAbilityRawCommandWithClientNote(
            ...
            prompt,
            "changed-payload");
        const string acceptedClientIntentId = "intent-malzahar-resource-before-stale-prompt-replay";
        const string staleClientIntentId = "intent-malzahar-resource-stale-prompt-replay";
        ...
        Assert.True(accepted.Accepted, accepted.ErrorMessage);
        Assert.Equal(
            ["ABILITY_ACTIVATED", "UNIT_EXHAUSTED", "UNIT_DESTROYED", "POWER_GAINED"],
        ...
        var acceptedTemporaryResourceId = AssertMalzaharActivatedState(accepted, expectedTemporaryResourceCount: 1);
        ...
        Assert.Equal(acceptedTemporaryResourceId, AssertMalzaharActivatedState(replay, expectedTemporaryResourceCount: 1));
        ...
        Assert.Equal(acceptedP1SessionSnapshotHash, MatchStateHasher.HashValue(session.SnapshotFor("P1")));
        Assert.Equal(acceptedP2SessionSnapshotHash, MatchStateHasher.HashValue(session.SnapshotFor("P2")));
        Assert.Equal(acceptedTemporaryResourceId, AssertMalzaharActivatedState(duplicateReplay, expectedTemporaryResourceCount: 1));
        Assert.Equal(2, journal.Entries.Count);
        ...
        Assert.Equal(acceptedP1SessionSnapshotHash, MatchStateHasher.HashValue(session.SnapshotFor("P1")));
        Assert.Equal(acceptedP2SessionSnapshotHash, MatchStateHasher.HashValue(session.SnapshotFor("P2")));
        Assert.Equal(acceptedTemporaryResourceId, AssertMalzaharActivatedState(conflict, expectedTemporaryResourceCount: 1));
        Assert.Equal(2, journal.Entries.Count);
        Assert.Equal(journalHashAfterReplay, MatchStateHasher.HashValue(journal.Entries));
    ...
    public async Task MalzaharTemporaryPaymentResourcePaysRuneCostAndCleansUpAtPaymentClose()
    {
        ...
        Assert.Equal(P4ActivatedAbilityCatalog.MalzaharResourceGeneratedPower, paymentResourcePowerByChoice[resourceAction]["power"]);
        ...
        Assert.Equal("PAY-POWER-1", spentEvent.Payload["paymentId"]);
        Assert.Equal("TEST_PENDING_PAY_COST", spentEvent.Payload["paymentWindow"]);
        ...
        Assert.Equal(MalzaharObjectId, spentEvent.Payload["sourceObjectId"]);
        ...
        Assert.Equal("PAY-POWER-1", cleanupEvent.Payload["paymentId"]);
        Assert.Equal("TEST_PENDING_PAY_COST", cleanupEvent.Payload["paymentWindow"]);
        ...
        Assert.Equal(1, cleanupEvent.Payload["remainingPowerBeforeCleanup"]);
        ...
        Assert.Equal("PAY-POWER-1", costEvent.Payload["paymentId"]);
        ...
        Assert.Equal("PAY-POWER-1", paymentWindowClosedEvent.Payload["paymentId"]);
        Assert.Equal("TEST_PENDING_PAY_COST", paymentWindowClosedEvent.Payload["paymentWindow"]);
    }

    [Fact]
    public async Task MalzaharTemporaryPaymentResourcesCombineForLargerRuneCostAndCleanEachLedger()
    {
        var baseState = BuildStateWithTemporaryResource();
        var firstTemporaryResource = Assert.Single(baseState.TemporaryPaymentResources);
        var secondTemporaryResource = new TemporaryPaymentResourceState(
            "MALZAHAR:TEMP-PAYMENT-RESOURCE-2",
            "P1",
            MalzaharObjectId,
            P4ActivatedAbilityCatalog.MalzaharResourceAbilityId,
            "ACTIVATE_ABILITY",
        ...
        var state = baseState with
        {
            PendingPayment = new PendingPaymentState(
                ...
                "P1",
                powerCost: 4,
                legalPaymentChoiceIds: ["SPEND_POWER:any:4"]),
            TemporaryPaymentResources = [firstTemporaryResource, secondTemporaryResource]
        };
        var temporaryResources = state.TemporaryPaymentResources.ToArray();
        var resourceActions = temporaryResources
        ...
        var paymentResourceChoices = Assert.IsAssignableFrom<IReadOnlyList<ActionPromptChoiceDto>>(
            metadata["paymentResourceChoices"]);
        Assert.Equal(resourceActions, paymentResourceChoices.Select(choice => choice.Id).ToArray());
        Assert.Equal(resourceActions, Assert.IsAssignableFrom<IReadOnlyList<string>>(metadata["paymentResourceActionIds"]));
        var paymentResourcePowerByChoice = Assert.IsAssignableFrom<IReadOnlyDictionary<string, IReadOnlyDictionary<string, object?>>>(
            metadata["paymentResourcePowerByChoice"]);
        ...
        var submittedChoices = resourceActions.Append("SPEND_POWER:any:4").ToArray();
        var result = await new CoreRuleEngine().ResolveAsync(
            state,
            new PlayerIntent("intent-malzahar-two-temp-resources-pay-cost", "P1", CommandTypes.PayCost),
            new PayCostCommand("PAY-POWER-4", "TEST_PENDING_PAY_COST", submittedChoices),
            CancellationToken.None);
        ...
        var spentEvents = result.Events
            ...
            .ToArray();
        Assert.Equal(2, spentEvents.Length);
        var cleanupEvents = result.Events
            ...
            .ToArray();
        Assert.Equal(2, cleanupEvents.Length);
        for (var index = 0; index < temporaryResources.Length; index++)
        {
            var temporaryResource = temporaryResources[index];
            Assert.Equal("PAY-POWER-4", spentEvents[index].Payload["paymentId"]);
            Assert.Equal("TEST_PENDING_PAY_COST", spentEvents[index].Payload["paymentWindow"]);
            Assert.Equal("P1", spentEvents[index].Payload["playerId"]);
            Assert.Equal(temporaryResource.ResourceId, spentEvents[index].Payload["temporaryPaymentResourceId"]);
            Assert.Equal(MalzaharObjectId, spentEvents[index].Payload["sourceObjectId"]);
            Assert.Equal(P4ActivatedAbilityCatalog.MalzaharResourceAbilityId, spentEvents[index].Payload["abilityId"]);
            Assert.Equal(P4ActivatedAbilityCatalog.MalzaharResourceGeneratedPower, spentEvents[index].Payload["consumedPower"]);
            Assert.Equal(0, spentEvents[index].Payload["remainingPower"]);
            Assert.Empty(Assert.IsAssignableFrom<IReadOnlyDictionary<string, int>>(spentEvents[index].Payload["remainingPowerByTrait"]));
            Assert.Equal([PaymentCostRules.RuneCostPaymentKind], Assert.IsType<string[]>(spentEvents[index].Payload["allowedPaymentKinds"]));
            Assert.Equal(true, spentEvents[index].Payload["paymentOnly"]);

            Assert.Equal("PAY-POWER-4", cleanupEvents[index].Payload["paymentId"]);
            Assert.Equal("TEST_PENDING_PAY_COST", cleanupEvents[index].Payload["paymentWindow"]);
            Assert.Equal("P1", cleanupEvents[index].Payload["playerId"]);
            Assert.Equal(temporaryResource.ResourceId, cleanupEvents[index].Payload["temporaryPaymentResourceId"]);
            Assert.Equal(0, cleanupEvents[index].Payload["remainingPowerBeforeCleanup"]);
            Assert.Empty(Assert.IsAssignableFrom<IReadOnlyDictionary<string, int>>(cleanupEvents[index].Payload["remainingPowerByTraitBeforeCleanup"]));
            Assert.Equal(true, cleanupEvents[index].Payload["paymentOnly"]);
        }

        var costEvent = Assert.Single(result.Events, gameEvent => string.Equals(gameEvent.Kind, "COST_PAID", StringComparison.Ordinal));
        Assert.Equal("PAY-POWER-4", costEvent.Payload["paymentId"]);
        Assert.Equal("TEST_PENDING_PAY_COST", costEvent.Payload["paymentWindow"]);
        Assert.Equal("P1", costEvent.Payload["playerId"]);
        Assert.Equal(resourceActions, Assert.IsType<string[]>(costEvent.Payload["paymentResourceActions"]));
        Assert.Equal(submittedChoices, Assert.IsType<string[]>(costEvent.Payload["paymentChoiceIds"]));
        Assert.Equal(["SPEND_POWER:any:4"], Assert.IsType<string[]>(costEvent.Payload["legalPaymentChoiceIds"]));
        Assert.Equal(temporaryResources.Select(resource => resource.ResourceId).ToArray(), Assert.IsType<string[]>(costEvent.Payload["temporaryPaymentResourceIds"]));
        Assert.Equal(4, costEvent.Payload["temporaryPaymentResourcePower"]);
        Assert.Equal(4, costEvent.Payload["power"]);
        Assert.Empty(Assert.IsAssignableFrom<IReadOnlyDictionary<string, int>>(costEvent.Payload["powerByTrait"]));
        Assert.Equal(0, costEvent.Payload["remainingPower"]);
        Assert.Empty(Assert.IsAssignableFrom<IReadOnlyDictionary<string, int>>(costEvent.Payload["remainingPowerByTrait"]));
        ...
        var paymentWindowClosedEvent = Assert.Single(result.Events, gameEvent =>
            string.Equals(gameEvent.Kind, "PAYMENT_WINDOW_CLOSED", StringComparison.Ordinal));
        Assert.Equal("PAY-POWER-4", paymentWindowClosedEvent.Payload["paymentId"]);
        Assert.Equal("TEST_PENDING_PAY_COST", paymentWindowClosedEvent.Payload["paymentWindow"]);
    }
    ...
    private static void AssertPromptExposesMalzaharActivation(ActionPromptDto prompt)
    {
        ...
        Assert.Contains(
            ...
            choice => string.Equals(choice.Id, FriendlyUnitObjectId, StringComparison.Ordinal));
    }

    private static string AssertMalzaharActivatedState(
        ...
        int expectedTemporaryResourceCount)
    {
        ...
        Assert.DoesNotContain(FriendlyUnitObjectId, result.State.PlayerZones["P1"].Base);
        Assert.Contains(FriendlyUnitObjectId, result.State.PlayerZones["P1"].Graveyard);
        Assert.Equal("P1", result.State.ObjectLocations[FriendlyUnitObjectId].PlayerId);
        Assert.Equal("GRAVEYARD", result.State.ObjectLocations[FriendlyUnitObjectId].Zone);
        Assert.Empty(result.State.StackItems);
        Assert.Equal(TimingStates.NeutralOpen, result.State.TimingState);
        Assert.Null(result.State.PriorityPlayerId);
        Assert.Equal(0, result.State.RunePools["P1"].Power);
        ...
        if (result.Events.Count > 0)
        {
            ...
            Assert.Equal(MalzaharObjectId, activatedEvent.Payload["sourceObjectId"]);
            Assert.Equal(P4ActivatedAbilityCatalog.MalzaharResourceAbilityId, activatedEvent.Payload["abilityId"]);
            Assert.Equal(FriendlyUnitObjectId, activatedEvent.Payload["destroyedCostObjectId"]);
            Assert.Equal(temporaryResource.ResourceId, activatedEvent.Payload["temporaryPaymentResourceId"]);

            var exhaustedEvent = Assert.Single(result.Events, gameEvent => string.Equals(gameEvent.Kind, "UNIT_EXHAUSTED", StringComparison.Ordinal));
            Assert.Equal(MalzaharObjectId, exhaustedEvent.Payload["sourceObjectId"]);
            Assert.Equal(MalzaharObjectId, exhaustedEvent.Payload["targetObjectId"]);
            Assert.Equal(P4ActivatedAbilityCatalog.MalzaharResourceAbilityId, exhaustedEvent.Payload["abilityId"]);
            ...
            Assert.Equal("RESOURCE_SKILL_COST", destroyedEvent.Payload["reason"]);
            Assert.Equal(FriendlyUnitObjectId, destroyedEvent.Payload["targetObjectId"]);
            Assert.Equal("ACTIVATE_ABILITY", destroyedEvent.Payload["paymentWindow"]);
            Assert.Equal(P4ActivatedAbilityCatalog.MalzaharResourceAbilityId, destroyedEvent.Payload["abilityId"]);

            var powerEvent = Assert.Single(result.Events, gameEvent => string.Equals(gameEvent.Kind, "POWER_GAINED", StringComparison.Ordinal));
            Assert.Equal("P1", powerEvent.Payload["playerId"]);
            Assert.Equal(MalzaharObjectId, powerEvent.Payload["sourceObjectId"]);
            Assert.Equal(P4ActivatedAbilityCatalog.MalzaharResourceAbilityId, powerEvent.Payload["abilityId"]);
            Assert.Equal(FriendlyUnitObjectId, powerEvent.Payload["destroyedCostObjectId"]);
            Assert.Equal(P4ActivatedAbilityCatalog.MalzaharResourceGeneratedPower, powerEvent.Payload["generatedPower"]);
            Assert.Equal(P4ActivatedAbilityCatalog.MalzaharResourceGeneratedPower, powerEvent.Payload["power"]);
            Assert.Equal(0, powerEvent.Payload["powerAfter"]);
            Assert.Equal(temporaryResource.ResourceId, powerEvent.Payload["temporaryPaymentResourceId"]);
        }
