...
public sealed class VexSpellshieldGuardTests
{
    private const string VexObjectId = "P1-UNIT-VEX";
    private const string VexCardNo = "UNL-150/219";
    private const string VexEffectKind = "VEX_SPELLSHIELD_OPPONENT_UNIT_STUN_STATIC";

    [Fact]
    public async Task VexPlayCardWithNoTargetsUsesStackAndResolvesToBase()
    {
        var engine = new CoreRuleEngine();
        var state = BuildVexState();

        var played = await PlayVexAsync(engine, state, "P1-UNIT-VEX", []);
        ...
        Assert.Contains(played.Events, gameEvent =>
            ...
            && string.Equals(gameEvent.Payload["sourceObjectId"] as string, "P1-UNIT-VEX", StringComparison.Ordinal));
        Assert.Contains(played.Events, gameEvent =>
        ...
        Assert.Contains(played.Events, gameEvent =>
            ...
            && string.Equals(gameEvent.Payload["sourceObjectId"] as string, "P1-UNIT-VEX", StringComparison.Ordinal)
            && string.Equals(gameEvent.Payload["effectKind"] as string, "VEX_SPELLSHIELD_OPPONENT_UNIT_STUN_STATIC", StringComparison.Ordinal));

        var p1Pass = await engine.ResolveAsync(
            played.State,
            new PlayerIntent("intent-vex-play-p1-pass", "P1", CommandTypes.PassPriority),
            new PassPriorityCommand(),
            CancellationToken.None);
        var p2Pass = await engine.ResolveAsync(
            p1Pass.State,
            new PlayerIntent("intent-vex-play-p2-pass", "P2", CommandTypes.PassPriority),
            new PassPriorityCommand(),
            CancellationToken.None);
        ...
        Assert.Equal(["P1-TARGET-UNIT", "P1-BASE-VEX", "P1-FACE-DOWN-STANDBY-VEX", "P1-UNIT-VEX"], p2Pass.State.PlayerZones["P1"].Base);
        Assert.Empty(p2Pass.State.PlayerZones["P1"].Hand);
        Assert.Empty(p2Pass.State.StackItems);

        var unit = p2Pass.State.CardObjects["P1-UNIT-VEX"];
        Assert.Equal("UNL-150/219", unit.CardNo);
        ...
        Assert.Contains(p2Pass.Events, gameEvent =>
            ...
            && string.Equals(gameEvent.Payload["sourceObjectId"] as string, "P1-UNIT-VEX", StringComparison.Ordinal)
            && string.Equals(gameEvent.Payload["unitObjectId"] as string, "P1-UNIT-VEX", StringComparison.Ordinal)
    ...
    public async Task VexPlayCardStalePromptReplayAfterStackPriorityStartsUsesRejectedCacheWithoutMutation()
    {
        ...
        const string acceptedClientIntentId = "intent-vex-before-stale-prompt-replay";
        const string staleClientIntentId = "intent-vex-stale-prompt-replay";
        ...
        var acceptedStackItem = AssertVexStackPriorityState(accepted);
        ...
        AssertVexStackPriorityState(replay, acceptedStackItem);
        ...
        AssertVexStackPriorityState(reorderedReplay, acceptedStackItem);
        Assert.Equal(p1PromptAfterAccepted, MatchStateHasher.HashValue(session.PromptFor("P1")));
        Assert.Equal(p2PromptAfterAccepted, MatchStateHasher.HashValue(session.PromptFor("P2")));
        Assert.Equal(p1SnapshotAfterAccepted, MatchStateHasher.HashValue(session.SnapshotFor("P1")));
        Assert.Equal(p2SnapshotAfterAccepted, MatchStateHasher.HashValue(session.SnapshotFor("P2")));
        ...
        AssertVexStackPriorityState(duplicateReplay, acceptedStackItem);
        Assert.Equal(p1PromptAfterAccepted, MatchStateHasher.HashValue(session.PromptFor("P1")));
        Assert.Equal(p2PromptAfterAccepted, MatchStateHasher.HashValue(session.PromptFor("P2")));
        Assert.Equal(p1SnapshotAfterAccepted, MatchStateHasher.HashValue(session.SnapshotFor("P1")));
        Assert.Equal(p2SnapshotAfterAccepted, MatchStateHasher.HashValue(session.SnapshotFor("P2")));
        ...
        AssertVexStackPriorityState(conflict, acceptedStackItem);
        Assert.Equal(p1PromptAfterAccepted, MatchStateHasher.HashValue(session.PromptFor("P1")));
        Assert.Equal(p2PromptAfterAccepted, MatchStateHasher.HashValue(session.PromptFor("P2")));
        Assert.Equal(p1SnapshotAfterAccepted, MatchStateHasher.HashValue(session.SnapshotFor("P1")));
        Assert.Equal(p2SnapshotAfterAccepted, MatchStateHasher.HashValue(session.SnapshotFor("P2")));
    ...
    [Theory]
    [InlineData("P1-UNIT-VEX", "P1-TARGET-UNIT", 4, ErrorCodes.InvalidTarget)]
    [InlineData("P1-BASE-VEX", "", 4, ErrorCodes.CardNotInHand)]
    [InlineData("P2-UNIT-VEX", "", 4, ErrorCodes.CardNotInHand)]
    [InlineData("P1-FACE-DOWN-STANDBY-VEX", "", 4, ErrorCodes.CardNotInHand)]
    [InlineData("P1-UNIT-VEX", "", 3, ErrorCodes.InsufficientCost)]
    public async Task VexPlayCardRejectsInvalidInputsWithoutMutation(
        string sourceObjectId,
        string targetObjectId,
        int mana,
        string expectedErrorCode)
    {
        var state = BuildVexState(mana);
        var targetObjectIds = string.IsNullOrWhiteSpace(targetObjectId) ? Array.Empty<string>() : [targetObjectId];

        var result = await PlayVexAsync(
            new CoreRuleEngine(),
            state,
            sourceObjectId,
            targetObjectIds);
        ...
        Assert.Equal(["P1-UNIT-VEX"], result.State.PlayerZones["P1"].Hand);
        Assert.Equal(["P1-TARGET-UNIT", "P1-BASE-VEX", "P1-FACE-DOWN-STANDBY-VEX"], result.State.PlayerZones["P1"].Base);
        Assert.Equal(["P2-UNIT-VEX"], result.State.PlayerZones["P2"].Hand);
        Assert.Empty(result.State.StackItems);
        Assert.Null(result.State.PendingPayment);
        Assert.False(result.State.CardObjects["P1-UNIT-VEX"].IsFaceDown);
        Assert.Null(result.State.CardObjects["P1-FACE-DOWN-STANDBY-VEX"].CardNo);
        Assert.True(result.State.CardObjects["P1-FACE-DOWN-STANDBY-VEX"].IsFaceDown);
        Assert.Equal(
            [CardObjectTags.UnitCard, CardObjectTags.Standby],
            result.State.CardObjects["P1-FACE-DOWN-STANDBY-VEX"].Tags);
        ...
        IReadOnlyList<string> targetObjectIds)
    {
        return await engine.ResolveAsync(
            state,
            new PlayerIntent("intent-vex-play", "P1", CommandTypes.PlayCard),
            new PlayCardCommand(
                sourceObjectId,
                "UNL-150/219",
                targetObjectIds),
        ...
        ActionPromptDto prompt)
    {
        ...
        Assert.Equal(VexObjectId, rawCommand.GetProperty("cardObjectId").GetString());
        Assert.Equal(VexCardNo, rawCommand.GetProperty("cardNo").GetString());
    ...
    private static StackItemState AssertVexStackPriorityState(
        ...
        StackItemState? expectedStackItem = null)
    {
        ...
        Assert.Equal(
            ["P1-TARGET-UNIT", "P1-BASE-VEX", "P1-FACE-DOWN-STANDBY-VEX"],
            result.State.PlayerZones["P1"].Base);
        Assert.Null(result.State.PendingPayment);
        Assert.Equal("STACK", result.State.ObjectLocations[VexObjectId].Zone);

        var unit = result.State.CardObjects[VexObjectId];
        Assert.Equal(VexCardNo, unit.CardNo);
        Assert.Equal("P1", unit.OwnerId);
        Assert.Equal("P1", unit.ControllerId);
        ...
        Assert.Equal(VexObjectId, stackItem.SourceObjectId);
        Assert.Equal(VexCardNo, stackItem.CardNo);
        Assert.Equal(VexEffectKind, stackItem.EffectKind);
    ...
    private static MatchState BuildVexState(int mana = 4)
    {
        return new MatchState(
            roomId: "vex-spellshield-guard-test",
            tick: 0,
            turnNumber: 1,
            activePlayerId: "P1",
            ...
            {
                ["P1-UNIT-VEX"] = Vex("P1-UNIT-VEX"),
                ["P1-BASE-VEX"] = Vex("P1-BASE-VEX"),
                ["P1-FACE-DOWN-STANDBY-VEX"] = Vex(
                    "P1-FACE-DOWN-STANDBY-VEX",
                    isFaceDown: true,
    ...
    private static CardObjectState Vex(
        string objectId,
        ...
        string controllerId = "P1")
    {
        return new CardObjectState(
            objectId,
            cardNo: isFaceDown ? null : "UNL-150/219",
