...
public sealed class HuntTheWeakDestroyGuardTests
{
    ...
    [Fact]
    public async Task HuntTheWeakPlayCardStalePromptReplayAfterStackPriorityStartsUsesRejectedCacheWithoutMutation()
    {
        ...
        const string acceptedClientIntentId = "intent-hunt-the-weak-before-stale-prompt-replay";
        const string staleClientIntentId = "intent-hunt-the-weak-stale-prompt-replay";
        ...
        AssertPromptScopedPlayCardRawCommand(reorderedStaleRawCommand, command, prompt);
        ...
        var acceptedStackItem = AssertHuntTheWeakStackPriorityState(accepted);
        ...
        AssertHuntTheWeakStackPriorityState(replay, acceptedStackItem);
        ...
        AssertHuntTheWeakStackPriorityState(reorderedReplay, acceptedStackItem);
        Assert.Equal(p1PromptAfterAccepted, MatchStateHasher.HashValue(session.PromptFor("P1")));
        Assert.Equal(p2PromptAfterAccepted, MatchStateHasher.HashValue(session.PromptFor("P2")));
        Assert.Equal(p1SnapshotAfterAccepted, MatchStateHasher.HashValue(session.SnapshotFor("P1")));
        Assert.Equal(p2SnapshotAfterAccepted, MatchStateHasher.HashValue(session.SnapshotFor("P2")));
        ...
        AssertHuntTheWeakStackPriorityState(duplicateReplay, acceptedStackItem);
        Assert.Equal(p1PromptAfterAccepted, MatchStateHasher.HashValue(session.PromptFor("P1")));
        Assert.Equal(p2PromptAfterAccepted, MatchStateHasher.HashValue(session.PromptFor("P2")));
        Assert.Equal(p1SnapshotAfterAccepted, MatchStateHasher.HashValue(session.SnapshotFor("P1")));
        Assert.Equal(p2SnapshotAfterAccepted, MatchStateHasher.HashValue(session.SnapshotFor("P2")));
        ...
        AssertHuntTheWeakStackPriorityState(conflict, acceptedStackItem);
        Assert.Equal(p1PromptAfterAccepted, MatchStateHasher.HashValue(session.PromptFor("P1")));
        Assert.Equal(p2PromptAfterAccepted, MatchStateHasher.HashValue(session.PromptFor("P2")));
        Assert.Equal(p1SnapshotAfterAccepted, MatchStateHasher.HashValue(session.SnapshotFor("P1")));
        Assert.Equal(p2SnapshotAfterAccepted, MatchStateHasher.HashValue(session.SnapshotFor("P2")));
    ...
    private static StackItemState AssertHuntTheWeakStackPriorityState(
        ...
        StackItemState? expectedStackItem = null)
    {
        ...
        Assert.Equal(1, result.State.CardObjects["P2-BATTLEFIELD-UNIT"].Damage);
        Assert.Equal(3, result.State.CardObjects["P2-BATTLEFIELD-UNIT"].Power);
        Assert.True(result.State.CardObjects.ContainsKey("P2-BATTLEFIELD-UNIT"));

        var stackItem = Assert.Single(result.State.StackItems);
        Assert.Equal("P1-SPELL-HUNT-THE-WEAK", stackItem.SourceObjectId);
        Assert.Equal("UNL-159/219", stackItem.CardNo);
        Assert.Equal(["P2-BATTLEFIELD-UNIT"], stackItem.TargetObjectIds);
        Assert.Empty(stackItem.OptionalCosts);
        Assert.Equal("HUNT_THE_WEAK_DESTROY_BATTLEFIELD_UNIT_POWER_3_OR_LESS", stackItem.EffectKind);
