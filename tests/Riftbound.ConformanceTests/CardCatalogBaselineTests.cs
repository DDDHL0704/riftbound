...
public sealed class CardCatalogBaselineTests
{
    ...
    public async Task BehaviorSpecsCoverEveryOfficialCardWithExplicitStatusAndReason()
    {
        ...
        Assert.Equal(878, report.StatusCounts[BehaviorImplementationStatuses.Implemented]);
        Assert.Equal(131, report.StatusCounts[BehaviorImplementationStatuses.ManualRuleRequired]);
    ...
    public async Task P6FunctionalUnitCoverageAuditsSameTextVariantsAndReprints()
    {
        ...
        Assert.Equal(724, coverage.ImplementedUnits);
        Assert.Equal(87, coverage.ManualRuleRequiredUnits);
        Assert.Equal(0, coverage.UnimplementedUnits);
        Assert.Equal(113, coverage.DuplicateGroups);
        Assert.Equal(84, coverage.ImplementedDuplicateGroups);
        Assert.Equal(238, coverage.ImplementedDuplicateEntries);
        Assert.Equal(29, coverage.PendingDuplicateGroups);
        Assert.Equal(73, coverage.PendingDuplicateEntries);
    ...
    public async Task P6HighFrequencyTemplateFamiliesReportEntryAndFunctionalUnitCoverage()
    {
        ...
        AssertFamily(report, BehaviorTemplateIds.Draw, 131, 112, 19, 0, 114, 101, 13);
    ...
    public async Task P6SecondaryTemplateFamiliesReportEntryAndFunctionalUnitCoverage()
    {
        ...
        AssertFamily(report, BehaviorTemplateIds.TempMight, 292, 261, 31, 0, 230, 211, 19);
    ...
    public async Task P6LifecycleTriggerReplacementFamiliesReportSpecAndExecutionBoundaryCoverage()
    {
        ...
        AssertTimingSurfaceCoverage(
            ...
            TimingSurfaceNames.Trigger,
            entries: 530,
            specImplementedEntries: 442,
            manualRuleRequiredEntries: 88,
            unimplementedEntries: 0,
            functionalUnits: 423,
            specImplementedFunctionalUnits: 364,
            pendingFunctionalUnits: 59);
        AssertTimingSurfaceCoverage(
            timingRows,
            TimingSurfaceNames.Replacement,
            entries: 28,
    ...
    public async Task P6CompletionAuditKeepsEveryFunctionalUnitImplementedOrExplicitlyDeferred()
    {
        ...
        Assert.Equal(131, manualSpecs.Length);
        Assert.Equal(
            87,
        ...
        var implementedSpecs = specs
            .Where(spec => string.Equals(
                ...
                StringComparison.Ordinal))
            .ToArray();
        Assert.Equal(878, implementedSpecs.Length);
