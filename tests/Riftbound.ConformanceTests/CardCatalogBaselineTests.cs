...
public sealed class CardCatalogBaselineTests
{
    ...
    public async Task BehaviorSpecsCoverEveryOfficialCardWithExplicitStatusAndReason()
    {
        ...
        Assert.Equal(953, report.StatusCounts[BehaviorImplementationStatuses.Implemented]);
        Assert.Equal(56, report.StatusCounts[BehaviorImplementationStatuses.ManualRuleRequired]);
    ...
    public async Task P6FunctionalUnitCoverageAuditsSameTextVariantsAndReprints()
    {
        ...
        Assert.Equal(758, coverage.ImplementedUnits);
        Assert.Equal(53, coverage.ManualRuleRequiredUnits);
        Assert.Equal(0, coverage.UnimplementedUnits);
        Assert.Equal(113, coverage.DuplicateGroups);
        Assert.Equal(110, coverage.ImplementedDuplicateGroups);
        Assert.Equal(305, coverage.ImplementedDuplicateEntries);
        Assert.Equal(3, coverage.PendingDuplicateGroups);
        Assert.Equal(6, coverage.PendingDuplicateEntries);
    ...
    public async Task P6HighFrequencyTemplateFamiliesReportEntryAndFunctionalUnitCoverage()
    {
        ...
        AssertFamily(report, BehaviorTemplateIds.Draw, 131, 125, 6, 0, 114, 108, 6);
        AssertFamily(report, BehaviorTemplateIds.Damage, 148, 144, 4, 0, 129, 125, 4);
    ...
    public async Task P6SecondaryTemplateFamiliesReportEntryAndFunctionalUnitCoverage()
    {
        ...
        AssertFamily(report, BehaviorTemplateIds.Move, 136, 130, 6, 0, 111, 105, 6);
        AssertFamily(report, BehaviorTemplateIds.Recycle, 63, 59, 4, 0, 51, 47, 4);
        AssertFamily(report, BehaviorTemplateIds.Banish, 11, 11, 0, 0, 9, 9, 0);
        AssertFamily(report, BehaviorTemplateIds.TempMight, 292, 283, 9, 0, 230, 221, 9);
        AssertFamily(report, BehaviorTemplateIds.Boon, 66, 63, 3, 0, 48, 45, 3);
        Assert.All(report.Families, family =>
        {
            Assert.Equal(family.Entries, family.ImplementedEntries + family.ManualRuleRequiredEntries + family.UnimplementedEntries);
            Assert.Equal(family.FunctionalUnits, family.ImplementedFunctionalUnits + family.PendingFunctionalUnits);
    ...
    public async Task P6LifecycleTriggerReplacementFamiliesReportSpecAndExecutionBoundaryCoverage()
    {
        ...
        AssertTimingSurfaceCoverage(
            ...
            TimingSurfaceNames.Trigger,
            entries: 530,
            specImplementedEntries: 491,
            manualRuleRequiredEntries: 39,
            unimplementedEntries: 0,
            functionalUnits: 423,
            specImplementedFunctionalUnits: 385,
            pendingFunctionalUnits: 38);
        AssertTimingSurfaceCoverage(
            timingRows,
            TimingSurfaceNames.Replacement,
            entries: 28,
    ...
    public async Task P6BattlefieldRuleDomainSurfacesReportManualBoundaryCoverage()
    {
        ...
        var implementedBattlefieldSpec = Assert.Single(battlefieldSpecs, spec => string.Equals(
            spec.Status,
            BehaviorImplementationStatuses.Implemented,
            StringComparison.Ordinal));
        Assert.Equal("OGN·280/298", implementedBattlefieldSpec.CardNo);
        Assert.Equal(OfficialRuleDomainBehaviorCatalog.BattlefieldRuleDomainEffectKind, implementedBattlefieldSpec.ImplementedEffectKind);
        Assert.Equal("OGN·280/298", implementedBattlefieldSpec.ImplementedByCardNo);
        Assert.Contains("P7.9 battlefield rule domain", implementedBattlefieldSpec.Reason, StringComparison.Ordinal);
        Assert.False(CardBehaviorRegistry.TryGetByCardNo(implementedBattlefieldSpec.CardNo, out _));
    ...
    public async Task P6CompletionAuditKeepsEveryFunctionalUnitImplementedOrExplicitlyDeferred()
    {
        ...
        Assert.Equal(56, manualSpecs.Length);
        Assert.Equal(
            53,
        ...
        var implementedSpecs = specs
            .Where(spec => string.Equals(
                ...
                StringComparison.Ordinal))
            .ToArray();
        Assert.Equal(953, implementedSpecs.Length);
