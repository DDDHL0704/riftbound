...
{
    private const string CharmObjectId = "P1-SPELL-CHARM";
    private const string CharmCardNo = "OGN·043/298";
    private const string CharmTargetObjectId = "P2-BATTLEFIELD-UNIT";
    ...
    public async Task CharmPlayCardStalePromptReplayAfterStackPriorityStartsUsesRejectedCacheWithoutMutation()
    {
        ...
        var acceptedStackItem = AssertCharmStackPriorityState(accepted);
        ...
        AssertCharmStackPriorityState(replay, acceptedStackItem);
        ...
        AssertCharmStackPriorityState(duplicateRejected, acceptedStackItem);
        Assert.Equal(p1PromptAfterAccepted, MatchStateHasher.HashValue(session.PromptFor("P1")));
        Assert.Equal(p2PromptAfterAccepted, MatchStateHasher.HashValue(session.PromptFor("P2")));
        Assert.Equal(p1SnapshotAfterAccepted, MatchStateHasher.HashValue(session.SnapshotFor("P1")));
        Assert.Equal(p2SnapshotAfterAccepted, MatchStateHasher.HashValue(session.SnapshotFor("P2")));
        ...
        AssertCharmStackPriorityState(conflict, acceptedStackItem);
        Assert.Equal(p1PromptAfterAccepted, MatchStateHasher.HashValue(session.PromptFor("P1")));
        Assert.Equal(p2PromptAfterAccepted, MatchStateHasher.HashValue(session.PromptFor("P2")));
        Assert.Equal(p1SnapshotAfterAccepted, MatchStateHasher.HashValue(session.SnapshotFor("P1")));
        Assert.Equal(p2SnapshotAfterAccepted, MatchStateHasher.HashValue(session.SnapshotFor("P2")));
        ...
        ActionPromptDto prompt)
    {
        ...
        Assert.Equal(CharmObjectId, rawCommand.GetProperty("cardObjectId").GetString());
        Assert.Equal(CharmCardNo, rawCommand.GetProperty("cardNo").GetString());
        Assert.Equal(
            [CharmTargetObjectId],
    ...
    private static StackItemState AssertCharmStackPriorityState(
        ...
        StackItemState? expectedStackItem = null)
    {
        ...
        Assert.Equal(CharmObjectId, stackItem.SourceObjectId);
        Assert.Equal(CharmCardNo, stackItem.CardNo);
        Assert.Equal([CharmTargetObjectId], stackItem.TargetObjectIds);
