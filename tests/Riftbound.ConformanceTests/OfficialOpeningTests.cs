...
public sealed class OfficialOpeningTests
{
    ...
    [Fact]
    public Task PromptIdOnlyRecycleRuneWithFirstReadyPromptAfterFinalReadyRejectsWithoutMutation()
    {
        return AssertRecycleRuneFirstReadyEnvelopeAfterFinalReadyRejectsWithoutMutation(
            "official-recycle-rune-prompt-id-only-with-first-ready-prompt-after-final-ready-room",
            "recycle-rune-prompt-id-only-with-first-ready-prompt-after-final-ready",
            prompt => PromptIdOnlyBasicRawCommand(CommandTypes.RecycleRune, prompt),
            ErrorCodes.PromptExpired,
            "行动窗口已过期，请按最新提示重新提交。");
    }

    [Fact]
    public Task SnapshotOnlyRecycleRuneWithFirstReadyPromptAfterFinalReadyRejectsWithoutMutation()
    {
        return AssertRecycleRuneFirstReadyEnvelopeAfterFinalReadyRejectsWithoutMutation(
            "official-recycle-rune-snapshot-only-with-first-ready-prompt-after-final-ready-room",
            "recycle-rune-snapshot-only-with-first-ready-prompt-after-final-ready",
            prompt =>
            {
                Assert.NotNull(prompt.SnapshotTick);
    ...
    [Fact]
    public Task PromptIdOnlyHideCardWithFirstReadyPromptAfterFinalReadyRejectsWithoutMutation()
    {
        return AssertHideCardFirstReadyEnvelopeAfterFinalReadyRejectsWithoutMutation(
            "official-hide-card-prompt-id-only-with-first-ready-prompt-after-final-ready-room",
            "hide-card-prompt-id-only-with-first-ready-prompt-after-final-ready",
            prompt => PromptIdOnlyBasicRawCommand(CommandTypes.HideCard, prompt),
            ErrorCodes.PromptExpired,
            "行动窗口已过期，请按最新提示重新提交。");
    }

    [Fact]
    public Task SnapshotOnlyHideCardWithFirstReadyPromptAfterFinalReadyRejectsWithoutMutation()
    {
        return AssertHideCardFirstReadyEnvelopeAfterFinalReadyRejectsWithoutMutation(
            "official-hide-card-snapshot-only-with-first-ready-prompt-after-final-ready-room",
            "hide-card-snapshot-only-with-first-ready-prompt-after-final-ready",
            prompt =>
            {
                Assert.NotNull(prompt.SnapshotTick);
                return SnapshotOnlyBasicRawCommand(CommandTypes.HideCard, prompt.SnapshotTick.Value);
            },
            ErrorCodes.PhaseNotAllowed,
            "待命埋伏只能在当前玩家的开放主阶段提交。");
    }

    private static async Task AssertHideCardFirstReadyEnvelopeAfterFinalReadyRejectsWithoutMutation(
        string sessionName,
        string intentId,
        Func<ActionPromptDto, JsonElement> rawCommandFactory,
        string expectedErrorCode,
    ...
    [Fact]
    public async Task OfficialFirstTurnTapRuneDuplicateClientIntentReplaysExactRawButRejectsChangedRawPayload()
    {
        var journal = new RecordingMatchJournal();
        var context = await BuildFinalMulliganFirstTurnAuditContext(
            "official-first-turn-tap-rune-raw-intent-room",
            journal);
        ...
        var sourceIds = (tapRuneCandidate.Sources ?? []).Select(source => source.Id).ToArray();
        Assert.NotEmpty(sourceIds);
        var sourceId = sourceIds[0];
        Assert.Contains(sourceId, context.CalledRuneObjectIds);
        var command = new TapRuneCommand(sourceId);
        ...
        var journalEntryCountBeforeTapRune = journal.Entries.Count;

        var accepted = await context.Session.SubmitAsync(
            context.ActivePlayerId,
            "first-turn-tap-rune-raw-intent",
            command,
            rawCommand,
            CancellationToken.None);
    ...
    [Fact]
    public async Task OfficialFirstTurnSurrenderFreshEndTurnAfterMatchFinishedThrowsStableErrorWithoutMutation()
    {
        var journal = new RecordingMatchJournal();
        var context = await BuildFinalMulliganFirstTurnAuditContext(
            "official-first-turn-surrender-fresh-end-turn-after-match-finished-room",
            journal);
        ...
        var accepted = await context.Session.SubmitAsync(
            context.ActivePlayerId,
            "first-turn-surrender-before-fresh-end-turn-after-match-finished",
            new SurrenderCommand(),
        ...
        var journalEntryCountBeforeFreshEndTurn = journal.Entries.Count;

        var rejected = await Assert.ThrowsAsync<MatchSessionException>(() =>
            context.Session.SubmitAsync(
                context.ActivePlayerId,
                "fresh-end-turn-after-first-turn-surrender-match-finished",
                new EndTurnCommand(),
                RawCommandWithClientNote(CommandTypes.EndTurn, "fresh-after-match-finished"),
                CancellationToken.None).AsTask());

        Assert.Equal(ErrorCodes.MatchFinished, rejected.Code);
        Assert.Equal("对局已经结束，不能继续提交行动。", rejected.Message);
        Assert.Equal(journalEntryCountBeforeFreshEndTurn, journal.Entries.Count);
        Assert.DoesNotContain(journal.Entries, entry =>
            string.Equals(
    ...
    [Fact]
    public async Task OfficialFirstTurnSurrenderFreshPlayCardAfterMatchFinishedThrowsStableErrorWithoutMutation()
    {
        var journal = new RecordingMatchJournal();
        var context = await BuildFinalMulliganFirstTurnAuditContext(
            "official-first-turn-surrender-fresh-play-card-after-match-finished-room",
            journal);
        ...
        var accepted = await context.Session.SubmitAsync(
            context.ActivePlayerId,
            "first-turn-surrender-before-fresh-play-card-after-match-finished",
            new SurrenderCommand(),
            PromptScopedBasicRawCommand(CommandTypes.Surrender, firstTurnPrompt),
            CancellationToken.None);
        ...
        var activePromptBefore = JsonSerializer.Serialize(context.Session.PromptFor(context.ActivePlayerId));
        var secondPromptBefore = JsonSerializer.Serialize(context.Session.PromptFor(context.SecondPlayerId));
        var journalEntryCountBeforeFreshPlayCard = journal.Entries.Count;

        var rejected = await Assert.ThrowsAsync<MatchSessionException>(() =>
            context.Session.SubmitAsync(
                context.ActivePlayerId,
                "fresh-play-card-after-first-turn-surrender-match-finished",
                new PlayCardCommand("missing-source", "missing-card", []),
                RawCommandWithClientNote(CommandTypes.PlayCard, "fresh-play-card-after-match-finished"),
                CancellationToken.None).AsTask());

        Assert.Equal(ErrorCodes.MatchFinished, rejected.Code);
        Assert.Equal("对局已经结束，不能继续提交行动。", rejected.Message);
        Assert.Equal(journalEntryCountBeforeFreshPlayCard, journal.Entries.Count);
        Assert.DoesNotContain(journal.Entries, entry =>
            string.Equals(
                entry.ClientIntentId,
                "fresh-play-card-after-first-turn-surrender-match-finished",
    ...
    public async Task OfficialFirstTurnSurrenderFreshMoveUnitAfterMatchFinishedThrowsStableErrorWithoutMutation()
    {
        var journal = new RecordingMatchJournal();
        var context = await BuildFinalMulliganFirstTurnAuditContext(
            "official-first-turn-surrender-fresh-move-unit-after-match-finished-room",
            journal);
        ...
        var accepted = await context.Session.SubmitAsync(
            context.ActivePlayerId,
            "first-turn-surrender-before-fresh-move-unit-after-match-finished",
            new SurrenderCommand(),
            PromptScopedBasicRawCommand(CommandTypes.Surrender, firstTurnPrompt),
            CancellationToken.None);
        ...
        var activePromptBefore = JsonSerializer.Serialize(context.Session.PromptFor(context.ActivePlayerId));
        var secondPromptBefore = JsonSerializer.Serialize(context.Session.PromptFor(context.SecondPlayerId));
        var journalEntryCountBeforeFreshMoveUnit = journal.Entries.Count;

        var rejected = await Assert.ThrowsAsync<MatchSessionException>(() =>
            context.Session.SubmitAsync(
                context.ActivePlayerId,
                "fresh-move-unit-after-first-turn-surrender-match-finished",
                new MoveUnitCommand("missing-source", "missing-origin", "missing-destination"),
                RawCommandWithClientNote(CommandTypes.MoveUnit, "fresh-move-unit-after-match-finished"),
                CancellationToken.None).AsTask());

        Assert.Equal(ErrorCodes.MatchFinished, rejected.Code);
        Assert.Equal("对局已经结束，不能继续提交行动。", rejected.Message);
        Assert.Equal(journalEntryCountBeforeFreshMoveUnit, journal.Entries.Count);
        Assert.DoesNotContain(journal.Entries, entry =>
            string.Equals(
                entry.ClientIntentId,
                "fresh-move-unit-after-first-turn-surrender-match-finished",
    ...
    [Fact]
    public async Task OfficialFirstTurnSurrenderFreshAssembleEquipmentAfterMatchFinishedThrowsStableErrorWithoutMutation()
    {
        var journal = new RecordingMatchJournal();
        var context = await BuildFinalMulliganFirstTurnAuditContext(
            "official-first-turn-surrender-fresh-assemble-equipment-after-match-finished-room",
            journal);
        ...
        var accepted = await context.Session.SubmitAsync(
            context.ActivePlayerId,
            "first-turn-surrender-before-fresh-assemble-equipment-after-match-finished",
            new SurrenderCommand(),
            PromptScopedBasicRawCommand(CommandTypes.Surrender, firstTurnPrompt),
            CancellationToken.None);
        ...
        var activePromptBefore = JsonSerializer.Serialize(context.Session.PromptFor(context.ActivePlayerId));
        var secondPromptBefore = JsonSerializer.Serialize(context.Session.PromptFor(context.SecondPlayerId));
        var journalEntryCountBeforeFreshAssembleEquipment = journal.Entries.Count;

        var rejected = await Assert.ThrowsAsync<MatchSessionException>(() =>
            context.Session.SubmitAsync(
                context.ActivePlayerId,
                "fresh-assemble-equipment-after-first-turn-surrender-match-finished",
                new AssembleEquipmentCommand("missing-source", "missing-target"),
                RawCommandWithClientNote(CommandTypes.AssembleEquipment, "fresh-assemble-equipment-after-match-finished"),
                CancellationToken.None).AsTask());

        Assert.Equal(ErrorCodes.MatchFinished, rejected.Code);
        Assert.Equal("对局已经结束，不能继续提交行动。", rejected.Message);
        Assert.Equal(journalEntryCountBeforeFreshAssembleEquipment, journal.Entries.Count);
        Assert.DoesNotContain(journal.Entries, entry =>
            string.Equals(
                entry.ClientIntentId,
                "fresh-assemble-equipment-after-first-turn-surrender-match-finished",
    ...
    public async Task OfficialFirstTurnSurrenderFreshTapRuneAfterMatchFinishedThrowsStableErrorWithoutMutation()
    {
        var journal = new RecordingMatchJournal();
        var context = await BuildFinalMulliganFirstTurnAuditContext(
            "official-first-turn-surrender-fresh-tap-rune-after-match-finished-room",
            journal);
        ...
        var accepted = await context.Session.SubmitAsync(
            context.ActivePlayerId,
            "first-turn-surrender-before-fresh-tap-rune-after-match-finished",
            new SurrenderCommand(),
            PromptScopedBasicRawCommand(CommandTypes.Surrender, firstTurnPrompt),
            CancellationToken.None);
        ...
        var activePromptBefore = JsonSerializer.Serialize(context.Session.PromptFor(context.ActivePlayerId));
        var secondPromptBefore = JsonSerializer.Serialize(context.Session.PromptFor(context.SecondPlayerId));
        var journalEntryCountBeforeFreshTapRune = journal.Entries.Count;

        var rejected = await Assert.ThrowsAsync<MatchSessionException>(() =>
            context.Session.SubmitAsync(
                context.ActivePlayerId,
                "fresh-tap-rune-after-first-turn-surrender-match-finished",
                new TapRuneCommand("missing-rune-after-finished"),
                RawCommandWithClientNote(CommandTypes.TapRune, "fresh-tap-rune-after-match-finished"),
                CancellationToken.None).AsTask());

        Assert.Equal(ErrorCodes.MatchFinished, rejected.Code);
        Assert.Equal("对局已经结束，不能继续提交行动。", rejected.Message);
        Assert.Equal(journalEntryCountBeforeFreshTapRune, journal.Entries.Count);
        Assert.DoesNotContain(journal.Entries, entry =>
            string.Equals(
                entry.ClientIntentId,
                "fresh-tap-rune-after-first-turn-surrender-match-finished",
    ...
    [Fact]
    public async Task OfficialFirstTurnSurrenderFreshRecycleRuneAfterMatchFinishedThrowsStableErrorWithoutMutation()
    {
        var journal = new RecordingMatchJournal();
        var context = await BuildFinalMulliganFirstTurnAuditContext(
            "official-first-turn-surrender-fresh-recycle-rune-after-match-finished-room",
            journal);
        ...
        var accepted = await context.Session.SubmitAsync(
            context.ActivePlayerId,
            "first-turn-surrender-before-fresh-recycle-rune-after-match-finished",
            new SurrenderCommand(),
            PromptScopedBasicRawCommand(CommandTypes.Surrender, firstTurnPrompt),
            CancellationToken.None);
        ...
        var activePromptBefore = JsonSerializer.Serialize(context.Session.PromptFor(context.ActivePlayerId));
        var secondPromptBefore = JsonSerializer.Serialize(context.Session.PromptFor(context.SecondPlayerId));
        var journalEntryCountBeforeFreshRecycleRune = journal.Entries.Count;

        var rejected = await Assert.ThrowsAsync<MatchSessionException>(() =>
            context.Session.SubmitAsync(
                context.ActivePlayerId,
                "fresh-recycle-rune-after-first-turn-surrender-match-finished",
                new RecycleRuneCommand("missing-rune-after-finished"),
                RawCommandWithClientNote(CommandTypes.RecycleRune, "fresh-recycle-rune-after-match-finished"),
                CancellationToken.None).AsTask());

        Assert.Equal(ErrorCodes.MatchFinished, rejected.Code);
        Assert.Equal("对局已经结束，不能继续提交行动。", rejected.Message);
        Assert.Equal(journalEntryCountBeforeFreshRecycleRune, journal.Entries.Count);
        Assert.DoesNotContain(journal.Entries, entry =>
            string.Equals(
                entry.ClientIntentId,
                "fresh-recycle-rune-after-first-turn-surrender-match-finished",
    ...
    [Fact]
    public async Task OfficialFirstTurnSurrenderFreshHideCardAfterMatchFinishedThrowsStableErrorWithoutMutation()
    {
        var journal = new RecordingMatchJournal();
        var context = await BuildFinalMulliganFirstTurnAuditContext(
            "official-first-turn-surrender-fresh-hide-card-after-match-finished-room",
            journal);
        ...
        var accepted = await context.Session.SubmitAsync(
            context.ActivePlayerId,
            "first-turn-surrender-before-fresh-hide-card-after-match-finished",
            new SurrenderCommand(),
            PromptScopedBasicRawCommand(CommandTypes.Surrender, firstTurnPrompt),
            CancellationToken.None);
        ...
        var activePromptBefore = JsonSerializer.Serialize(context.Session.PromptFor(context.ActivePlayerId));
        var secondPromptBefore = JsonSerializer.Serialize(context.Session.PromptFor(context.SecondPlayerId));
        var journalEntryCountBeforeFreshHideCard = journal.Entries.Count;

        var rejected = await Assert.ThrowsAsync<MatchSessionException>(() =>
            context.Session.SubmitAsync(
                context.ActivePlayerId,
                "fresh-hide-card-after-first-turn-surrender-match-finished",
                new HideCardCommand("missing-source", "missing-card"),
                RawCommandWithClientNote(CommandTypes.HideCard, "fresh-hide-card-after-match-finished"),
                CancellationToken.None).AsTask());

        Assert.Equal(ErrorCodes.MatchFinished, rejected.Code);
        Assert.Equal("对局已经结束，不能继续提交行动。", rejected.Message);
        Assert.Equal(journalEntryCountBeforeFreshHideCard, journal.Entries.Count);
        Assert.DoesNotContain(journal.Entries, entry =>
            string.Equals(
                entry.ClientIntentId,
                "fresh-hide-card-after-first-turn-surrender-match-finished",
    ...
    [Fact]
    public async Task OfficialFirstTurnSurrenderFreshRevealCardAfterMatchFinishedThrowsStableErrorWithoutMutation()
    {
        var journal = new RecordingMatchJournal();
        var context = await BuildFinalMulliganFirstTurnAuditContext(
            "official-first-turn-surrender-fresh-reveal-card-after-match-finished-room",
            journal);
        ...
        var accepted = await context.Session.SubmitAsync(
            context.ActivePlayerId,
            "first-turn-surrender-before-fresh-reveal-card-after-match-finished",
            new SurrenderCommand(),
            PromptScopedBasicRawCommand(CommandTypes.Surrender, firstTurnPrompt),
            CancellationToken.None);
        ...
        var activePromptBefore = JsonSerializer.Serialize(context.Session.PromptFor(context.ActivePlayerId));
        var secondPromptBefore = JsonSerializer.Serialize(context.Session.PromptFor(context.SecondPlayerId));
        var journalEntryCountBeforeFreshRevealCard = journal.Entries.Count;

        var rejected = await Assert.ThrowsAsync<MatchSessionException>(() =>
            context.Session.SubmitAsync(
                context.ActivePlayerId,
                "fresh-reveal-card-after-first-turn-surrender-match-finished",
                new RevealCardCommand("missing-source", "missing-card", []),
                RawCommandWithClientNote(CommandTypes.RevealCard, "fresh-reveal-card-after-match-finished"),
                CancellationToken.None).AsTask());

        Assert.Equal(ErrorCodes.MatchFinished, rejected.Code);
        Assert.Equal("对局已经结束，不能继续提交行动。", rejected.Message);
        Assert.Equal(journalEntryCountBeforeFreshRevealCard, journal.Entries.Count);
        Assert.DoesNotContain(journal.Entries, entry =>
            string.Equals(
                entry.ClientIntentId,
                "fresh-reveal-card-after-first-turn-surrender-match-finished",
