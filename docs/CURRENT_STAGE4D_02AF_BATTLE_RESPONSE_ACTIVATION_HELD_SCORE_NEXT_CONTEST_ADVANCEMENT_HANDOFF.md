# Stage 4D-02AF Battle Response Activation Held-Score Next Contest Advancement Handoff
...
4D-02AD / 4D-02AE 已验收 actual Shadow activation / stack resolution / returned response 后 held-score `RECYCLE_RUNE:*` 与 `TEMP_PAYMENT_RESOURCE:*` payment-resource context preservation，并补了 no-response normalization。它们证明了单一 current battlefield 的 held-score payment / audit path，但没有把该 held-score branch 组合进 BF-A / BF-B 双争夺战场队列推进。

4D-02AF 锁定一个窄的 next-contested advancement representative：
...
- returned response pass-pass 后进入 held-score branch，消费 temporary payment resource 并让 P2 得分；
...
This composes 4D-02AE temporary held-score payment-resource context with existing next-contested battlefield advancement guards. Passing either side alone is not enough.
...
1. Extend or locally compose `BuildHeldScoreTemporaryPaymentResourceNaturalStartBattleState()` so it also includes the existing BF-B next-contest fixtures:
   - `NextBattlefieldObjectId`
   - `NextAttackerObjectId`
   - `NextDefenderObjectId`
2. P2 should start with `new RunePool(1, 4)` so Shadow can spend 1 mana / 1 power and the final held-score branch can still pay 4 power using the temporary resource.
3. Set:
...
9. Assert final BF-A branch:
   ...
   - `BATTLEFIELD_HELD` for BF-A happens before held-score trigger / payment / score;
   - `TEMPORARY_PAYMENT_RESOURCE_SPENT` and `TEMPORARY_PAYMENT_RESOURCE_CLEARED` preserve `HeldScoreTemporaryResourceId`;
...
11. Assert final cleanup:
    ...
    - `TemporaryPaymentResources` is empty;
...
This slice should be test-first. Runtime changes are allowed only if the new cross-product exposes that held-score branch completion fails to advance BF-B, advances BF-B too early, leaks the internal battle-response declaration context, leaves stale prompts, or mishandles the temporary payment resource during final resume.
