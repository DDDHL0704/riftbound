# Stage 4D-18DA Recovery Timing Temporary Payment Resource Ability Id Whitespace Value Audit
...
Added `RecoveryValidatorRejectsSpectatorReplayTimingTemporaryPaymentResourceKeyedAbilityIdWhitespaceValueWithCountMismatch` in `tests/Riftbound.ConformanceTests/MatchRecoveryTests.cs`.

The test builds an authoritative temporary payment resource from real `MatchState` source-object state, keeps the emitted spectator payload keyed to `temp-payment-resource-1`, changes `abilityId` to a whitespace-only value, then appends `temp-payment-resource-extra` to force resource-count mismatch.

It proves validation still emits:

- optional scalar required diagnostics for the whitespace-only `abilityId` payload;
...
`abilityId` is an optional payload field, so missing or null values are tolerated at the payload layer. The whitespace-only case is invalid scalar content, and the keyed authoritative comparison still rejects it when the authoritative resource has `TEST_TEMP_RESOURCE_ABILITY`.

## Validation

- Focused new test: `1/1`.
- Focused `TemporaryPaymentResource` filter: `100/100`.
- Focused recovery filter: `1133/1133`.
- Adjacent recovery/official-opening/Postgres recovery-store filter: `1714/1714`.
- Backend full: `7079/7079`.
