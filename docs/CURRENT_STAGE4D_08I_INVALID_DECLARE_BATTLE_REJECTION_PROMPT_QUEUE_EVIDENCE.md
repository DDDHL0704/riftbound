# Stage 4D-08I Invalid Declare Battle Rejection Prompt Queue Evidence
...
- `docs/CURRENT_STAGE4D_08I_INVALID_DECLARE_BATTLE_REJECTION_PROMPT_QUEUE_AUDIT.md`
- `docs/CURRENT_STAGE4D_08I_INVALID_DECLARE_BATTLE_REJECTION_PROMPT_QUEUE_EVIDENCE.md`

## Validation

```bash
/Users/dinghaolin/.dotnet/dotnet test Riftbound.slnx --no-restore --filter "FullyQualifiedName~ActiveStartBattleRejectsInvalidDeclareBattleWithoutMutation"
```
...
Runtime changed: no. Existing invalid `DECLARE_BATTLE` rejection paths already preserved state, pending queue and prompts.
