...
当前第二百四十五批是启动技能来源控制权错误文案运行时收口批。`CoreRuleEngine` 在 Vi / Xerath 代表性 `ACTIVATE_ABILITY` 来源不由当前玩家控制时拒绝并返回中文错误；核心回归测试断言不含 raw action kind；后端 full test 当前 HEAD 通过 3141/3141。此前第二百三十二批 Chrome 插件房间 `smoke-battlefield-held-score-1778247059745` 已覆盖战场得分 UI 代表路径。
