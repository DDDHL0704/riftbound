...
当前第二百五十批是启动技能未开放路径错误文案运行时收口批。`CoreRuleEngine` 在 `ACTIVATE_ABILITY` ability id 尚未进入服务端已开放技能目录时返回中文“当前启动技能路径尚未由服务端开放。”，并断言错误消息不含 raw `ACTIVATE_ABILITY`；相关 deferred surface 回归通过 8/8，后端 full test 当前 HEAD 通过 3146/3146，前端 build 通过。此前第二百三十二批 Chrome 插件房间 `smoke-battlefield-held-score-1778247059745` 已覆盖战场得分 UI 代表路径。
