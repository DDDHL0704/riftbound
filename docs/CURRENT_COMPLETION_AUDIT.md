...
当前第二百五十三批是声明战斗未开放路径错误文案运行时收口批。`CoreRuleEngine` 在 `DECLARE_BATTLE` 未进入当前代表性最小战斗计划时返回中文“当前声明战斗路径尚未由服务端开放。”；声明战斗相关回归通过 56/56，后端 full test 当前 HEAD 通过 3147/3147，前端 build 通过。此前第二百三十二批 Chrome 插件房间 `smoke-battlefield-held-score-1778247059745` 已覆盖战场得分 UI 代表路径。
