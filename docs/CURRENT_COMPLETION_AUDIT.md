...
当前第二百二十三批是官方起手调整候选 tooltip 展示收口批：服务端 `MULLIGAN` source choice reason 改为“起手调整候选”，前端起手候选按钮缺少 reason 时也只显示“服务端起手候选”，不再回退到 `choice.id`。验证已通过：`source scripts/dev-env.sh && dotnet build Riftbound.slnx --no-restore`；`source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-build --filter "FullyQualifiedName~OfficialDeckSubmitReadyAndMulliganFlowWorksThroughHub"`；`source ../../scripts/dev-env.sh && npm run build`。
