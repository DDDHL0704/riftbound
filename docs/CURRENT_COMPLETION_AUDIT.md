...
当前第二百五十一批是战场静态禁止出牌错误文案运行时收口批。`CoreRuleEngine` 在战场静态效果禁止单位打出到目标战场时返回中文“战场效果禁止将单位打出到该战场。”，Core 与 Hub 断言错误消息不含 raw `PLAY_CARD`；目标回归通过 2/2，后端 full test 当前 HEAD 通过 3146/3146，前端 build 通过。此前第二百三十二批 Chrome 插件房间 `smoke-battlefield-held-score-1778247059745` 已覆盖战场得分 UI 代表路径。
