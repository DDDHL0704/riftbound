...
当前第二百四十三批是出牌来源错误文案运行时收口批。`CoreRuleEngine` 在出牌来源不在自己手牌、来源缺少服务端确认牌号、提交牌号与来源身份不匹配或来源不由当前玩家控制时拒绝 `PLAY_CARD` 并返回中文错误；核心回归测试断言不含 raw action kind；后端 full test 当前 HEAD 通过 3141/3141。此前第二百三十二批 Chrome 插件房间 `smoke-battlefield-held-score-1778247059745` 已覆盖战场得分 UI 代表路径。
