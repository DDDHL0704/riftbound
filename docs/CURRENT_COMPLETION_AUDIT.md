...
当前第二百二十四批是事件/错误日志 hover 展示收口批：`EventLog` 与 `RoomPage` 不再把 `event.kind` 或 `error.code` 放入标题 tooltip，日志标题只展示中文 formatter 结果。验证已通过：静态扫描 `title={event.kind}` / `title={error.code}` 为 0；`source ../../scripts/dev-env.sh && npm run build`。
