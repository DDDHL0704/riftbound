...
当前第二百三十四批是 active battle task 错误文案运行时收口批。`CoreRuleEngine` 在错误战场的 `DECLARE_BATTLE` 命令上返回中文“声明战斗必须匹配当前争夺战场的开始战斗任务。”，对应测试断言不含 raw `DECLARE_BATTLE` / `START_BATTLE`；后端 full test 当前 HEAD 通过 3140/3140。此前第二百三十二批 Chrome 插件房间 `smoke-battlefield-held-score-1778247059745` 已覆盖战场得分 UI 代表路径。
