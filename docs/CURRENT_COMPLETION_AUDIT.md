...
当前第二百二十五批是卡牌详情操作选择 tooltip 展示收口批：`CardDetailDrawer` 的选择按钮 title 统一经过 `choiceTitle`，中文服务端原因保留，明显内部英文说明或协议 token 降级为“服务端候选”，不再直接显示历史 internal reason。验证已通过：`source ../../scripts/dev-env.sh && npm run build`；静态扫描不再存在 `title={choice.reason ?? undefined}`。
