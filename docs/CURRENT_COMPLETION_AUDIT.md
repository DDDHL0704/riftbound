...
当前第二百四十四批是全局已结束对局错误文案运行时收口批。`CoreRuleEngine.ResolveAsync` 在对局已结束后拒绝任意命令并返回中文错误；回归覆盖结束后提交 `END_TURN` 与 `MULLIGAN`，并断言不含 raw action kind；后端 full test 当前 HEAD 通过 3141/3141。此前第二百三十二批 Chrome 插件房间 `smoke-battlefield-held-score-1778247059745` 已覆盖战场得分 UI 代表路径。
