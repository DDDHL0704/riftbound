...
当前第二百四十八批是伏击出牌未开放路径错误文案运行时收口批。`CoreRuleEngine` 在 `PLAY_CARD` 的 `AMBUSH` 模式进入未开放/非法伏击出牌路径时仍拒绝并保持状态不变，错误消息改为中文“当前伏击出牌路径尚未由服务端开放。”；伏击相关回归通过 21/21，后端 full test 当前 HEAD 通过 3146/3146，前端 build 通过。此前第二百三十二批 Chrome 插件房间 `smoke-battlefield-held-score-1778247059745` 已覆盖战场得分 UI 代表路径。
