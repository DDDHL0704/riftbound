...
当前第二百五十二批是传奇行动未开放与非法时点错误文案运行时收口批。`CoreRuleEngine` 在未知传奇行动 ability id 或当前时点不允许使用该传奇行动时返回中文错误，并断言错误消息不含 raw `LEGEND_ACT`；目标回归通过 2/2，后端 full test 当前 HEAD 通过 3147/3147，前端 build 通过。此前第二百三十二批 Chrome 插件房间 `smoke-battlefield-held-score-1778247059745` 已覆盖战场得分 UI 代表路径。
