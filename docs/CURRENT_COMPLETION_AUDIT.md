...
当前第二百四十批是待命翻开/反应错误文案运行时收口批。`CoreRuleEngine` 在来源不在自己基地、非法模式、窗口不合法、无优先权、来源非面朝下、身份未知/不匹配、来源不受控或来源没有待命关键词时拒绝 `REVEAL_CARD` 并返回中文错误；常规翻开与反应回归测试断言不含 raw action kind；后端 full test 当前 HEAD 通过 3140/3140。此前第二百三十二批 Chrome 插件房间 `smoke-battlefield-held-score-1778247059745` 已覆盖战场得分 UI 代表路径。
