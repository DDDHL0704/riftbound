...
- `src/Riftbound.Engine/OfficialDeckRules.cs`
...
当前第二百五十七批是正式卡组与房间会话错误文案运行时收口批。`OfficialDeckValidator` 与 `MatchSession` 在非法卡组、未开局/已结束提交、未知命令、重复 intent 和正式房间未提交卡组准备时返回中文玩家文案；相关回归通过 16/16，后端 full test 当前 HEAD 通过 3154/3154，前端 build 通过。此前第二百三十二批 Chrome 插件房间 `smoke-battlefield-held-score-1778247059745` 已覆盖战场得分 UI 代表路径。
...
第二百五十七批无前端运行时代码变更，仅收口服务端正式卡组/房间错误文案和回归测试；本批未启动新的 API/Vite/Chrome smoke，提交前目标端口保持无监听。
