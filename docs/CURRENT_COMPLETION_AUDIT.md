...
当前第二百三十五批是 pass priority/focus 错误文案运行时收口批。`CoreRuleEngine` 在非法窗口的 `PASS_PRIORITY` / `PASS_FOCUS` 命令上返回中文错误，测试断言不含 raw action kind；后端 full test 当前 HEAD 通过 3140/3140。此前第二百三十二批 Chrome 插件房间 `smoke-battlefield-held-score-1778247059745` 已覆盖战场得分 UI 代表路径。
