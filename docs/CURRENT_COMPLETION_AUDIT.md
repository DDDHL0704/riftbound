...
当前第二百五十五批是战场效果目标选择错误文案运行时收口批。`CoreRuleEngine` 在普通战场误带目标、防守方坚守战场非法目标、以及防守方召回战场过量目标时返回中文玩家文案；相关 Hub seed 回归通过 3/3，后端 full test 当前 HEAD 通过 3153/3153，前端 build 通过。此前第二百三十二批 Chrome 插件房间 `smoke-battlefield-held-score-1778247059745` 已覆盖战场得分 UI 代表路径。
