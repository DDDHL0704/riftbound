...
当前第二百三十七批是结束回合错误文案运行时收口批。`CoreRuleEngine` 在非当前玩家、非开放主阶段或结算链未清空时拒绝 `END_TURN` 并返回中文错误；非当前玩家与关闭窗口回归测试断言不含 raw action kind；后端 full test 当前 HEAD 通过 3140/3140。此前第二百三十二批 Chrome 插件房间 `smoke-battlefield-held-score-1778247059745` 已覆盖战场得分 UI 代表路径。
