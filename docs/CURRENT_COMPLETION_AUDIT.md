...
当前第二百四十九批是回合开始非回合玩家推进错误文案运行时收口批。`CoreRuleEngine` 在 `TURN_START` 阶段拒绝非当前回合玩家推进时返回中文“回合开始只能由当前回合玩家推进。”，并断言错误消息不含 raw `TURN_START`；目标回归通过 1/1，后端 full test 当前 HEAD 通过 3146/3146，前端 build 通过。此前第二百三十二批 Chrome 插件房间 `smoke-battlefield-held-score-1778247059745` 已覆盖战场得分 UI 代表路径。
