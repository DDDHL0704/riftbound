...
当前第二百三十六批是符文资源动作错误文案运行时收口批。`CoreRuleEngine` 在 `TAP_RUNE` / `RECYCLE_RUNE` 的错误窗口、非法来源、已横置符文、缺少服务端确认牌号和非特性符文路径上返回中文错误；未知牌号回归测试断言不含 raw action kind；后端 full test 当前 HEAD 通过 3140/3140。此前第二百三十二批 Chrome 插件房间 `smoke-battlefield-held-score-1778247059745` 已覆盖战场得分 UI 代表路径。
