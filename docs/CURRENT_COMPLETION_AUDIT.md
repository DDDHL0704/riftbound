...
当前第二百二十七批是规则队列 fallback 展示收口批：`StackPanel` 的未知 phase、battle result 与 battlefield result kind 不再直接回显 raw enum，协议 token 降级为中文服务端占位，自然文本仍经过内部 ID 脱敏。验证已通过：`source ../../scripts/dev-env.sh && npm run build`。
