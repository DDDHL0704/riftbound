...
当前第二百四十七批是装备装配未开放路径错误文案运行时收口批。`CoreRuleEngine` 在装配来源/目标/窗口/费用未进入服务端代表性最小计划时仍拒绝并保持状态不变，错误消息改为中文“当前装备装配路径尚未由服务端开放。”；装配相关回归通过 28/28，后端 full test 当前 HEAD 通过 3146/3146，前端 build 通过。此前第二百三十二批 Chrome 插件房间 `smoke-battlefield-held-score-1778247059745` 已覆盖战场得分 UI 代表路径。
