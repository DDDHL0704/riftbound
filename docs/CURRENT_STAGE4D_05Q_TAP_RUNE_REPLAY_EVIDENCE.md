# Stage 4D-05Q Tap Rune Replay Evidence
...
- `docs/CURRENT_STAGE4D_05Q_TAP_RUNE_REPLAY_AUDIT.md`
- `docs/CURRENT_STAGE4D_05Q_TAP_RUNE_REPLAY_EVIDENCE.md`

## Validation

```bash
source scripts/dev-env.sh && dotnet test tests/Riftbound.ConformanceTests/Riftbound.ConformanceTests.csproj --no-restore --filter "FullyQualifiedName~CoreRuleEngineRejectsAcceptedTapRuneReplayWithoutMutation"
```

Result: passed `1/1`.

```bash
source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore --filter "FullyQualifiedName~TapRune|FullyQualifiedName~RecycleRune|FullyQualifiedName~MatchStateHasher"
```
