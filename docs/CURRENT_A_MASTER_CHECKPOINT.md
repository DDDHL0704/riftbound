...
状态：**focused foundation accepted；4D-03B focused slice accepted；4D-03C focused slice accepted；4D-03D handoff / baseline ready；项目仍 NOT READY。**
...
- 派发 4D-03D：让代表性 `ACTIVATE_ABILITY` 支付窗口支持 `RECYCLE_RUNE:*` payment resource action，并保持 prompt quote / command commit / `COST_PAID` audit parity。
