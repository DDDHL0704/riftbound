...
状态：**focused foundation accepted；4D-03B focused slice accepted；4D-03C focused slice accepted；4D-03D focused slice accepted；4D-03E focused slice accepted；4D-03F handoff ready；项目仍 NOT READY。**
...
- Adjacent baseline：229/229 通过。

4D-03F 当前目标：

- 让普通 pending `PAY_COST` 支持合法 `RECYCLE_RUNE:*` payment resource action 与 `SPEND_POWER:*` / typed `SPEND_POWER:<trait>:<amount>` 在同一支付窗口结算。
- 保持现有 mana-only `TRIGGER_PAYMENT` 代表路径兼容，不把当前 battlefield conquer / Vayne / Icevale / Jax trigger payment 改造成 power-cost trigger。
- 本批不修改前端、卡牌矩阵或未跟踪的 `riftbound-dotnet.sln`，不关闭 P0-005 full official。

下一步：

- 派发 B / Maxwell 实现 4D-03F pending `PAY_COST` resource action foundation，并由 A 复跑 focused / adjacent / backend full / `git diff --check` 后再收口审计与证据。
