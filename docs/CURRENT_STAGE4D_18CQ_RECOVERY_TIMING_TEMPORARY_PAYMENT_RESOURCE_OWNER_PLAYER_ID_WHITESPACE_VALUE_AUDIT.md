# Stage 4D-18CQ Recovery Timing Temporary Payment Resource Owner Player Id Whitespace Value Audit
...
Added `RecoveryValidatorRejectsSpectatorReplayTimingTemporaryPaymentResourceKeyedOwnerPlayerIdWhitespaceValueWithCountMismatch` in `tests/Riftbound.ConformanceTests/MatchRecoveryTests.cs`.

The test builds an authoritative temporary payment resource from real `MatchState` source-object state, keeps the emitted spectator payload keyed to `temp-payment-resource-1`, changes `ownerPlayerId` to a whitespace-only string, then appends `temp-payment-resource-extra` to force resource-count mismatch.
...
- Focused new test: `1/1` after tightening the mismatch assertion to stable diagnostic fragments.
- Focused `TemporaryPaymentResource` filter: `90/90`.
- Focused recovery filter: `1123/1123`.
- Adjacent recovery/official-opening/Postgres recovery-store filter: `1704/1704`.
- Backend full: `7069/7069`.
