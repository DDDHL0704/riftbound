# Stage 4D-08L Declare Battle Cleanup Block Prompt Queue Evidence
...
- `docs/CURRENT_STAGE4D_08L_DECLARE_BATTLE_CLEANUP_BLOCK_PROMPT_QUEUE_AUDIT.md`
- `docs/CURRENT_STAGE4D_08L_DECLARE_BATTLE_CLEANUP_BLOCK_PROMPT_QUEUE_EVIDENCE.md`

## Validation

```bash
/Users/dinghaolin/.dotnet/dotnet test Riftbound.slnx --no-restore --filter "FullyQualifiedName~ImmediateDeclareBattleDoesNotAdvanceNextContestedBattlefieldWhenCleanupBlocks"
```
...
Runtime changed: no. Existing declare-battle close / cleanup-block transition already kept BF-2 queued behind state-based cleanup.
