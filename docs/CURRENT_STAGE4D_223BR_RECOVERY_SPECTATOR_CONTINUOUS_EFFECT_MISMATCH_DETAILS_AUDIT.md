# Stage 4D-223BR Recovery Spectator Continuous Effect Mismatch Details Audit
...
Branch / commit: `main`, code commit `7dd4bb5b`
...
`MatchRecoveryValidator` now appends stable expected/actual details to spectator replay-frame timing continuous-effect keyed authoritative mismatch diagnostics. The existing diagnostic prefix remains intact, while the suffix reports strings, integers, string lists, `<missing>`, `<unreadable>`, `<empty>` or `[]` as applicable.
...
- Focused `RecoveryValidatorRejectsSpectatorReplayTimingContinuousEffectKeyedPropertyNameWithoutCountMismatch`: `1/1`.
...
External UI followup worktree `/Users/dinghaolin/MyProjects/riftbound-codex-ui-followup-20260616` on `codex/ui-followup-20260616` was clean at `01364ee2`, 61 commits behind current `main` after the code commit and with no commits ahead of `main`.
