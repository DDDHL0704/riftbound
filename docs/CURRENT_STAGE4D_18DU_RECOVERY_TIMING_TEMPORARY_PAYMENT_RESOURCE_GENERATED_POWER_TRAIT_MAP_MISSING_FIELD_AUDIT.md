# Stage 4D-18DU Recovery Timing Temporary Payment Resource Generated Power Trait Map Missing Field Audit
...
Added `RecoveryValidatorRejectsSpectatorReplayTimingTemporaryPaymentResourceKeyedGeneratedPowerTraitMapMissingFieldWithCountMismatch` in `tests/Riftbound.ConformanceTests/MatchRecoveryTests.cs`.

The test builds an authoritative temporary payment resource from real `MatchState` source-object state, keeps the emitted spectator payload keyed to `temp-payment-resource-1`, removes the emitted `generatedPowerByTrait` map, then appends `temp-payment-resource-extra` to force resource-count mismatch.
...
This closes the focused generated-power trait-map missing-field variant, keeping keyed authoritative generated-power trait validation independently locked for spectator timing temporary-payment-resource payloads when the spectator trait map is absent.

## Validation

- Focused new test: `1/1`.
- Focused `TemporaryPaymentResource` filter: `120/120`.
- Focused recovery filter: `1153/1153`.
- Adjacent recovery/official-opening/Postgres recovery-store filter: `1734/1734`.
- Backend full: `7099/7099`.
