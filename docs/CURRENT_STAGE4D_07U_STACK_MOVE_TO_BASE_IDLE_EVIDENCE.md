# Stage 4D-07U Stack Move To Base Idle Evidence
...
- `docs/CURRENT_STAGE4D_07U_STACK_MOVE_TO_BASE_IDLE_AUDIT.md`
- `docs/CURRENT_STAGE4D_07U_STACK_MOVE_TO_BASE_IDLE_EVIDENCE.md`

## Validation

```bash
/Users/dinghaolin/.dotnet/dotnet test Riftbound.slnx --no-restore --filter "FullyQualifiedName~StackResolutionMoveToBaseCanRemoveContestAndReturnQueueToIdle"
```
...
Runtime changed: no. Existing stack resolution already resolves the move-to-base effect, removes the contest and leaves the pending task queue idle.
