...
Stage 4D-03D activate ability payment resource handoff / baseline 新增，见 `docs/CURRENT_STAGE4D_03D_PAYMENT_ENGINE_ACTIVATE_RESOURCE_HANDOFF.md` 与 `docs/CURRENT_STAGE4D_03D_PAYMENT_ENGINE_ACTIVATE_RESOURCE_BASELINE_EVIDENCE.md`；focused baseline 79/79、adjacent baseline 252/252 通过。该基线只证明当前 `ACTIVATE_ABILITY`、基础符文资源技能和既有 payment-resource 代表路径绿色；下一实现片只处理 Vi / Xerath `ACTIVATE_ABILITY` 的 `RECYCLE_RUNE:*` payment resource action，不关闭 P0-005。
