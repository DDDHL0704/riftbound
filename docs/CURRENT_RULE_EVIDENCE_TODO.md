...
| `ORDER_TRIGGERS` | `CORE-260330` rules 333-340, 383.3.d-383.3.e；`JFAQ-251023` q2.2-q2.3, q2.5 | 阶段 3D 已补最小 runtime window / UI / evidence；阶段 4C-1 已补保守 APNAP controller-block 子集、battle initial stack 代表路径和 face-down standby source 脱敏；阶段 4C-2 已补 Watchful Sentinel 多触发真实 `UNIT_DESTROYED` -> `TriggerQueue` -> `ORDER_TRIGGERS` -> `StackItems` -> priority -> draw 代表路径，以及非法排序 no mutation 复核；完整 trigger engine、其他触发族、state-based cleanup 触发统一入队、trigger payment / decline、完整 effect resolution、FAQ regression、1009/811 full-official 仍缺 | B 主实现；E 触发族 / FAQ fixture；C 只提交服务端 prompt；D 文档 | 以 4C-2 真实入队代表路径为基线，继续扩 last-breath / friendly-destroyed / attack / conquer 触发族、cleanup 统一入队、触发费用拒付、effect resolution 和 FAQ 回归 |
...
- P0：其他 last-breath / friendly-destroyed / attack / conquer 触发族。
...
- D-FrontendContract：继续把正式复杂 prompt 字段草案拆成 `PAY_COST`、`ORDER_TRIGGERS`、`ASSIGN_COMBAT_DAMAGE`、`SPELL_DUEL_ACTION` 四张契约清单；标注 `PAY_COST` 只有最小 runtime、完整 PaymentEngine 未开放，`ASSIGN_COMBAT_DAMAGE` 只有最小 runtime、完整全规则矩阵未开放，`ORDER_TRIGGERS` 已到 Watchful Sentinel 真实多触发入队代表路径但完整 trigger engine / 其他触发族 / state-based cleanup 触发统一入队 / effect resolution / trigger payment / FAQ regression 未开放，`SPELL_DUEL_ACTION` 仍没有完整 runtime prompt，现有降级面板只能临时承接展示。
...
- E-Conformance：为仍未清零 P0 建最小官方场景，优先覆盖失控待命延迟移除、恶意收购非战斗法术对决、完整 APNAP 多玩家独立排序、其他 last-breath / friendly-destroyed / attack / conquer 触发族、state-based cleanup 触发统一入队、触发费用拒付、多单位伤害分配、战斗清理与征服/据守得分。
