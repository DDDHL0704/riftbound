...
- 2026-06-29 Plan B follow-up: the former Spirit Fire effect-specific target guard has been folded into shared `RequiresVisibleFieldUnitPrimitiveTarget` rules. `CoreRuleEngine` and `MatchSession` no longer branch on `SPIRIT_FIRE_DESTROY_BATTLEFIELD_UNITS_TOTAL_POWER_4`; the existing `DestroysTarget` + default `BATTLEFIELD_UNIT` scope + `MaxTotalTargetPower=4` metadata now drives public battlefield-unit target filtering and total-power legal selections.
- Validation: focused Spirit Fire / TargetGuard / Spellshield filter passed 48/48; ActionPrompt / Prompt / SpiritFire regression filter passed 112/112; backend full passed 3690/3690; frontend build passed; Chrome smoke passed. 2026-06-29 follow-up validation: `PrimitiveTargetGuardSourceTests|SpiritFireDestroyGuardTests` 14/14 passed; `PrimitiveTargetGuardSourceTests|SpiritFireDestroyGuardTests|CoreRuleEnginePlaysSpiritFire|SpiritFire|RealTriggerQueue|UndercoverAgentTrigger|MatchRecovery` adjacent 2077/2077 passed; backend full conformance 9023/9023 passed.
