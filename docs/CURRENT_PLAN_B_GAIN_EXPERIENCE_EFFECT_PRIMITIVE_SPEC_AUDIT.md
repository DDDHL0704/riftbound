...
This slice advances Plan B by moving direct and friendly-field-count gain-experience primitive metadata from the old P2 `CardBehaviorDefinition` surface into `BehaviorSpec.Effects`.

Implemented in this slice:

- `EffectPhraseSpec` now carries optional `ExperienceCount`, `ExperienceCountFormula`, and `ExperienceCountMultiplier`.
...
- `EffectPhraseParser` parses `场上每有一名友方单位，便获得1经验` into `ExperienceCountFormula=FRIENDLY_FIELD_UNIT_COUNT` and `ExperienceCountMultiplier=1`.
...
- `BehaviorTemplatePrimitiveExecutor` adds a `gain-experience` primitive kind and emits it when `BehaviorSpec.Effects` supplies either a positive `ExperienceCount` or a complete friendly-field-count formula.
...
- Dynamic experience formulas outside the friendly-field-unit count pattern remain delegated.
...
```bash
/Users/dinghaolin/.dotnet/dotnet test tests/Riftbound.ConformanceTests/Riftbound.ConformanceTests.csproj --filter "FullyQualifiedName~BehaviorSpecEffectPhrasesCarryGainExperiencePrimitiveMetadata|FullyQualifiedName~P4PrimitiveExecutorBuildsBasicActionPlansAndLeavesComplexRoutesDelegated" --no-restore --nologo
```
