# Stage 4D-18CP Recovery Timing Temporary Payment Resource Resource Id Whitespace Value Audit
...
Added `RecoveryValidatorRejectsSpectatorReplayTimingTemporaryPaymentResourceResourceIdWhitespaceValueWithCountMismatch` in `tests/Riftbound.ConformanceTests/MatchRecoveryTests.cs`.

The test builds an authoritative temporary payment resource from real `MatchState` source-object state, changes the emitted spectator payload's `resourceId` to a whitespace-only string, then appends `temp-payment-resource-extra` to force resource-count mismatch.

It proves validation still emits:

- missing resource-id required diagnostics for the malformed item;
...
- Focused `TemporaryPaymentResource` filter: `89/89`.
- Focused recovery filter: `1122/1122`.
- Adjacent recovery/official-opening/Postgres recovery-store filter: `1703/1703`.
- Backend full: `7068/7068`.
