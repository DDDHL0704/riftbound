...
- 2026-07-03 adjacent SourceUnitPlayed / UnitConquest graveyard-spell helper / FullGameEndToEnd / Stack / CardCatalogBaseline / MatchRecovery representatives after multiple-friendly-target no-auto-select guard: `3019/3019` passing.
- 2026-07-03 backend full conformance after multiple-friendly-target source-unit-played no-auto-select guard: `9167/9167` passing.

## Residual Risk

- Complete optional yes/no prompts, explicit graveyard spell selection, prompt-authored target selection, arbitrary / multi-target target-bearing spell legality, power-cost prompt routing, and broader spell effect state handoff remain open; the current guard only prevents unsafe automatic target choice when multiple friendly targets are legal.
