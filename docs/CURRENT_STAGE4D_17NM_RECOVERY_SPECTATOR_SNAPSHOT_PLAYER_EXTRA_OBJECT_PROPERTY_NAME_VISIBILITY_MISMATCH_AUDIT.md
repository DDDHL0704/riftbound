# Stage 4D-17NM Recovery Spectator Snapshot Player Extra Object Property Name Visibility Mismatch Audit

Date: 2026-05-28 05:28 CST

Status: accepted. Project remains **NOT READY**.

Scope: A_MAIN tightened spectator replay-frame snapshot player object recovery validation only. `MatchRecoveryValidator.ValidateSpectatorReplayFrame` now validates property names for object-shaped entries in `SpectatorSnapshot.Players[playerId].objects{}` that are not visible in the authoritative spectator view.

Runtime change: spectator snapshot player object validation now emits explicit property-name diagnostics for extra object-shaped `objects{}` entries outside the authoritative visible object-id set. Existing visible-object value, location and redaction parity still runs over the authoritative visible object set.

Test coverage: `RecoveryValidatorRejectsSpectatorReplaySnapshotPlayerExtraObjectPropertyNamesWithVisibilityMismatch` proves an extra hidden hand object payload with duplicate, surrounding-whitespace and blank property names emits explicit player object property-name diagnostics alongside the authoritative spectator visibility mismatch diagnostic.

Validation:

- Focused single: `1/1`
- Focused recovery: `396/396`
- Adjacent recovery/opening/store-smoke: `977/977`
- Backend full: `6342/6342`
...
- `docs/CURRENT_STAGE4D_17NM_RECOVERY_SPECTATOR_SNAPSHOT_PLAYER_EXTRA_OBJECT_PROPERTY_NAME_VISIBILITY_MISMATCH_AUDIT.md`
