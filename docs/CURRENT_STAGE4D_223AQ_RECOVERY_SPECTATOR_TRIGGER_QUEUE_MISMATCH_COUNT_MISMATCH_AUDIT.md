2026-06-16 14:22 CST

Stage 4D-223AQ recovery spectator trigger-queue mismatch count-mismatch validation accepted.
...
What changed:
- Added `RecoveryValidatorRejectsSpectatorReplayTimingTriggerQueueMismatchWithCountMismatch`.
- The test builds an authoritative visible-source and hidden-source `triggerQueue[]`, then corrupts the spectator replay timing queue while appending an extra spectator trigger.
- The test proves recovery validation still reports trigger-queue count mismatch, extra spectator trigger ids, missing authoritative trigger ids and keyed hidden-source visibility drift when ordered parity is skipped.
...
- Changed-class `MatchRecoveryTests`: `1945/1945`.
- Adjacent TriggerQueue/SpectatorReplayTiming filter: `1465/1465`.
- Backend full: `8275/8275`.
- `git diff --check` passed before code commit.
- Changed-test anchored conflict-marker scan passed before code commit.

Code commit:
- `ef3ecceb test: cover recovery trigger queue mismatch count drift`

Non-goals:
- Does not change recovery runtime behavior.
- Does not change trigger ordering, stack placement, Last Breath runtime behavior, hidden-source redaction or authoritative state serialization.
