# Stage 4D-18DB Recovery Timing Temporary Payment Resource Ability Id Empty Value Audit
...
Added `RecoveryValidatorRejectsSpectatorReplayTimingTemporaryPaymentResourceKeyedAbilityIdEmptyValueWithCountMismatch` in `tests/Riftbound.ConformanceTests/MatchRecoveryTests.cs`.

The test builds an authoritative temporary payment resource from real `MatchState` source-object state, keeps the emitted spectator payload keyed to `temp-payment-resource-1`, changes `abilityId` to `string.Empty`, then appends `temp-payment-resource-extra` to force resource-count mismatch.
...
`abilityId` is an optional payload field. Empty strings are accepted by optional scalar payload parsing, so this case intentionally does not assert a payload-layer required or invalid diagnostic. The keyed authoritative comparison still rejects the empty value when the authoritative resource has `TEST_TEMP_RESOURCE_ABILITY`.

## Validation

- Focused new test: `1/1`.
- Focused `TemporaryPaymentResource` filter: `101/101`.
- Focused recovery filter: `1134/1134`.
- Adjacent recovery/official-opening/Postgres recovery-store filter: `1715/1715`.
- Backend full: `7080/7080`.
