...
2026-07-03 follow-up: SFD Rumble's conquest text now parses to `UNIT_CONQUEST_RECYCLE_FRIENDLY_PLAY_GRAVEYARD_MECHANICAL_UNIT` and has a natural-conquest runtime representative for the zero-mana-after-reduction path. The execution bridge recycles one other controlled public field unit to the owner's main deck, then plays one controlled graveyard `机械` unit to base when the recycled unit's power reduces that unit's mana cost to 0. Complete optional target choice prompts, non-zero reduced-mana `PAY_COST`, battlefield destination choice, and full official Rumble breadth remain open.
...
- Focused Rumble recycle-friendly-unit graveyard-mechanical-unit TriggerSpec parser + natural conquest zero-mana-after-reduction representative: `3/3` passing.
...
- Rumble's recycle-friendly-unit graveyard-mechanical-unit execution is currently a zero-mana-after-reduction representative path. Complete optional target choice, non-zero reduced-mana `PAY_COST`, explicit graveyard mechanical unit selection, battlefield destination choice, and broader payment/zone handoff remain open.
