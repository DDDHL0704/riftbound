...
- Focused 天声玄龙 friendly-power behavior-spec/source-guard/runtime tests: `3/3` passing.
- Adjacent unit-conquest / `P79BattlefieldHeldActivateConquest...` representatives: `33/33` passing.
- `MatchRecovery`: `1989/1989` passing.
- Backend full conformance: `8496/8496` passing.
- No DevUi source or catalog TypeScript shape changed in this slice, so DevUi build was not rerun.

## Residual Risk

- The remaining destroy-equipment unit-conquest effect helper is still card-number based and must be migrated in a follow-up slice.
