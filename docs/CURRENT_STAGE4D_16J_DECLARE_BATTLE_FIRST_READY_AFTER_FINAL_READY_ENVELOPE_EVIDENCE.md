2026-05-24 Stage 4D-16J evidence.
...
```bash
source scripts/dev-env.sh && dotnet test tests/Riftbound.ConformanceTests/Riftbound.ConformanceTests.csproj --no-restore --filter "FullyQualifiedName~PromptIdOnlyDeclareBattleWithFirstReadyPromptAfterFinalReadyRejectsWithoutMutation|FullyQualifiedName~SnapshotOnlyDeclareBattleWithFirstReadyPromptAfterFinalReadyRejectsWithoutMutation"
```
...
Adjacent result: passed `993/993`.
...
Backend full result: passed `5945/5945`.
