...
- from `PreconstructedDeckCatalog.Build(...)`, the currently shippable `jhin-lowcurve`, `rumble-lowcurve`, and `lillia-lowcurve` decklists drive the same full `SUBMIT_DECK` -> `READY` -> `MULLIGAN` -> gameplay -> `DECLARE_BATTLE` -> score-victory action-log replay route to the same final state hash and recovered event payload hash in ring pairings that cover each deck as P1 and P2;
