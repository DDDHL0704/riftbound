...
- from a legal official LeBlanc opening-derived midgame state, official `UNL-199/219` LeBlanc legend and `UNL-090/219` LeBlanc champion make official `UNL-200/219` Mirror Image legal under `OfficialDeckValidator`; the server-authored `PLAY_CARD` prompt targets public `OGN·096/298` Watchful Sentinel, stack resolution creates an active `映像` copy token in P1 base with copied card/power/tags plus `瞬息`, records `UNL·T06` token factory metadata, then continues through score victory and action-log replay to the same final state hash;
...
- from a seated official Vex opening state, a verified legal official-deck opening feeds a focused midgame main-phase state with official `UNL-047/219` Moss Stepper / 踏苔蜥 public on P2's battlefield, P2 at 3 experience, and official `OGS·003/024` Incinerate in P1 hand; the target object deliberately has no materialized `法盾` tag, `RULE_TEXT:SOURCE_OBJECT_LEVEL_KEYWORD:*:法盾` projects from `BehaviorSpec.StaticAuras`, the server-authored `PLAY_CARD` route pays base 2 mana plus dynamic Spellshield tax 1, resolves damage, then continues through score victory and action-log replay to the same final state hash;
...
- from a seated official Jhin opening state, a verified legal official-deck opening feeds a focused midgame main-phase state with `OGN·100/298` Gemstone Seer in P1 base, official `SFD·075/221` Progress Glory in P1 hand, and a controlled main-deck top card; the server-authored `PLAY_CARD` prompt exposes only that top card as the optional other-friendly static-granted `预知` recycle target, Progress Glory has no printed `预知` tag, `OTHER_FRIENDLY_UNITS_KEYWORD` / `预知` projects after resolution, `CARDS_RECYCLED` moves the selected card to the bottom of P1's main deck, and the score-victory command stream replays through `MatchActionLogReplayer` to the same final state hash;
...
- from a seated official Rumble opening state, a verified legal official-deck opening feeds a focused midgame play state with P1 official `SFD·006/221` Aggressive Dragonhound in hand; the setup pre-exhausts that source object, playing it to a P1 battlefield resolves the parsed unconditional `SOURCE_UNIT_ENTER_READY`, emits self source entry metadata, enters active at printed 3 power, then continues through score victory and action-log replay to the same final state hash;
...
- from a seated official Poppy opening state, a verified legal official-deck opening feeds the same focused midgame Bandle Soldier play state with P1 at 2 experience and the source object pre-exhausted; playing Bandle Soldier to a P1 battlefield keeps it exhausted, emits no entry static-ability metadata, then continues through score victory and action-log replay to the same final state hash;
...
- from a seated official Poppy opening state without staging another public controlled Dragon, a verified legal official-deck opening feeds a focused midgame play state with P1 official `SFD·094/221` Fiercewing in hand and its source object pre-exhausted; playing Fiercewing to a P1 battlefield keeps it exhausted, emits no entry static-ability metadata, then continues through score victory and action-log replay to the same final state hash;
...
- Strengthened `tests/Riftbound.ConformanceTests/PreconstructedDeckCatalogTests.cs` with `BuildIncludesRumbleArmamentsDeckForEquipmentAndTemperedCoverage`.
- Strengthened `tests/Riftbound.ConformanceTests/PreconstructedDeckCatalogTests.cs` with `BuildIncludesVexSpellsDeckForSpellStackCoverage`.
- Strengthened `tests/Riftbound.ConformanceTests/PreconstructedDeckCatalogTests.cs` with `BuildIncludesPoppyStandbyDeckForStandbyAndBattlefieldDestinationCoverage`.
...
- Strengthened `tests/Riftbound.ConformanceTests/FullGameEndToEndTests.cs` with `OfficialDeckMidgameAppliesMossStepperSourceObjectLevelSpellshieldTaxAndScoreVictoryActionLogReplaysToFinalStateHash`.
...
- Strengthened `tests/Riftbound.ConformanceTests/FullGameEndToEndTests.cs` with `OfficialDeckMidgameResolvesBandleSoldierLevelActiveEntryAndScoreVictoryActionLogReplaysToFinalStateHash`.
- Strengthened `tests/Riftbound.ConformanceTests/FullGameEndToEndTests.cs` with `OfficialDeckMidgameSkipsBandleSoldierBelowLevelActiveEntryAndScoreVictoryActionLogReplaysToFinalStateHash`.
- Strengthened `tests/Riftbound.ConformanceTests/FullGameEndToEndTests.cs` with `OfficialDeckMidgameSkipsFiercewingNoControlledDragonActiveEntryAndScoreVictoryActionLogReplaysToFinalStateHash`.
...
- Strengthened `tests/Riftbound.ConformanceTests/FullGameEndToEndTests.cs` with `OfficialDeckMidgameResolvesGemstoneSeerOtherFriendlyStaticGrantedPredictRecycleAndScoreVictoryActionLogReplaysToFinalStateHash`.
...
This LeBlanc Mirror Image official-deck replay slice adds no runtime rule changes. It proves a legal official blue/yellow `UNL-199/219` LeBlanc legend / `UNL-090/219` champion deck can carry official `UNL-200/219` Mirror Image into a focused midgame `PLAY_CARD` route. The route uses the server-authored prompt to target a public battlefield `OGN·096/298` Watchful Sentinel, resolves stack pass-pass into an active `映像` copy token at P1 base with copied card number, power, tags, `瞬息`, and `UNL·T06` token factory metadata, then continues through score victory and action-log replay. This slice also records that Mirror Image is a LeBlanc exclusive spell and is not a legal Kai'Sa B0 representative; complete copy-text semantics, copied play-effect suppression breadth, ephemeral token cleanup breadth, LeBlanc legend image trigger, and READY remain open.
...
This Moss Stepper source-object level Spellshield increment adds no runtime rule changes. It proves a legal official Vex opening-derived state can carry `SOURCE_OBJECT_LEVEL_KEYWORD` / `法盾` through a real enemy spell target-tax payment, stack resolution, score victory, action-log replay, and hidden-info guarded full-game helpers while the target object has no materialized printed `法盾` tag. It still does not close complete source-object keyword breadth, complete target-tax timing breadth, complete official deck archetype breadth, P0, or READY.
...
This Plunder Alley increment additionally proves a legal official Jhin vs Vex deck opening can carry a BehaviorSpec-driven defended-battlefield move-friendly-unit-to-base route through server-authored `DECLARE_BATTLE`, parsed `BATTLEFIELD_DEFENSE_MOVE_FRIENDLY_UNIT_TO_BASE`, moving the selected same-battlefield friendly unit to its owner's base, score victory, and action-log replay. The later target-scope follow-up proves this target is not limited to declared defenders by covering an exhausted non-defending friendly unit. It still does not close complete optional yes/no trigger prompts, complete movement / control-zone edge cases, complete battlefield FUs, complete official deck archetype breadth, or READY.
...
This face-down Molten Drake skipped active-entry increment additionally proves the same legal official Jhin deck opening family ignores hidden standby source objects for `OTHER_FRIENDLY_UNITS_ENTER_READY`. The focused state keeps P1 `OGN·011/298` Molten Drake in base face down and official `OGN·010/298` Legion Rearguard in hand; after the server-authored `PLAY_CARD` to a P1 battlefield without `HASTE_READY`, Legion Rearguard stays exhausted, `UNIT_PLAYED_TO_BATTLEFIELD` omits entry static-ability metadata, and the command stream continues through score victory and final-state replay. It still does not close complete active-entry family breadth, complete standby/reaction breadth, complete official deck archetype breadth, P0 full objective, or READY.
...
This Bandle Soldier below-level skipped active-entry increment additionally proves the same legal official Poppy deck opening family keeps `SOURCE_UNIT_ENTER_READY` gated below the level-3 requirement. The focused state keeps P1 at 2 experience with official `UNL-151/219` Bandle Soldier in hand and pre-exhausts that source object so readiness must come from the parsed static ability; after the server-authored `PLAY_CARD` to a P1 battlefield, Bandle Soldier stays exhausted at printed 5 power, `UNIT_PLAYED_TO_BATTLEFIELD` omits entry static-ability metadata, and the command stream continues through score victory and final-state replay. It still does not close complete active-entry family breadth, complete level / experience breadth, complete official deck archetype breadth, P0 full objective, or READY.
...
This Fiercewing no-controlled-Dragon skipped active-entry increment additionally proves the same legal official Poppy deck opening family keeps `SOURCE_UNIT_ENTER_READY` gated when no other public controlled `龙` unit is present. The focused state keeps official `SFD·094/221` Fiercewing in P1 hand, excludes a staged Dune Drake, and pre-exhausts Fiercewing so readiness must come from the parsed static ability; after the server-authored `PLAY_CARD` to a P1 battlefield, Fiercewing stays exhausted at printed 7 power, `UNIT_PLAYED_TO_BATTLEFIELD` omits entry static-ability metadata, and the command stream continues through score victory and final-state replay. It still does not close complete active-entry family breadth, complete controlled-tag breadth, complete official deck archetype breadth, P0 full objective, or READY.
...
This extra-standby rejection increment additionally proves a legal official Poppy deck opening can carry the BehaviorSpec-driven `BATTLEFIELD_EXTRA_STANDBY_DESTINATION` guard into server-authored `HIDE_CARD` destination filtering, reject a direct hand-written battlefield standby destination when the player does not control Bandle Tree, journal and replay that rejected command, then continue through score victory and final-state replay. It still does not close base standby reaction current-battlefield context, remaining battlefield standby cleanup breadth, all standby replacement costs, complete hidden-information matrix, complete battlefield FUs, complete official deck archetype breadth, or READY.

This battlefield standby reveal increment additionally proves the shared standby reveal prompt and resolver no longer assume all revealable standby sources live in base. The follow-up battlefield-source, base/spell-duel, base/battle-response, and face-up defender Teemo target-damage increments additionally prove source-location, spell-duel context, battle-response context, or declared defender identity can drive same-battlefield target choice, top-five main-deck reveal/count, damage, and recycle through stack or battle resolution; the defender route now also exposes multi-target prompt metadata, accepts explicit same-battlefield target selection, and multiplexes official Fortified Position defender-target and Plunder Alley same-battlefield-friendly-unit battlefield effects with the Teemo enemy-target defend trigger. It still does not close non-spell-duel base contexts without a battlefield, illegal/lost-control standby cleanup breadth, all standby replacement costs, complete hidden-information matrix, complete battlefield FUs, complete official deck archetype breadth, or READY.
...
- broaden standby-heavy coverage beyond the OGN/SFD battlefield-source, base/spell-duel, base/battle-response, and face-up defender Teemo representatives into multi-target defender prompt selection, non-spell-duel base contexts without a battlefield, other standby reactions, remaining battlefield standby cleanup branches, and non-ready-base cleanup branches.
...
Latest LeBlanc Mirror Image official-deck replay validation passed:

```sh
/Users/dinghaolin/.dotnet/dotnet test tests/Riftbound.ConformanceTests/Riftbound.ConformanceTests.csproj --no-restore --filter "FullyQualifiedName~OfficialDeckMidgameResolvesLeblancMirrorImage"
/Users/dinghaolin/.dotnet/dotnet test tests/Riftbound.ConformanceTests/Riftbound.ConformanceTests.csproj --no-restore --filter "FullyQualifiedName~MirrorImage"
/Users/dinghaolin/.dotnet/dotnet test tests/Riftbound.ConformanceTests/Riftbound.ConformanceTests.csproj --no-restore --filter "FullyQualifiedName~FullGameEndToEndTests"
...
```text
Focused: Passed: 1, Failed: 0, Skipped: 0, Total: 1
MirrorImage adjacent: Passed: 8, Failed: 0, Skipped: 0, Total: 8
FullGameEndToEndTests: Passed: 126, Failed: 0, Skipped: 0, Total: 126
Backend full: Passed: 9182, Failed: 0, Skipped: 0, Total: 9182
```
...
```sh
/Users/dinghaolin/.dotnet/dotnet test tests/Riftbound.ConformanceTests/Riftbound.ConformanceTests.csproj --filter "FullyQualifiedName~PreconstructedDeckCatalogTests|FullyQualifiedName~PreconstructedCatalogDecksReachScoreVictoryActionLogReplaysToFinalStateHash"
```
...
Latest Poppy standby preconstructed catalog adjacent validation passed:

```sh
/Users/dinghaolin/.dotnet/dotnet test tests/Riftbound.ConformanceTests/Riftbound.ConformanceTests.csproj --filter "FullyQualifiedName~PreconstructedDeckCatalog|FullyQualifiedName~FullGameEndToEnd|FullyQualifiedName~MatchRecovery|FullyQualifiedName~CardCatalogBaseline|FullyQualifiedName~Standby|FullyQualifiedName~HideCard|FullyQualifiedName~RevealCard|FullyQualifiedName~BandleTree|FullyQualifiedName~Pakaa"
```

Result:

```text
Passed: 2640, Failed: 0, Skipped: 0, Total: 2640
```

Latest backend full validation after Poppy standby preconstructed catalog replay passed:
...
```text
Passed: 9055, Failed: 0, Skipped: 0, Total: 9055
```

Latest Vex spell-stack preconstructed catalog replay focused validation passed:
...
Latest Vex spell-stack preconstructed catalog adjacent validation passed:

```sh
/Users/dinghaolin/.dotnet/dotnet test tests/Riftbound.ConformanceTests/Riftbound.ConformanceTests.csproj --filter "FullyQualifiedName~PreconstructedDeckCatalog|FullyQualifiedName~FullGameEndToEnd|FullyQualifiedName~MatchRecovery|FullyQualifiedName~CardCatalogBaseline|FullyQualifiedName~CardTrick|FullyQualifiedName~FlowingTimeMirror|FullyQualifiedName~SpellPlayed"
```

Result:

```text
Passed: 2443, Failed: 0, Skipped: 0, Total: 2443
```

Latest backend full validation after Vex spell-stack preconstructed catalog replay passed:
...
```text
Passed: 9054, Failed: 0, Skipped: 0, Total: 9054
```
...
```sh
/Users/dinghaolin/.dotnet/dotnet test tests/Riftbound.ConformanceTests/Riftbound.ConformanceTests.csproj --no-restore --filter "FullyQualifiedName~P4RevealCardCommandBattlefieldStandbyReactionTargetDamageCountsTopFiveStandbyCards|FullyQualifiedName~ActionPromptRevealCardMetadataExposesBattlefieldStandbyReactionTargets|FullyQualifiedName~P4StandbyTargetContextGuardsCoverBaseRevealAndBaseReactionBranches|FullyQualifiedName~P4RevealCardCommandRejectsBaseReactionTargetWithoutSourceBattlefieldContext|FullyQualifiedName~P4RevealCardCommandRejectsBaseRevealTargetsWithoutSourceBattlefieldContext"
```
...
Latest battlefield-source standby reaction target-damage adjacent validation passed:

```sh
/Users/dinghaolin/.dotnet/dotnet test tests/Riftbound.ConformanceTests/Riftbound.ConformanceTests.csproj --no-restore --filter "FullyQualifiedName~RevealCard|FullyQualifiedName~StandbyReaction|FullyQualifiedName~ActionPrompt"
```
...
Latest battlefield-source standby reaction target-damage hidden-info / recovery adjacent validation passed:

```sh
/Users/dinghaolin/.dotnet/dotnet test tests/Riftbound.ConformanceTests/Riftbound.ConformanceTests.csproj --no-restore --filter "FullyQualifiedName~RevealCard|FullyQualifiedName~StandbyReaction|FullyQualifiedName~ActionPrompt|FullyQualifiedName~MatchRecovery"
```
...
Latest battlefield-source standby reaction target-damage backend full validation passed:
...
```sh
/Users/dinghaolin/.dotnet/dotnet test tests/Riftbound.ConformanceTests/Riftbound.ConformanceTests.csproj --no-restore --filter "FullyQualifiedName~ActionPromptFiltersRevealCardSourcesByWindowAndFaceDownStandby|FullyQualifiedName~P4RevealCardCommandRevealsStandbyCardAtBattlefield"
```
...
Latest battlefield standby reveal / standby adjacent validation passed:

```sh
/Users/dinghaolin/.dotnet/dotnet test tests/Riftbound.ConformanceTests/Riftbound.ConformanceTests.csproj --no-restore --filter "FullyQualifiedName~RevealCard|FullyQualifiedName~StandbyOfficialDecks|FullyQualifiedName~ActionPromptFiltersRevealCardSourcesByWindowAndFaceDownStandby"
```
...
Latest battlefield standby reveal backend full validation passed:
...
```text
Passed: 8889, Failed: 0, Skipped: 0, Total: 8889
```
...
Latest Molten Drake face-up/face-down other-friendly active-entry official-deck replay focused validation passed:

```sh
/Users/dinghaolin/.dotnet/dotnet test tests/Riftbound.ConformanceTests/Riftbound.ConformanceTests.csproj --no-restore --filter "FullyQualifiedName~OfficialDeckMidgameSkipsFaceDownMoltenDrakeOtherFriendlyActiveEntry|FullyQualifiedName~OfficialDeckMidgameResolvesMoltenDrakeOtherFriendlyActiveEntry"
```
...
Latest Molten Drake face-up/face-down active-entry replay / active-entry / hidden-info adjacent validation passed:

```sh
/Users/dinghaolin/.dotnet/dotnet test tests/Riftbound.ConformanceTests/Riftbound.ConformanceTests.csproj --no-restore --filter "FullyQualifiedName~OfficialDeckMidgameSkipsFaceDownMoltenDrakeOtherFriendlyActiveEntry|FullyQualifiedName~OfficialDeckMidgameResolvesMoltenDrakeOtherFriendlyActiveEntry|FullyQualifiedName~MoltenDrakeOtherFriendlyActiveEntry|FullyQualifiedName~LegionRearguardHasteReadyEntry|FullyQualifiedName~OfficialDeckMidgameSkipsDunehornBeastHighHandActiveEntry|FullyQualifiedName~OfficialDeckMidgameResolvesDunehornBeastLowHandActiveEntry|FullyQualifiedName~DunehornLowHandActiveEntryStaticAbility|FullyQualifiedName~MasterYiLevelActiveEntryStaticAbility|FullyQualifiedName~RenataTokenActiveEntryStaticAbility|FullyQualifiedName~FullGameEndToEndTests|FullyQualifiedName~MatchRecoveryTests"
```
...
Latest Molten Drake face-down skipped active-entry backend full validation passed:
...
```text
Passed: 9199, Failed: 0, Skipped: 0, Total: 9199
```
