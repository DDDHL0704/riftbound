...
- from `PreconstructedDeckCatalog.Build(...)`, the currently shippable `jhin-lowcurve`, `rumble-lowcurve`, `lillia-lowcurve`, and `rumble-armaments` decklists drive the same full `SUBMIT_DECK` -> `READY` -> `MULLIGAN` -> gameplay -> `DECLARE_BATTLE` -> score-victory action-log replay route to the same final state hash and recovered event payload hash across all twelve ordered distinct deck pairings; `rumble-armaments` requires official `SFD·085/221` Ornn, `SFD·008/221` Sentinel Adept, and `SFD·022/221` Long Sword so the served catalog covers equipment / Tempered representatives instead of only low-curve unit lists;
