# Stage 4D-18DM Recovery Timing Temporary Payment Resource Generated Power Value Audit
...
Added `RecoveryValidatorRejectsSpectatorReplayTimingTemporaryPaymentResourceKeyedGeneratedPowerValueWithCountMismatch` in `tests/Riftbound.ConformanceTests/MatchRecoveryTests.cs`.

The test builds an authoritative temporary payment resource from real `MatchState` source-object state, keeps the emitted spectator payload keyed to `temp-payment-resource-1`, changes the emitted `generatedPower` from `3` to `9`, then appends `temp-payment-resource-extra` to force resource-count mismatch.
...
This closes the focused generated-power value-drift variant outside duplicate-id and broader required-field tests, keeping keyed authoritative validation independently locked for valid numeric `generatedPower` payloads.

## Validation

- Focused new test: `1/1`.
- Focused `TemporaryPaymentResource` filter: `112/112`.
- Focused recovery filter: `1145/1145`.
- Adjacent recovery/official-opening/Postgres recovery-store filter: `1726/1726`.
- Backend full: `7091/7091`.
