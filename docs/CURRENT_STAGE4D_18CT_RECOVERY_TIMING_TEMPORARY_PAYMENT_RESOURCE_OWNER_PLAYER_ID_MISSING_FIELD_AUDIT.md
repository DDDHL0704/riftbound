# Stage 4D-18CT Recovery Timing Temporary Payment Resource Owner Player Id Missing Field Audit
...
Added `RecoveryValidatorRejectsSpectatorReplayTimingTemporaryPaymentResourceKeyedOwnerPlayerIdMissingFieldWithCountMismatch` in `tests/Riftbound.ConformanceTests/MatchRecoveryTests.cs`.

The test builds an authoritative temporary payment resource from real `MatchState` source-object state, keeps the emitted spectator payload keyed to `temp-payment-resource-1`, removes the `ownerPlayerId` field, then appends `temp-payment-resource-extra` to force resource-count mismatch.
...
- Focused `TemporaryPaymentResource` filter: `93/93`.
- Focused recovery filter: `1126/1126`.
- Adjacent recovery/official-opening/Postgres recovery-store filter: `1707/1707`.
- Backend full: `7072/7072`.
