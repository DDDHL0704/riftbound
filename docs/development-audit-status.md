...
1. 继续 P2 preflight：实现 `PASS_FOCUS` / 法术对决最小流程。
2. 后续单独补废牌堆也为空导致连续燃尽/胜利判定 fixture。
3. 再准备真实卡牌能力进入 FEPR 栈项目的最小 handler 边界。
