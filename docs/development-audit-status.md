...
| `PlaceholderRuleEngine` / `CoreRuleEngine` | `PlaceholderRuleEngine` 仍对齐旧 Java 的 `PASS`、`END_TURN`、重复 `PASS` 事件形状；`CoreRuleEngine` 已实现 P2 普通回合开始、短符文牌堆、1v1 P2 首回合额外符文、燃尽/连续燃尽立即胜利、`END_TURN -> 下一回合开始` 最小闭环、回合结束特殊清理中的伤害移除/本回合内效果失效，`PASS_PRIORITY` / FEPR 最小让过与结算，`PASS_FOCUS` / 法术对决焦点让过和关闭窗口，以及官方法术《惩戒》《渊海狩咒》《焚烧》《海克斯射线》《霹天雳地》《彗星坠击》《终极闪光》《超究极死神飞弹！》的最小 `PLAY_CARD -> 入栈 -> 结算伤害 -> END_TURN 清理伤害` 通道；《惩戒》《符文禁锢》已覆盖 `ANY_UNIT` 目标范围中的基地单位，《惩戒》已覆盖伤害达到战力后的致命摧毁，《碎裂之火》已覆盖目标被此法术摧毁后的抽牌，《星落》《艾卡西亚暴雨》已覆盖多次单位伤害选择和重复选择同一目标，《剑刃飓风》已覆盖 0 目标全战场单位各 1 点伤害路径和致命伤害摧毁路径，《走开》已覆盖从手牌打出时眩晕后抽牌，《处置命令》已覆盖抽牌模式和对手废牌堆最多三张回收模式，《冥想》已覆盖基础抽牌路径，《聚心凝神》已覆盖全额支付的抽 2 张基础路径，《借鉴历史》已覆盖从手牌打出的抽 2 张基础路径，《以战养战》已覆盖全额支付与本回合敌方单位被摧毁后的减费抽 2 张路径，《海克斯射线》已覆盖 `BATTLEFIELD_UNIT` 不能指定基地单位；《霹天雳地》已覆盖按控制单位最高战力降低法力费用和基础 5 点伤害路径；《星芒凝汇》已覆盖 `1-2` 目标范围和多目标逐个伤害；《火箭轰击》已覆盖 `PLAY_CARD.mode` 模式选择和 `BASE_UNIT` 目标范围；《台前作秀》已覆盖不支付回响的 0 目标抽牌基础路径和支付回响后的重复抽牌路径；《聚心凝神》《先知之兆》《借鉴历史》《进化日》已覆盖 0 目标多张抽牌路径；《复仇》《爆能术》《狩魂》《暗刃》《流沙陷坑》已覆盖主动摧毁单位并移入拥有者废牌堆，其中《狩魂》已覆盖不高于 3 战力目标限制和高战力拒绝，《暗刃》已覆盖目标控制者抽 2 张；《虚空索敌》已覆盖结算伤害后抽 1 张牌，以及结算抽牌触发燃尽/回收/抽牌分支；《符文禁锢》已覆盖 `PLAY_CARD -> 入栈 -> 施加 STUNNED -> END_TURN 清理失效` 通道；《渊海狩咒》已覆盖正面朝下卡牌条件伤害；三十张卡参数已进入最小 card behavior registry；并将 placeholder 作为 legacy fallback | 需要继续规则审查 | P2 新规则继续进入 `CoreRuleEngine`；legacy placeholder 只保留为旧 fixture 对照和未迁移命令兜底。 |
...
| P2 前置规则审查 | 已建立 `docs/p2-rules-preflight.md`，覆盖符文池、`END_TURN`、`PASS_PRIORITY`、`PASS_FOCUS`、清理/特殊清理、最小状态模型、事件词表和首批 P2 fixture；fixture schema、`MatchState` 基础权威字段、runner 初始状态应用、普通回合开始、短符文牌堆、1v1 首回合额外符文、燃尽/连续燃尽胜利、`END_TURN` 自动推进、特殊清理伤害/效果、清理重复、`PASS_PRIORITY` 误提交拒绝、FEPR 最小让过/结算、剩余结算链优先权、法术对决焦点让过和《惩戒》《渊海狩咒》《焚烧》《海克斯射线》《霹天雳地》《彗星坠击》《终极闪光》《超究极死神飞弹！》《剑刃飓风》《台前作秀》《聚心凝神》《先知之兆》《借鉴历史》《进化日》《以战养战》《复仇》《爆能术》《狩魂》《暗刃》《流沙陷坑》《星芒凝汇》《火箭轰击》《虚空索敌》《符文禁锢》《碎裂之火》《星落》《艾卡西亚暴雨》《走开》《处置命令》《冥想》打出/结算 fixture 已落地；《惩戒》《符文禁锢》已覆盖基地单位目标；《惩戒》已覆盖伤害达到战力后的致命摧毁；《碎裂之火》已覆盖条件抽牌；《星落》《艾卡西亚暴雨》已覆盖多次单位伤害选择和重复选择同一目标；《剑刃飓风》已覆盖全战场单位伤害和多单位致命摧毁；《走开》已覆盖眩晕后抽牌；《处置命令》已覆盖抽牌模式和对手废牌堆最多三张回收模式；《冥想》已覆盖基础抽牌路径；《聚心凝神》已覆盖全额支付的抽 2 张路径；《借鉴历史》已覆盖从手牌打出的抽 2 张路径；《以战养战》已覆盖全额支付和本回合敌方单位被摧毁后减费抽 2 张路径；《星芒凝汇》已覆盖多目标伤害；《火箭轰击》已覆盖模式选择；《台前作秀》已覆盖不支付回响的 0 目标抽牌基础路径和支付回响路径；《聚心凝神》《先知之兆》《借鉴历史》《进化日》已覆盖 0 目标多张抽牌；《复仇》《爆能术》《狩魂》《暗刃》《流沙陷坑》已覆盖主动摧毁单位，其中《狩魂》已覆盖不高于 3 战力目标限制和高战力拒绝，《暗刃》已覆盖目标控制者抽 2 张；《超究极死神飞弹！》已覆盖基础 5 点伤害；《虚空索敌》已覆盖结算抽牌触发燃尽/回收/抽牌；《符文禁锢》已覆盖本回合内眩晕随回合结束清理失效；《渊海狩咒》已覆盖正面朝下卡牌条件伤害 | 继续执行 | 下一步迁移更多低复杂度官方卡牌，并继续扩大通用 diff 覆盖面。 |
