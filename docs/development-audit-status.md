...
| `PlaceholderRuleEngine` / `CoreRuleEngine` | `PlaceholderRuleEngine` 仅保留 legacy fallback；`CoreRuleEngine` 已实现 P2 回合、优先权、焦点、燃尽、清理和多类 `PLAY_CARD` preflight 路径。当前短状态和 registry 百分比见 `docs/CURRENT_P2_STATUS.md`，完整 fixture 覆盖和证据见 `docs/p2-rules-preflight.md` 与 `docs/rules-evidence-index.md`。 | 需要继续规则审查 | P2 新规则继续进入 `CoreRuleEngine`；不要在本表重复粘贴完整卡牌清单。 |
