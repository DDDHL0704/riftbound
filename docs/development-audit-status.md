...
| `PostgresMatchJournal` 和 P1 SQL | 能记录命令、事件、snapshot、prompt，并写入 ruleset/FAQ/audit 元数据和 event sequence 边界 | 需要后续扩展 | 保留；后续补 command raw payload、recovery 读取路径和恢复校验。 |
