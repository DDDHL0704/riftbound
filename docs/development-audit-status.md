...
2. 再逐批迁移基础卡牌能力，先从能覆盖费用、目标、结算链和对象状态的低复杂度样例开始。
3. 后续补 richer expected canonical diff，减少测试中手写断言的比例。
