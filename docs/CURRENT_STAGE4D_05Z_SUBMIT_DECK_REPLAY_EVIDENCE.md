# Stage 4D-05Z Submit Deck Replay Evidence
...
- `docs/CURRENT_STAGE4D_05Z_SUBMIT_DECK_REPLAY_AUDIT.md`
- `docs/CURRENT_STAGE4D_05Z_SUBMIT_DECK_REPLAY_EVIDENCE.md`

## Validation

```bash
source scripts/dev-env.sh && dotnet test tests/Riftbound.ConformanceTests/Riftbound.ConformanceTests.csproj --no-restore --filter "FullyQualifiedName~SubmitDeckRejectsAcceptedReplayWithoutMutation"
```

Result: passed `1/1`.

```bash
source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore --filter "FullyQualifiedName~OfficialOpening|FullyQualifiedName~SubmitDeck|FullyQualifiedName~Ready|FullyQualifiedName~Mulligan|FullyQualifiedName~MatchStateHasher"
```
...
Result: passed. No whitespace warnings, conflict markers or trailing whitespace in the new 05J/05K/05L/05M/05N/05O/05P/05Q/05R/05S/05T/05U/05V/05W/05X/05Y/05Z evidence docs.

## Runtime And Protocol

Runtime changed: yes. `MatchSession.SubmitDeckAsync` now rejects exact duplicate normalized official deck submissions before ready without mutating state.
