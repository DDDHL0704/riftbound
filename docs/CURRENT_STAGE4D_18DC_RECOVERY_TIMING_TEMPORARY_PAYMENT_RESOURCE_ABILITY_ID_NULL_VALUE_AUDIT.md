# Stage 4D-18DC Recovery Timing Temporary Payment Resource Ability Id Null Value Audit
...
Added `RecoveryValidatorRejectsSpectatorReplayTimingTemporaryPaymentResourceKeyedAbilityIdNullValueWithCountMismatch` in `tests/Riftbound.ConformanceTests/MatchRecoveryTests.cs`.

The test builds an authoritative temporary payment resource from real `MatchState` source-object state, keeps the emitted spectator payload keyed to `temp-payment-resource-1`, changes `abilityId` to `null`, then appends `temp-payment-resource-extra` to force resource-count mismatch.
...
`abilityId` is an optional payload field. Null values are tolerated by optional scalar payload parsing, so this case intentionally does not assert a payload-layer required or invalid diagnostic. The keyed authoritative comparison still rejects the null value when the authoritative resource has `TEST_TEMP_RESOURCE_ABILITY`.

## Validation

- Focused new test: `1/1`.
- Focused `TemporaryPaymentResource` filter: `102/102`.
- Focused recovery filter: `1135/1135`.
- Adjacent recovery/official-opening/Postgres recovery-store filter: `1716/1716`.
- Backend full: `7081/7081`.
