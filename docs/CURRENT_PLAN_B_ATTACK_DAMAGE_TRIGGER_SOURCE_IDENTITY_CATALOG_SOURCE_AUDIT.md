...
|---|---|---|
| Sharpshooter Pirate attack-damage trigger source no longer directly selects by card number | `ResolveSharpshooterPirateAttackDamageTrigger` calls `IsControlledFaceUpFieldUnitWithEffectKind` instead of comparing `attackerState.CardNo` with `SharpshooterPirateCardNo` | Accepted |
| Runtime source check consumes registered source behavior row | source identity uses `SHARPSHOOTER_PIRATE_ATTACK_TRIGGER_PLAY_UNIT` through `CardBehaviorRegistry.IsImplementedUnitWithEffectKind` | Accepted |
