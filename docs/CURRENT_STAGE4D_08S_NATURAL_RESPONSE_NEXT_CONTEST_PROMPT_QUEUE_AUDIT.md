# Stage 4D-08S Natural Response Next-Contest Prompt Queue Audit

Date: 2026-05-23

Owner: `A_MAIN`

Status: accepted test-only server P0-005 natural battle-response activation next-contest prompt / queue audit slice. Project remains **NOT READY**.

## Scope

This slice strengthens `NaturalBattleResponseActivationImmediateBattleAdvancesNextContestedBattlefieldTask` in `BattleDamageAssignmentLifecycleTests`. It covers the path where a natural battle opens a battle-response priority window, P2 activates Shadow, the stack resolves, both players pass response priority, the battle closes without opening `ASSIGN_COMBAT_DAMAGE`, and the follow-up contested battlefield advances into active spell-duel focus.

Runtime behavior was not changed. The existing battle-response close path already advanced the next contested battlefield; this slice makes the post-close queue, snapshot queue, battlefield-task snapshot and active prompt invariants explicit.
...
- Strengthened `NaturalBattleResponseActivationImmediateBattleAdvancesNextContestedBattlefieldTask`.
- The audit proves the battle-response priority close does not open a battle-damage assignment prompt.
