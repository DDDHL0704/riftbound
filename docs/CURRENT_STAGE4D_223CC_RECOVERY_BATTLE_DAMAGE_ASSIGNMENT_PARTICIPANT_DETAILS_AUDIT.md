# Stage 4D-223CC Recovery Battle Damage Assignment Participant Details Audit

Status time: 2026-06-19 06:13 CST
...
Code commit: `27ad1418`

Project status: **NOT READY**

## Scope

This slice narrows recovery snapshot and spectator replay timing battle damage assignment diagnostics only. `MatchRecoveryValidator` now appends stable `expected ... but got ...` details when a battle damage assignment participant object id is not one of the enclosing battle participants.

Covered participant membership surfaces:

- `damagePool` source object ids.
- `legalTargets` source object ids.
- `legalTargets` legal target object ids.
...
- Latest core rules 454-461 for battle lifecycle, battle participants and identity context.
...
- `ValidateBattleDamageAssignmentParticipantObjectId` now appends `FormatExpectedActualForRecovery(expectedParticipantObjectIds, normalizedObjectId)` when an object id is outside the enclosing battle participant set.
- Expected participant object ids are ordered with `StringComparer.Ordinal` for deterministic recovery diagnostics.
- Existing prefix text is unchanged, preserving older broad assertions and log-search compatibility.

`tests/Riftbound.ConformanceTests/MatchRecoveryTests.cs`:

- `RecoveryValidatorRejectsSnapshotTimingBattleDamageAssignmentParticipantsOutsideBattle` now asserts detailed suffixes for all seven snapshot participant membership surfaces.
- `RecoveryValidatorRejectsSpectatorReplayTimingBattleDamageAssignmentParticipantsOutsideBattle` now asserts detailed suffixes for all seven spectator replay participant membership surfaces.

## Validation

Passed:

- Focused battle-damage-assignment participant detail tests: `2/2`.
...
External UI followup worktree `/Users/dinghaolin/MyProjects/riftbound-codex-ui-followup-20260616` on `codex/ui-followup-20260616` was clean at `01364ee2`, 99 commits behind the post-code `main` and `0` commits ahead at the pre-docs-sync divergence check. A_MAIN must inspect it before any UI followup integration and must not develop directly there.
