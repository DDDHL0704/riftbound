...
| 战斗 | RISKY | 已有显式 `BattleState` 参与者视图、`START_BATTLE` 任务视图和 `DECLARE_BATTLE.sourceRequirements`；4D-02B 到 4D-02G 已覆盖最小 response、assignment、task advancement、no-result 与一条带 Icevale Archer declaration context 的 response preservation representative，但完整官方 battle task 未关闭。 |
