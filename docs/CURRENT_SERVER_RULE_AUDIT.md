...
| 通用清理检查 | RISKY | 已有 `PendingCleanupTasks` 与移动/栈结算后的致命伤害/0 战力 cleanup loop；仍缺覆盖全部状态变化的统一任务队列。 |
