...
46. 下一步：4D-03BR-B 已把 target-bearing / typed / experience / Spellshield-tax activated ability representatives 扩成 executable target/tax matrix；后续仍需继续扩展 typed payment engine 到剩余 target-bearing activated abilities、rune/legend/battlefield/keyword 全路径，支持 `[A]` / `[C]` resource skills、替代费用、减费/加费、额外/可选费用。
