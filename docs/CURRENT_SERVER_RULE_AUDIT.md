...
- `src/Riftbound.Engine/MatchSession.cs:509` `NormalizePowerModifierLedger` 仍按 `EffectId` 排序 ledger entries；当前 ledger entry 还没有显式 application order / timestamp 字段。
...
- `src/Riftbound.Engine/MatchSession.cs:1776` 最终 continuous effect view 仍按 `Scope` / `TargetObjectId` / `Layer` / `EffectId` 排序，不能作为同层实际应用顺序的权威来源。
