...
51. 已验收 4D-02G focused slice：Battle response declaration-context focused 429/429、adjacent 608/608、backend full 4197/4197、`git diff --check` 通过；natural `DECLARE_BATTLE` 携带 Icevale Archer battlefield target context 且存在合法 Shadow battle response 时会先开启 response window，双方 pass 后保留原 declaration context 打开 trigger payment，internal carrier 不进入公开 snapshot / prompt。
52. 已验收 4D-02H test-only slice：Battle response Brush replacement context focused 430/430、adjacent 608/608、backend full 4198/4198、`git diff --check` 通过；`BRUSH_USE_REPLACED_BATTLEFIELD:*` optional cost 在 response pass 后进入 replacement-aware held-score path，internal carrier 继续不进入公开 snapshot / prompt。
53. 下一步：4D-03BR-B 已把 target-bearing / typed / experience / Spellshield-tax activated ability representatives 扩成 executable target/tax matrix；4D-03BS / 4D-03BT 已固定下一步 fresh-dispatch boundary 与 executable closure gate；4D-03BU 已把 resource skill official breadth 的 32 / 19 / 13 split 转成 executable verifier；4D-03BV / 4D-03BW 已把 13 个 deferred candidates 拆成 executable future legend bridge 与 non-legend runtime / verifier family manifest；4D-03BX 已把 4 个 non-legend runtime / verifier candidates 单独收窄为 future implementation boundary。后续仍需继续扩展 typed payment engine 到剩余 target-bearing activated abilities、rune/legend/battlefield/keyword 全路径，支持完整 `[A]` / `[C]` resource skills、替代费用、减费/加费、额外/可选费用。
54. 下一步：引入完整 continuous effect LayerEngine，并逐关键词、逐卡牌把 `Representative/FixturePass` 提升到 `FullOfficialRulePass`。
