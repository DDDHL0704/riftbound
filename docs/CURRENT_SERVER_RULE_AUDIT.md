...
证据点：当前 `CardBehaviorRegistry` 与 fixture 只覆盖 Malzahar 普通手牌打出进基地；`P4ActivatedAbilityCatalog` 未登记 Malzahar；`CoreRuleEngine.ResolveActivateAbility` 没有 destroy-friendly-unit-or-equipment-as-cost / gain payment-only power-resource 分支。官方 activated resource skill 需要 B 服务端实现、D/E FAQ 裁定和 C prompt-only UI 接线。
...
当前状态：**PARTIALLY RESOLVED / typed pool 已覆盖 PLAY_CARD、代表性非出牌支付、一个 battlefield trigger 支付路径、基础符文横置得法力、代表性回收得同特性符能和 PLAY_CARD 支付步骤回收符文资源动作，MOVE_UNIT、ASSEMBLE_EQUIPMENT、ACTIVATE_ABILITY 与 LEGEND_ACT 已有每来源服务端候选，急速额外费用已有资源动作回归证据；4D-03 PaymentEngine handoff / baseline 与 focused foundation 已验收，4D-03B non-play payment focused slice 已验收，完整 PaymentEngine 与 reaction payment window 仍待统一**
...
- 待补：4D-03C `PLAY_CARD` optional / extra / payment-resource focused slice、`[A]`、所有支付步骤中的 `[C]` 资源技能、单阵营/多阵营费用、Haste 的特殊/替代/加减费分支、Spellshield 加税的全支付窗口推广、Echo 费用。
...
8. 已准备 4D-03C handoff / baseline：下一服务端切片迁移 `PLAY_CARD` Haste / Echo / Spellshield / experience / payment-resource 代表路径的 quote / commit / audit 口径。
9. 下一步：扩展 typed payment engine 到 rune/legend/battlefield/keyword 全路径，支持 `[A]` / `[C]` resource skills、替代费用、减费/加费、额外/可选费用。
10. 下一步：引入完整 continuous effect LayerEngine，并逐关键词、逐卡牌把 `Representative/FixturePass` 提升到 `FullOfficialRulePass`。

Stage 4D 主控计划已将以上顺序拆为可执行写锁与验收门槛，见 `docs/CURRENT_STAGE4D_P0_P1_CLOSURE_PLAN.md`。4D-01 board task queue foundation 已验收；4D-02 battle/spell-duel focused slice 已验收但不关闭 full official P0-004；4D-03 PaymentEngine focused foundation 与 4D-03B non-play focused slice 已验收但不关闭 full official P0-005；4D-03C play optional / extra payment handoff / baseline 已准备。下一实现应继续扩展 full PaymentEngine breadth，进入 `CoreRuleEngine.cs` 集成前必须避开已完成切片的写锁冲突。
