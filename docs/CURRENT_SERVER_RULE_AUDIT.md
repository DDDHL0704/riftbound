...
6. 下一步：进入 4D-03 PaymentEngine unification；4D-02 focused slice 保留为 spell duel / battle task-scoped regression foundation。
...
Stage 4D 主控计划已将以上顺序拆为可执行写锁与验收门槛，见 `docs/CURRENT_STAGE4D_P0_P1_CLOSURE_PLAN.md`。4D-01 board task queue foundation 已验收；4D-02 battle/spell-duel focused slice 已验收但不关闭 full official P0-004。下一实现应进入 4D-03 PaymentEngine unification；进入 `CoreRuleEngine.cs` 集成前必须避开已完成切片的写锁冲突。
