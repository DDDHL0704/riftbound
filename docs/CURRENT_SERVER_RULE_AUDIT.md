...
43. 已验收 4D-03BE / 4D-03BF / 4D-03BG / 4D-03BH / 4D-03BJ / 4D-03BK / 4D-03BL-B / 4D-03BM focused verifiers，并已建立 4D-03BL rollback failure official matrix handoff / baseline：rollback failure row manifest focused 51/51、cross-window generation / consumption row manifest focused 56/56、card matrix alignment row manifest focused 61/61、missing row downstream coverage focused 64/64、representative seed upstream coverage focused 67/67、policy-deferred MOVE_UNIT boundary focused 70/70、rollback failure all-window matrix focused 75/75、cross-window generation / consumption all-window matrix focused 80/80；latest 4D-03BM adjacent PaymentEngine / resource skill / prompt / hub regression 638/638、backend full 4517/4517 通过。这些 verifiers / handoff 只固定 official matrix missing-row / representative-seed / policy-deferred routing completeness、rollback failure all-window audit contract 和 cross-window generated-resource all-window audit contract，不改 runtime/frontend/matrix，不关闭完整 PaymentEngine official matrix、P0-005 或 READY。
44. 已验收 4D-02G focused slice：Battle response declaration-context focused 429/429、adjacent 608/608、backend full 4197/4197、`git diff --check` 通过；natural `DECLARE_BATTLE` 携带 Icevale Archer battlefield target context 且存在合法 Shadow battle response 时会先开启 response window，双方 pass 后保留原 declaration context 打开 trigger payment，internal carrier 不进入公开 snapshot / prompt。
45. 已验收 4D-02H test-only slice：Battle response Brush replacement context focused 430/430、adjacent 608/608、backend full 4198/4198、`git diff --check` 通过；`BRUSH_USE_REPLACED_BATTLEFIELD:*` optional cost 在 response pass 后进入 replacement-aware held-score path，internal carrier 继续不进入公开 snapshot / prompt。
46. 下一步：继续扩展 typed payment engine 到剩余 target-bearing activated abilities、rune/legend/battlefield/keyword 全路径，支持 `[A]` / `[C]` resource skills、替代费用、减费/加费、额外/可选费用。
47. 下一步：引入完整 continuous effect LayerEngine，并逐关键词、逐卡牌把 `Representative/FixturePass` 提升到 `FullOfficialRulePass`。
