...
当前状态：**PARTIALLY RESOLVED / typed pool 已覆盖 PLAY_CARD、代表性非出牌支付、一个 battlefield trigger 支付路径、基础符文横置得法力、代表性回收得同特性符能和 PLAY_CARD 支付步骤回收符文资源动作，MOVE_UNIT、ASSEMBLE_EQUIPMENT、ACTIVATE_ABILITY 与 LEGEND_ACT 已有每来源服务端候选，急速额外费用已有资源动作回归证据；4D-03 PaymentEngine handoff / baseline 已建立，完整 PaymentEngine 与 reaction payment window 仍待统一**
...
6. 下一步：执行 4D-03 PaymentEngine unification；4D-03 handoff / baseline 已建立，4D-02 focused slice 保留为 spell duel / battle task-scoped regression foundation。
...
Stage 4D 主控计划已将以上顺序拆为可执行写锁与验收门槛，见 `docs/CURRENT_STAGE4D_P0_P1_CLOSURE_PLAN.md`。4D-01 board task queue foundation 已验收；4D-02 battle/spell-duel focused slice 已验收但不关闭 full official P0-004；4D-03 PaymentEngine handoff / baseline 已准备但未实现。下一实现应按 `docs/CURRENT_STAGE4D_03_PAYMENT_ENGINE_HANDOFF.md` 执行；进入 `CoreRuleEngine.cs` 集成前必须避开已完成切片的写锁冲突。
