# Stage 4D-15L Wrong Player First Turn Move Unit Prompt Audit

Status: accepted test-only server P0-005 official first-turn wrong-player current `MAIN_ACTION` prompt `MOVE_UNIT` boundary slice.
...
- `tests/Riftbound.ConformanceTests/OfficialOpeningTests.cs`
  - Added `WrongPlayerFirstTurnMoveUnitPromptRejectsWithoutMutation`.
  - Added `PromptIdOnlyWrongPlayerFirstTurnMoveUnitPromptRejectsWithoutMutation`.
  - Added `SnapshotOnlyWrongPlayerFirstTurnMoveUnitPromptRejectsWithoutMutation`.
  - Added `AssertWrongPlayerFirstTurnMoveUnitPromptRejectsWithoutMutation`.
...
- Non-active-player `MOVE_UNIT` carrying the active player's prompt-scoped or prompt-id-only current `MAIN_ACTION` prompt envelope rejects with `ErrorCodes.PromptExpired` and exact diagnostics: `行动窗口已过期，请按最新提示重新提交。`
- Non-active-player `MOVE_UNIT` carrying only the same-tick snapshot reaches the open-main current-player legality guard and rejects with `ErrorCodes.PhaseNotAllowed` and exact diagnostics: `移动单位只能在当前玩家的开放主阶段提交。`
...
Runtime behavior was not changed. This continues the current active-player `MAIN_ACTION` prompt wrong-player boundary beyond `END_TURN`, `MULLIGAN`, `PASS`, `PASS_PRIORITY` and `PASS_FOCUS`: prompt id ownership is rejected before command handling, while snapshot-only same-tick submissions still prove command-specific movement open-main legality without mutation.
