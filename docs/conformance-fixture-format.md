...
- P4.250 补 OGN·065《睿智长者》普通手牌打出带目标时的拒绝 fixture：不推进 tick、不写事件、不支付费用、不移动手牌、不入场单位、不创建结算链；拥有增益时静态 +1 战力由 BehaviorSpec `SOURCE_OBJECT_FILTERED_POWER` 通用路径覆盖。
...
- P4.285 补 SFD·039《皇家随从》普通手牌打出带非传奇无效目标时的拒绝 fixture：不推进 tick、不写事件、不支付费用、不移动手牌、不入场单位、不创建结算链；合法传奇目标与 READY/EXHAUST mode 另由 P2/P7.9 路径覆盖。
...
- `ASSEMBLE_EQUIPMENT` payload 支持 `sourceObjectId`、`targetObjectId` 和 `optionalCosts`；P4.66 锁定装备装配 command envelope。P4.388 将普通窗口《长剑》最小路径提升为 accepted fixture `p4-assemble-equipment-long-sword-attach.fixture.json`：`optionalCosts=["ASSEMBLE_RED"]` 时支付 1 红色符能，保留装备在基地列表中，设置 `attachedToObjectId`，写 `COST_PAID` / `EQUIPMENT_ATTACHED`，不创建结算链；第三百零八至第三百四十二批追加《灵魂之剑》`ASSEMBLE_GREEN`、《锯齿短匕》`ASSEMBLE_RED`、《反曲之弓》`ASSEMBLE_RED`、《阿瑞昂的陨落》`ASSEMBLE_RED`、《枯萎战斧》`ASSEMBLE_RED`、《残暴之力》`ASSEMBLE_GREEN`、《守护天使》`ASSEMBLE_GREEN`、《布甲》`ASSEMBLE_BLUE`、《海克斯注力刚壁》`ASSEMBLE_BLUE`、《云游图鉴》`ASSEMBLE_BLUE`、《Z型驱动》`ASSEMBLE_BLUE`、《斯特拉克的挑战护手》`ASSEMBLE_GREEN`、《斯弗尔尚歌》`ASSEMBLE_GREEN`、非灵便《多兰之盾》`ASSEMBLE_GREEN`、《多兰之戒》`ASSEMBLE_PURPLE`、《多兰之刃》`ASSEMBLE_ORANGE`、《海克斯饮魔刀》`ASSEMBLE_ORANGE`、《狂徒铠甲》`ASSEMBLE_ORANGE`、《三相之力》`ASSEMBLE_ORANGE`、《猎人的宽刃刀》`ASSEMBLE_ORANGE`、《牧人的传家宝》`SPEND_EXPERIENCE:1`、普通编号《碎骨棒》`ASSEMBLE_ORANGE`、promo 编号《碎骨棒》`ASSEMBLE_ORANGE`、《轻灵之靴》`ASSEMBLE_PURPLE`、《萃取》`ASSEMBLE_PURPLE`、《夜之锋刃》`ASSEMBLE_PURPLE`、《先锋之眼》`ASSEMBLE_YELLOW`、《暴风大剑》`ASSEMBLE_YELLOW`、《神圣剪刀》`ASSEMBLE_YELLOW`、《破败王者之刃》`ASSEMBLE_YELLOW + DESTROY_FRIENDLY_UNIT:<id>`、《旋转飞斧》`ASSEMBLE_ANY_POWER`、《炉火斗篷》`ASSEMBLE_ANY_POWER`、《灭世者的死亡之冠》`ASSEMBLE_ANY_POWER` 和《舒瑞娅的安魂曲》`ASSEMBLE_ANY_POWER` 的直接 Core/ActionPrompt 代表路径。P5.1 在 `p5-equipment-state-assemble-long-sword-owner-controller.fixture.json` 中补充 card object 的 `ownerId` / `controllerId` 局部比对和可见快照字段：已声明身份的装备与单位在代表装配路径中必须保留 owner/controller，并写入 `EQUIPMENT_ATTACHED.ownerId/controllerId`；身份和当前区域控制者冲突时仍拒绝且零副作用。P4.330 补 priority-window rejection fixture：即使 `NEUTRAL_CLOSED` 且 stack 非空，当前也不接受真实装配，不改变既有结算链。P4.331 补 already-attached source rejection fixture：已贴附装备再次执行 `ASSEMBLE_EQUIPMENT` 当前仍拒绝，并保留原 `attachedToObjectId`。P4.353/P4.375/P4.354/P4.376 补 missing/unknown target, missing source 和 unknown source rejection fixture：缺少 `targetObjectId`、未知 `targetObjectId`、缺少 `sourceObjectId` 或未知 `sourceObjectId` 当前仍拒绝。P4.355 补 hand-source rejection fixture：仍在手牌中的来源装备当前仍拒绝，不支付费用、不移动手牌或基地对象、不贴附、不创建结算链。P4.356 补 opponent-source rejection fixture：对手基地中的装备来源当前仍拒绝，不支付费用、不移动任一玩家基地对象、不贴附、不创建结算链。P4.357 补 non-equipment-source rejection fixture：单位等非装备来源当前仍拒绝，不支付费用、不移动基地对象、不贴附、不创建结算链。P4.358 补 non-unit-target rejection fixture：装备等非单位目标当前仍拒绝，不支付费用、不移动基地对象、不贴附、不创建结算链。P4.359 补 unsupported-optional-cost rejection fixture：`ECHO` 等非装配费用名当前仍拒绝，不支付费用、不移动基地对象、不贴附、不创建结算链。P4.360 补 missing-optional-cost rejection fixture：省略装配费用名当前仍拒绝，不支付费用、不移动基地对象、不贴附、不创建结算链。
- 第三百四十一批补 `UNL-158/219` 《牧人的传家宝》经验消耗装配代表路径：ActionPrompt 在经验足够时公开该来源、`SPEND_EXPERIENCE:1`、`experienceCost=1` 和合法单位目标，Core 对 `ASSEMBLE_EQUIPMENT` 携带 `SPEND_EXPERIENCE:1` accepted，扣 1 经验并贴附；经验不足时仍拒绝且不推进 tick、不写事件、不扣经验、不移动基地对象、不贴附、不创建结算链。
- 第三百四十二批补 `SFD·178/221` 《破败王者之刃》摧毁友方单位额外费用装配代表路径：ActionPrompt 公开 `additionalCostChoices` 与 `requiredAdditionalCostChoiceCount=1`，Core 对 `ASSEMBLE_EQUIPMENT` 携带 `ASSEMBLE_YELLOW` 和 `DESTROY_FRIENDLY_UNIT:<objectId>` accepted，扣 1 黄色符能、将费用单位移入墓地并贴附装备；缺少额外费用或摧毁目标与装配目标相同仍拒绝且不推进 tick、不写事件、不扣符能、不移动基地/墓地对象、不贴附、不创建结算链。
- 第三百四十三批补 `SFD·150/221` 《临终仪式》废牌堆回收额外费用装配代表路径：ActionPrompt 公开两个 `RECYCLE_GRAVEYARD_CARD:<objectId>` 额外费用候选与 `requiredAdditionalCostChoiceCount=2`，Core 对 `ASSEMBLE_EQUIPMENT` 携带 `ASSEMBLE_PURPLE` 和两个不同己方废牌堆回收目标 accepted，扣 1 紫色符能、将两张废牌堆卡牌回收到主牌堆底并贴附装备；缺少、重复或非法回收目标仍拒绝且不推进 tick、不写事件、不扣符能、不移动废牌堆/主牌堆对象、不贴附、不创建结算链。
- 第三百四十四批补 `UNL-188/219` 《海克斯科技护手》动态装配费用代表路径：ActionPrompt 公开 `ASSEMBLE_3_ANY_POWER`、基础 3 点法力、目标级法力费用与目标战力减免，并隐藏当前资源无法支付的低战力目标；Core 对 `ASSEMBLE_EQUIPMENT` 携带 `ASSEMBLE_3_ANY_POWER` accepted，按所选单位 2 战力把法力费用降至 1，支付 1 法力与 1 任意符能并贴附装备；法力不足仍拒绝且不推进 tick、不写事件、不扣资源、不贴附、不创建结算链。
...
- 全局 `untilEndOfTurnEffects` 可用于记录服务端回合记忆并在 expected 中断言保持，例如 `p2-preflight-play-rampaging-soul-discarded-hand-keyword-unit.fixture.json` 使用 `DISCARDED_HAND_CARD_THIS_TURN:P1` 覆盖《肆虐狂魂》条件满足时附加 `强攻` / `游走` 标签的路径，`p2-preflight-play-sly-salamander-experience-keyword-unit.fixture.json` 使用 `GAINED_EXPERIENCE_THIS_TURN:P1` 覆盖《狡猾的蝾螈》条件满足时的 +1 战力与 `游走` 标签路径，`p2-preflight-play-ascended-believer-four-plus-spell-unit.fixture.json` 使用 `PLAYED_FOUR_PLUS_COST_SPELL_THIS_TURN:P1` 覆盖《晋升信徒》条件满足时的 +4 战力路径，`p2-preflight-play-crescent-guard-spell-ready-payment.fixture.json` 使用 `PLAYED_SPELL_THIS_TURN:P1` 覆盖《新月禁卫》本回合打出过法术后的紫色额外费用与活跃入场路径。
