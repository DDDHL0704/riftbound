...
- `MOVE_UNIT` payload 支持 `sourceObjectId`、`origin`、`destination` 和 `optionalCosts`；P4.65 锁定游走/基础移动 command envelope，P4.98 补精确战场/`ROAM` 游走 `MOVE_UNIT` 显式拒绝 fixture：不推进 tick、不写事件、不支付费用、不移动对象、不改变战力、不创建结算链。P4.300 补 coarse `BASE` -> `BATTLEFIELD` 友方单位移动 fixture：推进 1 tick，写 `UNIT_MOVED_TO_BATTLEFIELD`，不支付费用、不改变战力、不创建结算链。
