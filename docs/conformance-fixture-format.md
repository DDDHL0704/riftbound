...
- 全局 `untilEndOfTurnEffects` 可用于记录服务端回合记忆并在 expected 中断言保持，例如 `p2-preflight-play-rampaging-soul-discarded-hand-keyword-unit.fixture.json` 使用 `DISCARDED_HAND_CARD_THIS_TURN:P1` 覆盖《肆虐狂魂》条件满足时附加 `强攻` / `游走` 标签的路径。
