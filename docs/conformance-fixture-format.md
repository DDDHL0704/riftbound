...
- `PLAY_CARD` payload 支持 `sourceObjectId`、`cardNo`、`targetObjectIds`、`mode`、`optionalCosts` 和 `destination`；P4.64 只为 `mode = "AMBUSH"` 锁定伏击战场目的地 envelope，Core 仍显式拒绝真实反应战场打出。P4.97 补伏击 `mode = "AMBUSH"` 显式拒绝 fixture：不推进 tick、不写事件、不支付费用、不移动手牌、不改变战场、不创建结算链。P4.101 补 `预知` 选择非顶部主牌堆牌时的显式拒绝 fixture：不推进 tick、不写事件、不支付费用、不移动手牌或主牌堆、不创建结算链。P4.102 补《游击战》选择废牌堆非待命目标时的显式拒绝 fixture：不推进 tick、不写事件、不支付费用、不移动手牌或废牌堆、不创建结算链。
