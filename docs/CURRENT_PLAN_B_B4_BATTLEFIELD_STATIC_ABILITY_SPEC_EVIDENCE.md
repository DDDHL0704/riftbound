...
- `StandbyOfficialDecksRejectBattlefieldExtraStandbyWithoutBandleTreeAndScoreVictoryActionLogReplaysToFinalStateHash` now proves the same extra-standby destination guard through legal official Poppy deck submission/opening without Bandle Tree, server-authored `HIDE_CARD` destination filtering, direct rejected `HIDE_CARD` to `BATTLEFIELD:<non-Bandle battlefield>`, unchanged state hash, score victory, and final-state action-log replay that includes the rejected command.
...
- The official-deck replay path now carries that same `BATTLEFIELD_EXTRA_STANDBY_DESTINATION` guard from server-authored `HIDE_CARD` prompt destination filtering through a replayable rejected `HIDE_CARD` to a non-Bandle battlefield destination, no-mutated state hash, unchanged rune pool / hand / object location, subsequent score victory, and final-state action-log replay.
...
No hidden-zone or opponent-hand projection logic was changed. The prevent move-to-base replay uses only public battlefield/base objects and asserts snapshot boundaries after the rejected command; the accepted extra-standby path still emits `CARD_HIDDEN` without revealing opponent hidden zones, the rejected extra-standby path emits no events and asserts snapshot boundaries after the rejected command, the battle-destroyed replacement and legend attach-armament paths move only public battlefield/base objects, and the representative GameHub tests still cover prompt/snapshot boundaries; MatchRecovery remained covered in adjacent validation.

## Validation

- latest extra-standby official-deck rejected-command replay focused validation: `1/1`;
- latest BandleTree / BattlefieldExtraStandby / HideCard / Standby / FullGameEndToEnd / MatchRecovery / CardCatalogBaseline adjacent validation: `2506/2506`;
- backend full conformance after the extra-standby replay increment: `8864/8864`;
