...
No hidden-zone or opponent-hand projection logic was changed. The extra-standby path still emits `CARD_HIDDEN` without revealing opponent hidden zones, and the representative GameHub tests still cover prompt/snapshot boundaries; MatchRecovery passed `1989/1989`.

## Validation

- latest focused behavior-spec/source guard/Bandle Tree extra-standby runtime representatives: `8/8`;
- latest adjacent BandleTree / BattlefieldExtraStandby / HideCard representatives: `61/61`;
- latest adjacent Standby / FullGameEndToEnd representatives: `94/94`;
- CardCatalog baseline: `166/166`;
- MatchRecovery: `1989/1989`;
- backend full conformance: `8457/8457`.
- DevUi build/browser smoke: not repeated; this slice did not touch DevUi files or frontend behavior.

## Non-Closure

This evidence proves two battlefield static restrictions, one battlefield static Echo cost reduction, one battlefield static equipment cost reduction, one battlefield granted unit-experience ability, one battlefield target spell/skill damage bonus, and one battlefield extra-standby destination permission have moved to BehaviorSpec-driven routing. It does not prove the complete B4 battlefield-effect family, complex Echo/equipment costs, all movement / play timing windows, all battlefield lifecycle rules, full activated ability modeling for granted abilities, all spell/skill damage modifier timing edges, complete extra-standby destination / hidden-info breadth, all card-effect families, frontend smoke or READY.
