# Stage 4D-15N Wrong Player First Turn Play Card Prompt Audit

Status: accepted test-only server P0-005 official first-turn wrong-player current `MAIN_ACTION` prompt `PLAY_CARD` boundary slice.
...
- `tests/Riftbound.ConformanceTests/OfficialOpeningTests.cs`
  - Added `WrongPlayerFirstTurnPlayCardPromptRejectsWithoutMutation`.
  - Added `PromptIdOnlyWrongPlayerFirstTurnPlayCardPromptRejectsWithoutMutation`.
  - Added `SnapshotOnlyWrongPlayerFirstTurnPlayCardPromptRejectsWithoutMutation`.
  - Added `AssertWrongPlayerFirstTurnPlayCardPromptRejectsWithoutMutation`.
...
- Non-active-player `PLAY_CARD` carrying the active player's prompt-scoped or prompt-id-only current `MAIN_ACTION` prompt envelope rejects with `ErrorCodes.PromptExpired` and exact diagnostics: `行动窗口已过期，请按最新提示重新提交。`
- Non-active-player `PLAY_CARD` carrying only the same-tick snapshot reaches the play-card behavior lookup and rejects with `ErrorCodes.UnsupportedCardBehavior` and exact diagnostics: `Unsupported card behavior or mode: missing-card `
...
Runtime behavior was not changed. This continues the current active-player `MAIN_ACTION` prompt wrong-player boundary beyond `END_TURN`, `MULLIGAN`, `PASS`, `PASS_PRIORITY`, `PASS_FOCUS`, `MOVE_UNIT` and `DECLARE_BATTLE`: prompt id ownership is rejected before command handling, while snapshot-only same-tick submissions still prove command-specific play-card behavior gating without mutation.
