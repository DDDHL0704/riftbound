...
The action-log replay regressions prove the score-victory command stream can be journaled and replayed to the same final state hash. `OfficialLowCurvePostBattleScoreVictoryActionLogReplaysToFinalStateHash` starts from the existing mirrored Jhin official low-curve battle-closed state and covers post-battle `END_TURN` scoring. `OfficialLowCurveFullGameScoreVictoryActionLogReplaysToFinalStateHash`, `DistinctOfficialLowCurveFullGameScoreVictoryActionLogReplaysToFinalStateHash`, and `StandbyHeavyOfficialLowCurveFullGameScoreVictoryActionLogReplaysToFinalStateHash` start from a seated official low-curve initial state, record `SUBMIT_DECK`, `READY`, `MULLIGAN`, tap/play/move/focus, reopened battle declaration, and score-victory `END_TURN` commands through `MatchJournal`, convert entries to recovered commands / events, and verify `MatchActionLogReplayer.VerifyFinalStateAsync` reaches the expected final state hash with no replay errors. `PreconstructedCatalogDecksReachScoreVictoryActionLogReplaysToFinalStateHash` now repeats that same seated-room replay route using every currently shippable decklist produced by `PreconstructedDeckCatalog.Build(...)`: `jhin-lowcurve`, `rumble-lowcurve`, `lillia-lowcurve`, `rumble-armaments`, `vex-spells`, `vex-battlefields`, `vex-response`, `poppy-standby`, and `poppy-demacia`. `rumble-armaments` is a legal Rumble preconstructed list that requires official `SFD·085/221` Ornn, `SFD·008/221` Sentinel Adept, and `SFD·022/221` Long Sword, so the served catalog includes equipment / Tempered representative cards. `vex-spells` is a legal Vex preconstructed list that requires official `UNL-232/219` Vex, `UNL-055/219` Vex champion, `OGN·183/298` Card Trick, and `OGN·180/298` Flowing Time Mirror, so the served catalog also includes implemented spell-stack representative cards. `vex-battlefields` is a legal Vex preconstructed list that requires those spell representatives plus official `UNL-211/219` Lost Library, `OGN·294/298` Trifarian Training Grounds, and `OGN·297/298` Wind Hill, so the served catalog includes battlefield high-cost spell insight, battlefield all-units power aura, and battlefield all-units RULE_TEXT Roam representatives. `vex-response` is a legal Vex preconstructed list that requires official `UNL-194/219` Shadow and `OGN·197/298` Teemo, so the served catalog includes battle-response / standby-reaction representative cards as well as hero-matching official exclusive main-deck cards. `poppy-standby` is a legal Poppy preconstructed list that requires official `UNL-203/219` Poppy, `UNL-116/219` Poppy champion, `OGN·135/298` Pakaa Cub, and `OGN·278/298` Bandle Tree, so the served catalog also includes standby / extra-standby destination representative cards. `poppy-demacia` is a legal Poppy preconstructed list that requires official `OGS·013/024` Garen, `UNL-092/219` Demacia Envoy, and `OGN·279/298` Fortified Position, so the served catalog includes same-battlefield static-aura and defend-grant-Steadfast battlefield representatives. All seventy-two ordered distinct pairings among those nine decks reach score victory and action-log replay, so the served preconstructed catalog is covered by the B0 score-victory replay gate instead of only by deck-validation tests. The B0 test driver writes replayable raw command payloads for prompt-derived object ids and destinations instead of only storing `cmdType`.

The armaments preconstructed catalog replay slice adds no runtime rule changes. Focused `PreconstructedDeckCatalogTests|PreconstructedCatalogDecksReachScoreVictoryActionLogReplaysToFinalStateHash` passed 7/7, adjacent `PreconstructedDeckCatalog|FullGameEndToEnd|MatchRecovery|CardCatalogBaseline|EquipmentKeyword|TemperedEquipment` passed 2471/2471, and backend full passed 9053/9053. This covers every currently shipped preconstructed deck against every other currently shipped preconstructed deck in both player positions, including one equipment / Tempered representative list, but does not close future preconstructed-archetype breadth, complete equipment attach lifecycle breadth, complete B0, or READY.

The Vex spell-stack preconstructed catalog replay slice adds no runtime rule changes. `PreconstructedDeckCatalog.Build(...)` now includes `vex-spells`; required preconstructed main-deck cards must match their official category and an implemented play behavior, so units require `PlaysSourceToBaseAsUnit`, equipment requires `PlaysSourceToBaseAsEquipment`, and spells require stack behavior rather than unit/equipment play behavior. The catalog still rejects exclusive, one-copy-group and unsupported trait candidates before `OfficialDeckValidator` performs final validation. Default low-curve filler now excludes `{{急速}}` units because the B0 auto-driver does not pay the Haste optional cost; this keeps served preconstructed replay stable instead of accepting an opening that can strand an exhausted base unit before the representative battle path. Focused `PreconstructedDeckCatalogTests|PreconstructedCatalogDecksReachScoreVictoryActionLogReplaysToFinalStateHash` passed 8/8, adjacent `PreconstructedDeckCatalog|FullGameEndToEnd|MatchRecovery|CardCatalogBaseline|CardTrick|FlowingTimeMirror|SpellPlayed` passed 2443/2443, and backend full passed 9054/9054. This covers `jhin-lowcurve`, `rumble-lowcurve`, `lillia-lowcurve`, `rumble-armaments`, and `vex-spells` across all twenty ordered distinct pairings, but does not close full spell-stack breadth, all target windows, complete preconstructed-archetype breadth, complete B0, P0, or READY.

The Poppy standby preconstructed catalog replay slice adds no gameplay runtime rule changes. `PreconstructedDeckCatalog.Build(...)` now includes `poppy-standby`; the preconstructed definition format can require battlefield representatives as well as main-deck representatives, and `BuildBattlefields(...)` validates required battlefield card numbers against the official catalog, rejects non-battlefield or duplicate battlefield-name requirements, then fills the remaining battlefield deck with the existing deterministic official-catalog defaults before `OfficialDeckValidator` performs final validation. Focused `PreconstructedDeckCatalogTests|PreconstructedCatalogDecksReachScoreVictoryActionLogReplaysToFinalStateHash` passed 9/9, adjacent `PreconstructedDeckCatalog|FullGameEndToEnd|MatchRecovery|CardCatalogBaseline|Standby|HideCard|RevealCard|BandleTree|Pakaa` passed 2640/2640, and backend full passed 9055/9055. This covers `jhin-lowcurve`, `rumble-lowcurve`, `lillia-lowcurve`, `rumble-armaments`, `vex-spells`, and `poppy-standby` across all thirty ordered distinct pairings, but does not close full standby reaction breadth, all hidden-info standby visibility edges, complete preconstructed-archetype breadth, complete B0, P0, or READY.

The Vex response preconstructed catalog replay slice adds no gameplay runtime rule changes. `PreconstructedDeckCatalog.Build(...)` now includes `vex-response`; required preconstructed main-deck category filtering allows official `专属单位`, `专属装备`, and `专属法术` only when the card's official hero tag matches the selected legend, while still requiring implemented play behavior and leaving final color, category, exclusive-count and copy-count legality to `OfficialDeckValidator`. Explicit exclusive representatives remain required-only and are not selected as default low-curve filler. Focused `PreconstructedDeckCatalogTests|PreconstructedCatalogDecksReachScoreVictoryActionLogReplaysToFinalStateHash` passed 10/10, adjacent `PreconstructedDeckCatalog|FullGameEndToEnd|MatchRecovery|CardCatalogBaseline|Shadow|BattleResponse|StandbyReaction|Teemo|RevealCard|ActivateAbility` passed 2739/2739, and backend full passed 9056/9056. This covers `jhin-lowcurve`, `rumble-lowcurve`, `lillia-lowcurve`, `rumble-armaments`, `vex-spells`, `vex-response`, and `poppy-standby` across all forty-two ordered distinct pairings, but does not close complete battle-response breadth, complete standby reaction breadth, all hidden-info standby visibility edges, complete preconstructed-archetype breadth, complete B0, P0, or READY.

The Vex battlefields preconstructed catalog replay slice adds no gameplay runtime rule changes. `PreconstructedDeckCatalog.Build(...)` now includes `vex-battlefields`; the preconstructed definition requires implemented spell-stack representatives and exactly three official battlefield representatives already covered by B0 official-deck midgame replay: `UNL-211/219` Lost Library, `OGN·294/298` Trifarian Training Grounds, and `OGN·297/298` Wind Hill. Focused `PreconstructedDeckCatalogTests|PreconstructedCatalogDecksReachScoreVictoryActionLogReplaysToFinalStateHash` passed 11/11, adjacent `PreconstructedDeckCatalog|FullGameEndToEnd|MatchRecovery|CardCatalogBaseline|BattlefieldAllUnits|BattlefieldHighCostSpellInsight|BattlefieldStatic|StaticAura|StaticKeyword|Roam|FlowingTimeMirror|CardTrick` passed 2582/2582, and backend full passed 9057/9057. This covers `jhin-lowcurve`, `rumble-lowcurve`, `lillia-lowcurve`, `rumble-armaments`, `vex-spells`, `vex-battlefields`, `vex-response`, and `poppy-standby` across all fifty-six ordered distinct pairings, but does not close complete battlefield rule breadth, complete static aura / RULE_TEXT battlefield breadth, complete spell-stack breadth, complete preconstructed-archetype breadth, complete B0, P0, or READY.

The Poppy Demacia preconstructed catalog replay slice adds no gameplay runtime rule changes. `PreconstructedDeckCatalog.Build(...)` now includes `poppy-demacia`; the preconstructed definition requires official `OGS·013/024` Garen, official `UNL-092/219` Demacia Envoy, and official `OGN·279/298` Fortified Position, all already covered by B0 official-deck replay evidence. Focused `PreconstructedDeckCatalogTests|PreconstructedCatalogDecksReachScoreVictoryActionLogReplaysToFinalStateHash` passed 12/12, adjacent `PreconstructedDeckCatalog|FullGameEndToEnd|MatchRecovery|CardCatalogBaseline|Garen|DemaciaEnvoy|FortifiedPosition|StaticAura|StaticPower|SameBattlefield|Steadfast|DeclareBattle` passed 2669/2669, and backend full passed 9058/9058. This covers `jhin-lowcurve`, `rumble-lowcurve`, `lillia-lowcurve`, `rumble-armaments`, `vex-spells`, `vex-battlefields`, `vex-response`, `poppy-standby`, and `poppy-demacia` across all seventy-two ordered distinct pairings, but does not close complete same-battlefield static-aura breadth, complete defend-grant-Steadfast breadth, complete preconstructed-archetype breadth, complete B0, P0, or READY.

The Vayne unit-conquest pay-return replay slice adds no runtime rule changes. `OfficialDeckMidgamePaysVayneConquestReturnAndScoreVictoryActionLogReplaysToFinalStateHash` starts from legal official Jhin/Lillia deck submission/opening, derives a focused midgame battle state with official `OGN·035/298` Vayne, official `OGN·096/298` Watchful Sentinel, and 1 P1 mana, then submits `DECLARE_BATTLE`, opens the BehaviorSpec-driven `UNIT_CONQUEST_PAY_1_RETURN_SELF_TO_HAND` `TRIGGER_PAYMENT` window, pays `PAY_COST(SPEND_MANA:1)`, returns the source unit to owner hand, closes the payment window, and continues through score victory with action-log replay to the same final state hash. Complete optional yes/no prompt breadth, full battle-lifecycle resource timing, complete unit-conquest family breadth, and READY remain open.

The LeBlanc Mirror Image official-deck replay slice adds no runtime rule changes. `OfficialDeckMidgameResolvesLeblancMirrorImageAndScoreVictoryActionLogReplaysToFinalStateHash` starts from legal official LeBlanc/Rumble deck submission/opening, derives a focused midgame state with official `UNL-200/219` Mirror Image in P1 hand, 3 P1 mana, and public official `OGN·096/298` Watchful Sentinel on P2's battlefield. The route uses the server-authored `PLAY_CARD` prompt to select the Mirror Image source and public unit target, resolves stack priority pass-pass, creates an active `映像` copy token in P1 base with copied card number, copied power, copied tags plus `瞬息`, and `UNL·T06` token factory metadata, moves the spell to P1 graveyard, then continues through score victory with action-log replay to the same final state hash. Focused validation passed 1/1, adjacent `MirrorImage` passed 8/8, `FullGameEndToEndTests` passed 126/126, and backend full conformance passed 9182/9182. This is a legal LeBlanc exclusive-spell B0 route; `UNL-200/219` is not a legal Kai'Sa official deck representative. Complete copy-text semantics, copied play-effect suppression breadth, ephemeral token cleanup breadth, LeBlanc legend image trigger, complete B0, and READY remain open.
...
The SFD Teemo shared standby-reaction metadata regressions prove official `SFD·230/221` and `SFD·230*/221` use the same target-damage resolver as the already covered Teemo route without adding an engine card-number branch. `PromptMetadataExposesSfdTeemoStandbyReactionTargets` verifies both SFD card numbers expose `ENEMY_UNIT_AT_SOURCE_BATTLEFIELD`, max target count 1, and only same-battlefield enemy unit choices for a face-down battlefield standby source. `BattlefieldStandbyReactionSfdTeemoDamagesAndRecyclesWithSharedResolver` verifies both source card numbers enter the stack with their own effect kinds, return to the precise battlefield, reveal the controller's top five main-deck cards, count cards tagged `待命`, apply that count as damage to the same-battlefield target, and recycle the looked cards. Focused regression passed 4/4, adjacent / hidden-info `StandbyReactionSfdTeemo|StandbyReaction|RevealCard|ActionPrompt|Teemo|MatchRecovery` passed 2158/2158, and backend full passed 8903/8903. This closes only the SFD Teemo shared metadata breadth; non-active battle/spell-duel base contexts, complete standby replacement-cost branches, full standby card-family breadth, complete B0, and READY remain open.

The face-up Teemo defend-trigger counted-damage regressions prove the same official top-five standby-count damage text now also resolves from a declared defender unit source without a card-number branch. `StandbyDefendTeemoTests.FaceUpTeemoDefendTriggerCountsTopFiveStandbyCardsAndRecycles` covers `OGN·121/298`, `OGN·121a/298`, `SFD·230/221`, and `SFD·230*/221`: `DECLARE_BATTLE` marks Teemo as the defender, `CoreRuleEngine` reads the new defend-trigger behavior metadata, reveals the defending player's top five main-deck cards, counts cards tagged `待命`, applies that count as damage to the same-battlefield enemy unit target, and recycles the looked cards before ordinary combat damage proceeds. `DeclareBattlePromptMetadataExposesTeemoDefendTriggerTargetChoices` and `FaceUpTeemoDefendTriggerUsesExplicitBattlefieldTargetWhenMultipleTargetsAreLegal` add the multi-legal-target route: prompt metadata exposes `battlefieldTargetChoicesByIndex` for same-battlefield enemy units and direct `DECLARE_BATTLE` accepts explicit `BattlefieldTargetObjectIds`. `DeclareBattlePromptMetadataMultiplexesBattlefieldSteadfastAndTeemoDefendTriggerTargets` / `FaceUpTeemoDefendTriggerMultiplexesBattlefieldSteadfastAndDamageTargets`, `DeclareBattlePromptMetadataMultiplexesBattlefieldMoveToBaseAndTeemoDefendTriggerTargets` / `FaceUpTeemoDefendTriggerMultiplexesBattlefieldMoveToBaseAndDamageTargets`, and the Plunder friendly-unit scope follow-up add the combined official battlefield-effect routes: prompt metadata exposes one battlefield-effect target slot and one Teemo enemy-unit damage target slot, then the resolver filters the shared submitted target list by each effect scope so `OGN·279/298` grants `坚守` to a defender, `OGN·285/298` can move any selected same-battlefield friendly unit, including an exhausted non-defender, to its owner's base, and Teemo damages the selected enemy unit. `DeclareBattlePromptBattlefieldDefendEffectTargetsUseGenericTriggerSpecs` guards that prompt target slots consume generic `TriggerSpec` timing/target-scope data instead of Fortified/Plunder-specific trigger-kind branches. Focused regression passed 13/13, adjacent / hidden-info `StandbyDefendTeemo|Plunder|BattlefieldDefend|DefendMoveToBase|DeclareBattle|FortifiedPosition|BattleResponse|MatchRecovery|Teemo` passed 2221/2221, and backend full passed 8916/8916. This closes only the representative defender-trigger target route and one Plunder target-scope correction; complete standby replacement-cost branches, full standby card-family breadth, complete B0, and READY remain open.
...
The Dunehorn Beast high-hand active-entry skipped action-log replay regression proves the same official-deck route does not apply `SOURCE_UNIT_ENTER_READY` when its hand-count condition is false. `OfficialDeckMidgameSkipsDunehornBeastHighHandActiveEntryAndScoreVictoryActionLogReplaysToFinalStateHash` records a legal Jhin deck opening, then starts from a focused midgame play state with P1 official `SFD·027/221` Dunehorn Beast in hand plus three other cards. The server-authored `PLAY_CARD` to a P1 battlefield leaves the controller with three hand cards, keeps Dunehorn Beast exhausted, emits `UNIT_PLAYED_TO_BATTLEFIELD` without entry static-ability metadata, and continues through score-victory action-log replay to the same final state hash with hidden-info guards. This slice changes only test / evidence coverage; it does not add runtime behavior or close complete active-entry family breadth, complete official deck archetype breadth, P0 full objective, or READY.

The Aggressive Dragonhound unconditional active-entry action-log replay regression proves a legal official deck opening can feed an unconditional `SOURCE_UNIT_ENTER_READY` spec into the B0 score-victory route. `OfficialDeckMidgameResolvesAggressiveDragonhoundUnconditionalActiveEntryAndScoreVictoryActionLogReplaysToFinalStateHash` records a legal Rumble deck opening, then starts from a focused midgame play state with P1 official `SFD·006/221` Aggressive Dragonhound in hand. The setup pre-exhausts that source object so readiness must come from the parsed static ability; the server-authored `PLAY_CARD` to a P1 battlefield resolves Aggressive Dragonhound ready, emits `UNIT_PLAYED_TO_BATTLEFIELD` with `entryStaticAbilityKind=SOURCE_UNIT_ENTER_READY`, self source object/card metadata, `isExhausted=false`, printed `power=3`, and continues through score-victory action-log replay to the same final state hash with hidden-info guards. This slice changes shared StaticAbilitySpec parsing / source-unit entry gating only; it does not add a runtime card-number branch or close complete active-entry family breadth, complete official deck archetype breadth, P0 full objective, or READY.
...
The face-down Molten Drake other-friendly active-entry skipped action-log replay regression proves the same official-deck route ignores hidden standby sources. `OfficialDeckMidgameSkipsFaceDownMoltenDrakeOtherFriendlyActiveEntryAndScoreVictoryActionLogReplaysToFinalStateHash` records the same legal Jhin deck opening, then starts from a focused midgame play state with P1 official `OGN·011/298` Molten Drake in base as a face-down standby object and official `OGN·010/298` Legion Rearguard in hand. The server-authored `PLAY_CARD` to a P1 battlefield leaves Legion Rearguard exhausted, emits `UNIT_PLAYED_TO_BATTLEFIELD` without entry static-ability metadata, preserves hidden-info guards, and continues through score-victory action-log replay to the same final state hash. This slice changes only test / evidence coverage; it does not add runtime behavior or close complete active-entry family breadth, complete standby/reaction breadth, complete official deck archetype breadth, P0 full objective, or READY.
...
The Master Yi below-level active-entry skipped action-log replay regression proves the same official-deck route keeps `FRIENDLY_UNITS_ENTER_READY` gated below level 11. `OfficialDeckMidgameSkipsMasterYiBelowLevelActiveEntryAndScoreVictoryActionLogReplaysToFinalStateHash` records the same legal Master Yi level deck opening, then starts from a focused midgame play state with P1 `UNL-191/219` in the legend zone, P1 at 10 experience, and official `UNL-092/219` Demacia Envoy in hand. The server-authored `PLAY_CARD` to a P1 battlefield leaves Demacia Envoy exhausted, emits `UNIT_PLAYED_TO_BATTLEFIELD` without entry static-ability metadata, preserves hidden-info guards, and continues through score-victory action-log replay to the same final state hash. This slice changes only test / evidence coverage; it does not add runtime behavior or close complete active-entry family breadth, complete level / experience breadth, complete official deck archetype breadth, P0 full objective, or READY.
...
The Moss Stepper source-object level Spellshield target-tax action-log replay regression proves a legal official deck path can carry a level-gated source-object `RULE_TEXT` resource keyword into a real enemy spell target-tax payment. `OfficialDeckMidgameAppliesMossStepperSourceObjectLevelSpellshieldTaxAndScoreVictoryActionLogReplaysToFinalStateHash` records a legal official Vex deck containing `UNL-047/219` Moss Stepper / 踏苔蜥, follows the normal official opening seed path, starts the focused midgame route at 3 P2 experience, places Moss Stepper on P2's public battlefield with no materialized `法盾` tag, verifies the projected `RULE_TEXT:SOURCE_OBJECT_LEVEL_KEYWORD:*:法盾` continuous effect, casts official `OGS·003/024` Incinerate from P1 at that target, observes `COST_PAID.mana = 3` with `baseMana = 2` and `spellshieldTaxMana = 1`, resolves two damage, then continues through score-victory replay to the same final state hash. This slice changes only test / evidence coverage; it does not close complete source-object keyword breadth, complete target-tax timing breadth, complete official deck archetype breadth, or READY.

The Flameclaw level-gated active-entry + source-object level static-aura action-log replay regression proves a legal official deck can carry both parsed source-unit active-entry and parsed source-object static power through the B0 full-game route. `OfficialDeckMidgameResolvesFlameclawLevelActiveEntryStaticAuraAndScoreVictoryActionLogReplaysToFinalStateHash` records a legal official Jhin deck containing `UNL-016/219` Flameclaw, follows the normal official opening seed path, starts the focused midgame route at 3 experience, plays Flameclaw directly to a battlefield through server-authored `PLAY_CARD`, verifies `SOURCE_UNIT_ENTER_READY` entry metadata with the source object itself, verifies `SOURCE_OBJECT_POWER` continuous effect projection with `PowerDelta=1`, submits a server-authorized `DECLARE_BATTLE`, observes Flameclaw's real attacker `DAMAGE_APPLIED` with `basePower=3`, `staticPowerBonus=1`, `combatPower=4`, and `damage=4`, then continues through score-victory replay to the same final state hash. This slice changes shared StaticAbilitySpec parsing / entry-requirement runtime only; it does not close complete source-unit active-entry breadth, complete source-object level family breadth, complete official deck archetype breadth, or READY.
...
Latest Crystalhand Hunter source-object level static-aura B0 score-victory replay focused validation passed:
...
Latest Crystalhand Hunter source-object level static-aura adjacent / recovery validation passed:
...
```text
Passed: 2243, Failed: 0, Skipped: 0, Total: 2243
```

Latest backend full after Crystalhand Hunter source-object level static-aura official-deck evidence passed:
...
Latest Flameclaw level-gated active-entry + source-object static-aura B0 score-victory replay focused validation passed:
...
Latest Flameclaw active-entry / static-aura adjacent / recovery validation passed:
...
```text
Passed: 2501, Failed: 0, Skipped: 0, Total: 2501
```

Latest backend full after Flameclaw level-gated active-entry + source-object static-aura official-deck evidence passed:
...
Latest Aggressive Dragonhound active-entry / full-game / hidden-info adjacent validation passed:
...
```text
Passed: 2468, Failed: 0, Skipped: 0, Total: 2468
```

Latest backend full after Aggressive Dragonhound unconditional active-entry official-deck evidence passed:
...
```text
Passed: 8997, Failed: 0, Skipped: 0, Total: 8997
```

Latest Moss Stepper source-object level Spellshield target-tax B0 score-victory replay focused validation passed:

```sh
/Users/dinghaolin/.dotnet/dotnet test tests/Riftbound.ConformanceTests/Riftbound.ConformanceTests.csproj --filter "FullyQualifiedName~OfficialDeckMidgameAppliesMossStepperSourceObjectLevelSpellshieldTax"
```
...
Latest Moss Stepper source-object level Spellshield target-tax adjacent / hidden-info validation passed:

```sh
/Users/dinghaolin/.dotnet/dotnet test tests/Riftbound.ConformanceTests/Riftbound.ConformanceTests.csproj --filter "FullyQualifiedName~OfficialDeckMidgameAppliesMossStepperSourceObjectLevelSpellshieldTax|FullyQualifiedName~SourceObjectLevelPower|FullyQualifiedName~SourceObjectKeyword|FullyQualifiedName~StaticAura|FullyQualifiedName~Spellshield|FullyQualifiedName~FullGameEndToEnd|FullyQualifiedName~MatchRecovery"
```

Result:

```text
Passed: 2258, Failed: 0, Skipped: 0, Total: 2258
```

Latest backend full validation after Moss Stepper source-object level Spellshield target-tax official-deck evidence passed:
...
```text
Passed: 9138, Failed: 0, Skipped: 0, Total: 9138
```
...
The current Moss Stepper increment additionally proves the source-object level keyword family can feed Spellshield target-tax payment from projected `RULE_TEXT` without materializing a printed `法盾` tag, then continue through stack resolution, score-victory replay, and hidden-info guarded full-game helpers. Complete source-object keyword breadth, complete target-tax timing breadth, and complete official deck archetype breadth remain open.

The current Aggressive Dragonhound increment additionally proves one legal official Rumble deck route can carry an unconditional source-unit active-entry spec through server-authored play, self source entry metadata, score-victory replay, and hidden-info guarded full-game helpers. Complete source-unit active-entry breadth and complete official deck archetype breadth remain open.

The current Flameclaw increment additionally proves one legal official Jhin deck route can carry source-unit level-gated active entry and source-object level static power together through server-authored play, real attacker damage, score-victory replay, and hidden-info guarded full-game helpers. Complete source-unit active-entry breadth and complete source-object level family breadth remain open.
...
The current Plunder Alley increment additionally proves one BehaviorSpec-driven battlefield-defended move-friendly-unit-to-base route through legal official Jhin vs Vex deck submission/opening, parsed `BATTLEFIELD_DEFENSE_MOVE_FRIENDLY_UNIT_TO_BASE`, server-authored same-battlefield friendly-unit target submission via `battlefieldTargetObjectIds`, movement of the selected friendly unit to its owner's base, score-victory replay, hidden-info guarded full-game helpers, and a follow-up non-defending exhausted friendly-unit target-scope regression. Complete optional yes/no trigger prompts, complete movement / control-zone edge cases, complete battlefield FUs, and complete official deck archetype breadth remain open.
...
The current Fiercewing controlled-Dragon active-entry increment additionally proves one BehaviorSpec-driven controlled-tag source-unit active-entry route through legal official Poppy deck submission/opening, parsed `SOURCE_UNIT_ENTER_READY`, `RequiredOtherControlledUnitTag=龙`, public controlled Dune Drake as the other Dragon participant, Fiercewing entering active on a P1 battlefield, self source entry metadata, score-victory replay, and hidden-info guarded full-game helpers. Complete active-entry family breadth, complete official deck archetype breadth, P0 full objective, and READY are still open.
