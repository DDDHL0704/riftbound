# Stage 4D-18DH Recovery Timing Temporary Payment Resource Payment Window Missing Field Audit
...
Added `RecoveryValidatorRejectsSpectatorReplayTimingTemporaryPaymentResourceKeyedPaymentWindowMissingFieldWithCountMismatch` in `tests/Riftbound.ConformanceTests/MatchRecoveryTests.cs`.

The test builds an authoritative temporary payment resource from real `MatchState` source-object state, keeps the emitted spectator payload keyed to `temp-payment-resource-1`, removes the emitted `paymentWindow` field, then appends `temp-payment-resource-extra` to force resource-count mismatch.
...
`paymentWindow` is an optional payload field. Missing values are tolerated by optional scalar payload parsing, so this case intentionally does not assert a payload-layer required or invalid diagnostic. The keyed authoritative comparison still rejects the missing value when the authoritative resource has `PAY_COST`.

## Validation

- Focused new test: `1/1`.
- Focused `TemporaryPaymentResource` filter: `107/107`.
- Focused recovery filter: `1140/1140`.
- Adjacent recovery/official-opening/Postgres recovery-store filter: `1721/1721`.
- Backend full: `7086/7086`.
