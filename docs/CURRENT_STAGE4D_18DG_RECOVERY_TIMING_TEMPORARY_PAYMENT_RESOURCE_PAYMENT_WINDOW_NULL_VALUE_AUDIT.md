# Stage 4D-18DG Recovery Timing Temporary Payment Resource Payment Window Null Value Audit
...
Added `RecoveryValidatorRejectsSpectatorReplayTimingTemporaryPaymentResourceKeyedPaymentWindowNullValueWithCountMismatch` in `tests/Riftbound.ConformanceTests/MatchRecoveryTests.cs`.

The test builds an authoritative temporary payment resource from real `MatchState` source-object state, keeps the emitted spectator payload keyed to `temp-payment-resource-1`, changes the emitted `paymentWindow` to `null`, then appends `temp-payment-resource-extra` to force resource-count mismatch.
...
`paymentWindow` is an optional payload field. Null values are tolerated by optional scalar payload parsing, so this case intentionally does not assert a payload-layer required or invalid diagnostic. The keyed authoritative comparison still rejects the null value when the authoritative resource has `PAY_COST`.

## Validation

- Focused new test: `1/1`.
- Focused `TemporaryPaymentResource` filter: `106/106`.
- Focused recovery filter: `1139/1139`.
- Adjacent recovery/official-opening/Postgres recovery-store filter: `1720/1720`.
- Backend full: `7085/7085`.
