...
| Runtime source selection reads TriggerSpec | `ResolveUnitLastBreathSourceBattlefieldAoeDamagePlayerId(...)` now uses `UnitDestroyedTriggerSpecRules.TryGetTrigger(..., IsLastBreathSourceBattlefieldAoeDamageTrigger, ...)` plus source unit / face-up / non-standby checks | Accepted |
...
| Recovery source-card validation reads TriggerSpec | 2026-07-02 follow-up removes `KogmawCardNoForRecovery`; recovered snapshot, authoritative-state and spectator replay source objects now pass only when `UnitDestroyedTriggerSpecRules.TryGetTrigger(sourceCardNo, IsLastBreathSourceBattlefieldAoeDamageTrigger, out _)` accepts the source card | Accepted |
