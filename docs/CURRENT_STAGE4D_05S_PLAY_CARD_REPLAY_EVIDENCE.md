# Stage 4D-05S Play Card Replay Evidence
...
- `docs/CURRENT_STAGE4D_05S_PLAY_CARD_REPLAY_AUDIT.md`
- `docs/CURRENT_STAGE4D_05S_PLAY_CARD_REPLAY_EVIDENCE.md`

## Validation

```bash
source scripts/dev-env.sh && dotnet test tests/Riftbound.ConformanceTests/Riftbound.ConformanceTests.csproj --no-restore --filter "FullyQualifiedName~CoreRuleEngineRejectsAcceptedPlayCardReplayWithoutMutation"
```

Result: passed `1/1`.

```bash
source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore --filter "FullyQualifiedName~PlayCard|FullyQualifiedName~Punishment|FullyQualifiedName~Stack|FullyQualifiedName~PassPriority|FullyQualifiedName~MatchStateHasher"
```
