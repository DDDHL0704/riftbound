# Stage 4D-18DV Recovery Timing Temporary Payment Resource Generated Power Trait Map Shape Audit
...
Added `RecoveryValidatorRejectsSpectatorReplayTimingTemporaryPaymentResourceKeyedGeneratedPowerTraitMapShapeWithCountMismatch` in `tests/Riftbound.ConformanceTests/MatchRecoveryTests.cs`.

The test builds an authoritative temporary payment resource from real `MatchState` source-object state, keeps the emitted spectator payload keyed to `temp-payment-resource-1`, changes `generatedPowerByTrait` to an unreadable array payload, then appends `temp-payment-resource-extra` to force resource-count mismatch.

It proves validation still emits:

- generated-power trait-map payload-required diagnostics for `temp-payment-resource-1`;
- generated-power trait-map invalid diagnostics for `temp-payment-resource-1`;
...
This closes the focused generated-power trait-map shape variant, keeping keyed authoritative generated-power trait validation independently locked for spectator timing temporary-payment-resource payloads when the spectator trait map has an unreadable payload shape.

## Validation

- Focused new test: `1/1`.
- Focused `TemporaryPaymentResource` filter: `121/121`.
- Focused recovery filter: `1154/1154`.
- Adjacent recovery/official-opening/Postgres recovery-store filter: `1735/1735`.
- Backend full: `7100/7100`.
