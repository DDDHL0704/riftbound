# Stage 4D-18DO Recovery Timing Temporary Payment Resource Remaining Power Null Value Audit
...
Added `RecoveryValidatorRejectsSpectatorReplayTimingTemporaryPaymentResourceKeyedRemainingPowerNullValueWithCountMismatch` in `tests/Riftbound.ConformanceTests/MatchRecoveryTests.cs`.

The test builds an authoritative temporary payment resource from real `MatchState` source-object state, keeps the emitted spectator payload keyed to `temp-payment-resource-1`, changes the emitted `remainingPower` from `1` to `null`, then appends `temp-payment-resource-extra` to force resource-count mismatch.
...
This closes the focused remaining-power null-value variant outside broad required-field and shape tests, keeping remaining-power required numeric validation and keyed authoritative validation independently locked for spectator timing temporary-payment-resource payloads.

## Validation

- Focused new test: `1/1`.
- Focused `TemporaryPaymentResource` filter: `114/114`.
- Focused recovery filter: `1147/1147`.
- Adjacent recovery/official-opening/Postgres recovery-store filter: `1728/1728`.
- Backend full: `7093/7093`.
