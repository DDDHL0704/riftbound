# Stage 4D-18EH Recovery Timing Temporary Payment Resource Payment-Only Shape Audit
...
A_MAIN added a focused server recovery regression test for spectator replay-frame timing `temporaryPaymentResources[]` same-key `paymentOnly` shape drift under resource-count mismatch.

The new `RecoveryValidatorRejectsSpectatorReplayTimingTemporaryPaymentResourceKeyedPaymentOnlyShapeWithCountMismatch` coverage builds an authoritative temporary payment resource from real `MatchState` source-object state with `paymentOnly: true`, `allowedPaymentKinds: [PaymentCostRules.RuneCostPaymentKind]`, `generatedPowerByTrait: { blue: 2 }` and `remainingPowerByTrait: { blue: 1 }`. It keeps the emitted spectator payload keyed to `temp-payment-resource-1`, changes `paymentOnly` to the unreadable string payload `"true"`, and appends `temp-payment-resource-extra` to force count mismatch.

This slice locks both the same-payload shape validation and keyed authoritative payment-only mismatch path under the count-mismatch branch.
...
- focused new payment-only shape test `1/1`
- focused `TemporaryPaymentResource` filter `133/133`
- focused recovery filter `1166/1166`
- adjacent recovery/official-opening/Postgres recovery-store filter `1747/1747`
- backend full `7112/7112`
