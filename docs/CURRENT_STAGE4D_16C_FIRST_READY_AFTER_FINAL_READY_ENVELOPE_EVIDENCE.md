2026-05-24 Stage 4D-16C evidence.
...
```bash
source scripts/dev-env.sh && dotnet test tests/Riftbound.ConformanceTests/Riftbound.ConformanceTests.csproj --no-restore --filter "FullyQualifiedName~PromptIdOnlyFirstReadyBothDecksPromptReplayAfterFinalReadyRejectsWithoutMutation|FullyQualifiedName~SnapshotOnlyFirstReadyBothDecksPromptAfterFinalReadyAcceptsWithoutMutation"
```
...
Adjacent result: passed `979/979`.
...
Backend full result: passed `5931/5931`.
