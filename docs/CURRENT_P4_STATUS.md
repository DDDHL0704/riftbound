...
- P4.381 提交：本提交 `feat: add p4 minimal declare battle path`
...
P4.381 更新：Combat keywords 行在 P4.374 基础上追加 `P4DeclareBattleCommandDeclaresSingleCombatantsForRepresentativePath`、`P4DeclareBattleCommandSingleCombatantsFixture` 和 `p4-declare-battle-single-combatants.fixture.json`，把 `DECLARE_BATTLE battlefieldId=BATTLEFIELD:P1-MAIN attackerObjectIds=["P1-BATTLEFIELD-GAREN"] defenderObjectIds=["P2-BATTLEFIELD-MUTANT-KITTEN"] optionalCosts=["COMBAT_ASSIGNMENT"]` 从前置拒绝提升为最小 accepted 代表路径：普通主阶段开放窗口、stack 为空、攻击者由命令玩家控制且在战场、防守者由对手控制且在战场时推进 1 tick、写入 `BATTLE_DECLARED`、设置 `isAttacking` / `isDefending`；战斗伤害、强攻/坚守数值、壁垒/后排承伤、多个攻防对象和非法/复杂声明仍保持 P4 显式拒绝或 deferred。
...
| P4.381 战斗声明 delta | `P4DeclareBattleCommandDeclaresSingleCombatantsForRepresentativePath`、`P4DeclareBattleCommandSingleCombatantsFixture`、`p4-declare-battle-single-combatants` | Partial：单攻单防代表路径 accepted，早期 `DECLARE_BATTLE` 全量拒绝说明已被部分 supersede；复杂声明和完整战斗结算仍 deferred。 |
...
- Accepted 路径推进 1 tick、写入 `BATTLE_DECLARED` 事件，并只设置 `P1-BATTLEFIELD-GAREN.isAttacking=true` 与 `P2-BATTLEFIELD-MUTANT-KITTEN.isDefending=true`；不计算伤害、不处理强攻/坚守数值、不处理壁垒/后排承伤、不移动对象、不创建 stack。
- 新增 fixture `p4-declare-battle-single-combatants.fixture.json` 和回放测试 `P4DeclareBattleCommandSingleCombatantsFixture`；直接测试 `P4DeclareBattleCommandDeclaresSingleCombatantsForRepresentativePath` 覆盖同一路径。
...
P4 当前整体进度：按 P4.381 审计后预计 `381/392 = 97.2%`。已完成 P4.1-P4.381：在 P4.380 全部内容基础上，P4.381 将一个低风险 `DECLARE_BATTLE` 单攻单防代表路径接入可玩状态写入，但不进入完整战斗结算。当前仍不能标记 P4 goal complete：战斗伤害、强攻/坚守数值、壁垒/后排承伤、真实跨战场游走、狩猎/战斗/移动触发经验、待命触发/目标伤害、伏击真实反应战场打出、更多技能目标税/通用 skill registry、装备关键词最小代表与 P5 边界、以及最后 SignalR 或等价 E2E/全量验证文档收口仍待完成。预计还需约 `11` 批。
...
进入 P4.382：优先做战斗伤害/强攻/坚守数值代表切片，继续限制在单攻单防和已验证 fixture 范围内。预计 P4 还需要约 `11` 批：战斗伤害与强攻/坚守数值、壁垒/后排承伤最小代表、真实游走、狩猎/经验触发、待命触发/目标伤害、伏击真实反应打出、通用 skill registry/技能目标税、装备关键词最小代表/P5 边界，以及最后 E2E/文档/全量验证收口。
