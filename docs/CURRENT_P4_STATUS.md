...
- P4.382 提交：本提交 `feat: add p4 combat damage representative`
...
| 壁垒 | 26 | 0 | 0 | High | P4.6 已识别 profile；P4.67 已新增 `DECLARE_BATTLE` typed command 与 Core 显式拒绝前置模型；P4.100 已补同一路径拒绝 fixture；P4.332 已补同一路径优先权窗口拒绝 fixture；P4.350/P4.351/P4.352/P4.374 已补空攻击者、空防守者、缺少战场 id 和未知战场 id 拒绝 fixture；P4.361-P4.373 已补战斗声明 unsupported optional cost、缺少战斗费用、攻击者非战场来源、防守者非战场来源、对手控制攻击者、当前玩家控制防守者、重复攻击者、重复防守者、攻防列表交叉、非主动玩家声明战斗、非主阶段声明战斗、未知攻击者和未知防守者拒绝 fixture；完整承伤顺序和同优先级选择仍 deferred。 |
| 后排 | 6 | 0 | 0 | High | P4.6 已识别 profile；P4.67 已新增 `DECLARE_BATTLE` typed command 与 Core 显式拒绝前置模型；P4.100 已补同一路径拒绝 fixture；P4.332 已补同一路径优先权窗口拒绝 fixture；P4.350/P4.351/P4.352/P4.374 已补空攻击者、空防守者、缺少战场 id 和未知战场 id 拒绝 fixture；P4.361-P4.373 已补战斗声明 unsupported optional cost、缺少战斗费用、攻击者非战场来源、防守者非战场来源、对手控制攻击者、当前玩家控制防守者、重复攻击者、重复防守者、攻防列表交叉、非主动玩家声明战斗、非主阶段声明战斗、未知攻击者和未知防守者拒绝 fixture；完整承伤顺序仍 deferred。 |
...
| P4.381/P4.382 战斗声明与伤害 delta | `P4DeclareBattleCommandDealsCombatDamageForRepresentativePath`、`P4DeclareBattleCommandSingleCombatantsFixture`、`p4-declare-battle-single-combatants` | Partial：单攻单防代表路径 accepted，并已应用强攻/坚守数值战斗伤害和致命清理；复杂声明、壁垒/后排承伤、得分和完整战斗结束清理仍 deferred。 |
...
P4 当前整体进度：按 P4.382 审计后预计 `382/392 = 97.4%`。已完成 P4.1-P4.382：在 P4.381 全部内容基础上，P4.382 将低风险 `DECLARE_BATTLE` 单攻单防代表路径升级为强攻/坚守数值战斗伤害和致命清理。当前仍不能标记 P4 goal complete：壁垒/后排承伤、真实跨战场游走、狩猎/战斗/移动触发经验、待命触发/目标伤害、伏击真实反应战场打出、更多技能目标税/通用 skill registry、装备关键词最小代表与 P5 边界、以及最后 SignalR 或等价 E2E/全量验证文档收口仍待完成。预计还需约 `10` 批。
...
- `source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore --filter "FullyQualifiedName~P4DeclareBattleCommand"`：pass，38/38
- `source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore --filter "FullyQualifiedName~P4CombatKeywordProfilesKeepExistingKeywordUnitFixturesGreen"`：pass，27/27
- `jq empty tests/Riftbound.ConformanceTests/Fixtures/p4-declare-battle-single-combatants.fixture.json`：pass
- `git diff --check`：pass

## Next Step

进入 P4.383：优先做壁垒/后排承伤最小代表或真实游走切片，继续限制在单战场/单代表 fixture 范围内。预计 P4 还需要约 `10` 批：壁垒/后排承伤最小代表、真实游走、狩猎/经验触发、待命触发/目标伤害、伏击真实反应打出、通用 skill registry/技能目标税、装备关键词最小代表/P5 边界，以及最后 E2E/文档/全量验证收口。
