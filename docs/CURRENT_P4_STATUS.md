...
- P4.388 提交：本提交 `feat: add p4 long sword assemble attachment`
...
P4 当前整体进度：按 P4.388 审计后预计 `388/392 = 99.0%`。已完成 P4.1-P4.388：在 P4.387 全部内容基础上，P4.388 将官网《长剑》最小 `ASSEMBLE_EQUIPMENT` 装配贴附接到 Core 可玩路径，仍保留灵便自动贴附、百炼 optional attach、owner/controller、装备随动和未激活文本为 deferred。当前仍不能标记 P4 goal complete：更多技能目标税/通用 skill registry、待命目标伤害或明确 deferred、以及最后 SignalR 或等价 E2E/全量验证文档收口仍待完成。预计还需约 `4` 批。
...
- `source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore --filter "FullyQualifiedName~P4AssembleEquipmentCommand"`：pass，26/26
- `source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore --filter "FullyQualifiedName~P4EquipmentKeywordProfilesKeepExistingNoAttachFixturesGreen"`：pass，19/19
- `jq empty tests/Riftbound.ConformanceTests/Fixtures/p4-assemble-equipment-long-sword-attach.fixture.json`：pass
- `git diff --check`：pass

## Next Step

进入 P4.389：优先在通用 skill registry/技能目标税，或待命目标伤害明确 deferred 中选择一个最低风险单 fixture 切片。预计 P4 还需要约 `4` 批：通用 skill registry/技能目标税、待命目标伤害或明确 deferred，以及最后 E2E/文档/全量验证收口。
