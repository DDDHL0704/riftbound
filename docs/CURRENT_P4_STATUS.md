...
| P4.8 装备关键词/剩余执行小批 | Pending | 0% | 优先识别装配/灵便/百炼 profile，避免直接进入贴附大系统。 |

P4 当前整体进度：按当前 part 计 `8/9 = 88.9%`；P4.1 已验证 `5` 个基础模板可安全委托到现有 P2 手写行为，P4.2 已新增最小权限关键词模型和 `1` 条 `迅捷` 法术对决焦点窗口可玩路径，P4.3 已新增 `瞬息` 开始阶段到期摧毁路径，P4.4 已新增 `回响` mana-only optional cost/repeat 显式模型，P4.5 已新增 `5` 个基础动作 primitive plan 并锁定 `move`/`recall` 继续委托 P2，P4.6 已新增 `5` 个战斗关键词 profile，P4.7 已新增 `4` 个资源关键词 profile。
...
- `source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore`：通过 `1665/1665`
- `source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore --filter "FullyQualifiedName~ConformanceFixtureRunnerTests"`：通过 `1600/1600`
- `source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore --filter "FullyQualifiedName~CardCatalogBaselineTests"`：通过 `15/15`
- `source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore --filter "FullyQualifiedName~P4Resource"`：通过 `6/6`
- `git diff --check`：通过

## Next Step

进入 P4.8：补装备关键词 profile，优先只识别 `装配` / `灵便` / `百炼` 与 no-optional/fixture 边界，暂不进入贴附、卸除、owner/controller 和武装费用大系统。
