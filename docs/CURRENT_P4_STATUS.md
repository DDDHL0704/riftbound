...
- P4.132 提交：本提交 `test: add p4 center stage echo cost fixture`
...
- `UNL-007/219 处罚` 继续作为非回响法术拒绝 `ECHO` optional cost 的负例；P4.132 已把《台前作秀》支付 `ECHO` 但 mana 不足的费用校验拒绝边界提升为 fixture。
- 复用并重新锁定三条已审计 conformance fixture：`p2-preflight-play-center-stage-echo-draw-stack`、`p2-preflight-play-the-curtain-rises-echo-ready-unit`、`p2-preflight-play-sandcraft-echo-create-two-sand-soldiers-base`；P4.132 额外锁定 `p4-play-center-stage-echo-insufficient-mana-rejected`。
...
| 补测试/文档/状态文件并提交 | `CardCatalogBaselineTests`、`ConformanceFixtureRunnerTests`、README、本文件、git commit | Covered for P4.132 once committed. |
...
- `source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore`：pass，1960/1960
- `source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore --filter "FullyQualifiedName~ConformanceFixtureRunnerTests"`：pass，1879/1879
...
- `source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore --filter "FullyQualifiedName~P4CenterStageEchoInsufficientManaRejectedFixture"`：pass，1/1
- `source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore --filter "FullyQualifiedName~P4EchoKeywordKeepsExistingP2FixturesGreen"`：pass，4/4
- `source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore --filter "FullyQualifiedName~P4InteractionKeywordProfilesKeepExistingRepresentativeFixturesGreen"`：pass，14/14
- `git diff --check`：pass

## Next Step

进入 P4.133：继续基于 completion audit 选择一个低风险、可验证的小批次。当前不能标记 P4 goal complete：更多技能目标税/通用 skill registry、待命触发/完整隐藏区/目标伤害、伏击真实反应战场打出、游走真实移动、完整战斗、完整装备装配/灵便/百炼、战斗/移动触发经验、《不死军团》废牌堆打出、德莱厄斯活跃/光环和其他急速牌彩色资源/活跃分支等仍有明确 deferred 项。
