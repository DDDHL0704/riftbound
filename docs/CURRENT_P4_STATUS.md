...
| P4.7 资源/装备/剩余关键词小批 | Pending | 0% | 优先选择法盾目标税、经验/等级/鼓舞或装备关键词中风险最低的一项。 |

P4 当前整体进度：按当前 part 计 `7/8 = 87.5%`；P4.1 已验证 `5` 个基础模板可安全委托到现有 P2 手写行为，P4.2 已新增最小权限关键词模型和 `1` 条 `迅捷` 法术对决焦点窗口可玩路径，P4.3 已新增 `瞬息` 开始阶段到期摧毁路径，P4.4 已新增 `回响` mana-only optional cost/repeat 显式模型，P4.5 已新增 `5` 个基础动作 primitive plan 并锁定 `move`/`recall` 继续委托 P2，P4.6 已新增 `5` 个战斗关键词 profile。
...
- `source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore`：通过 `1659/1659`
- `source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore --filter "FullyQualifiedName~ConformanceFixtureRunnerTests"`：通过 `1595/1595`
- `source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore --filter "FullyQualifiedName~CardCatalogBaselineTests"`：通过 `14/14`
- `source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore --filter "FullyQualifiedName~P4Combat"`：通过 `7/7`
- `git diff --check`：通过

## Next Step

进入 P4.7：继续补资源或装备关键词中的最低风险小批次；当前倾向先评估 `法盾` 目标税或经验/等级 profile，避免直接进入装配/贴附大系统。
