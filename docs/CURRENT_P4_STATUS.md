...
P4 当前整体进度：按计划 part 计 `3/7 = 42.9%`；P4.1 已验证 `5` 个基础模板可安全委托到现有 P2 手写行为，P4.2 已新增最小权限关键词模型和 `1` 条 `迅捷` 法术对决焦点窗口可玩路径。
...
- `source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore`：通过 `1640/1640`
- `source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore --filter "FullyQualifiedName~ConformanceFixtureRunnerTests"`：通过 `1579/1579`
...
进入 P4.3：实现生命周期/资源低风险小批，优先拆 `瞬息` 到期或 `预知` 最小回收分支；如果规则/状态模型仍缺口过大，先明确 blocked reason 并保持 P2/P2.5/P3/P4.2 绿色。
