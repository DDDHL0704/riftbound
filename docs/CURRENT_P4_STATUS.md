...
2026-06-28 B0 Interaction 注记：上表的“目标伤害 deferred”当前只适用于非 active battle/spell-duel base 上下文、其他待命牌族、combined battlefield-effect / defend-trigger target multiplexing 等未覆盖广度；battlefield-source OGN/SFD Teemo、base/spell-duel Teemo、base/battle-response Teemo 与 face-up defender Teemo 的同战场代表目标、顶五张展示、按 `待命` 数量伤害和展示牌回收已由 direct runtime regressions 覆盖，face-up defender Teemo 另覆盖多合法目标 prompt metadata 与显式目标选择。
