...
- P4.69 提交：本提交 `feat: add p4 face down snapshot redaction`
...
| 待命 | 47 | 6 | 0 | High | P4.9 已识别 profile；P4.63 已新增 `HIDE_CARD` typed command 与 Core 显式拒绝前置模型；P4.68 已新增 `REVEAL_CARD` typed command 与 Core 显式拒绝前置模型；P4.69 已补对手视角正面朝下对象 snapshot redaction；真实 face-down 放置、翻开打出和位置限制仍 deferred。 |
...
| Interaction remaining | `OGN·121/298 提莫`：待命；`OGN·199/298 控潮者`：待命；`UNL-021/219 阴森药剂师`：伏击；`UNL-176a/219 蔚`：伏击。 | P2 已有普通打出/静态 fixture；`回响` 已有 P4.4 mana-only 执行路径；P4.63 新增 `P4HideCardCommandIsExplicitlyRejectedUntilStandbyHiddenInfoExists` / `GameCommandMapperParsesHideCardPayload`；P4.64 新增 `P4AmbushPlayCardModeIsExplicitlyRejectedUntilBattlefieldReactionPlayExists` / `GameCommandMapperParsesAmbushPlayCardDestination`；P4.68 新增 `P4RevealCardCommandIsExplicitlyRejectedUntilStandbyRevealExists` / `GameCommandMapperParsesRevealCardPayload`；P4.69 新增 `SnapshotsRedactOpponentFaceDownObjects`。 | P4.9 已建立 interaction keyword profile；P4.63 已建立 `HIDE_CARD` command envelope 并显式拒绝执行；P4.64 已建立 `PLAY_CARD mode=AMBUSH` + `destination` envelope 并显式拒绝执行；P4.68 已建立 `REVEAL_CARD` command envelope 并显式拒绝执行；P4.69 已补对手视角正面朝下对象不泄漏；真实待命 face-down 放置/翻开和伏击 reaction battlefield play 仍 deferred。 |
...
| 互动关键词：待命、回响、伏击 | `CardInteractionKeywordRules`、`P4InteractionKeywordProfilesMapOfficialTextToRegistryTags`、`P4EchoKeywordKeepsExistingP2FixturesGreen`、`P4HideCardCommandIsExplicitlyRejectedUntilStandbyHiddenInfoExists`、`GameCommandMapperParsesHideCardPayload`、`P4RevealCardCommandIsExplicitlyRejectedUntilStandbyRevealExists`、`GameCommandMapperParsesRevealCardPayload`、`SnapshotsRedactOpponentFaceDownObjects`、`P4AmbushPlayCardModeIsExplicitlyRejectedUntilBattlefieldReactionPlayExists`、`GameCommandMapperParsesAmbushPlayCardDestination`、3 条 remaining fixture | Partial：mana-only 回响可玩，P4.63 已建立待命 `HIDE_CARD` command envelope 且显式拒绝执行，P4.68 已建立待命 `REVEAL_CARD` command envelope 且显式拒绝执行，P4.69 已补对手视角正面朝下对象 redaction，P4.64 已建立伏击 `PLAY_CARD mode=AMBUSH` + `destination` envelope 且显式拒绝执行，真实待命 face-down 放置/翻开与伏击 reaction battlefield play deferred。 |
...
| 补测试/文档/状态文件并提交 | `CardCatalogBaselineTests`、`ConformanceFixtureRunnerTests`、README、本文件、git commit | Covered for P4.69 once committed. |
...
| 互动关键词 | 回响 mana-only 已执行；P4.63/P4.68 已建立待命 hide/reveal command envelope 并显式拒绝执行；P4.69 已补对手视角正面朝下对象 redaction；真实待命 face-down 放置/翻开与伏击反应战场打出仍 deferred。 |
...
本阶段继续 completion audit：P4 仍不能标记 goal complete。`待命` 的官方文本要求“支付 A 正面朝下放置此牌，之后可支付 0 将其当作反应牌打出”，但当前公开 snapshot 会暴露对象细节，不能在没有隐藏信息视图前直接实现 face-down 放置。因此本批次只补 `HIDE_CARD` command 前置模型。
...
- `CoreRuleEngine` 对 `HideCardCommand` 显式返回 `UNSUPPORTED_COMMAND`，避免该 typed command 落入 `PlaceholderRuleEngine` 后被误接受。
- 新增 `P4HideCardCommandIsExplicitlyRejectedUntilStandbyHiddenInfoExists`：用 `OGN·121/298 提莫` 的待命文本作为代表，验证当前不会扣费用、不会把手牌移出、不会设置 `IsFaceDown`、不会生成 stack item。
- 本批次没有实现正面朝下放置、待命区/战场位置选择、隐藏信息 snapshot、从正面朝下状态翻开打出、待命触发、伏击或 P5/P6 批量迁移。
...
本阶段继续 completion audit：P4 仍不能标记 goal complete。`待命` 已有 P4.63 `HIDE_CARD` 放置前置模型，但官方文本还要求之后可支付 0 将该正面朝下牌作为反应牌打出；当前仍缺隐藏信息 snapshot、face-down 对象所有权视图和翻开结算模型，因此本批次只补 `REVEAL_CARD` command 前置模型。
...
- `CoreRuleEngine` 对 `RevealCardCommand` 显式返回 `UNSUPPORTED_COMMAND`，避免该 typed command 落入 `PlaceholderRuleEngine` 后被误接受。
- 新增 `P4RevealCardCommandIsExplicitlyRejectedUntilStandbyRevealExists`：用 `OGN·121/298 提莫` 的待命文本作为代表，验证当前不会翻开 `IsFaceDown`、不会从正面朝下区域移出、不会伤害目标、不会生成 stack item。
- 本批次没有实现正面朝下放置、隐藏信息 snapshot、从正面朝下状态翻开打出、待命触发、目标结算、伏击共用隐藏信息或 P5/P6 批量迁移。
...
- `REVEAL_CARD` 已有 P4.68 command envelope 与 Core 显式拒绝前置模型，可供后续待命翻开/显露小批次复用；当前不翻开正面朝下牌。
- Face-down snapshot redaction 已有 P4.69 对手视角防泄漏测试，可供后续待命/伏击隐藏信息小批次复用；当前仍不创建隐藏区或执行翻开。
...
| P4.68 completion audit + 待命 REVEAL_CARD command 前置模型 | Done | 100% | 审计确认 P4 仍不能标记 goal complete；新增 `RevealCardCommand`、JSON mapper 支持和 Core 显式 `UNSUPPORTED_COMMAND`，为待命翻开/隐藏信息后续小批次提供安全协议边界。 |
...
| P4.70 next low-risk gap | Pending | 0% | 基于 P4.69 audit 继续选择低风险可验证小批次；优先从技能 stack/目标税最小模型、待命隐藏区/翻开前置、基础动作 delegation 断言或剩余可审计资源边界中选一项，仍不进入 P5/P6/P7。 |

P4 当前整体进度：按当前 part 计 `70/71 = 98.6%`。已完成 P4.1-P4.69：template delegation/primitive plan、权限关键词代表时机、瞬息到期、回响 mana-only、战斗/资源/装备/生命周期/互动/basic-action profile、固定/动态经验、经验费用、法盾法术目标税、多目标税聚合与友方目标 no-tax 边界、`ACTIVATE_ABILITY` / `HIDE_CARD` / `REVEAL_CARD` / `MOVE_UNIT` / `ASSEMBLE_EQUIPMENT` / `DECLARE_BATTLE` command 前置模型、对手正面朝下对象 snapshot redaction、`PLAY_CARD mode=AMBUSH` 目的地前置模型、回收/放逐/增益 template skeleton、多条等级/鼓舞代表路径、34 条急速 `HASTE_READY` 代表路径、P4.58《取放自如》武装贴附/卸除代表路径。当前仍不能标记 P4 goal complete：战斗承伤/强攻修正、游走真实移动、真实待命 face-down 放置/翻开、伏击真实反应战场打出、技能目标税执行、完整装备装配/灵便/百炼、战斗/移动触发经验和若干复杂卡牌分支仍 deferred。
...
- `source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore`：pass，1815/1815
- `source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore --filter "FullyQualifiedName~ConformanceFixtureRunnerTests"`：pass，1735/1735
...
- `source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore --filter "FullyQualifiedName~SnapshotsRedactOpponentFaceDownObjects"`：pass，1/1
- `git diff --check`：pass

## Next Step

进入 P4.70：继续基于 completion audit 选择一个低风险、可验证的小批次。当前不能标记 P4 goal complete：技能目标税执行、真实待命 face-down 放置/翻开、伏击真实反应战场打出、游走真实移动、完整战斗、完整装备装配/灵便/百炼、战斗/移动触发经验、《不死军团》废牌堆打出、德莱厄斯活跃/光环和其他急速牌彩色资源/活跃分支等仍有明确 deferred 项。
