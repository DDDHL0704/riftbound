...
- P4.329 提交：本提交 `test: add p4 move unit precise origin rejection`
...
| P4.330 next low-risk gap | Pending | 0% | 基于 P4.329 audit 继续选择低风险可验证小批次；优先从移动/伏击/待命边界、技能边界、更多法盾边界、基础动作目标合法性或装备前置边界中选一项，仍不进入 P5/P6/P7。 |

P4 当前整体进度：按当前 part 计 `330/331 = 99.7%`。已完成 P4.1-P4.329：在 P4.328 全部内容基础上，P4.329 新增 `MOVE_UNIT` precise origin rejection fixture，锁定当前 coarse 移动即使没有 `ROAM` 也不能把 `BATTLEFIELD:P1-MAIN` 等精确战场来源当作普通 `BATTLEFIELD` 来源；拒绝时不推进 tick、不写事件、不移动对象、不改变战力/横置状态/标签、不创建结算链项目。当前仍不能标记 P4 goal complete：战斗承伤/强攻修正、真实跨战场游走、待命触发/完整隐藏区/目标伤害、伏击真实反应战场打出、更多技能目标税/通用 skill registry、完整装备装配/灵便/百炼、战斗/移动触发经验和若干复杂卡牌分支仍 deferred。
...
- `jq empty tests/Riftbound.ConformanceTests/Fixtures/p4-move-unit-precise-origin-rejected.fixture.json`：pass
- `source scripts/dev-env.sh && dotnet build Riftbound.slnx --no-restore`：pass，0 warnings，0 errors
- `source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore`：pass，2385/2385
- `source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore --filter "FullyQualifiedName~ConformanceFixtureRunnerTests"`：pass，2304/2304
...
- `source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore --filter "FullyQualifiedName~P4MoveUnitCommand"`：pass，30/30
- `source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore --filter "FullyQualifiedName~P4BasicActionProfilesKeepExistingRepresentativeFixturesGreen"`：pass，213/213
- `source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore --filter "FullyQualifiedName~P4CombatKeywordProfilesKeepExistingKeywordUnitFixturesGreen"`：pass，8/8
- `git diff --check`：pass

## Next Step

进入 P4.330：继续基于 completion audit 选择一个低风险、可验证的小批次。当前不能标记 P4 goal complete：更多技能目标税/通用 skill registry、待命触发/完整隐藏区/目标伤害、伏击真实反应战场打出、真实跨战场游走、完整战斗、完整装备装配/灵便/百炼、战斗/移动触发经验、《不死军团》废牌堆打出、德莱厄斯活跃/光环和其他急速牌彩色资源/活跃分支等仍有明确 deferred 项。
