...
- P4.355 提交：本提交 `test: add p4 assemble equipment hand source rejection`
...
| 装配 | 51 | 0 | 0 | High | P4.8 已识别 profile；P4.58 已审计并锁定《取放自如》贴附/卸除武装的代表执行路径；P4.66 已新增 `ASSEMBLE_EQUIPMENT` typed command 与 Core 显式拒绝前置模型；P4.99 已补同一路径拒绝 fixture；P4.330 已补同一路径优先权窗口拒绝 fixture；P4.331 已补已贴附来源拒绝 fixture；P4.353/P4.354 已补缺少目标/来源拒绝 fixture；P4.355 已补手牌来源拒绝 fixture；P4.334 已补移动带贴附装备来源拒绝 fixture；装配费用、装备未激活文本和自动贴附仍 deferred。 |
| 灵便 | 6 | 0 | 0 | High | P4.8 已识别 profile；P4.58 的代表路径只覆盖已有反应法术《取放自如》；P4.66 已为灵便装备后续自动贴附保留 `ASSEMBLE_EQUIPMENT` envelope，P4.99/P4.330/P4.331/P4.353/P4.354/P4.355 已补前置、优先权窗口、已贴附来源、缺少目标、缺少来源和手牌来源拒绝 fixture；真实反应打出和自动贴附仍 deferred。 |
| 百炼 | 16 | 2 | 0 | High | P4.8 已识别 profile；P4.66 已为百炼 optional attach 保留 `ASSEMBLE_EQUIPMENT` envelope，P4.99/P4.330/P4.331/P4.353/P4.354/P4.355 已补前置、优先权窗口、已贴附来源、缺少目标、缺少来源和手牌来源拒绝 fixture；FAQ 指明的可选装配和贴附边界仍 deferred，P4.58/P4.66 不改变该边界。 |
...
| 补测试/文档/状态文件并提交 | `CardCatalogBaselineTests`、`ConformanceFixtureRunnerTests`、README、本文件、git commit | Covered for P4.354 once committed. |
...
| 装备关键词 | P4.58 覆盖《取放自如》武装贴附/卸除代表路径；P4.66 建立 `ASSEMBLE_EQUIPMENT` command envelope 且显式拒绝执行；P4.99/P4.330/P4.331/P4.353/P4.354/P4.355 已把 `ASSEMBLE_EQUIPMENT` 前置、优先权窗口、已贴附来源、缺少目标、缺少来源和手牌来源拒绝边界纳入 fixture；装配/灵便/百炼的完整贴附、费用和未激活文本仍 deferred。 |
...
- `ASSEMBLE_EQUIPMENT` 已有 P4.66 command envelope 与 Core 显式拒绝前置模型，P4.99/P4.330/P4.331/P4.353/P4.354/P4.355 已补普通窗口、优先权窗口、已贴附来源、缺少目标、缺少来源和手牌来源 conformance fixtures，可供后续装配/灵便/百炼小批次复用；当前不贴附装备。
...
| P4.356 next low-risk gap | Pending | 0% | 基于 P4.355 audit 继续选择低风险可验证小批次；优先从待命边界、技能边界、更多法盾边界、装备边界、战斗/移动边界或基础动作目标合法性中选一项，仍不进入 P5/P6/P7。 |

P4 当前整体进度：按当前 part 计 `356/357 = 99.7%`。已完成 P4.1-P4.355：在 P4.354 全部内容基础上，P4.355 新增 `ASSEMBLE_EQUIPMENT` hand source rejection fixture，锁定完整装备装配模型完成前手牌中的装备不能直接作为装配来源；拒绝时不推进 tick、不写事件、不支付费用、不移动手牌或基地对象、不设置贴附、不创建结算链。当前仍不能标记 P4 goal complete：战斗承伤/强攻修正、真实跨战场游走、装备随动/完整装配、待命触发/完整隐藏区/目标伤害、伏击真实反应战场打出、更多技能目标税/通用 skill registry、完整灵便/百炼、战斗/移动触发经验和若干复杂卡牌分支仍 deferred。
...
- `jq empty tests/Riftbound.ConformanceTests/Fixtures/p4-assemble-equipment-hand-source-rejected.fixture.json`：pass
- `source scripts/dev-env.sh && dotnet build Riftbound.slnx --no-restore`：pass，0 warnings，0 errors
- `source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore`：pass，2465/2465
- `source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore --filter "FullyQualifiedName~ConformanceFixtureRunnerTests"`：pass，2384/2384
...
- `source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore --filter "FullyQualifiedName~P4AssembleEquipmentCommand"`：pass，12/12
- `source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore --filter "FullyQualifiedName~P4EquipmentKeywordProfilesKeepExistingNoAttachFixturesGreen"`：pass，12/12
- `git diff --check`：pass

## Next Step

进入 P4.356：继续基于 completion audit 选择一个低风险、可验证的小批次。当前不能标记 P4 goal complete：更多技能目标税/通用 skill registry、待命触发/完整隐藏区/目标伤害、伏击真实反应战场打出、真实跨战场游走、完整战斗、完整装备装配/灵便/百炼、战斗/移动触发经验、《不死军团》废牌堆打出、德莱厄斯活跃/光环和其他急速牌彩色资源/活跃分支等仍有明确 deferred 项。
