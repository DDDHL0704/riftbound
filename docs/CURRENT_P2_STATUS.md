...
- 当前 P2 功能基线：已覆盖 `UNL-184/219 狩猎律动` 的当前 preflight 友方单位放逐后重新打出到战场路径；最新提交以 `git log -1 --oneline` 为准
- 上一个 P2 功能基线：`OGN·102/298 传送门大营救` 的当前 preflight 友方单位放逐后重新打出到所属基地路径
- 最近全量验证：`dotnet test Riftbound.slnx --no-restore` 通过 `348/348`
- 最小 card behavior registry：`170/811 = 21.0%`
- P2 preflight 清单：已完成到 `203`，下一项是 `204. 逐批迁移更多低复杂度官方卡牌`
