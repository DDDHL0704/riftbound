...
- 当前 P2 功能基线：已覆盖 `OGN·102/298 传送门大营救` 的当前 preflight 友方单位放逐后重新打出到所属基地路径；最新提交以 `git log -1 --oneline` 为准
- 上一个 P2 功能基线：`OGN·268/298 弹幕时间` 的当前 preflight 支付任意符能后按支付数值伤害敌方战场单位路径
- 最近全量验证：`dotnet test Riftbound.slnx --no-restore` 通过 `346/346`
- 最小 card behavior registry：`169/811 = 20.8%`
- P2 preflight 清单：已完成到 `202`，下一项是 `203. 逐批迁移更多低复杂度官方卡牌`
...
- `OGN·102/298 传送门大营救`：当前 preflight 覆盖友方战场单位被放逐后重新打出到所属基地，并清除场上伤害与本回合内效果。
