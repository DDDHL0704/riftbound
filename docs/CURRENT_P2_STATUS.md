...
- 当前 P2 功能基线：已覆盖简单装配装备对象小批次到 `SFD·051/221 守护天使` 的当前 preflight 装备入场路径；最新提交以 `git log -1 --oneline` 为准
- 上一个 P2 功能基线：`SFD·124/221 多兰之戒`、`SFD·153/221 先锋之眼`、`SFD·016/221 反曲之弓`、`SFD·042/221 残暴之力` 的当前 preflight 装备入场路径
- 最近全量验证：`dotnet test Riftbound.slnx --no-restore` 通过 `398/398`
- 最小 card behavior registry：`195/811 = 24.0%`
- P2 preflight 清单：已完成到 `228`，下一项是 `229. 按同能力族小批次迁移更多低复杂度官方卡牌`
