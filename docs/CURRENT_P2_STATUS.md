...
- 当前 P2 功能基线：已覆盖到 `OGN·224/298 废物利用` 装备摧毁分支；最新提交以 `git log -1 --oneline` 为准
- 上一个 P2 功能基线：`OGN·179/298 折戟再战`
- 最近全量验证：`dotnet test Riftbound.slnx --no-restore` 通过 `322/322`
- 最小 card behavior registry：`152/811 = 18.7%`
- P2 preflight 清单：已完成到 `184`，下一项是 `185. 逐批迁移更多低复杂度官方卡牌`
