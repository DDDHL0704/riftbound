...
- 当前 P2 功能基线：已覆盖 `SFD·198/221 沙兵现身` 在控制 0 件武装时让两名既有“黄沙士兵”变为活跃状态的当前 preflight 路径；最新提交以 `git log -1 --oneline` 为准
- 上一个 P2 功能基线：`SFD·004/221 丛林伏击` 打出休眠“金币”装备指示物的当前 preflight 路径
- 最近全量验证：`dotnet test Riftbound.slnx --no-restore` 通过 `331/331`
- 最小 card behavior registry：`158/811 = 19.5%`
- P2 preflight 清单：已完成到 `191`，下一项是 `192. 逐批迁移更多低复杂度官方卡牌`
