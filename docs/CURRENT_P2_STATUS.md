...
- 当前 P2 功能基线：已覆盖 `UNL-083/219 镜中幻影` 的当前 preflight 互换两名友方单位位置后抽 1 路径；最新提交以 `git log -1 --oneline` 为准
- 上一个 P2 功能基线：`OGN·153/298 公开行动` 的当前 preflight 无既有增益可消耗时给予所有友方单位增益路径
- 最近全量验证：`dotnet test Riftbound.slnx --no-restore` 通过 `339/339`
- 最小 card behavior registry：`165/811 = 20.3%`
- P2 preflight 清单：已完成到 `198`，下一项是 `199. 逐批迁移更多低复杂度官方卡牌`
