...
- 当前 P2 功能基线：已覆盖 `OGN·153/298 公开行动` 的当前 preflight 无既有增益可消耗时给予所有友方单位增益路径；最新提交以 `git log -1 --oneline` 为准
- 上一个 P2 功能基线：`SFD·188/221 虚空猛冲` 的当前 preflight 不选择免费打出、抽取两张展示牌路径
- 最近全量验证：`dotnet test Riftbound.slnx --no-restore` 通过 `337/337`
- 最小 card behavior registry：`164/811 = 20.2%`
- P2 preflight 清单：已完成到 `197`，下一项是 `198. 逐批迁移更多低复杂度官方卡牌`
