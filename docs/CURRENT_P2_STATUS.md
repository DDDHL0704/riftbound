...
- 当前 P2 功能基线：已覆盖 `OGN·250/298 天声震落` 的当前 preflight 按基地单位战力伤害敌方战场单位并移动到战场路径；最新提交以 `git log -1 --oneline` 为准
- 上一个 P2 功能基线：`UNL-083/219 镜中幻影` 的当前 preflight 互换两名友方单位位置后抽 1 路径
- 最近全量验证：`dotnet test Riftbound.slnx --no-restore` 通过 `340/340`
- 最小 card behavior registry：`166/811 = 20.5%`
- P2 preflight 清单：已完成到 `199`，下一项是 `200. 逐批迁移更多低复杂度官方卡牌`
