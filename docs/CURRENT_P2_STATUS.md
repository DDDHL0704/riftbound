...
- 当前 P2 功能基线：已覆盖 `OGN·270/298 叹为观止` 的当前 preflight 基地友方单位增益并移动到战场路径；最新提交以 `git log -1 --oneline` 为准
- 上一个 P2 功能基线：`OGN·053/298 秘奥义！慈悲度魂落` 的当前 preflight 单目标增益路径
- 最近全量验证：`dotnet test Riftbound.slnx --no-restore` 通过 `335/335`
- 最小 card behavior registry：`162/811 = 20.0%`
- P2 preflight 清单：已完成到 `195`，下一项是 `196. 逐批迁移更多低复杂度官方卡牌`
