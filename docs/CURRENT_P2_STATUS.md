...
- 当前 P2 功能基线：已覆盖 `UNL-101/219 战斗号令` 的当前 preflight 友方单位与对手所选单位移动到战场路径；最新提交以 `git log -1 --oneline` 为准
- 上一个 P2 功能基线：`OGN·250/298 天声震落` 的当前 preflight 按基地单位战力伤害敌方战场单位并移动到战场路径
- 最近全量验证：`dotnet test Riftbound.slnx --no-restore` 通过 `342/342`
- 最小 card behavior registry：`167/811 = 20.6%`
- P2 preflight 清单：已完成到 `200`，下一项是 `201. 逐批迁移更多低复杂度官方卡牌`
