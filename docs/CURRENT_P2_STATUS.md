...
- 当前 P2 功能基线：已覆盖简单装配装备对象小批次到 `UNL-096/219 猎人的宽刃刀` 的当前 preflight 装备入场路径；最新提交以 `git log -1 --oneline` 为准
- 上一个 P2 功能基线：`SFD·134/221 萃取` 的当前 preflight 装备入场路径
- 最近全量验证：`dotnet test Riftbound.slnx --no-restore` 通过 `418/418`
- 最小 card behavior registry：`205/811 = 25.3%`
- P2 preflight 清单：已完成到 `238`，下一项是 `239. 按同能力族小批次迁移更多低复杂度官方卡牌`
