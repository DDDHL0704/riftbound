...
- 当前 P2 功能基线：已覆盖简单装备对象小批次到 `OGN·143/298 海盗避风港` 的当前 preflight 装备入场路径；最新提交以 `git log -1 --oneline` 为准
- 上一个 P2 功能基线：`SFD·078/221 预时之门`、`OGN·032/298 邪鸦魔典`、`OGN·021/298 太阳圆盘`、`OGN·060/298 远见面具`、`OGN·072/298 烈阳圣坛` 的当前 preflight 装备入场路径
- 最近全量验证：`dotnet test Riftbound.slnx --no-restore` 通过 `448/448`
- 最小 card behavior registry：`220/811 = 27.1%`
- P2 preflight 清单：已完成到 `253`，下一项是 `254. 按同能力族小批次迁移更多低复杂度官方卡牌`
