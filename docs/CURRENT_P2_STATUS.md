...
- 当前 P2 功能基线：已覆盖 `SFD·004/221 丛林伏击` 打出休眠“金币”装备指示物的当前 preflight 路径；最新提交以 `git log -1 --oneline` 为准
- 上一个 P2 功能基线：`OGN·071/298 次元门狂欢` 在当前 2P preflight 中对手选择“卡牌”或“符文”的两个 mode 分支
- 最近全量验证：`dotnet test Riftbound.slnx --no-restore` 通过 `330/330`
- 最小 card behavior registry：`157/811 = 19.4%`
- P2 preflight 清单：已完成到 `190`，下一项是 `191. 逐批迁移更多低复杂度官方卡牌`
