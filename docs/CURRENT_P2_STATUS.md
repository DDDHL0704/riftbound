...
- 当前 P2 功能基线：已覆盖简单装备对象小批次到 `OGN·072/298 烈阳圣坛` 的当前 preflight 装备入场路径；最新提交以 `git log -1 --oneline` 为准
- 上一个 P2 功能基线：`UNL-019/219 枯萎战斧`、`SFD·118/221 碎骨棒`、`SFD·117/221 远古簇碑`、`SFD·083/221 海克斯异常体`、`OGN·098/298 能量通道` 的当前 preflight 装备入场路径
- 最近全量验证：`dotnet test Riftbound.slnx --no-restore` 通过 `438/438`
- 最小 card behavior registry：`215/811 = 26.5%`
- P2 preflight 清单：已完成到 `248`，下一项是 `249. 按同能力族小批次迁移更多低复杂度官方卡牌`
