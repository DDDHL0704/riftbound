...
- 当前 P2 功能基线：已覆盖 `SFD·184/221 冷酷追击` 的当前 preflight 友方单位移动到所属基地并获得本回合征服后可召回标记路径；最新提交以 `git log -1 --oneline` 为准
- 上一个 P2 功能基线：`OGN·258/298 猛龙摆尾` 的当前 preflight 敌方单位移动到另一名敌方单位所在位置后互伤路径
- 最近全量验证：`dotnet test Riftbound.slnx --no-restore` 通过 `356/356`
- 最小 card behavior registry：`174/811 = 21.5%`
- P2 preflight 清单：已完成到 `207`，下一项是 `208. 逐批迁移更多低复杂度官方卡牌`
