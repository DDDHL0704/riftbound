...
- 当前 P2 功能基线：已覆盖简单装配装备对象小批次到 `SFD·134/221 萃取` 的当前 preflight 装备入场路径；最新提交以 `git log -1 --oneline` 为准
- 上一个 P2 功能基线：`SFD·102/221 海克斯饮魔刀`、`SFD·108/221 狂徒铠甲`、`SFD·115/221 三相之力`、`SFD·133/221 轻灵之靴` 的当前 preflight 装备入场路径
- 最近全量验证：`dotnet test Riftbound.slnx --no-restore` 通过 `408/408`
- 最小 card behavior registry：`200/811 = 24.7%`
- P2 preflight 清单：已完成到 `233`，下一项是 `234. 按同能力族小批次迁移更多低复杂度官方卡牌`
