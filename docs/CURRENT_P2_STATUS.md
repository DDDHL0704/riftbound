...
- 当前 P2 功能基线：已覆盖 `SFD·046/221 魄罗佳肴` 的当前 preflight 装备入场到基地并抽 1 路径；最新提交以 `git log -1 --oneline` 为准
- 上一个 P2 功能基线：`SFD·184/221 冷酷追击` 的当前 preflight 友方单位移动到所属基地并获得本回合征服后可召回标记路径
- 最近全量验证：`dotnet test Riftbound.slnx --no-restore` 通过 `358/358`
- 最小 card behavior registry：`175/811 = 21.6%`
- P2 preflight 清单：已完成到 `208`，下一项是 `209. 逐批迁移更多低复杂度官方卡牌`
