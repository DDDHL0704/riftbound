...
- 当前 P2 功能基线：已覆盖到 `SFD·162/221 血钱`；最新提交以 `git log -1 --oneline` 为准
- 上一个 P2 功能基线：`SFD·077/221 火箭轰击` 装备摧毁模式
- 最近全量验证：`dotnet test Riftbound.slnx --no-restore` 通过 `318/318`
- 最小 card behavior registry：`151/811 = 18.6%`
- P2 preflight 清单：已完成到 `182`，下一项是 `183. 逐批迁移更多低复杂度官方卡牌`
