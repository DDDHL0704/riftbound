...
- 当前 P2 功能基线：已覆盖 `OGN·198/298 蚀魂夜` 的当前 preflight 己方废牌堆单位牌打出到基地路径；最新提交以 `git log -1 --oneline` 为准
- 上一个 P2 功能基线：`UNL-184/219 狩猎律动` 的当前 preflight 友方单位放逐后重新打出到战场路径
- 最近全量验证：`dotnet test Riftbound.slnx --no-restore` 通过 `350/350`
- 最小 card behavior registry：`171/811 = 21.1%`
- P2 preflight 清单：已完成到 `204`，下一项是 `205. 逐批迁移更多低复杂度官方卡牌`
