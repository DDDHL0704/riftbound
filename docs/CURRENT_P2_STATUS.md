...
- 当前 P2 功能基线：已覆盖 `OGN·268/298 弹幕时间` 的当前 preflight 支付任意符能后按支付数值伤害敌方战场单位路径；最新提交以 `git log -1 --oneline` 为准
- 上一个 P2 功能基线：`UNL-101/219 战斗号令` 的当前 preflight 友方单位与对手所选单位移动到战场路径
- 最近全量验证：`dotnet test Riftbound.slnx --no-restore` 通过 `344/344`
- 最小 card behavior registry：`168/811 = 20.7%`
- P2 preflight 清单：已完成到 `201`，下一项是 `202. 逐批迁移更多低复杂度官方卡牌`
