...
- 当前 P2 功能基线：已覆盖装备对象小批次到 `UNL-136/219 占卜花朵` 的当前 preflight 休眠装备入场路径；最新提交以 `git log -1 --oneline` 为准
- 上一个 P2 功能基线：`OGN·182/298 废料堆`、`UNL-078/219 精灵提灯`、`UNL-085/219 地沟区地图` 的当前 preflight 装备入场路径
- 最近全量验证：`dotnet test Riftbound.slnx --no-restore` 通过 `370/370`
- 最小 card behavior registry：`181/811 = 22.3%`
- P2 preflight 清单：已完成到 `214`，下一项是 `215. 按同能力族小批次迁移更多低复杂度官方卡牌`
