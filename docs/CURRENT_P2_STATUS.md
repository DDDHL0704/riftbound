...
- 当前 P2 功能基线：已覆盖 `OGN·129/298 迎敌号令` 的当前 preflight 即时抽 1 路径；最新提交以 `git log -1 --oneline` 为准
- 上一个 P2 功能基线：`SFD·166/221 集结部队` 的当前 preflight 即时抽 1 路径
- 最近全量验证：`dotnet test Riftbound.slnx --no-restore` 通过 `333/333`
- 最小 card behavior registry：`160/811 = 19.7%`
- P2 preflight 清单：已完成到 `193`，下一项是 `194. 逐批迁移更多低复杂度官方卡牌`
