...
- 当前 P2 功能基线：已覆盖简单装配装备对象小批次到 `SFD·095/221 多兰之刃` 的当前 preflight 装备入场路径；最新提交以 `git log -1 --oneline` 为准
- 上一个 P2 功能基线：`UNL-039/219 灵魂之剑`、`SFD·009/221 锯齿短匕`、`SFD·033/221 多兰之盾`、`SFD·073/221 海克斯注力刚壁` 的当前 preflight 装备入场路径
- 最近全量验证：`dotnet test Riftbound.slnx --no-restore` 通过 `388/388`
- 最小 card behavior registry：`190/811 = 23.4%`
- P2 preflight 清单：已完成到 `223`，下一项是 `224. 按同能力族小批次迁移更多低复杂度官方卡牌`
