...
- 当前 P2 功能基线：已覆盖 `SFD·192/221 舒瑞娅的安魂曲` 的当前 preflight 装备入场到基地并让所有友方单位变为活跃路径；最新提交以 `git log -1 --oneline` 为准
- 上一个 P2 功能基线：`SFD·046/221 魄罗佳肴` 的当前 preflight 装备入场到基地并抽 1 路径
- 最近全量验证：`dotnet test Riftbound.slnx --no-restore` 通过 `360/360`
- 最小 card behavior registry：`176/811 = 21.7%`
- P2 preflight 清单：已完成到 `209`，下一项是 `210. 逐批迁移更多低复杂度官方卡牌`
