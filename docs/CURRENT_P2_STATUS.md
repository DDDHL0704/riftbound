...
- 当前 P2 功能基线：已覆盖 `OGN·008/298 罪恶快感` 弃置友方手牌后按该牌法力费用对战场单位造成伤害的路径；最新提交以 `git log -1 --oneline` 为准
- 上一个 P2 功能基线：`UNL-200/219 镜花水月` 在当前最小对象模型中创建活跃 `瞬息`“映像”复制体
- 最近全量验证：`dotnet test Riftbound.slnx --no-restore` 通过 `327/327`
- 最小 card behavior registry：`154/811 = 19.0%`
- P2 preflight 清单：已完成到 `188`，下一项是 `189. 逐批迁移更多低复杂度官方卡牌`
