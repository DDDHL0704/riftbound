...
- 当前 P2 功能基线：已覆盖 `OGN·258/298 猛龙摆尾` 的当前 preflight 敌方单位移动到另一名敌方单位所在位置后互伤路径；最新提交以 `git log -1 --oneline` 为准
- 上一个 P2 功能基线：`UNL-202/219 虚空来袭` 的当前 preflight 友方单位与敌方单位移动到战场路径
- 最近全量验证：`dotnet test Riftbound.slnx --no-restore` 通过 `354/354`
- 最小 card behavior registry：`173/811 = 21.3%`
- P2 preflight 清单：已完成到 `206`，下一项是 `207. 逐批迁移更多低复杂度官方卡牌`
