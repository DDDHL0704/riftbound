...
- 当前 P2 功能基线：已覆盖 `SFD·188/221 虚空猛冲` 的当前 preflight 不选择免费打出、抽取两张展示牌路径；最新提交以 `git log -1 --oneline` 为准
- 上一个 P2 功能基线：`OGN·270/298 叹为观止` 的当前 preflight 基地友方单位增益并移动到战场路径
- 最近全量验证：`dotnet test Riftbound.slnx --no-restore` 通过 `336/336`
- 最小 card behavior registry：`163/811 = 20.1%`
- P2 preflight 清单：已完成到 `196`，下一项是 `197. 逐批迁移更多低复杂度官方卡牌`
