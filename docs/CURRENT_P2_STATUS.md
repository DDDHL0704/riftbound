...
- 当前 P2 功能基线：已覆盖装备对象小批次到 `OGN·090/298 懊悔法球` 的当前 preflight 装备入场路径；最新提交以 `git log -1 --oneline` 为准
- 上一个 P2 功能基线：`UNL-011/219 魔法鲜豆`、`OGN·017/298 钢铁弩炮`、`SFD·052/221 玄冰之心` 的当前 preflight 装备入场路径
- 最近全量验证：`dotnet test Riftbound.slnx --no-restore` 通过 `378/378`
- 最小 card behavior registry：`185/811 = 22.8%`
- P2 preflight 清单：已完成到 `218`，下一项是 `219. 按同能力族小批次迁移更多低复杂度官方卡牌`
