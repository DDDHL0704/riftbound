2026-05-24 Stage 4D-16K evidence.
...
```bash
source scripts/dev-env.sh && dotnet test tests/Riftbound.ConformanceTests/Riftbound.ConformanceTests.csproj --no-restore --filter "FullyQualifiedName~PromptIdOnlyPlayCardWithFirstReadyPromptAfterFinalReadyRejectsWithoutMutation|FullyQualifiedName~SnapshotOnlyPlayCardWithFirstReadyPromptAfterFinalReadyRejectsWithoutMutation"
```
...
Adjacent result: passed `995/995`.
...
Backend full result: passed `5947/5947`.
