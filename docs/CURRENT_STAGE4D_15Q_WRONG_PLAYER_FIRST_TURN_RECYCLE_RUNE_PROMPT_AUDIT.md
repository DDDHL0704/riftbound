# Stage 4D-15Q Wrong Player First Turn Recycle Rune Prompt Audit

Status: accepted test-only server P0-005 official first-turn wrong-player current `MAIN_ACTION` prompt `RECYCLE_RUNE` boundary slice.
...
- `tests/Riftbound.ConformanceTests/OfficialOpeningTests.cs`
  - Added `WrongPlayerFirstTurnRecycleRunePromptRejectsWithoutMutation`.
  - Added `PromptIdOnlyWrongPlayerFirstTurnRecycleRunePromptRejectsWithoutMutation`.
  - Added `SnapshotOnlyWrongPlayerFirstTurnRecycleRunePromptRejectsWithoutMutation`.
  - Added `AssertWrongPlayerFirstTurnRecycleRunePromptRejectsWithoutMutation`.
...
- Non-active-player `RECYCLE_RUNE` carrying the active player's prompt-scoped or prompt-id-only current `MAIN_ACTION` prompt envelope rejects with `ErrorCodes.PromptExpired` and exact diagnostics: `行动窗口已过期，请按最新提示重新提交。`
- Non-active-player `RECYCLE_RUNE` carrying only the same-tick snapshot reaches the open-main current-player legality guard and rejects with `ErrorCodes.PhaseNotAllowed` and exact diagnostics: `回收符文只能在当前玩家的开放主阶段提交。`
...
Runtime behavior was not changed. This continues the current active-player `MAIN_ACTION` prompt wrong-player boundary beyond `END_TURN`, `MULLIGAN`, `PASS`, `PASS_PRIORITY`, `PASS_FOCUS`, `MOVE_UNIT`, `DECLARE_BATTLE`, `PLAY_CARD`, `ACTIVATE_ABILITY` and `TAP_RUNE`: prompt id ownership is rejected before command handling, while snapshot-only same-tick submissions still prove command-specific recycle-rune current-player legality without mutation.
