2026-05-24 Stage 4D-16F evidence.
...
```bash
source scripts/dev-env.sh && dotnet test tests/Riftbound.ConformanceTests/Riftbound.ConformanceTests.csproj --no-restore --filter "FullyQualifiedName~PromptIdOnlyPassWithFirstReadyPromptAfterFinalReadyRejectsWithoutMutation|FullyQualifiedName~SnapshotOnlyPassWithFirstReadyPromptAfterFinalReadyRejectsWithoutMutation"
```
...
Adjacent result: passed `985/985`.
...
Backend full result: passed `5937/5937`.
