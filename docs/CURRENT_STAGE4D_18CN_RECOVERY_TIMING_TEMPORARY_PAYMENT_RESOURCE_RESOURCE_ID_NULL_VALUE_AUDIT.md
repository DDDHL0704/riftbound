# Stage 4D-18CN Recovery Timing Temporary Payment Resource Resource Id Null Value Audit
...
Added `RecoveryValidatorRejectsSpectatorReplayTimingTemporaryPaymentResourceResourceIdNullValueWithCountMismatch` in `tests/Riftbound.ConformanceTests/MatchRecoveryTests.cs`.

The test builds an authoritative temporary payment resource from real `MatchState` source-object state, changes the emitted spectator payload's `resourceId` to `null`, then appends `temp-payment-resource-extra` to force resource-count mismatch.

It proves validation still emits:

- null resource-id required diagnostics for the malformed item;
...
- Focused `TemporaryPaymentResource` filter: `87/87`.
- Focused recovery filter: `1120/1120`.
- Adjacent recovery/official-opening/Postgres recovery-store filter: `1701/1701`.
- Backend full: `7066/7066`.
