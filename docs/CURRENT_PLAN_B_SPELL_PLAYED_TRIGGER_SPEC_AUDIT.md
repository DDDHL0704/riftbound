...
## 2026-07-02 Follow-up: Spell-Played Trigger Predicate Surface

`SpellPlayedTriggerSpecRules` no longer exposes the four public per-effect getters for unit spell-play power, unit high-cost spell power, legend high-cost spell draw, or legend high-cost banish completion. The accepted API is now `TryGetTrigger(cardNo, predicate, out trigger)` plus shared `Is...Trigger` shape predicates. `CoreRuleEngine` and `MatchRecovery` route all current spell-play trigger lookups through that generic predicate surface, preserving OGS Lux compatibility `EffectKind`, Lux intro legend draw, Ravenbloom/Diana power modifiers, Jhin banish completion, trigger queue payloads, and recovery validation behavior. Validation: focused guard / representative runtime set `61/61`, adjacent spell-play / recovery / full-game set `2604/2604`, backend full conformance `9140/9140`. This follow-up does not introduce a new official-text interpretation and does not close optional Jhin prompts, APNAP ordering, complete spell-play trigger breadth, P0, or READY.
...
| Core no longer uses a Ravenbloom-specific spell-play resolver | `ResolveRavenbloomStudentSpellPlayedTriggers` is removed; `CoreRuleEngine` calls `ResolveUnitSpellPlayedPowerModifierTriggers` and checks `SpellPlayedTriggerSpecRules.TryGetTrigger(..., IsUnitSpellPlayedPowerModifierTrigger, ...)` | Accepted |
