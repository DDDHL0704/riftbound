# Stage 4D-18CW Recovery Timing Temporary Payment Resource Source Object Id Empty Value Audit
...
Added `RecoveryValidatorRejectsSpectatorReplayTimingTemporaryPaymentResourceKeyedSourceObjectIdEmptyValueWithCountMismatch` in `tests/Riftbound.ConformanceTests/MatchRecoveryTests.cs`.

The test builds an authoritative temporary payment resource from real `MatchState` source-object state, keeps the emitted spectator payload keyed to `temp-payment-resource-1`, changes `sourceObjectId` to an empty string, then appends `temp-payment-resource-extra` to force resource-count mismatch.
...
- Focused `TemporaryPaymentResource` filter: `96/96`.
- Focused recovery filter: `1129/1129`.
- Adjacent recovery/official-opening/Postgres recovery-store filter: `1710/1710`.
- Backend full: `7075/7075`.
