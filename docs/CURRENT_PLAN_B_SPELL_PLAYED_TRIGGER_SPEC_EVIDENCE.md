...
This file records concrete evidence for routing OGS Lux high-cost spell and Ravenbloom Student spell-play power representative triggers through BehaviorSpec instead of single-card Core resolvers.
...
`RuleTextParser` now parses the official Ravenbloom Student unit text into:
...
The parser evidence is covered by `BehaviorSpecCatalogParsesUnitSpellPlayedPowerModifierTrigger`.
...
- The current Ravenbloom Student power path keeps trigger id `RAVENBLOOM_STUDENT_SPELL_POWER_PLUS_1` for event / fixture compatibility; the trigger source and parameters are now BehaviorSpec driven.
...
This is a focused representative slice. It does not close complete spell-play trigger extraction, complete trigger ordering / APNAP, Jhin high-cost spell banish migration, PaymentEngine full breadth, MatchRecovery generic effectKind migration, or project READY.
