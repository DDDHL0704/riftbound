...
- `PLAY_CARD` 已通过最小 card behavior registry 支撑十七张官方伤害法术、三张眩晕法术、八条 0 目标抽牌路径、六条主动摧毁单位路径、一个战力上限目标限制、一个进攻方单位目标限制、一个伤害达到战力后的致命摧毁路径、一个结算后抽牌路径、一个“目标被此法术摧毁则抽牌”的条件抽牌路径、一条目标控制者抽牌路径、一条眩晕后抽牌路径、结算抽牌燃尽分支、两条费用修正路径（本回合敌方单位被摧毁、控制单位最高战力）、一条从对手废牌堆回收所选牌路径、一条全战场单位伤害路径、一条战场单位返回所属者手牌路径、一条不支付有色回响的 1-2 个不同战场单位伤害路径、《渊海狩咒》的控制正面朝下卡牌条件伤害、《雷霆突降》的进攻方目标条件伤害、`ANY_UNIT` / `BATTLEFIELD_UNIT` / `BASE_UNIT` / `ATTACKING_UNIT` / `OPPONENT_GRAVEYARD_CARD` 目标范围分流、`1-2` 目标数量范围、两次/六次单位伤害选择、允许重复选择同一目标的多次执行法术、`PLAY_CARD.mode` 模式选择，以及 `optionalCosts = ["ECHO"]` 的单次回响额外费用；richer expected diff 已开始覆盖出牌 fixture 的事件 tick/sequence/payload 局部字段、最终玩家区域、对象状态和结算链；下一步逐批迁移更多低复杂度官方卡牌，并继续扩大通用 diff 覆盖面。
...
| 对象状态 | `cardObjects[objectId].damage`, `cardObjects[objectId].power`, `cardObjects[objectId].untilEndOfTurnEffects`, `cardObjects[objectId].isFaceDown`, `cardObjects[objectId].isAttacking` | 回合结束特殊清理移除伤害、令本回合内效果失效；`power` 先服务伤害达到战力后的摧毁判定；`isFaceDown` 先服务待命/隐藏信息条件效果的规则前提；`isAttacking` 先服务进攻方单位目标限制，后续扩展控制者、附属关系、战斗状态等完整对象字段。 |
...
65. 下一步：逐批迁移更多低复杂度官方卡牌，并继续扩大通用 diff 覆盖面。
