...
- 对象持久标签添加已用 `OBJECT_TAG_ADDED` 事件表达，当前用于《暗影的召唤》让友方非瞬息单位获得 `瞬息` 标签、《逝水如镜》让战场单位获得 `瞬息` 标签，以及《背水一战》让友方单位获得 `瞬息` 标签；开始阶段摧毁瞬息单位暂缓到关键词清理模型。
...
176. 下一步：逐批迁移更多低复杂度官方卡牌，并继续扩大通用 diff 覆盖面。
