...
- `MatchState` 已加入 `turnPlayerId`、`phase`、`timingState`、`runePools`、`playerZones`、`playerScores`，snapshot timing 已投影 timing 字段，玩家 `handSize` 和 `score` 来自权威状态。
...
- `END_TURN` 完整特殊清理尚未建模：伤害移除、本回合内效果失效、清理重复与 FAQ 个案仍需单独 fixture。
...
| `p2-end-turn-special-cleanup` | 主阶段有未消耗资源、单位伤害和本回合内效果 | 记录回合结束声明，移除伤害，本回合内效果失效，清空符文池，推进回合玩家；若清理产生新状态变化则重复清理。 | `CORE-260330` p30-p31 rules 316.6-317.3; `JFAQ-251023` p6-p7 questions 5.1-5.2 |
...
10. 下一步：补 `END_TURN` 完整特殊清理 fixture，包括伤害移除、本回合内效果失效、清理重复与 FAQ 个案。
11. 实现 `PASS_PRIORITY` 和 FEPR 最小流程。
12. 实现 `PASS_FOCUS` 和法术对决最小流程。
