...
- `PLAY_CARD` 已通过最小 card behavior registry 支撑十六张官方伤害法术、三张眩晕法术、八条 0 目标抽牌路径、五条主动摧毁单位路径、一个战力上限目标限制、一个进攻方单位目标限制、一个伤害达到战力后的致命摧毁路径、一个结算后抽牌路径、一个“目标被此法术摧毁则抽牌”的条件抽牌路径、一条目标控制者抽牌路径、一条眩晕后抽牌路径、结算抽牌燃尽分支、两条费用修正路径（本回合敌方单位被摧毁、控制单位最高战力）、一条从对手废牌堆回收所选牌路径、一条全战场单位伤害路径、《渊海狩咒》的控制正面朝下卡牌条件伤害、《雷霆突降》的进攻方目标条件伤害、`ANY_UNIT` / `BATTLEFIELD_UNIT` / `BASE_UNIT` / `ATTACKING_UNIT` / `OPPONENT_GRAVEYARD_CARD` 目标范围分流、`1-2` 目标数量范围、两次/六次单位伤害选择、允许重复选择同一目标的多次执行法术、`PLAY_CARD.mode` 模式选择，以及 `optionalCosts = ["ECHO"]` 的单次回响额外费用；richer expected diff 已开始覆盖出牌 fixture 的事件 tick/sequence/payload 局部字段、最终玩家区域、对象状态和结算链；下一步逐批迁移更多低复杂度官方卡牌，并继续扩大通用 diff 覆盖面。
...
62. 下一步：逐批迁移更多低复杂度官方卡牌，并继续扩大通用 diff 覆盖面。
