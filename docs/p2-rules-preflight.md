...
- `MatchState` 已加入 `turnPlayerId`、`phase`、`timingState`、`runePools`、`playerZones`，snapshot timing 已投影 timing 字段，玩家 `handSize` 来自权威手牌区。
...
- 上述两个 fixture 已按 `CORE-260330` rule 481.7 调整为 P2 非首个回合；P2 首个回合额外召出符文需单独 fixture。

尚未完成：

- 主牌堆为空导致燃尽、1v1 首回合额外符文尚未实现。
- 回合开始自动结算尚未接入 `END_TURN` 后的回合推进。
...
7. 下一步：补主牌堆为空/燃尽、1v1 首回合额外符文 fixture。
8. 下一步：实现 `END_TURN` 的回合结束特殊清理与回合推进，并接入回合开始自动结算。
9. 实现 `PASS_PRIORITY` 和 FEPR 最小流程。
10. 实现 `PASS_FOCUS` 和法术对决最小流程。
