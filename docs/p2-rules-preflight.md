...
- `PLAY_CARD` 已通过最小 card behavior registry 支撑两张官方伤害法术和《渊海狩咒》的条件伤害；richer expected diff 已开始覆盖出牌 fixture 的最终玩家区域、对象状态和结算链；下一步逐批迁移更多低复杂度官方卡牌，并继续扩大通用 diff 覆盖面。
...
21. 下一步：逐批迁移更多低复杂度官方卡牌，并继续扩大通用 diff 覆盖面。
