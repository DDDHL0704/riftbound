...
- `PLAY_CARD` 已通过最小 card behavior registry 支撑八张官方伤害法术、一张眩晕法术、0 目标抽牌、一个结算后抽牌路径、结算抽牌燃尽分支、《渊海狩咒》的条件伤害、`ANY_UNIT` / `BATTLEFIELD_UNIT` / `BASE_UNIT` 目标范围分流、`1-2` 目标数量范围，以及 `PLAY_CARD.mode` 模式选择；richer expected diff 已开始覆盖出牌 fixture 的最终玩家区域、对象状态和结算链；下一步逐批迁移更多低复杂度官方卡牌，并继续扩大通用 diff 覆盖面。
...
34. 下一步：逐批迁移更多低复杂度官方卡牌，并继续扩大通用 diff 覆盖面。
