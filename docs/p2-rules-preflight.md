...
- `PLAY_CARD` 已通过最小 card behavior registry 支撑 P2 preflight 的代表性官方卡牌/模式路径，当前为 `149/811 = 18.4%`。已覆盖能力族包括费用、目标范围、模式选择、伤害、抽牌/燃尽、摧毁/回手/移动、战力修正、关键词/对象标签、装备对象、指示物、符文召出、可选/强制额外费用和部分替代效果；完整 fixture 列表、事件词表和逐项进度以本文件的 fixture 表与第 6 节进度清单为准，规则证据以 `docs/rules-evidence-index.md` 和 fixture 内 `rulesEvidence` 为准。
...
- 对象持久标签添加已用 `OBJECT_TAG_ADDED` 事件表达，当前用于《暗影的召唤》让友方非瞬息单位获得 `瞬息` 标签、《逝水如镜》让战场单位获得 `瞬息` 标签、《化为灰烬》让装备获得 `瞬息` 标签，以及《背水一战》让友方单位获得 `瞬息` 标签；`CARD_TYPE:EQUIPMENT` 暂用于《痛苦之酬》的“金币”装备指示物对象标签，以及《化为灰烬》《印爆术》《紧急召回》和《火箭轰击》摧毁装备模式的装备目标校验；装备摧毁已用 `EQUIPMENT_DESTROYED` 事件表达，装备回手已用 `EQUIPMENT_RETURNED_TO_HAND` 事件表达，且不会写入本回合单位摧毁记忆；开始阶段摧毁瞬息单位和金币资源技能暂缓到关键词/装备模型。
...
181. 下一步：逐批迁移更多低复杂度官方卡牌，并继续扩大通用 diff 覆盖面。
