...
- `p2-preflight-play-center-stage-draw-stack.fixture.json` 已验证官方法术 `UNL-061/219 台前作秀` 不支付回响时的 0 目标 `PLAY_CARD` 通道：支付 2 点费用，加入结算链，双方让过后抽 1 张牌并进入废牌堆。`回响`额外费用和重复效果暂缓。
...
- `PLAY_CARD` 已通过最小 card behavior registry 支撑八张官方伤害法术、一张眩晕法术、0 目标抽牌、一个结算后抽牌路径、结算抽牌燃尽分支、《渊海狩咒》的条件伤害、`ANY_UNIT` / `BATTLEFIELD_UNIT` / `BASE_UNIT` 目标范围分流、`1-2` 目标数量范围、`PLAY_CARD.mode` 模式选择，以及 `optionalCosts = ["ECHO"]` 的单次回响额外费用；richer expected diff 已开始覆盖出牌 fixture 的最终玩家区域、对象状态和结算链；下一步逐批迁移更多低复杂度官方卡牌，并继续扩大通用 diff 覆盖面。
...
| 对象状态 | `cardObjects[objectId].damage`, `cardObjects[objectId].untilEndOfTurnEffects`, `cardObjects[objectId].isFaceDown` | 回合结束特殊清理移除伤害、令本回合内效果失效；`isFaceDown` 先服务待命/隐藏信息条件效果的规则前提，后续扩展战力、控制者、附属关系等完整对象字段。 |
...
| `p2-play-center-stage-draw-stack` | P1 打出官方法术《台前作秀》，不支付回响 | 支付 2 点费用，0 目标卡牌入栈，双方让过后抽 1 张牌并进入废牌堆；`回响`额外费用和重复效果暂缓。 | `CATALOG` UNL-061/219; `CORE-260330` p39-p42 rules 355-356; p33-p35 rules 327-340; p57 rule 413.4 |
...
35. 下一步：逐批迁移更多低复杂度官方卡牌，并继续扩大通用 diff 覆盖面。
