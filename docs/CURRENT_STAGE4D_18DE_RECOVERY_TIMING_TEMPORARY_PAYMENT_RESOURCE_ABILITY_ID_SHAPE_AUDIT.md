# Stage 4D-18DE Recovery Timing Temporary Payment Resource Ability Id Shape Audit
...
Added `RecoveryValidatorRejectsSpectatorReplayTimingTemporaryPaymentResourceKeyedAbilityIdShapeWithCountMismatch` in `tests/Riftbound.ConformanceTests/MatchRecoveryTests.cs`.

The test builds an authoritative temporary payment resource from real `MatchState` source-object state, keeps the emitted spectator payload keyed to `temp-payment-resource-1`, changes the emitted `abilityId` to an unreadable array payload, then appends `temp-payment-resource-extra` to force resource-count mismatch.

It proves validation still emits:

- payload-layer invalid diagnostics for the unreadable `abilityId` shape;
...
`abilityId` is an optional payload field, but unreadable non-scalar shapes are still invalid. The keyed authoritative comparison also rejects the unreadable value when the authoritative resource has `TEST_TEMP_RESOURCE_ABILITY`.

## Validation

- Focused new test: `1/1`.
- Focused `TemporaryPaymentResource` filter: `104/104`.
- Focused recovery filter: `1137/1137`.
- Adjacent recovery/official-opening/Postgres recovery-store filter: `1718/1718`.
- Backend full: `7083/7083`.
