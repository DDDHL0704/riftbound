2026-05-24 Stage 4D-16E evidence.
...
```bash
source scripts/dev-env.sh && dotnet test tests/Riftbound.ConformanceTests/Riftbound.ConformanceTests.csproj --no-restore --filter "FullyQualifiedName~PromptIdOnlyDuplicateMulliganWithFirstReadyPromptAfterFinalReadyRejectsWithoutMutation|FullyQualifiedName~SnapshotOnlyDuplicateMulliganWithFirstReadyPromptAfterFinalReadyRejectsWithoutMutation"
```
...
Adjacent result: passed `983/983`.
...
Backend full result: passed `5935/5935`.
