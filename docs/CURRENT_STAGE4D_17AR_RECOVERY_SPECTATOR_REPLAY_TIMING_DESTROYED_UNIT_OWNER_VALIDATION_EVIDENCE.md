# Stage 4D-17AR Recovery Spectator Replay Timing Destroyed-Unit-Owner Validation Evidence

2026-05-24 Stage 4D-17AR evidence bundle for recovery spectator replay timing destroyed-unit-owner validation.

## Runtime/Test Delta

- `src/Riftbound.Engine/MatchRecovery.cs`
  - `MatchRecoveryValidator.ValidateSpectatorReplayFrame` rejects spectator replay snapshot timing `destroyedUnitOwnerIdsThisTurn` lists that disagree with the authoritative final state `DestroyedUnitOwnerIdsThisTurn`.
- `tests/Riftbound.ConformanceTests/MatchRecoveryTests.cs`
  - Added `RecoveryValidatorRejectsSpectatorReplayTimingDestroyedUnitOwnersMismatch`.
...
- Adjacent recovery/opening regression:
  ...
  - Result: passed `651/651`.
- Backend full:
  - `source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore`
  - Result: passed `6016/6016`.
