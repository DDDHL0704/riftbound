# Stage 4D-18CO Recovery Timing Temporary Payment Resource Resource Id Missing Field Audit
...
Added `RecoveryValidatorRejectsSpectatorReplayTimingTemporaryPaymentResourceResourceIdMissingFieldWithCountMismatch` in `tests/Riftbound.ConformanceTests/MatchRecoveryTests.cs`.

The test builds an authoritative temporary payment resource from real `MatchState` source-object state, removes the emitted spectator payload's `resourceId` field, then appends `temp-payment-resource-extra` to force resource-count mismatch.
...
- Focused `TemporaryPaymentResource` filter: `88/88`.
- Focused recovery filter: `1121/1121`.
- Adjacent recovery/official-opening/Postgres recovery-store filter: `1702/1702`.
- Backend full: `7067/7067`.
