# Stage 4D-17NP Recovery Spectator Snapshot Player Extra Object Numeric Scalar Visibility Mismatch Audit

Date: 2026-05-28 05:53 CST
...
Runtime change: spectator snapshot player object validation now emits explicit `damage`, `power`, `basePower`, `effectivePower`, `untilEndOfTurnPowerModifier` and `manaCost` numeric-scalar value-shape diagnostics for extra object-shaped `objects{}` entries outside the authoritative visible object-id set. Existing visible-object location, redaction, boolean and list parity still runs over the authoritative visible object set.

Test coverage: `RecoveryValidatorRejectsSpectatorReplaySnapshotPlayerExtraObjectNumericScalarValueShapeWithVisibilityMismatch` proves an extra hidden hand object payload with malformed damage, power, base power, effective power, until-end-of-turn power modifier and mana cost emits explicit player object numeric-scalar diagnostics alongside the authoritative spectator visibility mismatch diagnostic.

Validation:

- Focused single: `1/1`
- Focused recovery: `399/399`
- Adjacent recovery/opening/store-smoke: `980/980`
- Backend full: `6345/6345`
...
- `docs/CURRENT_STAGE4D_17NP_RECOVERY_SPECTATOR_SNAPSHOT_PLAYER_EXTRA_OBJECT_NUMERIC_SCALAR_VISIBILITY_MISMATCH_AUDIT.md`
