...
- `dotnet test Riftbound.slnx --no-build` 通过，当前 57 个测试。
...
1. 继续按 `docs/p2-rules-preflight.md` 扩展 P2 preflight：下一步补首回合额外符文和主牌堆为空/燃尽 fixture，再把回合开始规则从孤立 `TURN_START` 接到 `END_TURN` 后的自动推进。
