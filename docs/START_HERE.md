...
P7 产品级 Web 对战已完成，最终状态以 `docs/CURRENT_P7_STATUS.md` 为准。当前新目标是 P7.9 本地产品版全卡可玩，状态入口为 `docs/CURRENT_P7_9_STATUS.md`。P7.9.6 传奇规则域已完成；P7.9.7 战场规则域也已完成，最新新增 `OGN·286/298` 清算人竞技场：据守时由后端激活已支持单位的征服效果。已落地能力包括战场对象 destination、hold/conquer 事件、`OGN·278/298` / `OGN·278a/298` 班德尔树额外战场待命目的地、`SFD·208/221` 魄罗熔炉授予友方传奇贴附武装、`UNL-206/219` 鲜血祭坛战斗摧毁替代、`UNL-216/219` 皮城学院据守后让本回合下一个法术获得等同基础费用的 Echo、`SFD·209/221` 遗忘丰碑在玩家第三回合前阻止战场来源得分、`UNL-214/219` 鬼影湾返回单位后支付 1 叫符文、`UNL-213/219` 蜕变花园授予战场单位 `ACTIVATE_ABILITY`、`OGN·289/298` 征服后选择两枚符文并在回合结束时重置、`SFD·207/221` 征服后支付 1、返回己方战场单位并在战场打出黄沙士兵，以及 `UNL` / `OGN` / `SFD` 多个战场代表效果。当前实现 `811/811` 功能单元，manual deferred 剩余 `0/811`。P7.9 的核心任务转为战斗完整性复核、图鉴/页面操作整合、UI polish 和最终 Browser smoke/full audit。
...
当前完成阶段是 P7 产品级 Web 对战。当前推进阶段是 P7.9 本地产品版全卡可玩，P7.9.6 传奇功能单元已全部补齐，P7.9.7 战场功能单元已全部补齐，不进入 P8 生产化。

立即顺序：

1. 维护 `docs/CURRENT_P7_9_STATUS.md` 作为当前短交接入口。
2. 继续 P7.9.9：复核多单位战斗、伤害分配、得分、征服/据守触发、控制权、装备和 token/copy 边界，确保页面操作仍只来自服务端 prompt。
3. 继续 P7.9.10/P7.9.11：把全卡图鉴、卡牌详情、页面可操作路径、事件日志、战报、回放/观战只读边界和 UI polish 做到本地产品版标准。
4. 最终 P7.9 audit 需要保住 `811/811` 功能单元实现、`0/811` manual deferred、后端 full test、focused suites、前端 build、Browser smoke 和 clean status 全绿。
...
P7 产品级 Web 对战已完成，当前推进 P7.9 本地产品版全卡可玩：结构化 ActionPrompt、点击式 UI 操作、战场规则域补齐、最终关闭当前剩余的 1 个 manual deferred 功能单元。不要进入 P8 账号/匹配/部署/监控/风控，不要提交规则 PDF/FAQ，不要提交未跟踪的 riftbound-dotnet.sln，不要回退现有改动。
```
