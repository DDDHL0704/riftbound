...
- `dotnet test Riftbound.slnx --no-build` 通过，当前 93 个测试。
...
2. richer expected diff 已开始接入，当前可比较 final tick、event kinds、prompt actions、最终 timing、符文池、分数、玩家区域、对象状态和结算链；目标校验已能区分 `ANY_UNIT`、`BATTLEFIELD_UNIT`、`BASE_UNIT`，支持 `1-2` 这种目标数量范围，并可按 `PLAY_CARD.mode` 选择模式行为；下一步逐批迁移更多低复杂度官方卡牌，并继续扩大通用 diff 覆盖面。
