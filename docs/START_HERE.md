...
P7 产品级 Web 对战已完成，最终状态以 `docs/CURRENT_P7_STATUS.md` 为准。当前新目标是 P7.9 本地产品版全卡可玩，状态入口为 `docs/CURRENT_P7_9_STATUS.md`。当前 P7.9.6 传奇规则域已完成：`LEGEND_ACT` 结构化提示、页面操作、十个传奇主动/反应小批次（含 Jax 武装贴附/重贴附、Darius 鼓舞资源、Teemo 召回/待命替代、Azir 武装后打出黄沙士兵、Diana/Kai'Sa/Ornn 反应资源、Ezreal 反应抽牌，以及 Irelia 友方目标反应重置/征服重置传奇）、十七个自动触发/替换小批次（含 Jinx/Draven/Garen/Lux/Annie、Volibear/Fiora 强力单位召符文、Rengar 打出单位 S+1、Leona 眩晕后增益、Sivir 回收符文造金币/敌方单位摧毁重置、Jhin 高费法术放逐完成、Vi 过量伤害征服重置、Vex 据守抽牌、Renata 据守造金币、LeBlanc 征服/据守弃牌造映像、Rek'Sai 征服展示/打出/回收、Ivern 征服/据守替换草丛，以及 Sett 恩赐单位替代毁灭/征服重置传奇），以及 Rumble/Lucian/OGS Master Yi/Ahri/UNL Master Yi/Azir 六个静态传奇小批次已打通。当前实现 `757/811` 功能单元，`54/811` 功能单元仍是战场 manual deferred。P7.9 的核心任务就是继续关闭战场规则缺口，并让对应能力通过服务端 prompt 在页面可操作。
...
当前完成阶段是 P7 产品级 Web 对战。当前推进阶段是 P7.9 本地产品版全卡可玩，P7.9.6 传奇功能单元已全部补齐，下一步进入 P7.9.7 战场规则域，不进入 P8 生产化。

立即顺序：

1. 维护 `docs/CURRENT_P7_9_STATUS.md` 作为当前短交接入口。
2. 进入 P7.9.7：建立战场对象/control/hold/conquer 的规则域基础，补齐代表性战场效果和事件模型。
3. 每个战场规则小批次都要补 prompt 候选、页面操作边界、focused conformance/GameHub 测试和状态记录。
4. 战场规则域完成后，最终关闭当前剩余 `54/811` manual deferred 功能单元。
...
P7 产品级 Web 对战已完成，当前推进 P7.9 本地产品版全卡可玩：结构化 ActionPrompt、点击式 UI 操作、战场规则域补齐、最终关闭当前剩余的 54 个 manual deferred 功能单元。不要进入 P8 账号/匹配/部署/监控/风控，不要提交规则 PDF/FAQ，不要提交未跟踪的 riftbound-dotnet.sln，不要回退现有改动。
```
