...
2. 继续 P7.9.6：在已打通的 `LEGEND_ACT` 主动技能路径、Jax 武装贴附路径、Darius/Diana/Kai'Sa/Ornn 资源路径、Teemo 召回/待命替代路径和 Jinx/Draven/Garen/Lux/Annie/Volibear/Fiora/Rengar/Leona/Sivir 自动触发路径上继续补齐剩余传奇 active/static/passive/trigger/identity 功能单元。
3. 每个传奇规则小批次都要补 prompt 候选、页面操作边界、focused conformance/GameHub 测试和状态记录。
4. 传奇域完成后进入战场规则域，最终关闭当前剩余 `65/811` manual deferred 功能单元。
