...
> 2026-05-06 复审提示：本文件早期段落保留 P7/P7.9 已完成的历史口径；当前最新结论以 `docs/CURRENT_SERVER_RULE_AUDIT.md` 和 `docs/CURRENT_FRONTEND_REBUILD_PLAN.md` 为准。当前总体状态是 **NOT READY**，本轮工作目标是清理旧前端、重建产品级 Web 前端，并继续补齐服务端 P0/P1 规则缺口。
...
3. `docs/CURRENT_P7_9_STATUS.md`
4. `docs/CURRENT_P7_STATUS.md`
5. `docs/CURRENT_P6_STATUS.md`
6. `README.md`
...
最新复审状态：**NOT READY**。`docs/CURRENT_SERVER_RULE_AUDIT.md` 已确认当前仍缺完整 battlefield/standby/control task 状态机、central cleanup task queue、spell duel/battle lifecycle、PaymentEngine、LayerEngine 和全官方卡牌证据。产品前端必须只展示服务端 snapshot/prompt/action candidate 支持的能力；服务端未完整支持的能力必须禁用、过滤或明确提示。
...
3. 任何可玩能力必须来自服务端 `ActionPrompt` / snapshot / events；P6 deferred/manual 或服务端未完整支持的能力必须禁用、过滤或明确提示。
4. 持续修复 `docs/CURRENT_SERVER_RULE_AUDIT.md` 中 NOT READY 的 P0/P1 缺口，直到复审无 P0/P1。
