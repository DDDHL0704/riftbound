...
P7 产品级 Web 对战已完成，最终状态以 `docs/CURRENT_P7_STATUS.md` 为准。当前新目标是 P7.9 本地产品版全卡可玩，状态入口为 `docs/CURRENT_P7_9_STATUS.md`。当前推进到 P7.9.6：`LEGEND_ACT` 结构化提示、页面操作、六个传奇主动技能小批次（含 Jax 武装贴附/重贴附、Darius 鼓舞资源与 Teemo 召回/待命替代）、六个自动触发小批次（含 Jinx/Draven/Garen/Lux/Annie 与 Volibear/Fiora 强力单位召符文），以及 Rumble/Lucian/OGS Master Yi/Ahri/UNL Master Yi 五个静态传奇小批次已打通。当前实现 `738/811` 功能单元，`73/811` 功能单元仍是传奇/战场 manual deferred。P7.9 的核心任务就是关闭这些规则缺口，并让对应能力通过服务端 prompt 在页面可操作。
...
4. 传奇域完成后进入战场规则域，最终关闭当前剩余 `73/811` manual deferred 功能单元。
