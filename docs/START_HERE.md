...
P7 产品级 Web 对战已完成，最终状态以 `docs/CURRENT_P7_STATUS.md` 为准。当前新目标是 P7.9 本地产品版全卡可玩，状态入口为 `docs/CURRENT_P7_9_STATUS.md`。P7.9.6 传奇规则域已完成；P7.9.7 已进入战场规则域并完成二十个战场对象/战斗结果功能单元：`DECLARE_BATTLE` 可以指向服务端已知战场卡对象，ActionPrompt 会下发战场对象目的地，`UNL-208/219` 此处瞬息单位由后端获得坚守，`UNL-210/219` 此处孤立防守单位由后端受到坚守 -2，`OGN·275/298` 据守此处时由后端打出随从，`OGN·279/298` 防守此处时由后端选择战斗单位获得坚守2，`OGN·280/298` 据守此处时由后端抽 1 张，`OGN·282/298` 征服此处时由后端消耗增益并抽牌，`OGN·283/298` 据守此处时由后端给予单位增益，`OGN·285/298` 防守此处时由后端选择战斗单位移动到基地，`OGN·287/298` 征服此处时由后端回收己方基地符文，`OGN·288/298` 据守此处时由后端召 1 枚符文，`SFD·210/221` 征服此处时由后端支付 1 并让传奇变为活跃状态，`SFD·212/221` 征服此处时由后端将主牌堆顶两张放入废牌堆，`SFD·215/221` 防守此处时由后端展示顶牌并抽法术或回收，`SFD·217/221` 征服此处时由后端按其他受控战场数量抽牌，`SFD·218/221` 征服此处且留有强力单位时由后端支付 1 并抽牌，`SFD·219/221` 据守此处时由后端让双方召出符文，`SFD·220/221` 征服此处时由后端支付 1 并打出休眠金币，`SFD·221/221` 征服此处时由后端重置友方装备并可卸除武装，`OGN·294/298` 此处战斗单位由后端获得静态战力 +1，`OGN·298/298` 征服此处时由后端弃牌抽牌。当前实现 `777/811` 功能单元，`34/811` 功能单元仍是战场 manual deferred。P7.9 的核心任务就是继续关闭战场规则缺口，并让对应能力通过服务端 prompt 在页面可操作。
