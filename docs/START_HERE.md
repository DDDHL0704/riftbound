...
- `dotnet test Riftbound.slnx --no-build` 通过，当前 118 个测试。
...
1. 继续按 `docs/p2-rules-preflight.md` 扩展 P2 preflight：`PLAY_CARD` 已有最小 card behavior registry，并覆盖十三张官方伤害法术、两张眩晕法术、八条 0 目标抽牌路径、单次回响、主动摧毁单位、伤害达到战力后的致命摧毁、结算后抽牌/条件抽牌/抽牌燃尽路径、一条本回合敌方单位被摧毁后的费用修正路径，以及一条对手废牌堆回收路径；《渊海狩咒》已覆盖基础 2 点伤害和“控制正面朝下卡牌则改为 4 点伤害”的条件路径。
