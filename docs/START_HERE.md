...
P7 产品级 Web 对战已完成，最终状态以 `docs/CURRENT_P7_STATUS.md` 为准。当前新目标是 P7.9 本地产品版全卡可玩，状态入口为 `docs/CURRENT_P7_9_STATUS.md`。当前推进到 P7.9.6：`LEGEND_ACT` 结构化提示、页面操作、十个传奇主动/反应小批次（含 Jax 武装贴附/重贴附、Darius 鼓舞资源、Teemo 召回/待命替代、Azir 武装后打出黄沙士兵、Diana/Kai'Sa/Ornn 反应资源、Ezreal 反应抽牌，以及 Irelia 友方目标反应重置/征服重置传奇）、十三个自动触发小批次（含 Jinx/Draven/Garen/Lux/Annie、Volibear/Fiora 强力单位召符文、Rengar 打出单位 S+1、Leona 眩晕后增益、Sivir 回收符文造金币/敌方单位摧毁重置、Jhin 高费法术放逐完成、Vi 过量伤害征服重置、Vex 据守抽牌与 Renata 据守造金币），以及 Rumble/Lucian/OGS Master Yi/Ahri/UNL Master Yi/Azir 六个静态传奇小批次已打通。当前实现 `752/811` 功能单元，`59/811` 功能单元仍是传奇/战场 manual deferred。P7.9 的核心任务就是关闭这些规则缺口，并让对应能力通过服务端 prompt 在页面可操作。
...
2. 继续 P7.9.6：在已打通的 `LEGEND_ACT` 主动技能路径、Jax 武装贴附路径、Darius/Diana/Kai'Sa/Ornn/Ezreal/Irelia 资源与反应路径、Teemo 召回/待命替代路径和 Jinx/Draven/Garen/Lux/Annie/Volibear/Fiora/Rengar/Leona/Sivir/Jhin/Vi/Vex/Renata 自动触发路径上继续补齐剩余传奇 active/static/passive/trigger/identity 功能单元。
3. 每个传奇规则小批次都要补 prompt 候选、页面操作边界、focused conformance/GameHub 测试和状态记录。
4. 传奇域完成后进入战场规则域，最终关闭当前剩余 `59/811` manual deferred 功能单元。
...
P7 产品级 Web 对战已完成，当前推进 P7.9 本地产品版全卡可玩：结构化 ActionPrompt、点击式 UI 操作、传奇/战场规则域补齐、最终关闭当前剩余的 59 个 manual deferred 功能单元。不要进入 P8 账号/匹配/部署/监控/风控，不要提交规则 PDF/FAQ，不要提交未跟踪的 riftbound-dotnet.sln，不要回退现有改动。
```
