...
- `dotnet test Riftbound.slnx --no-build` 通过，当前 61 个测试。
...
1. 继续按 `docs/p2-rules-preflight.md` 扩展 P2 preflight：下一步补清理重复/常规清理的最小闭环与相关 FAQ 个案；废牌堆也为空导致连续燃尽/胜利判定另建后续 fixture。
2. 接着实现 `PASS_PRIORITY` / FEPR 最小流程，再做 `PASS_FOCUS` / 法术对决最小流程。
