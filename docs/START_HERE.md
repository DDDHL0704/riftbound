...
2. 继续 P7.9.6：在已打通的 `LEGEND_ACT` 主动技能路径、Jax 武装贴附路径和 Jinx/Draven/Garen/Lux/Annie/Volibear/Fiora 自动触发路径上继续补齐剩余传奇 active/static/passive/trigger/identity 功能单元。
3. 每个传奇规则小批次都要补 prompt 候选、页面操作边界、focused conformance/GameHub 测试和状态记录。
4. 传奇域完成后进入战场规则域，最终关闭当前剩余 `75/811` manual deferred 功能单元。
...
P7 产品级 Web 对战已完成，当前推进 P7.9 本地产品版全卡可玩：结构化 ActionPrompt、点击式 UI 操作、传奇/战场规则域补齐、最终关闭当前剩余的 75 个 manual deferred 功能单元。不要进入 P8 账号/匹配/部署/监控/风控，不要提交规则 PDF/FAQ，不要提交未跟踪的 riftbound-dotnet.sln，不要回退现有改动。
```
