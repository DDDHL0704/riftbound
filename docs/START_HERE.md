...
P7 产品级 Web 对战已完成，最终状态以 `docs/CURRENT_P7_STATUS.md` 为准。当前新目标是 P7.9 本地产品版全卡可玩，状态入口为 `docs/CURRENT_P7_9_STATUS.md`。P7.9.6 传奇规则域已完成；P7.9.7 已进入战场规则域并完成十个战场对象/战斗结果切片：`DECLARE_BATTLE` 可以指向服务端已知战场卡对象，ActionPrompt 会下发战场对象目的地，`UNL-208/219` 此处瞬息单位由后端获得坚守，`OGN·275/298` 据守此处时由后端打出随从，`OGN·279/298` 防守此处时由后端选择战斗单位获得坚守2，`OGN·280/298` 据守此处时由后端抽 1 张，`OGN·287/298` 征服此处时由后端回收己方基地符文，`SFD·212/221` 征服此处时由后端将主牌堆顶两张放入废牌堆，`SFD·215/221` 防守此处时由后端展示顶牌并抽法术或回收，`SFD·219/221` 据守此处时由后端让双方召出符文，`OGN·294/298` 此处战斗单位由后端获得静态战力 +1，`OGN·298/298` 征服此处时由后端弃牌抽牌。当前实现 `767/811` 功能单元，`44/811` 功能单元仍是战场 manual deferred。P7.9 的核心任务就是继续关闭战场规则缺口，并让对应能力通过服务端 prompt 在页面可操作。
...
2. 继续 P7.9.7：在已建立的战场对象 destination、hold/conquer 事件、`UNL-208/219` 瞬息坚守、`OGN·275/298` 据守随从、`OGN·279/298` 防守坚守2选择、`OGN·280/298` 据守抽牌、`OGN·287/298` 征服回收符文、`SFD·212/221` 征服弃置主牌堆、`SFD·215/221` 防守顶牌展示分支、`SFD·219/221` 据守双方召符文、`OGN·294/298` 静态战力和 `OGN·298/298` 征服弃牌抽牌路径上，继续补齐 control、得分、替换、静态和激活类代表战场效果。
3. 每个战场规则小批次都要补 prompt 候选、页面操作边界、focused conformance/GameHub 测试和状态记录。
4. 战场规则域完成后，最终关闭当前剩余 `44/811` manual deferred 功能单元。
...
P7 产品级 Web 对战已完成，当前推进 P7.9 本地产品版全卡可玩：结构化 ActionPrompt、点击式 UI 操作、战场规则域补齐、最终关闭当前剩余的 44 个 manual deferred 功能单元。不要进入 P8 账号/匹配/部署/监控/风控，不要提交规则 PDF/FAQ，不要提交未跟踪的 riftbound-dotnet.sln，不要回退现有改动。
```
