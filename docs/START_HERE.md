...
P7 产品级 Web 对战已完成，最终状态以 `docs/CURRENT_P7_STATUS.md` 为准。当前新目标是 P7.9 本地产品版全卡可玩，状态入口为 `docs/CURRENT_P7_9_STATUS.md`。当前已完成到 P7.9.5：`LEGEND_ACT` 代表路径、结构化提示候选和页面操作已打通。P6 最终状态仍保留一个关键规则边界：`713/811` 功能单元已实现，`98/811` 功能单元是传奇/战场 manual deferred。P7.9 的核心任务就是关闭这 98 个规则缺口，并让对应能力通过服务端 prompt 在页面可操作。
...
当前完成阶段是 P7 产品级 Web 对战。当前推进阶段是 P7.9 本地产品版全卡可玩，已完成到 P7.9.5；下一步是 P7.9.6 分批补齐传奇功能单元，不进入 P8 生产化。

立即顺序：

1. 维护 `docs/CURRENT_P7_9_STATUS.md` 作为当前短交接入口。
2. 继续 P7.9.6：在已打通的 `LEGEND_ACT` 代表路径上分批补齐传奇 active/static/passive/trigger/identity 功能单元。
3. 每个传奇规则小批次都要补 prompt 候选、页面操作边界、focused conformance/GameHub 测试和状态记录。
4. 传奇域完成后进入战场规则域，最终关闭 `98/811` manual deferred 功能单元。
