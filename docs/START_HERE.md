...
P7 产品级 Web 对战已完成，最终状态以 `docs/CURRENT_P7_STATUS.md` 为准。当前新目标是 P7.9 本地产品版全卡可玩，状态入口为 `docs/CURRENT_P7_9_STATUS.md`。P7.9.6 传奇规则域已完成；P7.9.7 已进入战场规则域并完成五十二个战场对象/战斗结果功能单元，最新新增 `SFD·208/221` 魄罗熔炉：控制该战场时，友方传奇获得横置并将受控武装贴附到受控单位上的 `LEGEND_ACT` 能力。已落地能力包括战场对象 destination、hold/conquer 事件、`UNL-206/219` 鲜血祭坛战斗摧毁替代、`UNL-216/219` 皮城学院据守后让本回合下一个法术获得等同基础费用的 Echo、`SFD·209/221` 遗忘丰碑在玩家第三回合前阻止战场来源得分、`UNL-214/219` 鬼影湾返回单位后支付 1 叫符文、`UNL-213/219` 蜕变花园授予战场单位 `ACTIVATE_ABILITY`、`OGN·289/298` 征服后选择两枚符文并在回合结束时重置、`SFD·207/221` 征服后支付 1、返回己方战场单位并在战场打出黄沙士兵，以及 `UNL` / `OGN` / `SFD` 多个战场代表效果。当前实现 `809/811` 功能单元，`2/811` 功能单元仍是战场 manual deferred。P7.9 的核心任务就是继续关闭战场规则缺口，并让对应能力通过服务端 prompt 在页面可操作。
...
2. 继续 P7.9.7：在已建立的战场对象 destination、hold/conquer 事件、`UNL-205/219`、`UNL-206/219`、`UNL-211/219`、`UNL-213/219`、`UNL-214/219`、`UNL-215/219`、`UNL-216/219`、`UNL-218/219`、`UNL-219/219`、`OGN·289/298`、`OGN·292/298`、`OGN·296/298`、`SFD·207/221`、`SFD·208/221`、`SFD·209/221`、`SFD·211/221`、`SFD·213/221`、`SFD·216/221` 和既有 `UNL` / `OGN` / `SFD` 代表战场路径上，继续补齐 control、得分、替换、静态和激活类代表战场效果。
3. 每个战场规则小批次都要补 prompt 候选、页面操作边界、focused conformance/GameHub 测试和状态记录。
4. 战场规则域完成后，最终关闭当前剩余 `2/811` manual deferred 功能单元。
...
P7 产品级 Web 对战已完成，当前推进 P7.9 本地产品版全卡可玩：结构化 ActionPrompt、点击式 UI 操作、战场规则域补齐、最终关闭当前剩余的 2 个 manual deferred 功能单元。不要进入 P8 账号/匹配/部署/监控/风控，不要提交规则 PDF/FAQ，不要提交未跟踪的 riftbound-dotnet.sln，不要回退现有改动。
```
