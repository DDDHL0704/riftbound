...
- `dotnet test Riftbound.slnx --no-build` 通过，当前 62 个测试。
...
1. 继续按 `docs/p2-rules-preflight.md` 扩展 P2 preflight：下一步实现 `PASS_PRIORITY` / FEPR 最小流程，先覆盖普通主阶段误提交 `PASS_PRIORITY` 不会结束回合，再进入有结算链项目的优先行动权让过。
