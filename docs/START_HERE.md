...
P7 产品级 Web 对战已完成，最终状态以 `docs/CURRENT_P7_STATUS.md` 为准。当前新目标是 P7.9 本地产品版全卡可玩，状态入口为 `docs/CURRENT_P7_9_STATUS.md`。P7.9.6 传奇规则域已完成；P7.9.7 已进入战场规则域并完成三十个战场对象/战斗结果功能单元：`DECLARE_BATTLE` 可以指向服务端已知战场卡对象，ActionPrompt 会下发战场对象目的地，`UNL-207/219` 据守此处时由后端将单位移动到基地，`UNL-208/219` 此处瞬息单位由后端获得坚守，`UNL-210/219` 此处孤立防守单位由后端受到坚守 -2，`UNL-212/219` 回合开始时由后端对战场单位造成伤害并清理致命伤害，`UNL-217/219` 征服此处且过量伤害不低于 3 时由后端打出战鹰，`OGN·275/298` 据守此处时由后端打出随从，`OGN·276/298` / `OGN·276a/298` 在场时由后端让赢得游戏所需分数 +1，`OGN·279/298` 防守此处时由后端选择战斗单位获得坚守2，`OGN·280/298` 据守此处时由后端抽 1 张，`OGN·281/298` 据守此处时由后端让英雄从废牌堆返回英雄区域，`OGN·282/298` 征服此处时由后端消耗增益并抽牌，`OGN·283/298` 据守此处时由后端给予单位增益，`OGN·284/298` 在双方各自第一个回合开始阶段由后端额外召出 1 枚符文，`OGN·285/298` 防守此处时由后端选择战斗单位移动到基地，`OGN·287/298` 征服此处时由后端回收己方基地符文，`OGN·288/298` 据守此处时由后端召 1 枚符文，`OGN·290/298` 在双方各自第一个回合开始阶段由后端获得 1 分，`OGN·291/298` 征服此处时由后端展示主牌堆顶两张并回收，`OGN·293/298` / `OGN·293a/298` 据守此处且控制至少 7 名战场单位时由后端直接判定胜利，`SFD·210/221` 征服此处时由后端支付 1 并让传奇变为活跃状态，`SFD·212/221` 征服此处时由后端将主牌堆顶两张放入废牌堆，`SFD·214/221` 据守此处时由后端支付 4 点能量并额外获得 1 分，`SFD·215/221` 防守此处时由后端展示顶牌并抽法术或回收，`SFD·217/221` 征服此处时由后端按其他受控战场数量抽牌，`SFD·218/221` 征服此处且留有强力单位时由后端支付 1 并抽牌，`SFD·219/221` 据守此处时由后端让双方召出符文，`SFD·220/221` 征服此处时由后端支付 1 并打出休眠金币，`SFD·221/221` 征服此处时由后端重置友方装备并可卸除武装，`OGN·294/298` 此处战斗单位由后端获得静态战力 +1，`OGN·298/298` 征服此处时由后端弃牌抽牌。当前实现 `787/811` 功能单元，`24/811` 功能单元仍是战场 manual deferred。P7.9 的核心任务就是继续关闭战场规则缺口，并让对应能力通过服务端 prompt 在页面可操作。
...
2. 继续 P7.9.7：在已建立的战场对象 destination、hold/conquer 事件、`UNL-207/219` 据守移动到基地、`UNL-208/219` 瞬息坚守、`UNL-210/219` 孤立防守坚守 -2、`UNL-212/219` 回合开始伤害、`UNL-217/219` 征服过量伤害战鹰、`OGN·275/298` 据守随从、`OGN·276/298` / `OGN·276a/298` 胜利分数 +1、`OGN·279/298` 防守坚守2选择、`OGN·280/298` 据守抽牌、`OGN·281/298` 据守英雄返回英雄区、`OGN·282/298` 征服消耗增益抽牌、`OGN·283/298` 据守增益、`OGN·284/298` 首回合额外符文、`OGN·285/298` 防守移动到基地、`OGN·287/298` 征服回收符文、`OGN·288/298` 据守召符文、`OGN·290/298` 首回合得分、`OGN·291/298` 征服展示回收顶牌、`OGN·293/298` / `OGN·293a/298` 据守七单位胜利、`SFD·210/221` 征服支付 1 重置传奇、`SFD·212/221` 征服弃置主牌堆、`SFD·214/221` 据守支付 4 点能量得分、`SFD·215/221` 防守顶牌展示分支、`SFD·217/221` 征服按其他战场抽牌、`SFD·218/221` 征服强力单位支付 1 抽牌、`SFD·219/221` 据守双方召符文、`SFD·220/221` 征服支付 1 打出金币、`SFD·221/221` 征服重置装备/卸除武装、`OGN·294/298` 静态战力和 `OGN·298/298` 征服弃牌抽牌路径上，继续补齐 control、得分、替换、静态和激活类代表战场效果。
3. 每个战场规则小批次都要补 prompt 候选、页面操作边界、focused conformance/GameHub 测试和状态记录。
4. 战场规则域完成后，最终关闭当前剩余 `24/811` manual deferred 功能单元。
...
P7 产品级 Web 对战已完成，当前推进 P7.9 本地产品版全卡可玩：结构化 ActionPrompt、点击式 UI 操作、战场规则域补齐、最终关闭当前剩余的 24 个 manual deferred 功能单元。不要进入 P8 账号/匹配/部署/监控/风控，不要提交规则 PDF/FAQ，不要提交未跟踪的 riftbound-dotnet.sln，不要回退现有改动。
```
