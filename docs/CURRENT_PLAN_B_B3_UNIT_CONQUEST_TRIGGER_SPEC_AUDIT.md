...
Status: focused unit-conquest draw-one, draw-or-call-rune, create-dormant-Gold, overkill create-dormant-Gold, attack-overkill gain-score, pay-return-self-to-hand, grant-self-boon, ready-self-once, grant-friendly-boon, additional-activation, friendly-power, destroy-equipment self-boon, natural battle-conquest TriggerSpec activation, natural Kai'Sa no-target draw/rune-call graveyard-spell representatives, official-deck midgame Treant replay, official-deck-derived Vayne pay-return replay, official-deck-derived Kai'Sa graveyard-spell replay, and official-deck-derived Rumble paid / insufficient-payment replay slices accepted; project remains **NOT READY**.
...
2026-07-03 follow-up: OGN Kai'Sa's graveyard-spell bridge now admits no-target rune-call spells in addition to no-target draw spells through the shared `IsNoTargetFreePlayRecycleRepresentativeSpell(...)` predicate. The natural `DECLARE_BATTLE` representative uses official `OGN·134/298` 动员 as a controlled 2-mana graveyard spell at score 3: one path calls an exhausted rune from rune deck, the other verifies 动员's official "if you can't, draw one" fallback when rune deck is empty, and both recycle the played spell to the main-deck bottom. Focused Kai'Sa graveyard spell representatives 3/3, adjacent Kai'Sa graveyard / UnitConquest / Stack / CardCatalogBaseline / MatchRecovery representatives 2889/2889, and backend full conformance 9161/9161 passed; complete optional spell selection, power-cost prompt routing, targeted spell resolution, and broader stack handoff remain open.
...
- `NaturalUnitConquestTriggerTests.KaisaPlaysLowCostGraveyardRuneSpellAndRecyclesItAfterNaturalBattlefieldConquest` and `KaisaGraveyardRuneSpellDrawsAndRecyclesWhenRuneCallFailsAfterNaturalBattlefieldConquest` carry official `OGN·134/298` 动员 through the same graveyard-spell bridge, covering no-target rune-call and failed-rune-call draw fallback stack resolution before recycling the played spell.
...
- It adds official-deck-derived score-victory action-log replay coverage for Kai'Sa's low-cost graveyard spell recycle representative, with natural-conquest breadth now covering no-target draw spells and no-target rune-call / failed-rune-call draw fallback spells.
