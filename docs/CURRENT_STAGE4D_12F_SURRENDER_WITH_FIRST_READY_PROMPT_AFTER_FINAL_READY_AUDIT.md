# Stage 4D-12F Surrender With First Ready Prompt After Final Ready Audit

Status: accepted test-only server P0-005 official SURRENDER command with first READY stale-prompt-after-final-READY / mulligan prompt preservation audit slice.

## Scope

This slice adds `SurrenderWithFirstReadyPromptAfterFinalReadyRejectsWithoutMutation` to `OfficialOpeningTests`.

It covers the official opening path where both players have submitted legal decks, P1 accepts the first prompt-scoped `READY`, P2 accepts final `READY` and advances the game into official mulligan, and then the active mulligan player carries P1's original first READY prompt id / snapshot tick on a `SURRENDER` command.
...
- The active mulligan player carrying P1's original first READY prompt id / snapshot tick on `SURRENDER` after final READY rejects with `ErrorCodes.PromptExpired`.
