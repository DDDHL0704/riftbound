...
The 2026-06-27 B0 follow-up adds an official-deck action-log replay for Plunder Alley (`OGN·285/298`). It selects the BehaviorSpec battlefield source through normal opening prompts, stages official `OGN·096/298` Watchful Sentinel and `UNL-057/219` Wildclaw Beastmaster at that battlefield, submits the defender through `battlefieldTargetObjectIds`, asserts the parsed `BATTLEFIELD_DEFENSE_MOVE_FRIENDLY_UNIT_TO_BASE` trigger and server-authored `UNIT_MOVED_TO_BASE` transition into the defender owner's base, then replays the resulting action log to the final score-victory state hash.
