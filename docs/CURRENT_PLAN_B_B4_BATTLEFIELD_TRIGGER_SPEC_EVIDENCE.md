...
The 2026-06-27 B0 follow-up adds an official-deck action-log replay for Piltover Academy (`UNL-216/219`). It selects the BehaviorSpec battlefield source through normal opening prompts, stages an official Watchful Sentinel attack into a surviving Crimson Signet Treant defender, resolves the `DECLARE_BATTLE` held path, and asserts the parsed held-next-spell Echo marker payload. It then derives the holder's next neutral-open spell window from that official state, submits official `Punishment` (`UNL-007/219`) with the granted Echo optional cost, asserts marker consumption, extra mana equal to base cost, repeat count `2`, and replays that spell action log to the stack state hash. This replay deliberately does not claim Swift stack-response `PLAY_CARD` prompts are complete.
...
- held-next-spell Echo Piltover Academy B0 action-log replay follow-up: `1/1`;
- held-next-spell Echo Piltover Academy B0 FullGame follow-up: `67/67`;
...
- backend full conformance after the Piltover Academy held-next-spell Echo B0 action-log replay follow-up: `8763/8763`;
