...
- `BattlefieldTriggerSpecRules.TryGetBattlefieldConquerPayReadyLegendTrigger(...)` is removed. The accepted path now uses `BattlefieldTriggerSpecRules.TryGetTrigger(cardNo, BattlefieldTriggerSpecRules.IsBattlefieldConquerPayReadyLegendTrigger, out trigger)`.
- `IsBattlefieldConquerPayReadyLegendTrigger` validates the parsed `BehaviorSpec.Triggers` shape: `Kind=BATTLEFIELD_CONQUERED_PAY_1_READY_LEGEND`, `Timing=BATTLEFIELD_CONQUERED`, `TargetScope=CONTROLLED_LEGEND`, positive `ManaCost`, and `LegendReadyCount=1`.
- `CoreRuleEngine.TryOpenBattlefieldConquerPayReadyLegendPaymentWindow` and `CoreRuleEngine.ResolveBattlefieldConquerReadyLegendTriggerPayment` use that generic predicate route and keep existing trigger-payment open / accept / decline behavior, exhausted controlled-legend selection, payment choice ids, `PAYMENT_WINDOW_OPENED`, `COST_PAID`, `BATTLEFIELD_TRIGGER_RESOLVED`, `LEGEND_READIED`, hidden-info guarded payloads, and official-deck replay semantics.
- Existing conquer pay-ready-legend paths continue to open `TRIGGER_PAYMENT`, accept `SPEND_MANA:1` or `DECLINE`, ready the selected exhausted controlled legend only after successful payment, preserve decline-without-ready behavior, and replay both paid and declined Hall of Legends official-deck midgames to score victory.
- Source guard: `CardCatalogBaselineTests.BattlefieldConquerPayReadyLegendTriggerUsesGenericSpecPredicate` rejects the removed getter across engine sources and requires the shared generic predicate route.
...
## 2026-07-02 Conquer Pay-Return-Unit Create-Sand-Soldier Execution Helper Evidence

- This follow-up does not introduce a new battlefield text interpretation. It preserves `SFD·207/221` Imperial Shrine / 帝王神坛 official text for paying 1 after conquest and returning a controlled unit at that battlefield to play a 2-power Sand Soldier at that battlefield.
- `BattlefieldTriggerSpecRules.TryGetBattlefieldConquerPayReturnUnitCreateSandSoldierTrigger(...)` is removed. The accepted path now uses `BattlefieldTriggerSpecRules.TryGetTrigger(cardNo, BattlefieldTriggerSpecRules.IsBattlefieldConquerPayReturnUnitCreateSandSoldierTrigger, out trigger)`.
- `IsBattlefieldConquerPayReturnUnitCreateSandSoldierTrigger` validates the parsed `BehaviorSpec.Triggers` shape: `Kind=BATTLEFIELD_CONQUERED_PAY_1_RETURN_UNIT_CREATE_SAND_SOLDIER`, `Timing=BATTLEFIELD_CONQUERED`, `TargetScope=CONTROLLED_UNIT_AT_THIS_BATTLEFIELD`, positive `ManaCost`, `ReturnCount=1`, `ReturnOriginZone=BATTLEFIELD`, `ReturnDestinationZone=HAND`, `CreatedTokenCount=1`, non-empty `CreatedTokenName`, positive `CreatedTokenPower`, and `CreatedTokenDestination=BATTLEFIELD`.
- `CoreRuleEngine.TryOpenBattlefieldConquerPayOneReturnUnitCreateSandSoldierPaymentWindow` and `CoreRuleEngine.ResolveBattlefieldConquerSandSoldierTriggerPayment` use that generic predicate route and keep existing trigger-payment open / accept / decline behavior, selected controlled-unit return, token identity lookup by name/power, token ready/exhausted handling, `PAYMENT_WINDOW_OPENED`, `COST_PAID`, `BATTLEFIELD_TRIGGER_RESOLVED`, `UNIT_RETURNED_TO_HAND`, token-created events, hidden-info guarded payloads, and official-deck replay semantics.
- Existing conquer pay-return-unit create-Sand-Soldier paths continue to open `TRIGGER_PAYMENT`, accept `SPEND_MANA:1` or `DECLINE`, return the selected controlled conquest attacker to hand only after successful payment, create a battlefield Sand Soldier token from `P6TokenFactoryCatalog`, preserve decline-without-return-or-token behavior, and replay both paid and declined Imperial Shrine official-deck midgames to score victory.
- Source guard: `CardCatalogBaselineTests.BattlefieldConquerPayReturnUnitCreateSandSoldierTriggerUsesGenericSpecPredicate` rejects the removed getter across engine sources and requires the shared generic predicate route.
...
- Existing representative tests `P79BattlefieldDefendMovesChosenSurvivingDefenderToBase`, `P79BattlefieldDefendMoveToBaseRejectsAttackerControlledBattlefield`, GameHub `P79BattlefieldDefendMoveToBaseSeedOffersBattlefieldDestinationAndChoice`, and the 2026-06-28 `StandbyDefendTeemoTests` Plunder friendly-unit scope follow-up remain the runtime evidence for the defend move-friendly-unit-to-base behavior.
...
The 2026-06-27 B0 follow-up adds an official-deck score-victory action-log replay for Back Alley Bar (`OGN·277/298`). It selects the BehaviorSpec battlefield source through normal opening prompts, stages an official Wildclaw Beastmaster at that battlefield, submits the existing server-authoritative battlefield-to-base `MOVE_UNIT` command path, asserts the parsed `BATTLEFIELD_UNIT_MOVED_AWAY_POWER_MODIFIER` trigger payload and until-end-of-turn power-modifier ledger, then continues through battlefield score victory and replays the resulting action log to the final score-victory state hash.
...
The 2026-06-27 B0 follow-up adds an official-deck score-victory action-log replay for Piltover Academy (`UNL-216/219`). It selects the BehaviorSpec battlefield source through normal opening prompts, stages an official Watchful Sentinel attack into a surviving Crimson Signet Treant defender, resolves the `DECLARE_BATTLE` held path, and asserts the parsed held-next-spell Echo marker payload. It then derives the holder's next neutral-open spell window from that official state, submits official `Punishment` (`UNL-007/219`) with the granted Echo optional cost, asserts marker consumption, extra mana equal to base cost, repeat count `2`, resolves the repeated spell stack, and continues through battlefield score victory to the final state hash. This replay deliberately does not claim natural same-turn non-active spell access or Swift stack-response `PLAY_CARD` prompts are complete.
...
The 2026-06-27 B0 follow-up adds an official-deck action-log replay for Vaults of Helia (`UNL-219/219`). It selects the BehaviorSpec battlefield source through normal opening prompts, starts a focused main-phase replay with the parsed held-unit-cost marker active, submits official `OGN·211/298` Loyal Craftsman through the server-authored `PLAY_CARD` prompt, asserts prompt source requirements and `COST_PAID` both charge base mana `3` plus `battlefieldHeldUnitCostIncreaseMana=1`, resolves the stack, and replays the resulting command log through score victory to the same final state hash.
...
The 2026-06-26 locality follow-up closes the remaining flat-zone boundary for `FRIENDLY_UNIT_AT_THIS_BATTLEFIELD`. When `ObjectLocations` carry a precise target `BattlefieldObjectId`, `CoreRuleEngine` now checks that id against each BehaviorSpec battlefield source before resolving the Dream Tree draw or Waste Hall power bonus. `P79BattlefieldFriendlySpellTargetSkipsDifferentBattlefieldUnit` proves Dream Tree does not draw for a friendly unit at another battlefield, while `P79BattlefieldSpellPowerBonusSkipsDifferentBattlefieldUnit` proves the same shared helper prevents Waste Hall from buffing across battlefield locations. Legacy no-location representative fixtures remain valid through the existing compatibility path.
...
The 2026-06-27 B0 follow-up adds an official-deck action-log replay for Idol Valley (`UNL-218/219`). It selects the BehaviorSpec battlefield source through normal opening prompts, plays a real unit to that battlefield, resolves the stack, asserts the parsed pay-1 boon trigger and hidden-safe boon event, then replays the resulting action log to the final score-victory state hash.
...
The 2026-06-27 B0 follow-up adds an official-deck action-log replay for Ghost Bay (`UNL-214/219`). It selects the BehaviorSpec battlefield source through normal opening prompts, stages legal blue official units at that battlefield, plays official blue spell `OGN·104/298` to return one friendly unit to hand and call a rune, asserts Ghost Bay pays the parsed mana cost and calls one additional rune, then replays the resulting action log to the final score-victory state hash.
...
The 2026-06-27 B0 follow-up adds an official-deck action-log replay for Meteor Spring (`UNL-215/219`). It selects the BehaviorSpec battlefield source through normal opening prompts, stages one friendly unit at that battlefield, plays another non-token unit to the same battlefield, resolves the stack, asserts the parsed once-per-turn move-other trigger and server-authored move event, then replays the resulting action log to the final score-victory state hash.
...
The 2026-06-27 B0 follow-up adds an official-deck action-log replay for Star Peak (`OGN·288/298`). It selects the BehaviorSpec battlefield source through normal opening prompts, stages a low-power official attacker and a surviving official defender at that battlefield, resolves the `DECLARE_BATTLE` held path, asserts the parsed rune-call trigger and hidden-safe rune movement into the holder's base, then replays the resulting action log to the final score-victory state hash.
...
The 2026-06-27 B0 follow-up adds an official-deck action-log replay for Confetti Tree (`SFD·219/221`). It selects the BehaviorSpec battlefield source through normal opening prompts, stages a low-power official attacker and a surviving official defender at that battlefield, resolves the `DECLARE_BATTLE` held path, asserts both players call one dormant rune through hidden-safe rune movement, then replays the resulting action log to the final score-victory state hash.
...
The 2026-06-27 B0 score-victory follow-up extends the official-deck action-log replay for Rehearsal Hall (`UNL-207/219`). It selects the BehaviorSpec battlefield source through normal opening prompts, stages a low-power official attacker and a surviving official defender at that battlefield, resolves the `DECLARE_BATTLE` held path, asserts the parsed move-unit-to-base trigger and server-authored `UNIT_MOVED_TO_BASE` state transition, then continues through battlefield score victory and replays the resulting action log to the final score-victory state hash. The runtime fix uses the shared effective field controller resolver for turn-start battlefield scoring, so official battlefield objects without an explicit `ControllerId` still score for their owner / effective controller.
...
The accepted `DECLARE_BATTLE` defended-battlefield path still requires an eligible battlefield source controlled or legacy-owned by the defending player, and a selected same-battlefield friendly unit matching `FRIENDLY_UNIT_AT_THIS_BATTLEFIELD`. The emitted `BATTLEFIELD_TRIGGER_RESOLVED` and `UNIT_MOVED_TO_BASE` payloads now use the parsed trigger kind. The optional no-target path and stale attacker-controlled battlefield rejection remain covered by existing Plunder Alley tests, and the 2026-06-28 follow-up proves the target set is not limited to declared defenders by moving an exhausted non-defending friendly unit while Teemo consumes a separate enemy-unit defend-trigger target.

The 2026-06-27 B0 follow-up adds an official-deck action-log replay for Plunder Alley (`OGN·285/298`). It selects the BehaviorSpec battlefield source through normal opening prompts, stages official `OGN·096/298` Watchful Sentinel and `UNL-057/219` Wildclaw Beastmaster at that battlefield, submits the friendly-unit target through `battlefieldTargetObjectIds`, asserts the parsed `BATTLEFIELD_DEFENSE_MOVE_FRIENDLY_UNIT_TO_BASE` trigger and server-authored `UNIT_MOVED_TO_BASE` transition into the target owner's base, then replays the resulting action log to the final score-victory state hash.

The defend grant-Steadfast follow-up parser path turns the Fortified Position official battlefield text into a structured `TriggerSpec` with `Kind=BATTLEFIELD_DEFENSE_GRANT_STEADFAST_TWO`, `Timing=BATTLEFIELD_DEFENDED`, `TargetScope=DEFENDER_UNIT_AT_THIS_BATTLEFIELD`, `GrantedKeyword=坚守`, and `KeywordBonus=2`. Runtime no longer checks `OGN·279/298` through `BattlefieldDefenderSteadfastTwoCardNo` / `IsBattlefieldDefenderSteadfastTwoCardNo`; it queries `BehaviorSpec.Triggers` via `BattlefieldTriggerSpecRules`.

The accepted `DECLARE_BATTLE` defended-battlefield path still requires an eligible battlefield source controlled or legacy-owned by the defending player and exactly one declared defender target when multiple targets could be present. The emitted `BATTLEFIELD_TRIGGER_RESOLVED` payload now uses the parsed trigger kind / keyword / bonus, and combat power reads the parsed keyword bonus instead of a hardcoded `+2`.

The 2026-06-27 B0 follow-up adds an official-deck action-log replay for Fortified Position (`OGN·279/298`). It selects the BehaviorSpec battlefield source through normal opening prompts, stages an official attacker and official Wildclaw Beastmaster defender at that battlefield, submits the defender through `battlefieldTargetObjectIds`, asserts the parsed `BATTLEFIELD_DEFENSE_GRANT_STEADFAST_TWO` trigger and defender combat-damage payload with `坚守` +2, then replays the resulting action log to the final score-victory state hash.
...
The 2026-06-27 B0 follow-up adds an official-deck action-log replay for Navori Arena (`OGN·283/298`). It selects the BehaviorSpec battlefield source through normal opening prompts, stages a low-power official attacker and a surviving official defender at that battlefield, resolves the `DECLARE_BATTLE` held path, asserts the parsed grant-boon trigger plus the server-authored `BOON_GRANTED` power/tag transition, then replays the resulting action log to the final score-victory state hash.
...
The 2026-06-27 B0 follow-up adds an official-deck action-log replay for Unity Sanctum (`OGN·275/298`). It selects the BehaviorSpec battlefield source through normal opening prompts, stages a low-power official attacker and a surviving official defender at that battlefield, resolves the `DECLARE_BATTLE` held path, asserts the parsed create-minion trigger plus the server-authored `UNIT_TOKEN_CREATED` base-zone token transition for `OGN·271/298`, then replays the resulting action log to the final score-victory state hash.
...
The 2026-06-27 B0 follow-up adds an official-deck action-log replay for Hallowed Tomb (`OGN·281/298`). It selects the BehaviorSpec battlefield source through normal opening prompts, stages a low-power official attacker, a surviving official defender, and the real official `UNL-055/219` Vex champion object in the holder's graveyard with an empty champion zone, resolves the `DECLARE_BATTLE` held path, asserts the parsed return-hero trigger plus the server-authored champion-zone/object-location transition, then replays the resulting action log to the final score-victory state hash. The first focused red run exposed that official card objects only carried coarse `CARD_TYPE` tags; `MatchSession` now adds generic `CARD_CATEGORY:<official category name>` tags for every official object so BehaviorSpec `TAG:CARD_CATEGORY:英雄单位` filters can resolve against real deck objects without a card-number branch.
...
The B0 full-game replay follow-up now also covers this same parsed Sunken Temple route through a legal official-deck opening. `OfficialDeckMidgamePaysSunkenTemplePowerfulDrawAndScoreVictoryActionLogReplaysToFinalStateHash` verifies that official `SFD·218/221` can be selected in opening setup, that a focused midgame `START_BATTLE` state opens `TRIGGER_PAYMENT` after conquest only with a surviving powerful attacker, that replayable `PAY_COST(SPEND_MANA:1)` draws one controlled main-deck card, and that the command journal replays through score victory to the same final state hash.

The conquer pay-return-unit create-Sand-Soldier follow-up parser path turns the Imperial Shrine official battlefield text into a structured `TriggerSpec` with `Kind=BATTLEFIELD_CONQUERED_PAY_1_RETURN_UNIT_CREATE_SAND_SOLDIER`, `Timing=BATTLEFIELD_CONQUERED`, `TargetScope=CONTROLLED_UNIT_AT_THIS_BATTLEFIELD`, `ManaCost=1`, `ReturnCount=1`, `ReturnOriginZone=BATTLEFIELD`, `ReturnDestinationZone=HAND`, `CreatedTokenCount=1`, `CreatedTokenName=黄沙士兵`, `CreatedTokenPower=2`, `CreatedTokenDestination=BATTLEFIELD`, and `CreatedTokenExhausted=false`. Runtime no longer checks `SFD·207/221` through `BattlefieldConquerPayOneReturnUnitCreateSandSoldierCardNo` / `IsBattlefieldConquerPayOneReturnUnitCreateSandSoldierCardNo`, and no longer uses `BattlefieldSandSoldierManaCost`; it queries `BehaviorSpec.Triggers` via generic `BattlefieldTriggerSpecRules.TryGetTrigger(cardNo, BattlefieldTriggerSpecRules.IsBattlefieldConquerPayReturnUnitCreateSandSoldierTrigger, out trigger)`.
...
The B0 full-game replay follow-up now also covers this same parsed Imperial Shrine route through a legal official-deck opening. `OfficialDeckMidgameResolvesImperialShrineSandSoldierAndScoreVictoryActionLogReplaysToFinalStateHash` verifies that official `SFD·207/221` can be selected in opening setup, that a focused midgame `START_BATTLE` state opens `TRIGGER_PAYMENT`, that replayable `PAY_COST(SPEND_MANA:1)` pays the parsed cost, returns the controlled conquest attacker to hand, and creates a ready 2-power `SFD·T02` Sand Soldier token at that battlefield, and that `OfficialDeckMidgameDeclinesImperialShrineSandSoldierAndScoreVictoryActionLogReplaysToFinalStateHash` closes the same trigger window with replayable `PAY_COST(DECLINE)` without cost, return, or token creation. Both command journals replay through score victory to the same final state hash. The same follow-up tightens `MatchSession` snapshot redaction so the returned hidden hand object's id is not leaked through opponent-view battle / battlefield metadata ids or object-id collections.

The conquer pay-ready-legend follow-up parser path turns the Hall of Legends official battlefield text into a structured `TriggerSpec` with `Kind=BATTLEFIELD_CONQUERED_PAY_1_READY_LEGEND`, `Timing=BATTLEFIELD_CONQUERED`, `TargetScope=CONTROLLED_LEGEND`, `ManaCost=1`, and `LegendReadyCount=1`. Runtime no longer checks `SFD·210/221` through `BattlefieldConquerPayOneReadyLegendCardNo` / `IsBattlefieldConquerPayOneReadyLegendCardNo`, and no longer uses `BattlefieldReadyLegendManaCost`; it queries `BehaviorSpec.Triggers` via generic `BattlefieldTriggerSpecRules.TryGetTrigger(cardNo, BattlefieldTriggerSpecRules.IsBattlefieldConquerPayReadyLegendTrigger, out trigger)`.

The B0 full-game replay follow-up now also covers this same parsed Hall of Legends route through a legal official-deck opening. `OfficialDeckMidgameResolvesHallOfLegendsReadyLegendAndScoreVictoryActionLogReplaysToFinalStateHash` verifies that official `SFD·210/221` can be selected in opening setup, that a focused midgame `START_BATTLE` state with an exhausted controlled legend opens `TRIGGER_PAYMENT`, that replayable `PAY_COST(SPEND_MANA:1)` pays the parsed cost and readies that legend, and that `OfficialDeckMidgameDeclinesHallOfLegendsReadyLegendAndScoreVictoryActionLogReplaysToFinalStateHash` closes the same trigger window with replayable `PAY_COST(DECLINE)` without cost and leaves that legend exhausted. Both command journals replay through score victory to the same final state hash.
...
The B0 full-game replay follow-up now also covers this same parsed Ravenbloom route through legal official-deck openings. `OfficialDeckMidgameResolvesRavenbloomDefendRevealSpellAndScoreVictoryActionLogReplaysToFinalStateHash` verifies that official `SFD·215/221` can be selected in opening setup, that a focused midgame `START_BATTLE` state with official `SFD·087/221` on top of the defending player's controlled main deck reveals that card, recognizes it as a spell, moves it to hand, and that the command journal replays through score victory to the same final state hash.

The non-spell B0 full-game replay follow-up now covers the miss branch of the same parsed Ravenbloom route. `OfficialDeckMidgameResolvesRavenbloomDefendRevealNonSpellRecycleAndScoreVictoryActionLogReplaysToFinalStateHash` verifies that a focused midgame `START_BATTLE` state with a controlled official `UNL-036/219` unit on top of the defending player's main deck reveals that card, recognizes it is not a spell, recycles it to the bottom of that main deck, and that the command journal replays through score victory to the same final state hash.
...
The 2026-06-27 B0 follow-up adds an official-deck score-victory action-log replay for Glory Arena (`OGN·290/298`). It verifies a legal Vex official opening selects the BehaviorSpec battlefield source, derives a focused first-turn replay state from that official opening, submits replayable `END_TURN`, asserts P2's first turn start emits `BATTLEFIELD_TRIGGER_RESOLVED` and `SCORE_GAINED` with `BATTLEFIELD_FIRST_TURN_GAIN_SCORE` and amount `1`, then continues through battlefield score victory and replays the resulting action log to the final score-victory state hash.
...
The 2026-06-27 B0 follow-up adds an official-deck score-victory action-log replay for Forgotten Monument (`SFD·209/221`). It verifies a legal Vex official opening selects the BehaviorSpec battlefield source, derives a focused first-turn replay state from that official opening with official `OGN·290/298` Glory Arena as the first-turn score source, submits replayable `END_TURN`, asserts P2's first turn start emits `BATTLEFIELD_SCORE_PREVENTED` with `trigger=BATTLEFIELD_SCORE_DELAY_UNTIL_THIRD_TURN`, `preventedReason=BATTLEFIELD_FIRST_TURN_GAIN_SCORE`, `turnOrdinal=1`, and `releasedTurnOrdinal=3`, then continues through battlefield score victory and replays the resulting action log to the final score-victory state hash.
...
The 2026-06-27 B0 follow-up adds an official-deck action-log replay for Energy Hub (`SFD·214/221`). It selects the BehaviorSpec battlefield source through normal opening prompts, stages a low-power official attacker and a surviving official defender at that battlefield, gives the holder exactly four power, resolves the `DECLARE_BATTLE` held path, asserts the parsed pay-four-power score trigger, PaymentEngine `COST_PAID` payload, and `SCORE_GAINED` state transition, then replays the resulting action log to the final score-victory state hash. The positive replay uses an attacking deck without `SFD·209/221` so it exercises Energy Hub's scoring path; existing score-delay representatives remain the evidence for the prevented branch.
...
The 2026-06-27 B0 follow-up adds an official-deck action-log replay for Reckoner Arena (`OGN·286/298`). It selects the BehaviorSpec battlefield source through normal opening prompts, stages official `UNL-029/219` Crimson Signet Treant as the battlefield holder's surviving unit, resolves the `DECLARE_BATTLE` held path, asserts Reckoner Arena activates the parsed unit conquest family with `BATTLEFIELD_HELD_ACTIVATE_UNIT_CONQUEST_EFFECTS`, asserts Treant's `UNIT_CONQUEST_GRANT_FRIENDLY_BOON` effect and server-authored `BOON_GRANTED` state transition, then replays the resulting action log to the final score-victory state hash.
...
- moved-unit Back Alley Bar B0 score-victory action-log replay follow-up: `1/1`;
- moved-unit Back Alley Bar B0 FullGame follow-up: `76/76`;
- moved-unit Back Alley Bar B0 adjacent BattlefieldMovedUnit / MoveUnit / FullGame / MatchRecovery follow-up: `2152/2152`;
- moved-unit Back Alley Bar B0 backend full follow-up: `8832/8832`;
- held-next-spell Echo focused behavior-spec/source guard/runtime/GameHub representative: `5/5`;
- held-next-spell Echo Piltover Academy B0 score-victory action-log replay follow-up: `1/1`;
- held-next-spell Echo Piltover Academy B0 FullGame follow-up: `76/76`;
- held-next-spell Echo Piltover Academy B0 adjacent BattlefieldHeld / BattlefieldTriggerSpec / Stack / FullGame / MatchRecovery follow-up: `2655/2655`;
- held-next-spell Echo Piltover Academy B0 backend full follow-up: `8832/8832`;
- held unit-cost increase focused behavior-spec/source guard/runtime/GameHub representative: `5/5`;
- held unit-cost increase Vaults of Helia B0 action-log replay follow-up: `2/2`;
- held unit-cost increase Vaults of Helia B0 FullGame follow-up: `73/73`;
- held unit-cost increase Vaults of Helia adjacent / hidden-info follow-up: `3445/3445`;
- held unit-cost increase Vaults of Helia backend full follow-up: `8829/8829`;
...
- held call-rune Star Peak B0 full-game follow-up: `56/56`;
- held call-rune Star Peak B0 adjacent BattlefieldHeld / BattlefieldTriggerSpec / CallRune follow-up: `132/132`;
...
- held each-player call-rune Confetti Tree B0 action-log replay follow-up: `1/1`;
- held each-player call-rune Confetti Tree B0 full-game follow-up: `57/57`;
- held each-player call-rune Confetti Tree B0 adjacent BattlefieldHeld / BattlefieldTriggerSpec / CallRune follow-up: `133/133`;
- held move-unit-to-base focused behavior-spec/source guard/runtime/GameHub representative: `4/4`;
- held move-unit-to-base Rehearsal Hall B0 score-victory owner-fallback follow-up: `1/1`;
- held move-unit-to-base Rehearsal Hall B0 full-game follow-up: `76/76`;
- held move-unit-to-base Rehearsal Hall B0 adjacent BattlefieldHeld / MoveUnit / FullGame / MatchRecovery follow-up: `2233/2233`;
- held move-unit-to-base Rehearsal Hall B0 backend full follow-up: `8832/8832`;
...
- held grant-boon Navori Arena B0 action-log replay follow-up: `1/1`;
- held grant-boon Navori Arena B0 full-game follow-up: `59/59`;
- held grant-boon Navori Arena B0 adjacent BattlefieldHeld / BattlefieldTriggerSpec / Boon / FullGame follow-up: `236/236`;
...
- held create-minion Unity Sanctum B0 action-log replay follow-up: `1/1`;
- held create-minion Unity Sanctum B0 full-game follow-up: `60/60`;
- held create-minion Unity Sanctum B0 adjacent BattlefieldHeld / BattlefieldTriggerSpec / Token / FullGame follow-up: `233/233`;
...
- held return-hero Hallowed Tomb B0 action-log replay follow-up: `1/1`;
- held return-hero Hallowed Tomb B0 full-game follow-up: `61/61`;
- held return-hero Hallowed Tomb B0 adjacent BattlefieldHeld / BattlefieldTriggerSpec / ChampionZone / FullGame / GameHub follow-up: `356/356`;
...
- held seven-units win Grand Plaza B0 action-log replay follow-up: `1/1`;
- held seven-units win Grand Plaza B0 full-game follow-up: `62/62`;
- held seven-units win Grand Plaza B0 adjacent BattlefieldHeld / BattlefieldTriggerSpec / FullGame / GameHub / MatchRecovery follow-up: `2345/2345`;
- held pay-power score Energy Hub B0 action-log replay follow-up: `1/1`;
- held pay-power score Energy Hub B0 full-game follow-up: `63/63`;
- held pay-power score Energy Hub B0 adjacent BattlefieldHeldScore / BrushReplacement / BattleDamageAssignmentLifecycle / PaymentEngine / BattlefieldTriggerSpec / FullGame / GameHub / MatchRecovery follow-up: `3126/3126`;
- held activate-unit-conquest-effects Reckoner Arena B0 action-log replay follow-up: `1/1`;
- held activate-unit-conquest-effects Reckoner Arena B0 full-game follow-up: `64/64`;
- held activate-unit-conquest-effects Reckoner Arena B0 adjacent UnitConquest / BattlefieldHeldActivateConquest / BattleDamageAssignmentLifecycle / BattlefieldTriggerSpec / FullGame / GameHub / MatchRecovery follow-up: `2361/2361`;
- defend move-friendly-unit-to-base Plunder Alley B0 action-log replay follow-up: `1/1`;
- defend move-friendly-unit-to-base Plunder Alley B0 full-game follow-up: `68/68`;
- defend move-friendly-unit-to-base Plunder Alley B0 adjacent Plunder / BattlefieldDefend / BattlefieldTriggerSpec / DeclareBattle / GameHub / FullGame / MatchRecovery follow-up: `2396/2396`;
- defend grant-Steadfast Fortified Position B0 action-log replay follow-up: `1/1`;
- defend grant-Steadfast Fortified Position B0 full-game follow-up: `65/65`;
- defend grant-Steadfast Fortified Position B0 adjacent BattlefieldDefender / BattlefieldDefend / DeclareBattle / FullGame / GameHub / MatchRecovery follow-up: `2393/2393`;
...
- conquer discard-draw Zaun Sump B0 FullGameEndToEnd follow-up: `84/84`;
- conquer discard-draw Zaun Sump B0 adjacent / hidden-info follow-up: `2077/2077`;
...
- conquer ready-equipment Moonveil Altar B0 action-log replay follow-up: `1/1`;
- conquer ready-equipment Moonveil Altar B0 FullGameEndToEnd follow-up: `87/87`;
- conquer ready-equipment Moonveil Altar B0 adjacent / hidden-info follow-up: `2157/2157`;
- conquer ready-equipment Moonveil Altar backend full follow-up: `8851/8851`;
...
- conquer powerful pay-draw B0 action-log replay follow-up: `1/1`;
- conquer powerful pay-draw B0 full-game follow-up: `43/43`;
...
- conquer pay-return-unit create-Sand-Soldier adjacent BattlefieldConquer / TriggerPayment / BattlefieldTriggerSpec / SandSoldier representatives: included in combined `2222/2222`;
- conquer pay-return-unit create-Sand-Soldier B0 action-log replay pay + decline follow-up: included in focused `4/4`;
- conquer pay-return-unit create-Sand-Soldier B0 full-game follow-up: included in `72/72`;
- conquer pay-return-unit create-Sand-Soldier B0 adjacent MatchRecovery follow-up: included in combined `2222/2222`;
- conquer pay-ready-legend focused behavior-spec/source guard/runtime/GameHub representative: `5/5`;
- conquer pay-ready-legend adjacent BattlefieldConquer / BattlefieldTriggerSpec / ReadyLegend / LegendReadied representatives: included in combined `2222/2222`;
- conquer pay-ready-legend B0 action-log replay pay + decline follow-up: included in focused `4/4`;
- conquer pay-ready-legend B0 full-game follow-up: included in `72/72`;
- conquer pay-ready-legend B0 adjacent MatchRecovery follow-up: included in combined `2222/2222`;
...
- defend reveal-spell-or-recycle B0 action-log replay follow-up: `1/1`;
- defend reveal-spell-or-recycle B0 full-game follow-up: `46/46`;
- defend reveal-spell-or-recycle B0 adjacent MatchRecovery follow-up: `2056/2056`;
- defend reveal-spell-or-recycle B0 non-spell replay follow-up: `1/1`;
- defend reveal-spell-or-recycle B0 non-spell full-game follow-up: `47/47`;
...
- friendly-spell draw / spell-power `FRIENDLY_UNIT_AT_THIS_BATTLEFIELD` locality focused follow-up: `8/8`;
- friendly-spell draw Dream Tree B0 action-log replay follow-up: `1/1`;
- friendly-spell draw Dream Tree B0 full-game follow-up: `49/49`;
- friendly-spell draw Dream Tree B0 adjacent MatchRecovery follow-up: `2051/2051`;
- spell-power bonus Waste Hall B0 action-log replay follow-up: `1/1`;
- spell-power bonus Waste Hall B0 full-game follow-up: `50/50`;
- spell-power bonus Waste Hall B0 adjacent MatchRecovery follow-up: `2045/2045`;
- high-cost spell insight Lost Library B0 action-log replay follow-up: `1/1`;
- high-cost spell insight Lost Library B0 full-game follow-up: `51/51`;
- high-cost spell insight Lost Library B0 adjacent MatchRecovery follow-up: `2046/2046`;
- unit-play boon Idol Valley B0 action-log replay follow-up: `1/1`;
- unit-play boon Idol Valley B0 full-game follow-up: `52/52`;
- unit-play boon Idol Valley B0 adjacent BattlefieldTriggerSpec / MatchRecovery follow-up: `1995/1995`;
- first-unit move-other Meteor Spring B0 action-log replay follow-up: `1/1`;
- first-unit move-other Meteor Spring B0 full-game follow-up: `53/53`;
- first-unit move-other Meteor Spring B0 adjacent BattlefieldTriggerSpec / MatchRecovery follow-up: `2333/2333`;
- unit-returned call-rune Ghost Bay B0 action-log replay follow-up: `1/1`;
- unit-returned call-rune Ghost Bay B0 full-game follow-up: `54/54`;
- unit-returned call-rune Ghost Bay B0 adjacent BattlefieldTriggerSpec / MatchRecovery follow-up: `2093/2093`;
...
- turn-start damage-units Frost Hold B0 action-log replay follow-up: `1/1`;
- turn-start damage-units Frost Hold B0 FullGameEndToEnd follow-up: `77/77`;
- turn-start damage-units Frost Hold B0 adjacent BattlefieldTurnStartDamage / BattlefieldTriggerSpec / FullGameEndToEnd / MatchRecovery follow-up: `2071/2071`;
...
- first-turn extra-rune Power Obelisk B0 action-log replay follow-up: `1/1`;
- first-turn extra-rune Power Obelisk B0 FullGameEndToEnd follow-up: `79/79`;
- first-turn extra-rune Power Obelisk B0 adjacent PowerObelisk / FirstTurnRune / BattlefieldFirstTurn / BattlefieldTriggerSpec / FullGameEndToEnd / MatchRecovery follow-up: `2076/2076`;
...
- first-turn score Glory Arena B0 action-log replay follow-up: `1/1`;
- first-turn score Glory Arena B0 FullGameEndToEnd follow-up: `80/80`;
- first-turn score Glory Arena B0 adjacent GloryArena / FirstTurnScore / ScoreDelay / BattlefieldFirstTurn / BattlefieldTriggerSpec / FullGameEndToEnd / MatchRecovery follow-up: `2083/2083`;
...
- score-delay Forgotten Monument B0 action-log replay follow-up: `1/1`;
- score-delay Forgotten Monument B0 FullGameEndToEnd follow-up: `81/81`;
- score-delay Forgotten Monument B0 adjacent ForgottenMonument / ScoreDelay / ScorePrevented / FirstTurnScore / BattlefieldHeldScore / BattleResponse / BattlefieldTriggerSpec / FullGameEndToEnd / MatchRecovery follow-up: `2140/2140`;
...
- winning-score increase B0 action-log replay follow-up: `1/1`;
- winning-score increase B0 FullGameEndToEnd follow-up: `82/82`;
- winning-score increase B0 adjacent WinningScore / BattlefieldStaticWinningScore / MindAndBalance / FullGameEndToEnd / MatchRecovery follow-up: `2082/2082`;
...
- backend full conformance after the Imperial Shrine / Hall of Legends B0 trigger-payment pay + decline follow-up: `8828/8828`;
...
- backend full conformance after the Dream Tree locality / B0 action-log replay follow-up: `8745/8745`;
- backend full conformance after the Waste Hall B0 action-log replay follow-up: `8746/8746`;
- backend full conformance after the Lost Library B0 action-log replay follow-up: `8747/8747`;
- backend full conformance after the Idol Valley B0 action-log replay follow-up: `8748/8748`;
- backend full conformance after the Meteor Spring B0 action-log replay follow-up: `8749/8749`;
- backend full conformance after the Ghost Bay B0 action-log replay follow-up: `8750/8750`;
- backend full conformance after the `OGN·280/298` held draw-one B0 action-log replay follow-up: `8751/8751`;
- backend full conformance after the Star Peak held call-rune B0 action-log replay follow-up: `8752/8752`;
- backend full conformance after the Confetti Tree held each-player call-rune B0 action-log replay follow-up: `8753/8753`;
- backend full conformance after the Rehearsal Hall held move-unit-to-base B0 action-log replay follow-up: `8754/8754`;
- backend full conformance after the Navori Arena held grant-boon B0 action-log replay follow-up: `8755/8755`;
- backend full conformance after the Unity Sanctum held create-minion B0 action-log replay follow-up: `8756/8756`;
- backend full conformance after the Hallowed Tomb held return-hero B0 action-log replay follow-up: `8757/8757`;
- backend full conformance after the Grand Plaza held seven-units win B0 action-log replay follow-up: `8758/8758`;
- backend full conformance after the Energy Hub held pay-power score B0 action-log replay follow-up: `8759/8759`;
- backend full conformance after the Reckoner Arena held activate-unit-conquest B0 action-log replay follow-up: `8760/8760`;
- backend full conformance after the Fortified Position defend grant-Steadfast B0 action-log replay follow-up: `8761/8761`;
- backend full conformance after the Back Alley Bar moved-unit power B0 score-victory follow-up: `8832/8832`;
- backend full conformance after the Piltover Academy held-next-spell Echo B0 score-victory follow-up: `8832/8832`;
- backend full conformance after the Frost Hold turn-start damage B0 score-victory follow-up: `8833/8833`;
- backend full conformance after the Duskpetal Lab turn-start destroy-draw B0 score-victory follow-up: `8834/8834`;
- backend full conformance after the Power Obelisk first-turn extra-rune B0 score-victory follow-up: `8835/8835`;
- backend full conformance after the Glory Arena first-turn score B0 score-victory follow-up: `8836/8836`;
- backend full conformance after the Forgotten Monument score-delay B0 score-victory follow-up: `8837/8837`;
- backend full conformance after the winning-score increase B0 score-victory follow-up: `8838/8838`;
- backend full conformance after the Piltover Academy held-next-spell Echo B0 stack-state follow-up: `8763/8763`;
- backend full conformance after the Plunder Alley defend move-friendly-unit-to-base B0 action-log replay follow-up: `8765/8765`;
...
This Moonveil Altar B0 follow-up additionally proves the parsed conquer ready-equipment route can carry from a legal official Rumble opening into a real battle declaration, choose an exhausted controlled official `SFD·022/221` Long Sword equipment object from P1 base, ready it, detach it from its attached `SFD·006/221` Aggressive Dragonhound host when it is an armament, score-victory replay, and hidden-info guarded full-game helpers. Complete optional equipment-selection prompts, complete armament attachment edge cases, complete battlefield FUs, and complete official deck archetype breadth remain open.
...
This Piltover Academy B0 score-victory follow-up additionally proves the parsed held-next-spell Echo marker can carry from a legal official opening into a real held battle declaration, a derived next-spell window, Echo optional payment, repeated spell stack resolution, score-victory replay, and hidden-info guarded full-game helpers. Natural same-turn non-active spell access, Swift stack-response `PLAY_CARD` prompts, complete Echo optional prompt breadth, complete battlefield FUs, and complete official deck archetype breadth remain open.

This Frost Hold B0 score-victory follow-up additionally proves the parsed turn-start damage-units route can carry from a legal official opening into replayable `END_TURN`, pre-scoring same-battlefield damage to units controlled by both players, score-victory replay, and hidden-info guarded full-game helpers. Complete optional trigger prompts, complete turn-start battlefield trigger breadth, complete battlefield FUs, and complete official deck archetype breadth remain open.

This Duskpetal Lab B0 score-victory follow-up additionally proves the parsed turn-start destroy-draw route can carry from a legal official opening into replayable `END_TURN`, pre-scoring same-battlefield controlled-unit destruction, one-card draw, offsite controlled-unit preservation, score-victory replay, and hidden-info guarded full-game helpers. Complete optional yes/no trigger prompts, complete turn-start battlefield trigger breadth, complete battlefield FUs, and complete official deck archetype breadth remain open.

This Power Obelisk B0 score-victory follow-up additionally proves the parsed first-turn extra-rune route can carry from a legal official opening-selected source into a focused first-turn replay, replayable `END_TURN`, P2 first-turn `RUNES_CALLED` count four, score-victory replay, and hidden-info guarded full-game helpers. Natural earliest opening rune-call timing, complete turn-start battlefield trigger breadth, complete battlefield FUs, and complete official deck archetype breadth remain open.

This Glory Arena B0 score-victory follow-up additionally proves the parsed first-turn score route can carry from a legal official opening-selected source into a focused first-turn replay, replayable `END_TURN`, P2 first-turn `BATTLEFIELD_FIRST_TURN_GAIN_SCORE` amount one, score-victory replay, and hidden-info guarded full-game helpers. Natural earliest opening score timing, complete turn-start battlefield trigger breadth, complete battlefield FUs, and complete official deck archetype breadth remain open.

This Forgotten Monument B0 score-victory follow-up additionally proves the parsed score-delay route can carry from a legal official opening-selected source into a focused first-turn replay, replayable `END_TURN`, P2 first-turn `BATTLEFIELD_SCORE_DELAY_UNTIL_THIRD_TURN` prevention against `BATTLEFIELD_FIRST_TURN_GAIN_SCORE`, score-victory replay, and hidden-info guarded full-game helpers. Complete physical `此处` score scoping, natural earliest opening score timing, complete turn-start battlefield trigger breadth, complete battlefield FUs, and complete official deck archetype breadth remain open.

This winning-score increase B0 score-victory follow-up additionally proves the parsed static winning-score route can carry from a legal official opening-selected source into a focused first-turn replay, replayable `END_TURN`, P2 eighth score without a win, later score-victory at `winningScore=9`, and hidden-info guarded full-game helpers. Multiple-source stacking breadth, natural earliest opening score timing, complete battlefield FUs, and complete official deck archetype breadth remain open.
...
This evidence proves thirty-nine battlefield trigger representatives plus two battlefield static representatives have moved to BehaviorSpec-driven routing, the friendly-spell draw / spell-power / Plunder Alley `FRIENDLY_UNIT_AT_THIS_BATTLEFIELD` trigger scopes now honor precise `ObjectLocations`, four parsed battlefield-conquer payment routes now have B0 official-deck action-log replay follow-ups with pay/decline semantics, the parsed Moonveil Altar ready-equipment route now has a B0 official-deck score-victory replay for readying and detaching an exhausted controlled armament, the parsed Back Alley Bar moved-unit power route now has a B0 official-deck score-victory action-log replay, the parsed Piltover Academy held-next-spell Echo route now has a B0 official-deck score-victory replay for the held marker plus derived next-spell Echo stack resolution, the parsed Frost Hold turn-start damage-units route now has a B0 official-deck score-victory replay for replayable pre-scoring same-battlefield damage, the parsed Duskpetal Lab turn-start destroy-draw route now has a B0 official-deck score-victory replay for replayable pre-scoring same-battlefield destruction/draw and offsite preservation, the parsed Power Obelisk first-turn extra-rune route now has a B0 official-deck score-victory replay for a replayable P2 first-turn four-rune call, the parsed Glory Arena first-turn score route now has a B0 official-deck score-victory replay for a replayable P2 first-turn one-score gain, the parsed Forgotten Monument score-delay route now has a B0 official-deck score-victory replay for replayable first-turn score prevention, the parsed winning-score increase route now has a B0 official-deck score-victory replay for replayable threshold-nine victory, the parsed Imperial Shrine pay-return-unit create-Sand-Soldier route now has B0 official-deck action-log replay coverage for both `PAY_COST(SPEND_MANA:1)` and `PAY_COST(DECLINE)` plus returned-hidden-object metadata redaction coverage, the parsed Hall of Legends pay-ready-legend route now has B0 official-deck action-log replay coverage for both `PAY_COST(SPEND_MANA:1)` and `PAY_COST(DECLINE)`, the parsed Ravenbloom defend reveal route now has B0 official-deck action-log replay coverage for both spell-hit and non-spell-recycle branches, the parsed Hunting Grounds overkill create-Warhawk route now has a B0 official-deck action-log replay, the parsed Candlelit Sanctum conquer reveal/recycle route now has a B0 official-deck action-log replay, the parsed Minefield conquer mill route now has a B0 official-deck action-log replay, the parsed Dream Tree friendly-spell draw route now has a B0 official-deck action-log replay, the parsed Waste Hall spell-power bonus route now has a B0 official-deck action-log replay, the parsed Lost Library high-cost spell insight route now has a B0 official-deck action-log replay, the parsed Idol Valley unit-play pay-boon route now has a B0 official-deck action-log replay, the parsed Meteor Spring first-unit move-other route now has a B0 official-deck action-log replay, the parsed Ghost Bay unit-returned call-rune route now has a B0 official-deck action-log replay, the parsed `OGN·280/298` held draw-one route now has a B0 official-deck action-log replay, the parsed Star Peak held call-rune route now has a B0 official-deck action-log replay, the parsed Confetti Tree held each-player call-rune route now has a B0 official-deck action-log replay, the parsed Rehearsal Hall held move-unit-to-base route now has a B0 official-deck score-victory action-log replay backed by effective-controller turn-start scoring, the parsed Navori Arena held grant-boon route now has a B0 official-deck action-log replay, the parsed Unity Sanctum held create-minion route now has a B0 official-deck action-log replay, the parsed Hallowed Tomb held return-hero route now has a B0 official-deck action-log replay, the parsed Grand Plaza held seven-units win route now has a B0 official-deck action-log replay, the parsed Energy Hub held pay-power score route now has a B0 official-deck action-log replay, the parsed Reckoner Arena held activate-unit-conquest route now has a B0 official-deck action-log replay, the parsed Plunder Alley defend move-friendly-unit-to-base route now has B0 official-deck action-log replay plus non-defending friendly-unit target-scope coverage, and the parsed Fortified Position defend grant-Steadfast route now has a B0 official-deck action-log replay. It does not prove the complete B4 battlefield-effect family, all trigger timing windows, natural same-turn non-active spell access, Swift stack-response `PLAY_CARD` prompts, all movement / control-zone edge cases, optional trigger choice prompts for non-payment battlefield families, natural earliest opening rune-call timing, natural earliest opening score timing, complete `此处` physical battlefield scoping for score prevention, broader Fortified Position target-choice semantics, complete winning-score multiple-source stacking breadth, complete legend target-choice breadth, complete reveal / recycle hidden-info breadth, complete token lifecycle breadth, the remaining individual unit conquest-effect helper breadth activated by Reckoner Arena, B0 full real-deck end-to-end game completion, all card-effect families, frontend smoke or READY.
