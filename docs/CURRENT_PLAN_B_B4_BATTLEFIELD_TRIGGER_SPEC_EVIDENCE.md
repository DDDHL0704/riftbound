...
The 2026-06-27 B0 follow-up adds an official-deck action-log replay for Rehearsal Hall (`UNL-207/219`). It selects the BehaviorSpec battlefield source through normal opening prompts, stages a low-power official attacker and a surviving official defender at that battlefield, resolves the `DECLARE_BATTLE` held path, asserts the parsed move-unit-to-base trigger and server-authored `UNIT_MOVED_TO_BASE` state transition, then replays the resulting action log to the triggered final state hash. This replay intentionally stops at the trigger result because Rehearsal Hall moves the only defending unit to its owner's base, so the generic score-victory driver is not a stable acceptance condition for this narrow route.
...
- moved-unit Back Alley Bar B0 action-log replay follow-up: `1/1`;
- moved-unit Back Alley Bar B0 FullGame follow-up: `66/66`;
- moved-unit Back Alley Bar B0 adjacent BattlefieldMovedUnit / MoveUnit / FullGame / MatchRecovery follow-up: `2142/2142`;
...
- held move-unit-to-base Rehearsal Hall B0 action-log replay follow-up: `1/1`;
- held move-unit-to-base Rehearsal Hall B0 full-game follow-up: `58/58`;
- held move-unit-to-base Rehearsal Hall B0 adjacent BattlefieldHeld / BattlefieldTriggerSpec / MoveUnit / FullGame follow-up: `224/224`;
...
- backend full conformance after the Back Alley Bar moved-unit power B0 action-log replay follow-up: `8762/8762`;
