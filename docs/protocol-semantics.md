...
| `END_TURN` | 表明主阶段没有要执行的自决行动，进入回合结束流程 | 普通开环主阶段，依据 `CORE-260330` p30 rules 316.1-316.6 和 p30-p31 rules 317.1-317.3 | `CoreRuleEngine` 已产出正式 P2 事件序列并自动串到下一回合开始；完整特殊清理细节仍需补齐。 |
