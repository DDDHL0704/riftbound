...
| `PLAY_CARD` | 从手牌打出卡牌 | 打出卡牌流程、目标选择、费用和结算链，依据 `CORE-260330` p39-p42 rules 355-356, p33-p35 rules 327-340 | 当前最小实现按 card behavior registry 支持 P2 preflight：校验手牌、费用、目标范围、对象标签、模式、可选费用和强制额外费用，入栈后由服务端权威结算。已覆盖的效果族包括伤害、抽牌、摧毁、回手、移动、战力修正、标签、装备对象、指示物、符文召出、回响和部分替代效果；完整卡牌/模式清单见 `docs/p2-rules-preflight.md` 与 `docs/rules-evidence-index.md`。 |
