...
| `END_TURN` | 表明主阶段没有要执行的自决行动，进入回合结束流程 | 普通开环主阶段，依据 `CORE-260330` p30 rules 316.1-316.6 和 p30-p31 rules 317.1-317.3 | `CoreRuleEngine` 已产出正式 P2 事件序列并自动串到下一回合开始；伤害移除和本回合内效果失效已接入，清理重复仍需补齐。 |
...
- Hub 错误统一返回 `ErrorDto(code, message)`；当前已覆盖 `ROOM_FULL`、`PLAYER_NOT_IN_ROOM`、`CLIENT_INTENT_ID_REQUIRED`、`MATCH_NOT_STARTED`、`INVALID_RECONNECT_TOKEN` 和 command 层拒绝。
