# Stage 4D-05U Hide Card Replay Evidence
...
- `docs/CURRENT_STAGE4D_05U_HIDE_CARD_REPLAY_AUDIT.md`
- `docs/CURRENT_STAGE4D_05U_HIDE_CARD_REPLAY_EVIDENCE.md`

## Validation

```bash
source scripts/dev-env.sh && dotnet test tests/Riftbound.ConformanceTests/Riftbound.ConformanceTests.csproj --no-restore --filter "FullyQualifiedName~P4HideCardCommandRejectsAcceptedReplayWithoutMutation"
```

Result: passed `1/1`.

```bash
source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore --filter "FullyQualifiedName~HideCard|FullyQualifiedName~Standby|FullyQualifiedName~MatchStateHasher"
```
