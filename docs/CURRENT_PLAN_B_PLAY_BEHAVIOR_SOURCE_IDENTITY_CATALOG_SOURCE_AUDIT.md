...
|---|---|---|
| Raging Drake play resolution source identity no longer selects by card number or runtime effect kind | `CoreRuleEngine` now reads `SourceNextSpellCostReductionMana` / `SourceNextSpellCostReductionEffectKind` instead of `behavior.CardNo == RagingDrakeCardNo` or `behavior.EffectKind == RAGING_DRAKE_NEXT_SPELL_COST_PLAY_UNIT`; `MatchSession` prompt cost also reads marker amounts through the behavior row | Accepted |
| Poro Herder play resolution source identity no longer selects by card number or runtime effect kind | `CoreRuleEngine` now reads `SourceBoonConditionKind` / `SourceBoonRequiredControlledUnitTag` / `SourceBoonDrawCount` instead of `behavior.CardNo == PoroHerderCardNo` or `behavior.EffectKind == PORO_HERDER_NO_PORO_STATIC_PLAY_UNIT` | Accepted |
...
Latest Raging Drake source-next-spell cost-reduction behavior-field follow-up:
...
Latest Poro Herder source-boon draw behavior-field follow-up:
...
- This does not generalize every play-trigger representative; it removes the current Raging Drake, Poro Herder, Balanced Disciple, Crescent Guard, Ascended Believer, Sly Salamander, Rampaging Soul, Armed Assaulter, and Akshan source card-number dependencies and routes those branches through implemented catalog effect kinds / behavior rows. Raging Drake, Poro Herder, Balanced Disciple, Crescent Guard, Armed Assaulter, and Akshan now use narrower behavior fields where available.
