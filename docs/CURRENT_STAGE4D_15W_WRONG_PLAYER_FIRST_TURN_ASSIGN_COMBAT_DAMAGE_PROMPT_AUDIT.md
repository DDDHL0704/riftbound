# Stage 4D-15W Wrong Player First Turn Assign Combat Damage Prompt Audit

Status: accepted test-only server P0-005 official first-turn wrong-player current `MAIN_ACTION` prompt `ASSIGN_COMBAT_DAMAGE` boundary slice.
...
- `tests/Riftbound.ConformanceTests/OfficialOpeningTests.cs`
  - Added `WrongPlayerFirstTurnAssignCombatDamagePromptRejectsWithoutMutation`.
  - Added `PromptIdOnlyWrongPlayerFirstTurnAssignCombatDamagePromptRejectsWithoutMutation`.
  - Added `SnapshotOnlyWrongPlayerFirstTurnAssignCombatDamagePromptRejectsWithoutMutation`.
  - Added `AssertWrongPlayerFirstTurnAssignCombatDamagePromptRejectsWithoutMutation`.
...
- Accepted first-turn state has no active battle.
- Non-active-player `ASSIGN_COMBAT_DAMAGE` carrying the active player's prompt-scoped or prompt-id-only current `MAIN_ACTION` prompt envelope rejects with `ErrorCodes.PromptExpired` and exact diagnostics: `行动窗口已过期，请按最新提示重新提交。`
- Non-active-player `ASSIGN_COMBAT_DAMAGE` carrying only the same-tick snapshot reaches the command payload shell and rejects with `ErrorCodes.InvalidPayload` and exact diagnostics: `ASSIGN_COMBAT_DAMAGE 需要 battleId、battlefieldId 与非空 assignments。`
