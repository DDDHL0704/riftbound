# Stage 4D-18DR Recovery Timing Temporary Payment Resource Remaining Power Value Audit
...
Added `RecoveryValidatorRejectsSpectatorReplayTimingTemporaryPaymentResourceKeyedRemainingPowerValueWithCountMismatch` in `tests/Riftbound.ConformanceTests/MatchRecoveryTests.cs`.

The test builds an authoritative temporary payment resource from real `MatchState` source-object state, keeps the emitted spectator payload keyed to `temp-payment-resource-1`, changes the emitted `remainingPower` from authoritative `1` to a valid numeric value `2`, then appends `temp-payment-resource-extra` to force resource-count mismatch.
...
This closes the focused remaining-power value-drift variant, keeping keyed authoritative remaining-power validation independently locked for spectator timing temporary-payment-resource payloads when the spectator value is readable and otherwise valid.

## Validation

- Focused new test: `1/1`.
- Focused `TemporaryPaymentResource` filter: `117/117`.
- Focused recovery filter: `1150/1150`.
- Adjacent recovery/official-opening/Postgres recovery-store filter: `1731/1731`.
- Backend full: `7096/7096`.
