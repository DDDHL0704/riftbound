# Stage 4D-223AD GameHub TapRune After-Finished Error Envelope Audit
...
`TapRuneAfterFinishedRedactsSentinelPayloadAndDoesNotBroadcastOrMutate` now proves the after-finished `TAP_RUNE` caller error is a `MessageType.ERROR` envelope, preserves normalized `roomId` / `P1` routing, and carries default protocol/schema versions.
...
- no `intentId`, `TAP_RUNE`, `SubmitIntent`, `MatchFinished` or `MatchFinished` code leakage
...
Latest core rules 302, 323.1 and 467 anchor the game-ending and win-state context. Latest core rules 160-166 anchor rune identity, rune-deck return behavior and rune-pool resource storage/spending. Latest core rules 414-415 anchor rest/ready object state and rest-as-cost context. Latest core rule 429 anchors resource gain into the rune pool. Latest core rule 430 anchors summoning runes from the rune deck into play. Latest core rules 649-652, especially 651.3, anchor player removal and the rule that a removed player cannot choose or otherwise affect the game after removal.

This is protocol-error-envelope coverage only. It does not change rune tap legality, resource generation, rune-pool spending, rune summon/recycle behavior, hidden rune-deck order, win/score/surrender/removal behavior, or redaction text.

## Validation

- Focused test: `1/1`.
- Changed class `GameHubJoinTests`: `217/217`.
- Adjacent Hub/after-finished/protocol/error/TapRune/Rune/RecycleRune/Resource/Pay/Raw/ClientIntent filter: `2500/2500`.
