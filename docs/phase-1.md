...
下一步继续 P2 preflight：逐批迁移更多低复杂度官方卡牌，并继续扩大通用 expected diff 覆盖面。普通主阶段误提交 `PASS_PRIORITY` / `PASS_FOCUS` 已能以 `PHASE_NOT_ALLOWED` 拒绝；FEPR、法术对决、连续燃尽胜利、六张官方伤害法术、一张官方眩晕法术和结算后抽牌/抽牌燃尽路径已通过 fixture，且《惩戒》《渊海狩咒》《焚烧》《海克斯射线》《彗星坠击》《虚空索敌》《符文禁锢》已进入最小 card behavior registry，《惩戒》《符文禁锢》已覆盖基地单位目标，《海克斯射线》已覆盖不能指定基地单位的战场目标限制，《渊海狩咒》正面朝下条件伤害已覆盖，《虚空索敌》结算抽牌触发燃尽/回收/抽牌已覆盖，《海克斯射线》真实伤害和《符文禁锢》`STUNNED` 随 `END_TURN` 特殊清理移除/失效已覆盖。协议版本治理剩余 TypeScript DTO 生成、客户端兼容策略、SignalR 方法版本和事件 upcaster；不要在没有前端接入点前过度设计。新增 fixture 不再使用裸 `PASS`。
