...
- 当前测试覆盖：
  ...
  - `MatchState` 已加入 `turnPlayerId`、`phase`、`timingState`、`runePools`、`playerZones`、`playerScores`、`cardObjects`、`priorityPlayerId`、`passedPriorityPlayerIds`、`stackItems`、`focusPlayerId`、`passedFocusPlayerIds`、`winnerPlayerId`、`destroyedUnitOwnerIdsThisTurn`、`seed`、`rngCursor`，snapshot timing 已投影这些 P2 权威 timing 字段、公开结算链、焦点、赢家字段、最小本回合摧毁记忆和可回放随机游标；当前已覆盖最小回合开始、`END_TURN` 自动推进、燃尽/连续燃尽胜利、伤害/本回合内效果清理、清理重复、FEPR 最小让过/结算、法术对决焦点让过，以及官方法术《惩戒》《渊海狩咒》的打出/入栈/结算伤害；`cardObjects.power` 已能支撑伤害达到战力后的致命摧毁，`cardObjects.untilEndOfTurnPowerModifier` 已能支撑本回合内战力修正的结算与清理，`cardObjects.isFaceDown` 已能支撑《渊海狩咒》的正面朝下条件伤害。后续扩展战场控制和卡牌对象状态时必须继续维护 `state_snapshots.payload`。
...
下一步继续 P2 preflight：逐批迁移更多低复杂度官方卡牌，并继续扩大通用 expected diff 覆盖面。普通主阶段误提交 `PASS_PRIORITY` / `PASS_FOCUS` 已能以 `PHASE_NOT_ALLOWED` 拒绝；FEPR、法术对决、连续燃尽胜利、十七张官方伤害法术、三张官方眩晕法术、八条 0 目标抽牌路径、单次回响、六条主动摧毁单位路径、伤害达到战力后的致命摧毁、条件抽牌、目标控制者抽牌、结算后抽牌/抽牌燃尽路径、两条费用修正路径（本回合敌方单位被摧毁、控制单位最高战力），以及一条本回合内战力修正后抽牌路径、一条回响重复战力修正路径、一条进攻方目标条件伤害路径、一条对手废牌堆回收路径已通过 fixture，且《惩戒》《渊海狩咒》《焚烧》《海克斯射线》《雷霆突降》《透体圣光》《霹天雳地》《彗星坠击》《终极闪光》《超究极死神飞弹！》《剑刃飓风》《台前作秀》《聚心凝神》《先知之兆》《借鉴历史》《进化日》《以战养战》《复仇》《爆能术》《狩魂》《暗刃》《流沙陷坑》《破败之咒》《责退》《罡风》《训练有素》《蛮荒之力》《星芒凝汇》《火箭轰击》《虚空索敌》《符文禁锢》《碎裂之火》《星落》《艾卡西亚暴雨》《走开》《扑咚！》《处置命令》《冥想》已进入最小 card behavior registry，《惩戒》《符文禁锢》已覆盖基地单位目标，《惩戒》已覆盖致命伤害摧毁，《碎裂之火》已覆盖目标被此法术摧毁后的抽牌，《星落》《艾卡西亚暴雨》已覆盖多次单位伤害选择和重复选择同一目标，《剑刃飓风》已覆盖 0 目标全战场单位各 1 点伤害路径和致命伤害摧毁路径，《走开》已覆盖从手牌打出时眩晕后抽牌，《扑咚！》已覆盖进攻方单位目标限制和基础眩晕路径，《处置命令》已覆盖抽牌模式和对手废牌堆最多三张回收模式，《冥想》已覆盖基础抽牌路径，《聚心凝神》已覆盖全额支付的抽 2 张路径，《借鉴历史》已覆盖从手牌打出的抽 2 张路径，《以战养战》已覆盖全额支付和本回合敌方单位被摧毁后减费的抽 2 张路径，《训练有素》已覆盖本回合内战力 +2 后抽牌和回合结束修正移除，《蛮荒之力》已覆盖支付回响后重复战力修正，《超究极死神飞弹！》已覆盖基础 5 点伤害路径，《海克斯射线》已覆盖不能指定基地单位的战场目标限制，《雷霆突降》已覆盖进攻方目标条件伤害，《霹天雳地》已覆盖按控制单位最高战力降低法力费用和基础 5 点伤害路径，《星芒凝汇》已覆盖 1-2 目标范围和多目标伤害，《火箭轰击》已覆盖 `PLAY_CARD.mode` 模式选择和基地单位目标，《台前作秀》已覆盖不支付回响和支付回响的抽牌路径，《聚心凝神》《先知之兆》《借鉴历史》《进化日》已覆盖多张抽牌路径，《复仇》《爆能术》《狩魂》《暗刃》《流沙陷坑》《破败之咒》已覆盖主动摧毁单位，其中《狩魂》已覆盖不高于 3 战力目标限制和高战力拒绝，《暗刃》已覆盖目标控制者抽 2 张，《渊海狩咒》正面朝下条件伤害已覆盖，《雷霆突降》进攻方目标条件伤害已覆盖，《透体圣光》基础多目标伤害已覆盖，《虚空索敌》结算抽牌触发燃尽/回收/抽牌已覆盖，《海克斯射线》真实伤害、《符文禁锢》`STUNNED` 和《训练有素》战力修正随 `END_TURN` 特殊清理移除/失效已覆盖。协议版本治理剩余 TypeScript DTO 生成、客户端兼容策略、SignalR 方法版本和事件 upcaster；不要在没有前端接入点前过度设计。新增 fixture 不再使用裸 `PASS`。
