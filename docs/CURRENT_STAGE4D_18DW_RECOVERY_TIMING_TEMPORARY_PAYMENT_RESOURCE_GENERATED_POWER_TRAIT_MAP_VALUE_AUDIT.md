# Stage 4D-18DW Recovery Timing Temporary Payment Resource Generated Power Trait Map Value Audit
...
Added `RecoveryValidatorRejectsSpectatorReplayTimingTemporaryPaymentResourceKeyedGeneratedPowerTraitMapValueWithCountMismatch` in `tests/Riftbound.ConformanceTests/MatchRecoveryTests.cs`.

The test builds an authoritative temporary payment resource from real `MatchState` source-object state, keeps the emitted spectator payload keyed to `temp-payment-resource-1`, changes the legal `generatedPowerByTrait` map from authoritative `{ blue: 2 }` to `{ blue: 1 }`, then appends `temp-payment-resource-extra` to force resource-count mismatch.
...
This closes the focused generated-power trait-map value-drift variant, keeping keyed authoritative generated-power trait validation independently locked for spectator timing temporary-payment-resource payloads when the spectator trait map is structurally valid but semantically different.

## Validation

- Focused new test: `1/1`.
- Focused `TemporaryPaymentResource` filter: `122/122`.
- Focused recovery filter: `1155/1155`.
- Adjacent recovery/official-opening/Postgres recovery-store filter: `1736/1736`.
- Backend full: `7101/7101`.
