2026-05-24 Stage 4D-16M evidence.
...
```bash
source scripts/dev-env.sh && dotnet test tests/Riftbound.ConformanceTests/Riftbound.ConformanceTests.csproj --no-restore --filter "FullyQualifiedName~PromptIdOnlyTapRuneWithFirstReadyPromptAfterFinalReadyRejectsWithoutMutation|FullyQualifiedName~SnapshotOnlyTapRuneWithFirstReadyPromptAfterFinalReadyRejectsWithoutMutation"
```
...
Adjacent result: passed `999/999`.
...
Backend full result: passed `5951/5951`.
