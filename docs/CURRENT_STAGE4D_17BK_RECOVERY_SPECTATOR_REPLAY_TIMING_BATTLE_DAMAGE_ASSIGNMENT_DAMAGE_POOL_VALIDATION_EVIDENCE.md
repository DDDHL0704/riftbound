# Stage 4D-17BK Recovery Spectator Replay Timing Battle Damage-Assignment Damage-Pool Validation Evidence

2026-05-24 Stage 4D-17BK evidence bundle for recovery spectator replay timing battle damage-assignment damage-pool validation.

## Runtime/Test Delta

- `src/Riftbound.Engine/MatchRecovery.cs`
  - `BattleDamageAssignmentMatches` now validates pending nested spectator replay snapshot `Timing["battle"]["damageAssignment"]["damagePool"]` against authoritative `ResolutionResult.BattleDamagePoolFor(state, state.BattleState)`.
  - Added int-dictionary parsing/comparison helpers for object and JSON spectator timing payloads.
- `tests/Riftbound.ConformanceTests/MatchRecoveryTests.cs`
  - Added `RecoveryValidatorRejectsSpectatorReplayTimingBattleDamageAssignmentDamagePoolMismatch`.
...
- Adjacent recovery/opening regression:
  ...
  - Result: passed `670/670`.
- Backend full:
  - `source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore`
  - Result: passed `6035/6035`.
