...
| `p2-preflight-play-scrap-heap-equipment-draw` | `RULE_AUDITED` | `CATALOG` OGN·182/298；`CORE-260330` p4-p8 rules 107-129；p14-p15 rules 142-143；p39-p42 rules 355-356；p57 rule 413.4 | 已验证官方装备《废料堆》从手牌打出时支付 2 点费用、0 目标加入结算链、双方让过后源牌进入控制者基地成为带 `CARD_TYPE:EQUIPMENT` 标签的装备对象，然后控制者抽 1 张牌；带目标打出由直接测试拒绝，弃置和摧毁触发抽牌分支暂缓。 |
| `p2-preflight-play-sprite-lantern-equipment-create-sprite` | `RULE_AUDITED` | `CATALOG` UNL-078/219；`CORE-260330` p4-p8 rules 107-129；p14-p15 rules 142-143；p39-p42 rules 355-356；p92-p105 keyword rules 800+ | 已验证官方装备《精灵提灯》从手牌打出时支付 2 点费用、0 目标加入结算链、双方让过后源牌进入控制者基地成为带 `CARD_TYPE:EQUIPMENT` 和 `瞬息` 标签的装备对象，然后打出一名带 `瞬息` 标签的 3 战力“精灵”到控制者基地；带目标打出由直接测试拒绝，绝念和开始阶段瞬息摧毁暂缓。 |
| `p2-preflight-play-sumpworks-map-equipment-ephemeral` | `RULE_AUDITED` | `CATALOG` UNL-085/219；`CORE-260330` p4-p8 rules 107-129；p14-p15 rules 142-143；p39-p42 rules 355-356；p92-p105 keyword rules 800+ | 已验证官方装备《地沟区地图》从手牌打出时支付 2 点费用、0 目标加入结算链、双方让过后源牌进入控制者基地成为带 `CARD_TYPE:EQUIPMENT` 和 `瞬息` 标签的装备对象；带目标打出由直接测试拒绝，对手得分触发抽牌和开始阶段瞬息摧毁暂缓。 |
| `p2-preflight-play-scrying-blossom-equipment-exhausted` | `RULE_AUDITED` | `CATALOG` UNL-136/219；`CORE-260330` p4-p8 rules 107-129；p14-p15 rules 142-143；p31-p35 rules 318-340；p39-p42 rules 355-356 | 已验证官方装备《占卜花朵》从手牌打出时支付 1 点费用、0 目标加入结算链、双方让过后源牌进入控制者基地成为休眠的 `CARD_TYPE:EQUIPMENT` 装备对象；带目标打出由直接测试拒绝，洞察/抽牌/经验激活技能暂缓。 |
| `p2-preflight-play-magic-beans-equipment` | `RULE_AUDITED` | `CATALOG` UNL-011/219；`CORE-260330` p4-p8 rules 107-129；p14-p15 rules 142-143；p39-p42 rules 355-356 | 已验证官方装备《魔法鲜豆》从手牌打出时支付 2 点费用、0 目标加入结算链、双方让过后源牌进入控制者基地成为带 `CARD_TYPE:EQUIPMENT` 标签的装备对象；带目标打出由直接测试拒绝，法术对决期间单位打出触发抽牌分支暂缓。 |
| `p2-preflight-play-steel-ballista-equipment-exhausted` | `RULE_AUDITED` | `CATALOG` OGN·017/298；`CORE-260330` p4-p8 rules 107-129；p14-p15 rules 142-143；p31-p35 rules 318-340；p39-p42 rules 355-356 | 已验证官方装备《钢铁弩炮》从手牌打出时支付 3 点费用、0 目标加入结算链、双方让过后源牌进入控制者基地成为休眠的 `CARD_TYPE:EQUIPMENT` 装备对象；带目标打出由直接测试拒绝，横置伤害技能暂缓。 |
| `p2-preflight-play-heart-of-ice-equipment` | `RULE_AUDITED` | `CATALOG` SFD·052/221；`CORE-260330` p4-p8 rules 107-129；p14-p15 rules 142-143；p39-p42 rules 355-356 | 已验证官方装备《玄冰之心》从手牌打出时支付 3 点费用、0 目标加入结算链、双方让过后源牌进入控制者基地成为带 `CARD_TYPE:EQUIPMENT` 标签的装备对象；带目标打出由直接测试拒绝，横置战力修正技能暂缓。 |
| `p2-preflight-play-remorse-orb-equipment` | `RULE_AUDITED` | `CATALOG` OGN·090/298；`CORE-260330` p4-p8 rules 107-129；p14-p15 rules 142-143；p39-p42 rules 355-356 | 已验证官方装备《懊悔法球》从手牌打出时支付 1 点费用、0 目标加入结算链、双方让过后源牌进入控制者基地成为带 `CARD_TYPE:EQUIPMENT` 标签的装备对象；带目标打出由直接测试拒绝，横置负战力修正技能暂缓。 |
...
| `p2-preflight-play-void-assault-move-friendly-and-enemy-unit` | `RULE_AUDITED` | `CATALOG` UNL-202/219；`CORE-260330` p4-p8 rules 107-129；p14-p15 rules 142-143；p39-p42 rules 355-356 | 已验证官方专属法术《虚空来袭》按顺序选择友方单位和敌方单位，双方让过后将两名基地单位移动到当前粗粒度战场区域；目标顺序反转由直接测试拒绝，战场控制/进攻方细节暂缓。 |
