...
| A 维护 checkpoint | `docs/CURRENT_A_MASTER_CHECKPOINT.md` 最新、可恢复、含当前结论 | 文件顶部记录 4D-04C accepted、focused 57/57、rejected/shape 113/113、adjacent 207/207、keyword guard 17/17、backend full 4368/4368、项目 NOT READY；本 checklist 已挂回 checkpoint | PARTIAL | 后续每批继续保持 checkpoint 同步 |
| A 维护任务拆分 / 子 agent 分工 | A-master agent pool、写锁、下一步计划 | `A_MASTER_AGENT_GOAL.md` §7/§8；checkpoint 记录 A/B/C/D/E 职责；`CURRENT_STAGE4D_NEXT_DISPATCH_AND_WRITELOCKS.md` 已记录 4D-04C-B implemented/A-validated and no open write locks | ONGOING | 后续 B / frontend / matrix 仍需单独写锁 |
...
| A 控制写入范围 | 不并行改核心模块；本批只打开 4D-04C 窄 runtime/test 写锁 | B worker 只改 Agile direct attach 相关 `CardEquipmentKeywordRules.cs`、`CoreRuleEngine.cs`、`MatchSession.cs`、focused tests 与 necessary fixtures；A 更新 checkpoint / completion audit / server audit / closure plan / dispatch docs 与 audit/evidence docs | OK FOR THIS SLICE | 后续 runtime / frontend / matrix 改动必须按 dispatch 文档独占 owner |
| 默认不写功能代码 | A 不亲自修改 runtime；功能代码由 B worker scoped 实现 | 4D-04C runtime/test diff 由 B worker Singer 完成；A 做派发、审计、验收、测试运行和文档收口 | OK FOR THIS SLICE | 不代表后续功能缺口已解决 |
...
| 后端 full test | `source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore` | 4D-04C A-side verification 记录 backend full 4368/4368 通过 | PASS AS LATEST CODE EVIDENCE | 只证明当前代码测试绿；不证明 P0/P1 全部满足 |
...
| §2.6 后端 full test | full test 绿 | 4D-04C 4368/4368 | PASS BUT NOT SUFFICIENT |
...
| §6 A 边界 | A 读文档、规划、审计；默认不写功能代码 | 4D-04C runtime 由 B worker 实现，A 做派发、验收、测试与 audit | OK FOR THIS SLICE |
...
| §8 写入边界 | B/C/D/E 各自写入范围，不并行改核心模块 | `CURRENT_STAGE4D_NEXT_DISPATCH_AND_WRITELOCKS.md` 已明确 4D-04C-B accepted、禁改文件和后续 frontend / matrix write windows closed | OK FOR THIS SLICE / ONGOING |
...
|---|---|---|
| 1. 修改文件列表 | 最近批次修改 Agile direct attach 相关 engine / session / keyword profile code、focused tests、fixtures 与 checkpoint / completion audit / server audit / closure plan / dispatch / active checklist docs | DONE FOR THIS SLICE / NOT FINAL |
| 2. 新增文件列表 | 新增 4D-04C Agile equipment direct attach audit / evidence docs 与 focused test file | DONE FOR THIS SLICE / NOT FINAL |
...
| 8. 后端 full test 命令和结果 | 4D-04C `dotnet test` 4368/4368 | PASS AS LATEST CODE EVIDENCE |
...
- `dotnet test` 4368/4368 通过不能替代 P0/P1 清零。
- 4D-04C focused 57/57、rejected/shape 113/113、adjacent 207/207、keyword guard 17/17、historical recheck 6/6、backend full 4368/4368 通过不能替代 P1-002 keyword full official closure；它只证明 printed Agile equipment direct-play attach representative。
