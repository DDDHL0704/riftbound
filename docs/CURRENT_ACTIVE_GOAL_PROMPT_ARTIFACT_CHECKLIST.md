...
| 后端 full test | `source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore` | 4D-03AL evidence 记录 backend full 4245/4245 通过 | PASS AS LATEST CODE EVIDENCE | 只证明当前代码测试绿；不证明 P0/P1 全部满足 |
...
| §2.6 后端 full test | full test 绿 | 4D-03AL 4245/4245 | PASS BUT NOT SUFFICIENT |
...
| 8. 后端 full test 命令和结果 | 4D-03AL `dotnet test` 4245/4245 | PASS AS LATEST CODE EVIDENCE |
...
- `dotnet test` 4245/4245 通过不能替代 P0/P1 清零。
