...
- 2026-06-29 Plan B follow-up code evidence: `src/Riftbound.Engine/CoreRuleEngine.cs` and `src/Riftbound.Engine/MatchSession.cs` no longer contain `IsSpiritFireTargetAllowed`, `IsPromptSpiritFireTargetAllowed`, or the `SPIRIT_FIRE_DESTROY_BATTLEFIELD_UNITS_TOTAL_POWER_4` runtime target-filtering branch. Spirit Fire now reuses shared `RequiresVisibleFieldUnitPrimitiveTarget` target filtering through `DestroysTarget` + default `BATTLEFIELD_UNIT` scope while `MaxTotalTargetPower=4` continues to reject illegal total-power selections.
...
- 2026-06-29 Plan B follow-up focused validation: `PrimitiveTargetGuardSourceTests|SpiritFireDestroyGuardTests` passed 14/14.
- 2026-06-29 Plan B follow-up adjacent validation: `PrimitiveTargetGuardSourceTests|SpiritFireDestroyGuardTests|CoreRuleEnginePlaysSpiritFire|SpiritFire|RealTriggerQueue|UndercoverAgentTrigger|MatchRecovery` passed 2077/2077.
- 2026-06-29 Plan B follow-up backend full validation: `tests/Riftbound.ConformanceTests/Riftbound.ConformanceTests.csproj --no-restore --nologo` passed 9023/9023.
