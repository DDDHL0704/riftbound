# Stage 4D-02AF Battle Response Activation Held-Score Next Contest Advancement Evidence
...
```sh
source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore --filter "FullyQualifiedName~NaturalBattleResponseActivationHeldScoreTemporaryPaymentAdvancesNextContestedBattlefieldTask"
```
...
4D-02AF is test-only. It adds one BF-A / BF-B cross-product guard proving held-score temporary payment / score / current battle close completes before BF-B `BATTLEFIELD_CONTESTED` / `SPELL_DUEL_STARTED` advancement.
