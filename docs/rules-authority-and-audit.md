...
2026-07-03 补充：Plan B / B3 兰博单位征服触发以官网卡面与核心规则为权威来源。`SFD·026/221` / `SFD·026a/221` 兰博官方文本要求征服战场后可选择回收另一名友方单位，以此从控制者废牌堆打出一名 `机械` 属性单位，并将所需法力费用减去被回收单位的战力；本批实现并记录回收 4 战力友方单位后把 4 费墓地机械单位减到 0 费并打到基地的代表路径，以及回收 2 战力友方单位后打开 `TRIGGER_PAYMENT` / `PAY_COST(SPEND_MANA:2)`、支付后再回收/打出、拒绝时不移动区域的代表路径，证据见 `docs/CURRENT_PLAN_B_B3_UNIT_CONQUEST_TRIGGER_SPEC_EVIDENCE.md` 与 `docs/rules-evidence-index.md` 的 2026-07-03 Plan B / B3 Rumble 条目。完整可选选择、显式回收/墓地目标选择、支付不足 guard、战场目的地选择和完整兰博家族 breadth 仍不得标记完成。
