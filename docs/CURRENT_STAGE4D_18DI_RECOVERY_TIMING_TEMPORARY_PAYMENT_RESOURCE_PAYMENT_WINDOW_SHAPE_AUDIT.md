# Stage 4D-18DI Recovery Timing Temporary Payment Resource Payment Window Shape Audit
...
Added `RecoveryValidatorRejectsSpectatorReplayTimingTemporaryPaymentResourceKeyedPaymentWindowShapeWithCountMismatch` in `tests/Riftbound.ConformanceTests/MatchRecoveryTests.cs`.

The test builds an authoritative temporary payment resource from real `MatchState` source-object state, keeps the emitted spectator payload keyed to `temp-payment-resource-1`, changes the emitted `paymentWindow` to an unreadable array payload, then appends `temp-payment-resource-extra` to force resource-count mismatch.

It proves validation still emits:

- payment-window invalid payload-shape diagnostics;
...
This closes the focused payment-window shape variant outside the broader required-field shape test, keeping payload-shape validation and keyed authoritative validation independently locked for the optional `paymentWindow` field.

## Validation

- Focused new test: `1/1`.
- Focused `TemporaryPaymentResource` filter: `108/108`.
- Focused recovery filter: `1141/1141`.
- Adjacent recovery/official-opening/Postgres recovery-store filter: `1722/1722`.
- Backend full: `7087/7087`.
