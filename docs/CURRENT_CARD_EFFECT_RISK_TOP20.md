...
阶段 4C-50 追加 Draven keyword-unit combat text ordinary play-unit + `法盾` tag guard representative baseline；阶段 4C-51 追加 Rek'Sai attack reveal / movement text ordinary play-unit guard representative baseline；阶段 4C-52 追加 Rek'Sai no-optional haste / overwhelm keyword ordinary play-unit + keyword tag guard representative baseline。它们只降低对应 FU 的局部 guard 证据风险，不改变 Top20 full-official 阻断或 4B primary status。
...
后续批量顺序建议：Vex / `FU-9f7cb73dc4` 已进入 4C-48；`UNL-150a/219` / `FU-4d8ee1696b` 保持未标记；Sett / `FU-6308c2db01` 继续等待 `LEGEND_ACTION_DOMAIN` shared-oracle design batch。
...
## 55A. Stage 4C-48 Vex Spellshield Opponent-Unit Stun Guard Overlay

4C-48 只更新覆盖矩阵 / 风险证据，不升级 full-official。正确身份为 `FU-9f7cb73dc4` / representative `UNL-150/219` / cardId `34697` / Vex / 《薇古丝》，snapshot entry 为 `UNL-150/219` / `34697`，oracle/effectId 为 `VEX_SPELLSHIELD_OPPONENT_UNIT_STUN_STATIC`。

4C-48 已部分降低的 blocker：

- test-only representative `PLAY_CARD` 0-target + `法盾` / `约德尔人` tag guard baseline。
- invalid input rejected without mutation or leak。
- 4B primary status / flags 不变：`IMPLEMENTED_TESTED`，`IMPLEMENTED_TESTED`、`NEEDS_ENGINE_SUPPORT`、`NEEDS_FAQ_REVIEW`；`fullOfficial=false`，FAQ refs remain open，full-official upgrades = 0。
- `UNL-150a/219` / cardId `34698` / `FU-4d8ee1696b` 是另一个 FU，不由 4C-48 标记。

矩阵数字口径：`stage4C48` verified FUs = 1，verified snapshot entries = 1，cumulative spellshield opponent-unit stun guard verified FUs = 1，full-official upgrades = 0，full-official still uncovered FUs = 811。

本批不关闭 actual opponent-unit stun / cannot-move static、spell duel lifecycle、movement/control-zone full matrix、cleanup/replacement/duration、PaymentEngine、FAQ adjudication for Vex refs、all Vex variants、1009/811 full-official 或正式 18-step E2E。

后续批量顺序建议：`UNL-150a/219` / `FU-4d8ee1696b` 可单独评估 parity coverage；Ezreal / `FU-2dca1ad450` 已进入 4C-49 ordinary play-unit guard；Sett / `FU-6308c2db01` 继续等待 `LEGEND_ACTION_DOMAIN` shared-oracle design batch。

仍缺：actual opponent-unit stun / cannot-move static、spell duel lifecycle、movement/control-zone、cleanup/replacement/duration、PaymentEngine、FAQ adjudication、1009/811 full-official 覆盖、正式 18-step E2E。

## 55B. Stage 4C-49 Ezreal Combat-Damage Text Play-Unit Guard Overlay

4C-49 只更新覆盖矩阵 / 风险证据，不升级 full-official。正确身份为 `FU-2dca1ad450` / representative `SFD·082/221` / cardId `33162` / Ezreal / 《伊泽瑞尔》，snapshot entries 为 `SFD·082/221` / `33162`、`SFD·082a/221` / `33163` 与 `SFD·082b/221·P` / `33164`，oracle/effectId 为 `SFD_082_EZREAL_COMBAT_DAMAGE_MOVE_PLAY_UNIT` / `SFD_082A_EZREAL_COMBAT_DAMAGE_MOVE_PLAY_UNIT` / `SFD_082B_EZREAL_COMBAT_DAMAGE_MOVE_PLAY_UNIT`。

4C-49 已部分降低的 blocker：

- ordinary play-unit guard baseline for the Ezreal combat-damage text card family。
- invalid input rejected without mutation or leak。
- 4B primary status / flags 不变：`IMPLEMENTED_TESTED`，`IMPLEMENTED_TESTED`、`SHARED_ORACLE_IMPLEMENTATION`、`NEEDS_ENGINE_SUPPORT`、`NEEDS_FAQ_REVIEW`；`fullOfficial=false`，FAQ refs remain open，full-official upgrades = 0。
- actual combat damage trigger/runtime and move-after-combat-damage behavior remain holdback。

矩阵数字口径：`stage4C49` verified FUs = 1，verified snapshot entries = 3，cumulative combat-damage text play-unit guard verified FUs = 1，full-official upgrades = 0，full-official still uncovered FUs = 811。
...
## 55C. Stage 4C-50 Draven Keyword-Unit Guard Overlay

4C-50 只更新覆盖矩阵 / 风险证据，不升级 full-official。正确身份为 `FU-104211dbbc` / representative `SFD·148/221` / cardId `33240` / Draven / 《德莱文》，snapshot entries 为 `SFD·148/221` / `33240` 与 `SFD·148a/221` / `33241`，oracle/effectId 为 `SFD_DRAVEN_PLAY_KEYWORD_UNIT` / `SFD_DRAVEN_ALT_A_PLAY_KEYWORD_UNIT`。

4C-50 已部分降低的 blocker：

- ordinary play-unit + `法盾` tag guard baseline for the Draven keyword-unit combat text family。
- invalid input rejected without mutation or leak。
- 4B primary status / flags 不变：`IMPLEMENTED_TESTED`，`IMPLEMENTED_TESTED`、`SHARED_ORACLE_IMPLEMENTATION`、`NEEDS_ENGINE_SUPPORT`、`NEEDS_FAQ_REVIEW`；`fullOfficial=false`，FAQ refs remain open，full-official upgrades = 0。
- battle win score、destroyed-in-battle score、Spellshield target tax and full battle scoring remain holdback。

矩阵数字口径：`stage4C50` verified FUs = 1，verified snapshot entries = 2，full-official upgrades = 0，full-official still uncovered FUs = 811。
...
后续批量顺序建议：Rek'Sai attack reveal / movement `FU-422b450261` 已进入 4C-51 ordinary play-unit guard；Rek'Sai play-unit no-optional keyword `FU-1945f6918c` 已进入 4C-52 ordinary no-optional play-unit + keyword tag guard；Sett / `FU-6308c2db01` 继续等待 `LEGEND_ACTION_DOMAIN` shared-oracle design batch。
...
后续批量顺序建议：Rek'Sai play-unit no-optional keyword `FU-1945f6918c` 已进入 4C-52 ordinary no-optional play-unit + keyword tag guard；Sett / `FU-6308c2db01` 继续等待 `LEGEND_ACTION_DOMAIN` shared-oracle design batch。
...
## 55E. Stage 4C-52 Rek'Sai Haste/Overwhelm Keyword Guard Overlay

4C-52 只更新覆盖矩阵 / 风险证据，不升级 full-official。正确身份为 `FU-1945f6918c` / representative `SFD·029/221` / cardId `33104` / Rek'Sai / 《雷克塞》，snapshot entries 为 `SFD·029/221` / `33104` 与 `SFD·029a/221` / `33105`，oracle/effectId 为 `REKSAI_PLAY_UNIT_NO_OPTIONAL_HASTE_OVERWHELM` / `REKSAI_ALT_A_PLAY_UNIT_NO_OPTIONAL_HASTE_OVERWHELM`。

4C-52 已部分降低的 blocker：

- ordinary no-optional play-unit + keyword tag guard baseline for the Rek'Sai haste/overwhelm keyword family。
- invalid input rejected without mutation or leak。
- 4B primary status / flags 不变：`IMPLEMENTED_TESTED`，`IMPLEMENTED_TESTED`、`SHARED_ORACLE_IMPLEMENTATION`、`NEEDS_ENGINE_SUPPORT`、`NEEDS_FAQ_REVIEW`；`fullOfficial=false`，FAQ refs remain open，full-official upgrades = 0。
...
矩阵数字口径：`stage4C52` verified FUs = 1，verified snapshot entries = 2，full-official upgrades = 0，full-official still uncovered FUs = 811。

验证记录：focused 305/305 passed；backend full 3641/3641 passed；frontend build passed；Chrome smoke passed。

本批不关闭 `HASTE_READY` paid branch full matrix、red resource exactness、Overwhelm / 强攻 battle modifier、`ASSIGN_COMBAT_DAMAGE` overflow、non-hand friendly unit gains haste、LayerEngine、hidden-info、FAQ adjudication for Rek'Sai refs、all Rek'Sai official text、1009/811 full-official 或正式 18-step E2E。

后续批量顺序建议：Sett / `FU-6308c2db01` 继续等待 `LEGEND_ACTION_DOMAIN` shared-oracle design batch。
...
|---:|---|---|---:|---|---|---|---|
| 1 | `FU-fb79eea7fc` | `OGN·077/298` 中娅沙漏 | 装备 / 1 | 代表路径：ZHONYAS_HOURGLASS_PLAY_EQUIPMENT；FRIENDLY_UNIT_DESTROYED_DESTROY_SOURCE_RECALL_EXHAUSTED | SOUL-JFAQ-260114 p15<br>SOUL-JFAQ-260114 p2<br>SOUL-JFAQ-260114 p23<br>SOUL-JFAQ-260114 p9<br>SOUL-OFAQ-260114 p8 | 清理/替换/持续时间、控制权/区域移动、FAQ 提及、隐藏信息/随机/牌堆、费用/支付、目标/结算链/时机 | CleanupQueue/ReplacementEffects, FEPR/Targeting/TimingWindows, PaymentEngine/PAY_COST, VisibilityFilter/RandomAndHiddenZones, ZoneOwnership/ControlChange/Movement |
...
- 4C-52 只验证 Rek'Sai no-optional haste / overwhelm keyword ordinary play-unit + keyword tag guard 的代表切片；`HASTE_READY` paid branch full matrix、red resource exactness、Overwhelm / 强攻 battle modifier、`ASSIGN_COMBAT_DAMAGE` overflow、non-hand friendly unit gains haste、LayerEngine、hidden-info、FAQ adjudication 与全 Rek'Sai card-family matrix 仍未完成。
- 正式 18 步 E2E 未最终收口。
