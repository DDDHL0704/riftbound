...
- `UnitLastBreathDrawIfAloneTriggerDoesNotUseCardNumberAllowList` verifies that `CoreRuleEngine` no longer contains `SadPoroOriginalCardNo`, `SadPoroUnleashedCardNo`, or `IsSadPoroCardNo`, and that `MatchRecovery` no longer contains the corresponding Sad Poro recovery source-card constants.
...
- `UnitLastBreathDrawIfNotAloneTriggerDoesNotUseCardNumberAllowList` verifies that `CoreRuleEngine` no longer contains `LoyalPoroCardNo` or `LoyalPoroLastBreathDrawEffectKind`, and that `MatchRecovery` no longer contains `LoyalPoroCardNoForRecovery`.
...
- `UnitLastBreathCallRuneTriggerDoesNotUseCardNumberAllowList` verifies that `CoreRuleEngine` no longer contains `ScoutingWarhawkCardNo` or `ScoutingWarhawkLastBreathCallRuneEffectKind`, and that `MatchRecovery` no longer contains `ScoutingWarhawkCardNoForRecovery`.
...
- `UnitLastBreathCreateDormantGoldTriggerDoesNotUseCoreCardNumberBehavior` verifies that `CoreRuleEngine` no longer contains the old Honest Broker Core card-number / effect-kind branch, and that `MatchRecovery` no longer contains `HonestBrokerCardNoForRecovery`.
- `UnitLastBreathDiscardDrawTriggerDoesNotUseCoreCardNumberBehavior` verifies that `CoreRuleEngine` no longer contains the old Undercover Agent Core card-number / effect-kind branch, and that `MatchRecovery` no longer contains `UndercoverAgentCardNoForRecovery`.
- `UnitLastBreathPowerfulDrawTriggerDoesNotUseCoreCardNumberBehavior` verifies that `CoreRuleEngine` no longer contains the old Unsung Hero Core card-number / effect-kind branch, and that `MatchRecovery` no longer contains `UnsungHeroCardNoForRecovery`.
- `UnitLastBreathCreateBaseUnitTriggerDoesNotUseCardNumberAllowList` verifies that `CoreRuleEngine` no longer contains the old Mechanical Trickster, Ironclad Vanguard, or Muddy Dredger Core card-number/effect-kind constants or card-specific resolver names; it also verifies that `MatchRecovery` no longer contains their recovery source-card constants or `GetStandardLastBreathSourceCardNosForRecovery`.
- `UnitDestroyedTriggerRoutingUsesGenericSpecPredicates` verifies that `UnitDestroyedTriggerSpecRules` no longer exposes public per-effect selectors and that runtime/recovery routing uses `TryGetTrigger(cardNo, predicate, out trigger)` plus shared public shape predicates.
...
- `MatchRecovery` source-card validation for `SAVAGE_JAWFISH_FRIENDLY_DESTROYED_EXPERIENCE_1` now calls `UnitDestroyedTriggerSpecRules.TryGetTrigger(..., IsFriendlyDestroyedGainExperienceTrigger, ...)` instead of comparing against `SavageJawfishCardNoForRecovery`.
...
- `MatchRecovery` source-card validation for `GHOSTLY_CENTAUR_FRIENDLY_DESTROYED_POWER_2` now calls `UnitDestroyedTriggerSpecRules.TryGetTrigger(..., IsFriendlyDestroyedPowerUntilEndTrigger, ...)` instead of comparing against `GhostlyCentaurCardNoForRecovery`.
- `RecoveryValidatorRejectsSnapshotTimingTriggerQueueFriendlyDestroyedSourceCardContextDrift`, `RecoveryValidatorRejectsAuthoritativeStateTriggerQueueFriendlyDestroyedSourceCardContextDrift`, `RecoveryValidatorRejectsSpectatorReplayTimingTriggerQueueFriendlyDestroyedSourceCardContextDriftWithoutCountMismatch`, and `RecoveryValidatorRejectsSpectatorReplayTimingTriggerQueueFriendlyDestroyedSourceCardContextDriftWithCountMismatch` now expect a BehaviorSpec trigger-shape mismatch for Ghostly Centaur instead of a fixed `UNL-068/219` card-number mismatch.
...
- `MatchRecovery` source-card validation for `RESONANT_SOUL_FIRST_FRIENDLY_DESTROYED_DRAW_1` now calls `UnitDestroyedTriggerSpecRules.TryGetTrigger(..., IsFirstFriendlyDestroyedDrawTrigger, ...)` instead of comparing against `ResonantSoulCardNoForRecovery`.
...
- `MatchRecovery` source-card validation for `VIKTOR_DESTROYED_NON_MINION_CREATE_MINION` now calls `UnitDestroyedTriggerSpecRules.TryGetTrigger(..., IsDestroyedNonMinionCreateMinionTrigger, ...)` instead of comparing against the old Viktor destroyed-non-minion recovery source-card allow-list; the shared friendly-destroyed recovery source-card allow-list helper is now removed.
...
- `MatchRecovery` source-card validation for `WATCHFUL_SENTINEL_LAST_BREATH_DRAW_1` now calls `UnitDestroyedTriggerSpecRules.TryGetTrigger(..., IsLastBreathDrawOneTrigger, ...)` instead of comparing against `WatchfulSentinelCardNoForRecovery`.
...
- `MatchRecovery` source-card validation for `SAD_PORO_LAST_BREATH_DRAW_1`, `LOYAL_PORO_LAST_BREATH_DRAW_1`, `UNSUNG_HERO_LAST_BREATH_POWERFUL_DRAW_2`, `SCOUTING_WARHAWK_LAST_BREATH_CALL_RUNE_1`, `HONEST_BROKER_LAST_BREATH_CREATE_GOLD`, and `UNDERCOVER_AGENT_LAST_BREATH_PLAY_UNIT` now calls the corresponding `UnitDestroyedTriggerSpecRules.TryGetTrigger(..., IsLastBreath...Trigger, ...)` predicate instead of comparing against recovery card numbers.
- `MatchRecovery` source-card validation for `MECHANICAL_TRICKSTER_LAST_BREATH_CREATE_MINIONS`, `IRONCLAD_VANGUARD_LAST_BREATH_CREATE_ROBOTS`, and `MUDDY_DREDGER_LAST_BREATH_CREATE_WARHAWK` now calls `UnitDestroyedTriggerSpecRules.TryGetTrigger(sourceCardNo, trigger => IsLastBreathCreateBaseUnitTrigger(trigger) && trigger.Kind == expectedEffectKind, out _)`, preserving effect-kind specificity while removing the old standard last-breath source-card allow-list helper.
...
- 2026-06-30 standard last-breath recovery follow-up focused guards `UnitLastBreathDrawIfAloneTriggerDoesNotUseCardNumberAllowList|UnitLastBreathDrawIfNotAloneTriggerDoesNotUseCardNumberAllowList|UnitLastBreathCallRuneTriggerDoesNotUseCardNumberAllowList|UnitLastBreathCreateDormantGoldTriggerDoesNotUseCoreCardNumberBehavior|UnitLastBreathDiscardDrawTriggerDoesNotUseCoreCardNumberBehavior|UnitLastBreathPowerfulDrawTriggerDoesNotUseCoreCardNumberBehavior|UnitLastBreathCreateBaseUnitTriggerDoesNotUseCardNumberAllowList`: `7/7` passing.
- 2026-06-30 standard last-breath recovery follow-up adjacent / hidden-info gate `LastBreath|StateBasedCleanup|CardCatalogBaselineTests|MatchRecovery|TriggerSourceIdentityGuard`: `2392/2392` passing.
- 2026-06-30 standard last-breath recovery follow-up backend full conformance: `9049/9049` passing.
- 2026-07-02 selector-surface cleanup focused red/green guard `UnitDestroyedTriggerRoutingUsesGenericSpecPredicates`: red `1/1` failing before implementation, then green `1/1` passing.
- 2026-07-02 selector-surface cleanup focused `CardCatalogBaselineTests`: `375/375` passing.
- 2026-07-02 selector-surface cleanup adjacent / hidden-info gate `LastBreath|FriendlyDestroyed|ViktorDestroyedNonMinion|StateBasedCleanup|MatchRecovery|TriggerSourceIdentityGuard|CardCatalogBaselineTests`: `2451/2451` passing.
- 2026-07-02 selector-surface cleanup backend full conformance: `9142/9142` passing.
