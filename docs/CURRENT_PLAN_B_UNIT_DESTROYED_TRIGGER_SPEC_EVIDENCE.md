...
- `MatchRecovery` source-card validation for `GHOSTLY_CENTAUR_FRIENDLY_DESTROYED_POWER_2` now calls `UnitDestroyedTriggerSpecRules.TryGetFriendlyDestroyedPowerUntilEndTrigger(...)` instead of comparing against `GhostlyCentaurCardNoForRecovery`.
...
- `MatchRecovery` source-card validation for `RESONANT_SOUL_FIRST_FRIENDLY_DESTROYED_DRAW_1` now calls `UnitDestroyedTriggerSpecRules.TryGetFirstFriendlyDestroyedDrawTrigger(...)` instead of comparing against `ResonantSoulCardNoForRecovery`.
...
- `MatchRecovery` source-card validation for `SAD_PORO_LAST_BREATH_DRAW_1`, `LOYAL_PORO_LAST_BREATH_DRAW_1`, `UNSUNG_HERO_LAST_BREATH_POWERFUL_DRAW_2`, `SCOUTING_WARHAWK_LAST_BREATH_CALL_RUNE_1`, `HONEST_BROKER_LAST_BREATH_CREATE_GOLD`, and `UNDERCOVER_AGENT_LAST_BREATH_PLAY_UNIT` now calls the corresponding `UnitDestroyedTriggerSpecRules.TryGetLastBreath...` selector instead of comparing against recovery card numbers.
- `MatchRecovery` source-card validation for `MECHANICAL_TRICKSTER_LAST_BREATH_CREATE_MINIONS`, `IRONCLAD_VANGUARD_LAST_BREATH_CREATE_ROBOTS`, and `MUDDY_DREDGER_LAST_BREATH_CREATE_WARHAWK` now calls exact `UnitDestroyedTriggerSpecRules.TryGetTrigger(sourceCardNo, expectedEffectKind, out _)`, preserving effect-kind specificity while removing the old standard last-breath source-card allow-list helper.
