...
4C-49 只追加 Ezreal combat-damage text ordinary play-unit guard representative overlay；4C-50 只追加 Draven keyword-unit combat text ordinary play-unit + `法盾` tag guard representative overlay；4C-51 只追加 Rek'Sai attack reveal / movement text ordinary play-unit guard representative overlay；4C-52 只追加 Rek'Sai no-optional haste / overwhelm keyword ordinary play-unit + keyword tag guard representative overlay。这些 overlay 均不改变 4B primary status、statusFlags、1009/811 frozen counts 或 full-official 口径。
...
## 40. Post-Freeze 4C-32 Overlay

4C-32 不改变 4B frozen counts、primary status counts 或 full-official 口径，只记录局部 friendly battlefield unit ready + movement target guard runtime 证据：

| 项 | 4C-32 记录 |
|---|---|
| verified FU | `FU-6f84196631` / `OGN·173/298` Ride the Wind / 《驭风而行》 |
| cardId | `31403` |
| oracle/effectId | `RIDE_THE_WIND_MOVE_FRIENDLY_BATTLEFIELD_UNIT_TO_BASE_READY` |
| route | Ride the Wind played -> valid friendly public battlefield unit target selected -> service-authoritative target guard -> target readied and moved to owner base |
| guard | enemy battlefield unit、friendly base unit、stale object、face-down standby、friendly battlefield equipment、friendly battlefield spell object、friendly battlefield rune object no mutation / no leak |
| automated evidence | focused 11/11 passed；adjacent guard 32/32 passed；backend full 3479/3479 passed；frontend build passed；Chrome smoke passed。 |
| FAQ boundary | No FAQ refs in matrix；4C-32 does not close complete FEPR target selection, movement/roam/precise battlefield semantics, swift / reaction timing, visibility, all movement cards, or FAQ adjudication。 |
| status impact | 4B `freezeStatus` / `statusFlags` unchanged；`fullOfficial=false`；full-official upgrades = 0。 |
| non-covered FUs | Hostile Takeover / `FU-00ee09c2cc`、Berserk Impulse / `FU-b05eda44ce`、Edge of Night / `FU-804412488c` remain high-risk later candidates；Karthus / `FU-ee1dfb3ed3` and Aphelios / `FU-67c6b0186e` remain design-gated。 |
...
4C-32 不关闭 all movement cards、full Ride the Wind official completion beyond this representative guard slice、complete FEPR target selection / target-change matrix、complete ZoneOwnership / ControlChange / Movement matrix、hidden / face-down / standby target visibility model、swift / reaction timing、FAQ adjudication 或 1009/811 full-official。

## 41. Post-Freeze 4C-33 Overlay

4C-33 不改变 4B frozen counts、primary status counts 或 full-official 口径，只记录局部 enemy battlefield unit movement target guard runtime 证据：

| 项 | 4C-33 记录 |
|---|---|
| verified FU | `FU-1586b6cdd9` / `OGN·043/298` Charm / 《魅惑妖术》 |
| cardId | `31255` |
| oracle/effectId | `CHARM_MOVE_ENEMY_BATTLEFIELD_UNIT_TO_BASE` |
| route | Charm played -> valid enemy public battlefield unit target selected -> service-authoritative target guard -> move target to owner base |
| guard | friendly battlefield unit、enemy base unit、stale object、face-down standby、enemy battlefield equipment、enemy battlefield spell object、enemy battlefield rune object no mutation / no leak |
| automated evidence | focused 35/35 passed；adjacent guard 40/40 passed；backend full 3487/3487 passed；frontend build passed；Chrome smoke passed。 |
| FAQ boundary | No FAQ refs in matrix；4C-33 does not close complete FEPR target selection, movement/roam/precise battlefield semantics, visibility, all movement cards, or FAQ adjudication。 |
| status impact | 4B `freezeStatus` / `statusFlags` unchanged；`fullOfficial=false`；full-official upgrades = 0。 |
| non-covered FUs | Hostile Takeover / `FU-00ee09c2cc`、Berserk Impulse / `FU-b05eda44ce`、Edge of Night / `FU-804412488c` remain high-risk later candidates；Karthus / `FU-ee1dfb3ed3` and Aphelios / `FU-67c6b0186e` remain design-gated。 |
...
4C-33 不关闭 all movement cards、full Charm official completion beyond this representative guard slice、complete FEPR target selection / target-change matrix、complete ZoneOwnership / ControlChange / Movement matrix、hidden / face-down / standby target visibility model、FAQ adjudication 或 1009/811 full-official。

## 42. Post-Freeze 4C-34 Overlay

4C-34 不改变 4B frozen counts、primary status counts 或 full-official 口径，只记录局部 enemy battlefield unit movement no-draw target guard runtime 证据：

| 项 | 4C-34 记录 |
|---|---|
| verified FU | `FU-175d573ae4` / `UNL-124/219` Isolate / 《隔绝》 |
| cardId | `34667` |
| oracle/effectId | `ISOLATE_MOVE_ENEMY_BATTLEFIELD_UNIT_TO_BASE_NO_DRAW` |
| route | Isolate played -> valid enemy public battlefield unit target selected -> service-authoritative target guard -> move target to owner base -> no card draw |
| guard | friendly battlefield unit、enemy base unit、stale object、face-down standby、enemy battlefield equipment、enemy battlefield spell object、enemy battlefield rune object no mutation / no draw / no leak |
| automated evidence | focused 46/46 passed；adjacent guard 48/48 passed；backend full 3495/3495 passed；frontend build passed；Chrome smoke passed。 |
| FAQ boundary | No FAQ refs in matrix；4C-34 does not close Isolate isolated-enemy draw branch, complete FEPR target selection, movement/roam/precise battlefield semantics, visibility, all movement cards, or FAQ adjudication。 |
| status impact | 4B `freezeStatus` / `statusFlags` unchanged；`fullOfficial=false`；full-official upgrades = 0。 |
| non-covered FUs | Vengeance / `FU-07104fa58a` remains next low-coupling destroy-target candidate；Hostile Takeover / `FU-00ee09c2cc`、Berserk Impulse / `FU-b05eda44ce`、Edge of Night / `FU-804412488c` remain high-risk later candidates；Karthus / `FU-ee1dfb3ed3` and Aphelios / `FU-67c6b0186e` remain design-gated。 |
...
## 44. Post-Freeze 4C-36 Overlay

4C-36 不改变 4B frozen counts、primary status counts 或 full-official 口径，只记录局部 control-ready target guard runtime 证据：

| 项 | 4C-36 记录 |
|---|---|
| verified FU | `FU-00ee09c2cc` / `SFD·202/221` Hostile Takeover / 《恶意收购》 |
| cardId | `33301` |
| oracle/effectId | `HOSTILE_TAKEOVER_GAIN_CONTROL_READY_ENEMY_BATTLEFIELD_UNIT` |
| route | Hostile Takeover played -> valid enemy public battlefield unit target selected -> service-authoritative target guard -> gain control -> ready target |
| guard | friendly battlefield unit、enemy base unit、stale object、face-down standby、enemy battlefield equipment、enemy battlefield spell object、enemy battlefield rune object no mutation / no control change / no leak |
| automated evidence | focused 265/265 passed；adjacent guard 157/157 passed；backend full 3515/3515 passed；frontend build passed；Chrome smoke passed。 |
| FAQ boundary | `SOUL-JFAQ-260114 p22` / `SOUL-OFAQ-260114 p21` remain open；4C-36 does not close complete control-change, ready / exhaust semantics, FEPR target selection, payment / battle / hidden-zone interactions, all gain-control cards, or FAQ adjudication。 |
| status impact | 4B `freezeStatus` / `statusFlags` unchanged；`fullOfficial=false`；full-official upgrades = 0。 |
| non-covered FUs | Berserk Impulse / `FU-b05eda44ce` and Edge of Night / `FU-804412488c` remain high-risk later candidates；Karthus / `FU-ee1dfb3ed3` and Aphelios / `FU-67c6b0186e` remain design-gated。 |
...
4C-36 不关闭 all gain-control cards、full Hostile Takeover official completion beyond this representative guard slice、complete FEPR target selection / target-change matrix、complete control-change matrix、ready / exhaust semantics、payment / battle / hidden-zone interactions、FAQ adjudication 或 1009/811 full-official。

## 45. Post-Freeze 4C-37 Overlay

4C-37 不改变 4B frozen counts、primary status counts 或 full-official 口径，只记录局部 opponent top main-deck unit target guard runtime 证据：

| 项 | 4C-37 记录 |
|---|---|
| verified FU | `FU-b05eda44ce` / `OGN·025/298` Berserk Impulse / 《暴怒冲动》 |
| cardId | `31231` |
| oracle/effectId | `BERSERK_IMPULSE_PLAY_OPPONENT_TOP_UNIT` |
| route | Berserk Impulse played -> service-authoritative opponent top main-deck object guard -> valid opponent top unit route -> representative play-opponent-top-unit slice |
| guard | opponent top non-unit、non-top object、friendly deck object、private / unauthorized object、face-down / hidden invalid object、dirty resolution / stale top object no play / no mutation / no leak |
| automated evidence | focused 17/17 passed；backend full 3529/3529 passed; frontend build passed; Chrome smoke passed。 |
| FAQ boundary | `SOUL-OFAQ-260114 p4` remains open；4C-37 does not close full reveal / choice / recycle semantics, hidden-zone visibility matrix, FEPR timing / target selection, payment / battle / control-zone interactions, all opponent-deck play effects, or FAQ adjudication。 |
...
| non-covered FUs | Edge of Night / `FU-804412488c` remains high-risk later candidate；Karthus / `FU-ee1dfb3ed3` and Aphelios / `FU-67c6b0186e` remain design-gated。 |
...
## 47. Post-Freeze 4C-39 Overlay

4C-39 不改变 4B frozen counts、primary status counts 或 full-official 口径，只记录局部 Zhonya play-equipment guard runtime 证据：

| 项 | 4C-39 记录 |
|---|---|
| verified FU | `FU-fb79eea7fc` / `OGN·077/298` Zhonya's Hourglass / 《中娅沙漏》 |
| cardId | `31291` |
| oracle/effectId | `ZHONYAS_HOURGLASS_PLAY_EQUIPMENT` |
| route | Ultra-narrow representative play-equipment guard baseline for Zhonya's Hourglass |
| guard | invalid source / target / cost rejected without mutation or leak |
| automated evidence | focused 268/268 passed；backend full 3552/3552 passed；frontend build passed；Chrome smoke passed。 |
| FAQ boundary | `SOUL-JFAQ-260114 p15` / `SOUL-JFAQ-260114 p2` / `SOUL-JFAQ-260114 p23` / `SOUL-JFAQ-260114 p9` / `SOUL-OFAQ-260114 p8` remain open；4C-39 does not close full standby / reaction semantics, destroy replacement, recall interactions, equipment layer / continuous-effect matrix, or FAQ adjudication。 |
| status impact | 4B `freezeStatus` / `statusFlags` unchanged；`NEEDS_FAQ_REVIEW` remains primary status；`IMPLEMENTED_UNTESTED` / `NEEDS_ENGINE_SUPPORT` / `NEEDS_FAQ_REVIEW` remain flags；`fullOfficial=false`；full-official upgrades = 0。 |
| non-covered FUs | Karthus / `FU-ee1dfb3ed3` and Aphelios / `FU-67c6b0186e` remain design-gated。 |
...
4C-39 不关闭 all equipment effects、full Zhonya official completion beyond this representative guard slice、full standby / reaction semantics、destroy replacement matrix、recall interactions、complete equipment layer / continuous-effect matrix、FAQ adjudication 或 1009/811 full-official。

## 48. Post-Freeze 4C-40 Overlay

4C-40 不改变 4B frozen counts、primary status counts 或 full-official 口径，只记录局部 Sea Monster Hook play-equipment guard runtime 证据：

| 项 | 4C-40 记录 |
|---|---|
| verified FU | `FU-2653af0380` / `OGN·242/298` Sea Monster Hook / 《海兽钓钩》 |
| cardId | `31482` |
| oracle/effectId | `SEA_MONSTER_HOOK_PLAY_EQUIPMENT` |
| route | Ultra-narrow representative play-equipment guard baseline for Sea Monster Hook |
| guard | invalid source / target / cost rejected without mutation or leak |
| automated evidence | focused 272/272 passed；backend full 3558/3558 passed；frontend build passed；Chrome smoke passed。 |
| FAQ boundary | `BREAK-JFAQ-260416 p9` / `JFAQ-251023 p2` / `JFAQ-251023 p3` / `SOUL-JFAQ-260114 p22` remain open；4C-40 does not close activated ability, top-five search / reveal / choice, free play, recycle, equipment layer / continuous-effect matrix, or FAQ adjudication。 |
| status impact | 4B `freezeStatus` / `statusFlags` unchanged；`NEEDS_FAQ_REVIEW` remains primary status；`IMPLEMENTED_UNTESTED` / `NEEDS_ENGINE_SUPPORT` / `NEEDS_FAQ_REVIEW` remain flags；`fullOfficial=false`；full-official upgrades = 0。 |
| non-covered FUs | Karthus / `FU-ee1dfb3ed3` and Aphelios / `FU-67c6b0186e` remain design-gated。 |
...
4C-40 不关闭 all equipment effects、full Sea Monster Hook official completion beyond this representative guard slice、activated ability、top-five search / reveal / choice、free play、recycle、complete equipment layer / continuous-effect matrix、FAQ adjudication 或 1009/811 full-official。

## 49. Post-Freeze 4C-41 Overlay

4C-41 不改变 4B frozen counts、primary status counts 或 full-official 口径，只记录局部 Giant Arm Kato play-keyword-unit guard runtime 证据：

| 项 | 4C-41 记录 |
|---|---|
| verified FU | `FU-464ec8c275` / `SFD·112/221` Giant Arm Kato / 《巨腕加藤》 |
| cardId | `33198` |
| oracle/effectId | `GIANT_ARM_KATO_PLAY_KEYWORD_UNIT` |
| route | Ultra-narrow representative play-keyword-unit guard baseline for Giant Arm Kato |
| guard | invalid source / target / cost rejected without mutation or leak |
| automated evidence | focused 99/99 passed；backend full 3564/3564 passed；frontend build passed；Chrome smoke passed。 |
| FAQ boundary | `SOUL-JFAQ-260114 p12` / `SOUL-JFAQ-260114 p3` / `SOUL-OFAQ-260114 p12` remain open；4C-41 does not close Spellshield target tax, move trigger, keyword grant, +power until EOT, LayerEngine / continuous-effect matrix, or FAQ adjudication。 |
| status impact | 4B `freezeStatus` / `statusFlags` unchanged；`NEEDS_FAQ_REVIEW` remains primary status；`IMPLEMENTED_UNTESTED` / `NEEDS_ENGINE_SUPPORT` / `NEEDS_FAQ_REVIEW` remain flags；`fullOfficial=false`；full-official upgrades = 0。 |
| non-covered FUs | Karthus / `FU-ee1dfb3ed3` and Aphelios / `FU-67c6b0186e` remain design-gated。 |
...
4C-41 不关闭 all keyword-unit effects、full Giant Arm Kato official completion beyond this representative guard slice、Spellshield target tax、move trigger、keyword grant、+power until EOT、complete LayerEngine / continuous-effect matrix、FAQ adjudication 或 1009/811 full-official。

## 50. Post-Freeze 4C-42 Overlay

4C-42 不改变 4B frozen counts、primary status counts 或 full-official 口径，只记录局部 Time Gate play-equipment guard runtime 证据：

| 项 | 4C-42 记录 |
|---|---|
| verified FU | `FU-081d97eb3e` / `SFD·078/221` Time Gate / 《预时之门》 |
| cardId | `33158` |
| oracle/effectId | `TIME_GATE_PLAY_EQUIPMENT` |
| route | Ultra-narrow representative play-equipment guard baseline for Time Gate |
| guard | invalid source / target / cost rejected without mutation or leak |
| automated evidence | focused 292/292 passed；backend full 3570/3570 passed；frontend build passed；Chrome smoke passed。 |
| FAQ boundary | `BREAK-JFAQ-260416 p11` / `SOUL-JFAQ-260114 p15` / `SOUL-JFAQ-260114 p19` / `SOUL-JFAQ-260114 p25` / `SOUL-JFAQ-260114 p6` / `SOUL-OFAQ-260114 p21` remain open；4C-42 does not close activated/tap ability, next spell Echo, optional echo payment/repeat, duration cleanup, or FAQ adjudication。 |
| status impact | 4B `freezeStatus` / `statusFlags` unchanged；`NEEDS_FAQ_REVIEW` remains primary status；`IMPLEMENTED_UNTESTED` / `NEEDS_ENGINE_SUPPORT` / `NEEDS_FAQ_REVIEW` remain flags；`fullOfficial=false`；full-official upgrades = 0。 |
| non-covered FUs | Karthus / `FU-ee1dfb3ed3` and Aphelios / `FU-67c6b0186e` remain design-gated。 |
...
4C-42 不关闭 all equipment effects、full Time Gate official completion beyond this representative guard slice、activated/tap ability、next spell Echo、optional echo payment/repeat、duration cleanup、FAQ adjudication 或 1009/811 full-official。

## 51. Post-Freeze 4C-43 Overlay

4C-43 不改变 4B frozen counts、primary status counts 或 full-official 口径，只记录局部 Sfur Song play-equipment guard runtime 证据：

| 项 | 4C-43 记录 |
|---|---|
| verified FU | `FU-9a623b3185` / `SFD·059/221` Sfur Song / 《斯弗尔尚歌》 |
| cardId | `33139` |
| oracle/effectId | `SFUR_SONG_PLAY_EQUIPMENT` |
| route | Ultra-narrow representative play-equipment guard baseline for Sfur Song |
| guard | invalid source / target / cost rejected without mutation or leak |
| automated evidence | focused 268/268 passed；backend full 3576/3576 passed；frontend build passed；Chrome smoke passed。 |
| FAQ boundary | `SOUL-JFAQ-260114 p24` / `SOUL-JFAQ-260114 p25` / `SOUL-JFAQ-260114 p8` / `SOUL-OFAQ-260114 p18` / `SOUL-OFAQ-260114 p19` remain open；4C-43 does not close copied host skill text, continuous text/layer, complete assemble/equipment attach lifecycle, equipment control/zone movement, or FAQ full behavior。 |
| status impact | 4B `freezeStatus` / `statusFlags` unchanged；`NEEDS_FAQ_REVIEW` remains primary status；`IMPLEMENTED_UNTESTED` / `NEEDS_ENGINE_SUPPORT` / `NEEDS_FAQ_REVIEW` remain flags；`fullOfficial=false`；full-official upgrades = 0。 |
| non-covered FUs | Akshan / `FU-7419ee7d9d` and Switcheroo / `FU-0b6332bbf0` remain follow-up candidates；Void Burrower / `FU-6e7d0dba2c` and Sett / `FU-6308c2db01` remain legend-domain/shared-oracle candidates。 |
...
4C-43 不关闭 all equipment effects、full Sfur Song official completion beyond this representative guard slice、copied host skill text、continuous text / LayerEngine、complete assemble / equipment attach lifecycle、equipment control / zone movement、FAQ full behavior 或 1009/811 full-official。

## 52. Post-Freeze 4C-44 Overlay

4C-44 不改变 4B frozen counts、primary status counts 或 full-official 口径，只记录局部 Akshan play-unit guard runtime 证据：

| 项 | 4C-44 记录 |
|---|---|
| verified FU | `FU-7419ee7d9d` / `SFD·109/221` Akshan / 《阿克尚》 |
| cardId | `33194` |
| oracle/effectId | `AKSHAN_NO_OPTIONAL_ASSEMBLE_NO_EXTRA_PLAY_UNIT` |
| route | Ultra-narrow representative play-unit guard baseline for Akshan |
| guard | invalid source / target / cost rejected without mutation or leak |
| automated evidence | focused 189/189 passed；backend full 3582/3582 passed；frontend build passed；Chrome smoke passed。 |
| FAQ boundary | `SOUL-JFAQ-260114 p18` / `SOUL-JFAQ-260114 p23` remain open；4C-44 does not close optional assemble, extra play, cleanup / replacement / duration, movement / control-zone, payment / cost windows, targeting / stack timing, LayerEngine / continuous effects, or FAQ adjudication。 |
| status impact | 4B `freezeStatus` / `statusFlags` unchanged；`NEEDS_FAQ_REVIEW` remains primary status；`IMPLEMENTED_UNTESTED` / `NEEDS_ENGINE_SUPPORT` / `NEEDS_FAQ_REVIEW` remain flags；`fullOfficial=false`；full-official upgrades = 0。 |
| non-covered FUs | Switcheroo / `FU-0b6332bbf0` remains follow-up candidate；Void Burrower / `FU-6e7d0dba2c` and Sett / `FU-6308c2db01` remain legend-domain/shared-oracle candidates。 |
...
4C-44 不关闭 all Akshan official text、optional assemble、extra play semantics、cleanup / replacement / duration matrix、movement / control-zone matrix、payment / cost windows、targeting / stack timing、LayerEngine / continuous effects、FAQ adjudication 或 1009/811 full-official。

## 53. Post-Freeze 4C-45 Overlay

4C-45 不改变 4B frozen counts、primary status counts 或 full-official 口径，只记录局部 Switcheroo battlefield power-swap guard runtime 证据：

| 项 | 4C-45 记录 |
|---|---|
| verified FU | `FU-0b6332bbf0` / `SFD·145/221` Switcheroo / 《换换乐》 |
| cardId | `33237` |
| oracle/effectId | `SWITCHEROO_SWAP_TWO_BATTLEFIELD_UNIT_POWERS` |
| route | Ultra-narrow representative battlefield power-swap guard baseline for Switcheroo |
| guard | invalid source / target / timing rejected without mutation or leak |
| automated evidence | focused 284/284 passed；backend full 3594/3594 passed；frontend build passed；Chrome smoke passed。 |
| FAQ boundary | `SOUL-JFAQ-260114 p14` remains open；4C-45 does not close full battle math, LayerEngine / continuous effect semantics, cleanup / replacement / duration, hidden / random-zone behavior, payment / timing, or FAQ adjudication。 |
| status impact | 4B `freezeStatus` / `statusFlags` unchanged；`IMPLEMENTED_TESTED` remains primary status；`IMPLEMENTED_TESTED` / `NEEDS_ENGINE_SUPPORT` / `NEEDS_FAQ_REVIEW` remain flags；`fullOfficial=false`；full-official upgrades = 0。 |
| non-covered FUs | Void Burrower / `FU-6e7d0dba2c` and Sett / `FU-6308c2db01` remain legend-domain/shared-oracle candidates。 |
...
4C-45 不关闭 all Switcheroo official text、full battle math、LayerEngine / continuous effect semantics、cleanup / replacement / duration matrix、hidden / random-zone behavior、payment / timing matrix、FAQ adjudication 或 1009/811 full-official。

## 54. Post-Freeze 4C-46 Overlay

4C-46 不改变 4B frozen counts、primary status counts 或 full-official 口径，只记录局部 Void Burrower legend-domain shared-oracle design gate：

| 项 | 4C-46 记录 |
|---|---|
| design-gated FU | `FU-6e7d0dba2c` / `SFD·187/221` Void Burrower / 《虚空遁地兽》 |
| cardIds | `SFD·187/221` / `33285`；`SFD·243/221` / `33354` |
| oracle/effectId | `LEGEND_ACTION_DOMAIN` |
| route | NO-GO direct implementation；legend-domain shared-oracle design gate only |
...
| automated evidence | no verified implementation evidence claimed；focused/backend/build/smoke counters remain outside this overlay。 |
| FAQ boundary | `SOUL-JFAQ-260114 p14` / `SOUL-OFAQ-260114 p4` remain open；4C-46 does not close FAQ adjudication。 |
| status impact | 4B `freezeStatus` / `statusFlags` unchanged；`SHARED_ORACLE_IMPLEMENTATION` remains primary status；`IMPLEMENTED_UNTESTED` / `SHARED_ORACLE_IMPLEMENTATION` / `NEEDS_ENGINE_SUPPORT` / `NEEDS_FAQ_REVIEW` remain flags；`fullOfficial=false`；full-official upgrades = 0。 |
| follow-on candidate | Sett / `FU-6308c2db01` / representative `OGN·269/298` / cardId `31512` remains a 3-entry follow-on design candidate and is not tagged by 4C-46。 |
...
4C-46 不关闭 direct implementation、verified runtime slice、full legend action domain、battle / spell duel lifecycle、movement / control-zone matrix、hidden / reveal / redaction matrix、LayerEngine / continuous effects、targeting / stack timing、FAQ adjudication 或 1009/811 full-official。

## 55. Post-Freeze 4C-47 Overlay

4C-47 不改变 4B frozen counts、primary status counts 或 full-official 口径，只记录局部 Draven battle body / play-unit guard runtime 证据：

| 项 | 4C-47 记录 |
|---|---|
| verified FU | `FU-964b214448` / `SFD·020/221` Draven / 《德莱文》 |
| cardIds | `SFD·020/221` / `33092`；`SFD·020a/221` / `33093` |
| oracle/effectId | `SFD_020_DRAVEN_VANILLA_PLAY_UNIT` / `SFD_020A_DRAVEN_VANILLA_PLAY_UNIT` |
| route | Ultra-narrow representative battle body / play-unit guard baseline for Draven shared-oracle FU |
| guard | invalid source / target / timing rejected without mutation or leak |
| automated evidence | focused 14/14 passed；backend full pending A final verification；frontend build / Chrome smoke pending A final verification。 |
| FAQ boundary | `BREAK-JFAQ-260416 p4` / `SOUL-JFAQ-260114 p25` / `SOUL-JFAQ-260114 p4` / `SOUL-OFAQ-260114 p16` / `SOUL-OFAQ-260114 p17` remain open；4C-47 does not close battle win Gold, attack/defense red payment, +2 until EOT, PaymentEngine, Layer/duration, or FAQ adjudication。 |
...
| follow-on candidates | Vex / `FU-9f7cb73dc4` moves to 4C-48；`UNL-150a/219` / `FU-4d8ee1696b` remains untagged；Sett / `FU-6308c2db01` remains legend-domain/shared-oracle design candidate。 |
...
4C-47 不关闭 all Draven official text、battle win Gold、attack / defense red payment、+2 until EOT、PaymentEngine、Layer / duration、battle lifecycle full matrix、FAQ adjudication 或 1009/811 full-official。

## 56. Post-Freeze 4C-48 Overlay

4C-48 不改变 4B frozen counts、primary status counts 或 full-official 口径，只记录局部 Vex spellshield opponent-unit stun guard 证据：

| 项 | 4C-48 记录 |
|---|---|
| verified FU | `FU-9f7cb73dc4` / `UNL-150/219` Vex / 《薇古丝》 |
| cardIds | `UNL-150/219` / `34697` |
| oracle/effectId | `VEX_SPELLSHIELD_OPPONENT_UNIT_STUN_STATIC` |
| route | Test-only representative `PLAY_CARD` 0-target + `法盾` / `约德尔人` tag guard baseline |
| guard | invalid input rejected without mutation or leak |
| automated evidence | focused 35/35 passed；backend full 3607/3607 passed；frontend build passed；Chrome smoke passed。 |
| FAQ boundary | `BREAK-JFAQ-260416 p5` / `BREAK-JFAQ-260416 p9` / `SOUL-JFAQ-260114 p17` remain open；4C-48 does not close actual opponent-unit stun / cannot-move static, spell duel lifecycle, movement/control-zone, PaymentEngine, or FAQ adjudication。 |
| status impact | 4B `freezeStatus` / `statusFlags` unchanged；`IMPLEMENTED_TESTED` remains primary status；`IMPLEMENTED_TESTED` / `NEEDS_ENGINE_SUPPORT` / `NEEDS_FAQ_REVIEW` remain flags；`fullOfficial=false`；full-official upgrades = 0。 |
| not tagged | `UNL-150a/219` / cardId `34698` / `FU-4d8ee1696b` remains separate and untagged by 4C-48。 |
...
4C-48 不关闭 all Vex variants、actual opponent-unit stun / cannot-move static、spell duel lifecycle、movement/control-zone full matrix、cleanup / replacement / duration、PaymentEngine、FAQ adjudication 或 1009/811 full-official。

## 57. Post-Freeze 4C-49 Overlay

4C-49 不改变 4B frozen counts、primary status counts 或 full-official 口径，只记录局部 Ezreal combat-damage text play-unit guard 证据：

| 项 | 4C-49 记录 |
|---|---|
| verified FU | `FU-2dca1ad450` / `SFD·082/221` Ezreal / 《伊泽瑞尔》 |
| cardIds | `SFD·082/221` / `33162`；`SFD·082a/221` / `33163`；`SFD·082b/221·P` / `33164` |
| oracle/effectId | `SFD_082_EZREAL_COMBAT_DAMAGE_MOVE_PLAY_UNIT` / `SFD_082A_EZREAL_COMBAT_DAMAGE_MOVE_PLAY_UNIT` / `SFD_082B_EZREAL_COMBAT_DAMAGE_MOVE_PLAY_UNIT` |
| route | Representative ordinary hand play-unit guard baseline for the Ezreal combat-damage text card family |
| guard | invalid target、wrong zone-source、opponent source、face-down standby source、insufficient mana rejected without mutation or leak |
...
4C-49 不关闭 all Ezreal official text、actual combat damage trigger/runtime、move-after-combat-damage behavior、battle lifecycle full matrix、movement/control-zone full matrix、LayerEngine / continuous effects、PaymentEngine、targeting / stack timing、FAQ adjudication 或 1009/811 full-official。

## 58. Post-Freeze 4C-50 Overlay

4C-50 不改变 4B frozen counts、primary status counts 或 full-official 口径，只记录局部 Draven keyword-unit combat text play-unit guard 证据：

| 项 | 4C-50 记录 |
|---|---|
| verified FU | `FU-104211dbbc` / `SFD·148/221` Draven / 《德莱文》 |
| cardIds | `SFD·148/221` / `33240`；`SFD·148a/221` / `33241` |
| oracle/effectId | `SFD_DRAVEN_PLAY_KEYWORD_UNIT` / `SFD_DRAVEN_ALT_A_PLAY_KEYWORD_UNIT` |
| route | Representative ordinary hand play-unit + `法盾` tag guard baseline for the Draven keyword-unit combat text family |
| guard | invalid input rejected without mutation or leak |
...
| status impact | 4B `freezeStatus` / `statusFlags` unchanged；`IMPLEMENTED_TESTED` remains primary status；`IMPLEMENTED_TESTED` / `SHARED_ORACLE_IMPLEMENTATION` / `NEEDS_ENGINE_SUPPORT` / `NEEDS_FAQ_REVIEW` remain flags；`fullOfficial=false`；full-official upgrades = 0。 |
| follow-on candidates | Rek'Sai attack reveal / movement / `FU-422b450261` entered 4C-51 ordinary play-unit guard；Rek'Sai play-unit no-optional keyword / `FU-1945f6918c` entered 4C-52 ordinary no-optional play-unit + keyword tag guard；Sett / `FU-6308c2db01` remains a high-pressure follow-on candidate。 |
...
| status impact | 4B `freezeStatus` / `statusFlags` unchanged；`IMPLEMENTED_TESTED` remains primary status；`IMPLEMENTED_TESTED` / `SHARED_ORACLE_IMPLEMENTATION` / `NEEDS_ENGINE_SUPPORT` / `NEEDS_FAQ_REVIEW` remain flags；`fullOfficial=false`；full-official upgrades = 0。 |
| follow-on candidates | Rek'Sai play-unit no-optional keyword / `FU-1945f6918c` entered 4C-52 ordinary no-optional play-unit + keyword tag guard；Sett / `FU-6308c2db01` remains a high-pressure follow-on candidate。 |
...
## 60. Post-Freeze 4C-52 Overlay

4C-52 不改变 4B frozen counts、primary status counts 或 full-official 口径，只记录局部 Rek'Sai no-optional haste / overwhelm keyword play-unit guard 证据：

| 项 | 4C-52 记录 |
|---|---|
| verified FU | `FU-1945f6918c` / `SFD·029/221` Rek'Sai / 《雷克塞》 |
| cardIds | `SFD·029/221` / `33104`；`SFD·029a/221` / `33105` |
| oracle/effectId | `REKSAI_PLAY_UNIT_NO_OPTIONAL_HASTE_OVERWHELM` / `REKSAI_ALT_A_PLAY_UNIT_NO_OPTIONAL_HASTE_OVERWHELM` |
| route | Representative ordinary no-optional hand play-unit + keyword tag guard baseline for the Rek'Sai haste/overwhelm keyword family |
| guard | invalid input rejected without mutation or leak |
| automated evidence | focused 305/305 passed；backend full 3641/3641 passed；frontend build passed；Chrome smoke passed。 |
| FAQ boundary | `BREAK-JFAQ-260416 p3` / `SOUL-JFAQ-260114 p19` / `SOUL-OFAQ-260114 p4` remain open；4C-52 does not close `HASTE_READY` paid branch full matrix、red resource exactness、Overwhelm / 强攻 battle modifier、`ASSIGN_COMBAT_DAMAGE` overflow、non-hand friendly unit gains haste、LayerEngine、hidden-info 或 FAQ adjudication。 |
| status impact | 4B `freezeStatus` / `statusFlags` unchanged；`IMPLEMENTED_TESTED` remains primary status；`IMPLEMENTED_TESTED` / `SHARED_ORACLE_IMPLEMENTATION` / `NEEDS_ENGINE_SUPPORT` / `NEEDS_FAQ_REVIEW` remain flags；`fullOfficial=false`；full-official upgrades = 0。 |
| follow-on candidates | Sett / `FU-6308c2db01` remains a high-pressure `LEGEND_ACTION_DOMAIN` shared-oracle design candidate。 |
...
## 62. Post-Freeze 4C-54 Overlay

4C-54 不改变 4B frozen counts、primary status counts 或 full-official 口径，只记录局部 Void Burrower `LEGEND_ACTION_DOMAIN` representative automated evidence：

| 项 | 4C-54 记录 |
|---|---|
| verified FU | `FU-6e7d0dba2c` / `SFD·187/221` Void Burrower / 《虚空遁地兽》 |
| cardIds | `SFD·187/221` / `33285`；`SFD·243/221` / `33354` |
| oracle/effectId | `LEGEND_ACTION_DOMAIN` |
| route | Representative legend-domain evidence for conquer trigger -> reveal top two -> play one unit when available -> recycle rest -> exhaust / make legend dormant |
| guard | no top unit route recycles both revealed cards；inactive / exhausted legend no trigger |
| automated evidence | focused 32/32 passed；backend full 3650/3650 passed；frontend build passed；Chrome smoke passed。 |
| FAQ boundary | `SOUL-JFAQ-260114 p14` / `SOUL-OFAQ-260114 p4` remain open；4C-54 does not close optional trigger offering / decline / replay semantics、`RevealChoice` full design、hidden / reveal / redaction matrix、shared-oracle official mapping、BattleEngine / SpellDuel lifecycle、`ASSIGN_COMBAT_DAMAGE`、FEPR targeting / timing、LayerEngine、control-zone movement 或 FAQ/full behavior。 |
| status impact | 4B `freezeStatus` / `statusFlags` unchanged；`SHARED_ORACLE_IMPLEMENTATION` remains primary status；`IMPLEMENTED_UNTESTED` / `SHARED_ORACLE_IMPLEMENTATION` / `NEEDS_ENGINE_SUPPORT` / `NEEDS_FAQ_REVIEW` remain flags；`fullOfficial=false`；full-official upgrades = 0。 |
| follow-on candidates | Zhonya / `FU-fb79eea7fc` and Sea Monster Hook / `FU-2653af0380` full behavior remain high-pressure candidates outside prior play-equipment guard overlays。 |
