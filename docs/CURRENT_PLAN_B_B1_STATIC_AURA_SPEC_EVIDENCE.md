...
- `StaticAuraSpecRules.IsBattlefieldKeywordStaticAura` remains the shared predicate for battlefield RULE_TEXT keyword auras; `BattlefieldKeywordStaticAuraExecutionRoutesThroughBehaviorSpecScope` now also blocks reintroducing the removed battlefield keyword helpers in `StaticAuraSpecRules`.
- `StaticAuraSpecRules.IsFriendlySingleDefendingUnitPowerStaticAura` is now the shared predicate for `STATIC_AURA` POWER auras whose target and participant scopes are `FRIENDLY_SINGLE_DEFENDING_BATTLEFIELD_UNIT` / `SINGLE_FRIENDLY_DEFENDING_BATTLEFIELD_UNIT`.
- `CoreRuleEngine.ResolveFriendlySingleDefendingUnitPowerBonus` now enumerates `StaticAuraSpecRules.GetStaticAuras(sourceState.CardNo)` and filters through that predicate instead of filtering by `StaticAuraKinds.FriendlySingleDefendingUnitPower`.
- Existing recovery-sensitive metadata remains compatible: `RULE_TEXT:BATTLEFIELD_ALL_UNITS_KEYWORD`, `RULE_TEXT:BATTLEFIELD_FILTERED_UNITS_KEYWORD`, and `FRIENDLY_SINGLE_DEFENDING_UNIT_POWER`.
- `BattlefieldStaticAuraSpecRoutingGuardTests.FriendlySingleDefendingPowerStaticAuraExecutionRoutesThroughBehaviorSpecScope` blocks reintroducing the removed friendly single-defender helper or kind-filtered Core routing.
- Validation passed: baseline backend full conformance 9076/9076; red/green focused guards 2/2; focused battlefield keyword / friendly single-defender / Master Yi / MatchRecovery regression 2022/2022; StaticAura / StaticPower / StaticKeyword / Battlefield / FriendlySingle / MasterYi / ContinuousEffect / FullGameEndToEnd / MatchRecovery adjacent 2875/2875; backend full conformance 9077/9077.
- Non-closure: this does not finish complete LayerEngine timestamp/order behavior, complete B1/B2 breadth, full P0, or READY.
...
- `StaticAuraSpecRules.IsSameBattlefieldOtherFriendlyFilteredUnitsPowerStaticAura` is now the shared predicate for same-battlefield other-friendly filtered POWER static auras whose target and participant scopes are `SAME_BATTLEFIELD_OTHER_FRIENDLY_FILTERED_UNITS` / `SAME_BATTLEFIELD_OTHER_FRIENDLY_FILTERED_PUBLIC_UNITS`.
- `MatchRecovery` source-card spec consistency validation now enumerates `StaticAuraSpecRules.GetStaticAuras(sourceCardNo)` and filters through those predicates instead of calling `StaticAuraSpecRules.TryGetSameBattlefieldOtherFriendlyUnitsPowerAura` / `TryGetSameBattlefieldOtherFriendlyFilteredUnitsPowerAura`.
- Existing recovery-sensitive metadata remains compatible: `SAME_BATTLEFIELD_OTHER_FRIENDLY_UNITS_POWER_PLUS_ONE` and `SAME_BATTLEFIELD_OTHER_FRIENDLY_FILTERED_UNITS_POWER`.
- `StaticAuraSpecRules.TryGetSameBattlefieldOtherFriendlyUnitsPowerAura` and `TryGetSameBattlefieldOtherFriendlyFilteredUnitsPowerAura` have been removed from the engine helper surface.
- `BattlefieldStaticAuraSpecRoutingGuardTests.SameBattlefieldOtherFriendlyPowerStaticAuraRecoveryRoutesThroughBehaviorSpecScope` blocks reintroducing the removed helpers in MatchRecovery or StaticAuraSpecRules.
- Validation passed: baseline backend full conformance 9074/9074; red/green focused guard 1/1; focused same-battlefield other-friendly POWER / MatchRecovery regression 2006/2006; StaticAura / StaticPower / SameBattlefield / ContinuousEffect / FullGameEndToEnd / MatchRecovery adjacent 2231/2231; backend full conformance 9075/9075.
- Non-closure: this does not finish other static-aura recovery helper families, complete LayerEngine timestamp/order behavior, complete B1/B2 breadth, full P0, or READY.
...
- `tests/Riftbound.ConformanceTests/SourceObjectLevelPowerStaticAuraTests.cs` now covers `UNL-075/219` Windrunner Fox's level-gated source-object `RULE_TEXT` keyword aura: at 3 experience the source projects `RULE_TEXT:SOURCE_OBJECT_LEVEL_KEYWORD:*:游走`, below 3 experience it projects no level-keyword effect, and a Windrunner Fox object without printed `游走` can pay precise `ROAM` movement from the parsed `BehaviorSpec.StaticAuras` route without materializing `游走` into object tags.
- `tests/Riftbound.ConformanceTests/CardCatalogBaselineTests.cs` now asserts level-gated source-object keyword parsing for `UNL-047/219` Moss Stepper (`法盾` at level 3), `UNL-075/219` Windrunner Fox (`游走` at level 3), and `UNL-113/219` / `UNL-113a/219` Yi (`法盾` and `游走` at level 6), all as `SOURCE_OBJECT_LEVEL_KEYWORD` with `RequiredPlayerExperience`.
...
- Latest source-object level RULE_TEXT keyword focused representative: 8/8 passed.
- Latest SourceObjectLevelPower / SourceObjectKeyword / StaticAura / Roam / MoveUnit / MatchRecovery adjacent representative: 2197/2197 passed.
- Latest backend full after source-object level RULE_TEXT keyword evidence: 9133/9133 passed.
