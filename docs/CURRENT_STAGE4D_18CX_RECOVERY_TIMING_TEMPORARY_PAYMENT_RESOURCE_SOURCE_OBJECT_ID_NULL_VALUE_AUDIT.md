# Stage 4D-18CX Recovery Timing Temporary Payment Resource Source Object Id Null Value Audit
...
Added `RecoveryValidatorRejectsSpectatorReplayTimingTemporaryPaymentResourceKeyedSourceObjectIdNullValueWithCountMismatch` in `tests/Riftbound.ConformanceTests/MatchRecoveryTests.cs`.

The test builds an authoritative temporary payment resource from real `MatchState` source-object state, keeps the emitted spectator payload keyed to `temp-payment-resource-1`, changes `sourceObjectId` to `null`, then appends `temp-payment-resource-extra` to force resource-count mismatch.
...
`sourceObjectId` is an optional payload field, so the null-value case intentionally does not assert a required or invalid scalar diagnostic. The keyed authoritative comparison still rejects the null value when the authoritative resource has `source-1`.

## Validation

- Focused new test: `1/1`.
- Focused `TemporaryPaymentResource` filter: `97/97`.
- Focused recovery filter: `1130/1130`.
- Adjacent recovery/official-opening/Postgres recovery-store filter: `1711/1711`.
- Backend full: `7076/7076`.
