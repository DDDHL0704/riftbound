# Stage 4D-08V Stale Target Response Next-Contest Prompt Queue Evidence
...
- `docs/CURRENT_STAGE4D_08V_STALE_TARGET_RESPONSE_NEXT_CONTEST_PROMPT_QUEUE_AUDIT.md`
- `docs/CURRENT_STAGE4D_08V_STALE_TARGET_RESPONSE_NEXT_CONTEST_PROMPT_QUEUE_EVIDENCE.md`

## Validation

```bash
/Users/dinghaolin/.dotnet/dotnet test Riftbound.slnx --no-restore --filter "FullyQualifiedName~NaturalBattleResponseActivationNoEffectForStaleTargetReturnsToResponseBeforeAdvancement"
```
...
Runtime changed: no. Existing stale-target no-effect battle-response close to next-contest behavior already advanced BF-NEXT into spell-duel focus after response priority closed.
