# Stage 4D-08Q Natural No Result Idle Prompt Queue Evidence
...
- `docs/CURRENT_STAGE4D_08Q_NATURAL_NO_RESULT_IDLE_PROMPT_QUEUE_AUDIT.md`
- `docs/CURRENT_STAGE4D_08Q_NATURAL_NO_RESULT_IDLE_PROMPT_QUEUE_EVIDENCE.md`

## Validation

```bash
/Users/dinghaolin/.dotnet/dotnet test Riftbound.slnx --no-restore --filter "FullyQualifiedName~NaturalAssignCombatDamageEmitsNoResultWhenAllParticipantsDestroyed"
```
...
Runtime changed: no. Existing no-result battle-close behavior already cleared the pending queue and returned to ordinary neutral action.
