# Stage 4D-08J Declare Battle Accepted Replay Prompt Queue Evidence
...
- `docs/CURRENT_STAGE4D_08J_DECLARE_BATTLE_ACCEPTED_REPLAY_PROMPT_QUEUE_AUDIT.md`
- `docs/CURRENT_STAGE4D_08J_DECLARE_BATTLE_ACCEPTED_REPLAY_PROMPT_QUEUE_EVIDENCE.md`

## Validation

```bash
/Users/dinghaolin/.dotnet/dotnet test Riftbound.slnx --no-restore --filter "FullyQualifiedName~ActiveStartBattleDeclareBattleClearsTaskAndPreservesRepresentativeEvents|FullyQualifiedName~ActiveStartBattleDeclareBattleRejectsAcceptedCommandReplayWithoutMutation"
```
...
Runtime changed: no. Existing accepted `DECLARE_BATTLE` close and replay rejection paths already cleared the BF-1 battle task and prompt scope.
