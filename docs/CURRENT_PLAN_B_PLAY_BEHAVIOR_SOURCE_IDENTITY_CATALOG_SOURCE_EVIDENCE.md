...
- `CoreRuleEngine` and `MatchSession` now resolve source-next-spell marker amounts through `CardBehaviorRegistry.TryGetSourceNextSpellCostReductionByEffectKind` instead of a hard-coded `RagingDrakeNextSpellCostReductionMana`.
- `CoreRuleEngine` now validates the Poro Herder boon/draw play-behavior source branch through `SourceBoonConditionKind=CONTROLLER_CONTROLS_FACE_UP_UNIT_WITH_TAG`, `SourceBoonRequiredControlledUnitTag=魄罗`, and `SourceBoonDrawCount=1`.
...
- The previous `RagingDrakeNextSpellCostSourceEffectKind` Core selector and hard-coded Raging Drake next-spell cost-reduction amount were deleted from Core / MatchSession prompt cost calculation.
- The previous `PoroHerderBoonDrawSourceEffectKind` Core selector and `ControllerControlsFaceUpPoroUnit` helper were deleted.
...
- `RagingDrakeNextSpellCostPlaySourceUsesBehaviorFields` blocks reintroducing `RagingDrakeCardNo`, `RagingDrakeNextSpellCostSourceEffectKind`, `RAGING_DRAKE_NEXT_SPELL_COST_PLAY_UNIT`, direct `behavior.CardNo` comparisons, or hard-coded Raging Drake next-spell reduction amounts in `CoreRuleEngine.cs` / `MatchSession.cs`, and requires the source-next-spell behavior fields.
- `PoroHerderBoonDrawPlaySourceUsesBehaviorFields` blocks reintroducing `PoroHerderCardNo`, `PoroHerderBoonDrawSourceEffectKind`, `PORO_HERDER_NO_PORO_STATIC_PLAY_UNIT`, or direct `behavior.CardNo` comparisons in `CoreRuleEngine.cs`, and requires the source-boon behavior fields.
...
```sh
/Users/dinghaolin/.dotnet/dotnet test tests/Riftbound.ConformanceTests/Riftbound.ConformanceTests.csproj --no-restore --nologo --filter "FullyQualifiedName~RagingDrakeNextSpellCostPlaySourceUsesBehaviorFields|FullyQualifiedName~RagingDrakeSourceNextSpellCostReductionCarriesOfficialAmount|FullyQualifiedName~P79RagingDrake|FullyQualifiedName~LuxPaidCostHighPrintedSpellReducedBelowThresholdDoesNotTriggerUnitOrLegend"
```

Result after implementation: 7/7 passed; adjacent / hidden-info gate `PlayBehaviorSourceIdentityGuardTests|RagingDrake|LuxHighCost|CardCatalogBaselineTests|PaymentEngineCoverageAuditTests|MatchRecovery` passed 3035/3035; backend full conformance passed 9028/9028.
...
```sh
/Users/dinghaolin/.dotnet/dotnet test tests/Riftbound.ConformanceTests/Riftbound.ConformanceTests.csproj --no-restore --nologo --filter "FullyQualifiedName~PoroHerderBoonDrawPlaySourceUsesBehaviorFields|FullyQualifiedName~PoroHerderSourceBoonDrawCarriesOfficialControlledPoroCondition|FullyQualifiedName~P79PoroHerderGrantsBoonAndDrawsWhenControllerHasPoro"
```

Result after implementation: 3/3 passed; adjacent / hidden-info gate `PlayBehaviorSourceIdentityGuardTests|PoroHerder|CardCatalogBaselineTests|PaymentEngineCoverageAuditTests|MatchRecovery` passed 3026/3026; backend full conformance passed 9027/9027.
