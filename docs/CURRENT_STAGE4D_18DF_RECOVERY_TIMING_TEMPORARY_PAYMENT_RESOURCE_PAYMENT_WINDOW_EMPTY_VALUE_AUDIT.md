# Stage 4D-18DF Recovery Timing Temporary Payment Resource Payment Window Empty Value Audit
...
Added `RecoveryValidatorRejectsSpectatorReplayTimingTemporaryPaymentResourceKeyedPaymentWindowEmptyValueWithCountMismatch` in `tests/Riftbound.ConformanceTests/MatchRecoveryTests.cs`.

The test builds an authoritative temporary payment resource from real `MatchState` source-object state, keeps the emitted spectator payload keyed to `temp-payment-resource-1`, changes the emitted `paymentWindow` to `string.Empty`, then appends `temp-payment-resource-extra` to force resource-count mismatch.

It proves validation still emits:

- keyed authoritative payment-window mismatch diagnostics for `temp-payment-resource-1`;
- unknown extra-resource diagnostics;
- resource-count mismatch diagnostics.

`paymentWindow` is an optional payload field. Empty values are tolerated by optional scalar payload parsing, so this case intentionally does not assert a payload-layer required or invalid diagnostic. The keyed authoritative comparison still rejects the empty value when the authoritative resource has `PAY_COST`.

## Validation

- Focused new test: `1/1`.
- Focused `TemporaryPaymentResource` filter: `105/105`.
- Focused recovery filter: `1138/1138`.
- Adjacent recovery/official-opening/Postgres recovery-store filter: `1719/1719`.
- Backend full: `7084/7084`.
