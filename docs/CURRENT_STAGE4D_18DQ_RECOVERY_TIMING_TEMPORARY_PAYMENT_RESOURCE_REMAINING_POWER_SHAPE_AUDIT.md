# Stage 4D-18DQ Recovery Timing Temporary Payment Resource Remaining Power Shape Audit
...
Added `RecoveryValidatorRejectsSpectatorReplayTimingTemporaryPaymentResourceKeyedRemainingPowerShapeWithCountMismatch` in `tests/Riftbound.ConformanceTests/MatchRecoveryTests.cs`.

The test builds an authoritative temporary payment resource from real `MatchState` source-object state, keeps the emitted spectator payload keyed to `temp-payment-resource-1`, changes the emitted `remainingPower` from `1` to an unreadable array payload, then appends `temp-payment-resource-extra` to force resource-count mismatch.

It proves validation still emits:

- remaining-power invalid diagnostics;
...
This closes the focused remaining-power shape variant outside broad required-field shape tests, keeping remaining-power payload-shape validation and keyed authoritative validation independently locked for spectator timing temporary-payment-resource payloads.

## Validation

- Focused new test: `1/1`.
- Focused `TemporaryPaymentResource` filter: `116/116`.
- Focused recovery filter: `1149/1149`.
- Adjacent recovery/official-opening/Postgres recovery-store filter: `1730/1730`.
- Backend full: `7095/7095`.
