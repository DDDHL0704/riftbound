# Stage 4D-05T Activate Ability Replay Evidence
...
- `docs/CURRENT_STAGE4D_05T_ACTIVATE_ABILITY_REPLAY_AUDIT.md`
- `docs/CURRENT_STAGE4D_05T_ACTIVATE_ABILITY_REPLAY_EVIDENCE.md`

## Validation

```bash
source scripts/dev-env.sh && dotnet test tests/Riftbound.ConformanceTests/Riftbound.ConformanceTests.csproj --no-restore --filter "FullyQualifiedName~FluftPoroRejectsAcceptedActivationReplayWithoutMutation"
```

Result: passed `1/1`.

```bash
source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore --filter "FullyQualifiedName~FluftPoro|FullyQualifiedName~ActivateAbility|FullyQualifiedName~Stack|FullyQualifiedName~PassPriority|FullyQualifiedName~MatchStateHasher"
```

Result: passed `547/547`.

```bash
source scripts/dev-env.sh && dotnet test Riftbound.slnx --no-restore
```

Result: passed `5355/5355`.
...
Result: passed. No whitespace warnings, conflict markers or trailing whitespace in the new 05J/05K/05L/05M/05N/05O/05P/05Q/05R/05S/05T evidence docs.
