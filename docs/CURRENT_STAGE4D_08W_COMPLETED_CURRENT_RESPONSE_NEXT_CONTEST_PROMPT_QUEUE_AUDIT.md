# Stage 4D-08W Completed Current Response Next-Contest Prompt Queue Audit

Date: 2026-05-23

Owner: `A_MAIN`

Status: accepted test-only server P0-005 completed-current battle-response next-contest prompt / queue audit slice. Project remains **NOT READY**.

## Scope

This slice strengthens `NaturalBattleResponseActivationImmediateBattleSkipsCompletedCurrentBattlefieldBeforeAdvancingNextTask` in `BattleDamageAssignmentLifecycleTests`. It covers the path where a natural battle opens a battle-response priority window while the current battlefield already has a completed spell-duel marker, P2 activates Shadow from the current battlefield, response priority closes, the current battlefield remains skipped for new contest / spell-duel startup, and the follow-up contested battlefield advances into active spell-duel focus.

Runtime behavior was not changed. The existing completed-current battlefield path already skipped the current battlefield and advanced BF-NEXT; this slice makes the final post-close queue, snapshot queue, battlefield-task snapshot and active prompt invariants explicit.
...
- Strengthened `NaturalBattleResponseActivationImmediateBattleSkipsCompletedCurrentBattlefieldBeforeAdvancingNextTask`.
- The audit proves the completed current battlefield marker remains present through response close.
- It proves the Shadow source stays on the current battlefield and the current battlefield does not emit a new contest or spell-duel start.
