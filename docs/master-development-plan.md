...
状态：P7 已完成；P7.9 已完成 P7.9.6 传奇规则域，并在 P7.9.7 进入战场规则域十五个功能单元：`DECLARE_BATTLE` 支持服务端已知战场卡对象目的地，ActionPrompt 下发战场对象候选，`UNL-208/219` 此处瞬息单位获得坚守、`UNL-210/219` 此处孤立防守单位受到坚守 -2、`OGN·275/298` 据守打出随从、`OGN·279/298` 防守此处选择单位获得坚守2、`OGN·280/298` 据守抽牌、`OGN·287/298` 征服回收符文、`SFD·210/221` 征服支付 1 重置传奇、`SFD·212/221` 征服弃置主牌堆顶两张、`SFD·215/221` 防守展示顶牌并抽法术/回收、`SFD·217/221` 征服按其他受控战场抽牌、`SFD·218/221` 征服强力单位支付 1 抽牌、`SFD·219/221` 据守双方召符文、`SFD·220/221` 征服支付 1 打出休眠金币、`OGN·294/298` 此处单位静态战力 +1、`OGN·298/298` 征服弃牌抽牌已由后端结算。当前实现 `772/811` 功能单元，manual deferred 剩余 `39/811`，全部集中在战场。当前短交接和批次计划见 `docs/CURRENT_P7_9_STATUS.md`。
...
4. 战场规则域：
   ...
   - P7.9.7 已迁移 `15/54` 个战场功能单元；继续关闭 P6 中剩余 `39` 个战场 manual deferred 功能单元。
5. 战斗和长尾整合：
...
4. 分批补齐战场规则域，最终关闭当前剩余 `39/811` manual deferred 功能单元。
