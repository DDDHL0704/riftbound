...
状态：P7 已完成；P7.9 正在 P7.9.6，`LEGEND_ACT` 已打通结构化提示、页面操作和三个传奇主动技能小批次，并已补入 Jinx 回合开始自动触发小批次；当前实现 `723/811` 功能单元，manual deferred 剩余 `88/811`。当前短交接和批次计划见 `docs/CURRENT_P7_9_STATUS.md`。
...
3. 传奇规则域：
   ...
   - P7.9.6 已迁移 `10/44` 个传奇功能单元；继续关闭剩余 `34` 个传奇 manual deferred 功能单元。
4. 战场规则域：
...
4. 分批补齐传奇和战场规则域，最终关闭当前剩余 `88/811` manual deferred 功能单元。
