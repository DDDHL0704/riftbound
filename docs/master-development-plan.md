...
- 已完成 schema v2 形状读取、runner 初始状态应用、`MatchState` 基础权威字段、普通回合开始最小规则行为、`END_TURN` 自动推进到下一回合开始的最小闭环，以及回合结束特殊清理中的伤害移除/本回合内效果失效。
- 下一步补清理重复/常规清理最小闭环；连续燃尽/胜利判定后续单独建 fixture。
...
新窗口接手时，先读 `docs/START_HERE.md`，再回到本节确认当前只推进 P1 范围。
