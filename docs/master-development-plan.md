...
状态：P7 已完成；P7.9 已完成 P7.9.6 传奇规则域，并在 P7.9.7 进入战场规则域前三个切片：`DECLARE_BATTLE` 支持服务端已知战场卡对象目的地，ActionPrompt 下发战场对象候选，`OGN·280/298` 据守抽牌、`SFD·212/221` 征服弃置主牌堆顶两张、`OGN·294/298` 此处单位静态战力 +1 已由后端结算。当前实现 `760/811` 功能单元，manual deferred 剩余 `51/811`，全部集中在战场。当前短交接和批次计划见 `docs/CURRENT_P7_9_STATUS.md`。
...
4. 战场规则域：
   ...
   - P7.9.7 已迁移 `3/54` 个战场功能单元；继续关闭 P6 中剩余 `51` 个战场 manual deferred 功能单元。
5. 战斗和长尾整合：
...
4. 分批补齐战场规则域，最终关闭当前剩余 `51/811` manual deferred 功能单元。
