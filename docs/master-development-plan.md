...
状态：P7 已完成；P7.9 已完成 P7.9.6 传奇规则域，并在 P7.9.7 进入战场规则域五十二个功能单元：`DECLARE_BATTLE` 支持服务端已知战场卡对象目的地，ActionPrompt 下发战场对象候选；`SFD·208/221` 魄罗熔炉在控制该战场时授予友方传奇横置贴附受控武装的 `LEGEND_ACT` 能力；`UNL-206/219` 鲜血祭坛在战斗中替代此处单位摧毁，支付 3 后移除伤害、休眠并召回；`UNL-216/219` 皮城学院据守后让本回合下一个法术获得等同基础费用的 Echo；`SFD·209/221` 遗忘丰碑在玩家第三回合前阻止战场来源得分；`UNL-214/219` 鬼影湾在战场单位返回手牌后支付 1 叫一枚休眠符文；`UNL-213/219` 蜕变花园授予战场单位 `ACTIVATE_ABILITY` 横置获得 1 经验；`OGN·289/298` 征服后选择两枚符文并在回合结束时重置，`SFD·207/221` 征服后支付 1、返回己方战场单位并在战场打出黄沙士兵，以及已落地的 `UNL` / `OGN` / `SFD` 多个战场代表效果均已由后端结算。当前实现 `809/811` 功能单元，manual deferred 剩余 `2/811`，全部集中在战场。当前短交接和批次计划见 `docs/CURRENT_P7_9_STATUS.md`。
...
4. 战场规则域：
   ...
   - P7.9.7 已迁移 `52/54` 个战场功能单元；继续关闭 P6 中剩余 `2` 个战场 manual deferred 功能单元。
5. 战斗和长尾整合：
...
4. 分批补齐战场规则域，最终关闭当前剩余 `2/811` manual deferred 功能单元。
