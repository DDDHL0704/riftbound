# Stage 4D-08U Multiple Response Sources Next-Contest Prompt Queue Audit

Date: 2026-05-23

Owner: `A_MAIN`

Status: accepted test-only server P0-005 multiple battle-response sources next-contest prompt / queue audit slice. Project remains **NOT READY**.

## Scope

This slice strengthens `NaturalBattleResponseAllowsMultipleLegalSourcesSequentiallyBeforeAdvancement` in `BattleDamageAssignmentLifecycleTests`. It covers the path where a natural battle opens a battle-response priority window, P2 has two legal Shadow response sources, each source activates and resolves sequentially, the available response-source list is consumed before advancement, both players pass response priority, the battle closes without opening `ASSIGN_COMBAT_DAMAGE`, and the follow-up contested battlefield advances into active spell-duel focus.

Runtime behavior was not changed. The existing multiple-source battle-response path already kept BF-NEXT blocked until both Shadow response opportunities were exhausted; this slice makes the final post-close queue, snapshot queue, battlefield-task snapshot and active prompt invariants explicit.
...
- Strengthened `NaturalBattleResponseAllowsMultipleLegalSourcesSequentiallyBeforeAdvancement`.
- The audit proves the first Shadow activation removes only the exhausted source from the response prompt while leaving the second source legal.
- It proves the second Shadow activation exhausts the remaining source and BF-NEXT still does not advance until response priority is passed.
