# Stage 4D-223U GameHub DeclareBattle After-Finished Error Envelope Audit
...
`DeclareBattleAfterFinishedRedactsSentinelPayloadAndDoesNotBroadcastOrMutate` now proves the after-finished `DECLARE_BATTLE` caller error is a `MessageType.ERROR` envelope, preserves normalized `roomId` / `P1` routing, and carries default protocol/schema versions.
...
Latest core rules 302, 323.1 and 467 anchor the game-ending and win-state context. Latest core rules 323.13 and 417 provide the battle / combat-damage command-surface context. Latest core rules 649-652, especially 651.3, anchor player removal and the rule that a removed player cannot choose or otherwise affect the game after removal.

This is protocol-error-envelope coverage only. It does not change declare-battle legality, battle resolution, combat damage, win/score/surrender/removal behavior, or redaction text.

## Validation

- Focused test: `1/1`.
- Changed class `GameHubJoinTests`: `217/217`.
- Adjacent Hub/after-finished/protocol/error/DeclareBattle filter: `362/362`.
