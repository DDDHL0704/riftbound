...
- 当前推进 P7.9 本地产品版全卡可玩：P7.9.6 已完成七个传奇主动技能小批次（含 Jax 武装贴附/重贴附、Darius 鼓舞资源、Teemo 召回/待命替代与 Azir 武装后打出黄沙士兵）、九个自动触发小批次（含 Jinx/Draven/Garen/Lux/Annie、Volibear/Fiora 强力单位召符文、Rengar 打出单位 S+1、Leona 眩晕后增益与 Sivir 回收符文造金币/敌方单位摧毁重置），以及 Rumble/Lucian/OGS Master Yi/Ahri/UNL Master Yi/Azir 六个静态传奇小批次，传奇规则域现在覆盖 29 个传奇功能单元 / 71 个官方条目；当前实现 `742/811` 个功能单元，manual deferred 剩余 `69/811`，全部集中在传奇/战场。详见 `docs/CURRENT_P7_9_STATUS.md`。
