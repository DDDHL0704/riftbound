...
- 当前推进 P7.9 本地产品版全卡可玩：P7.9.6 已完成全部传奇规则域，P7.9.7 已进入战场规则域并完成五十个战场对象/战斗结果功能单元，最新新增 `UNL-216/219` 皮城学院据守后让本回合下一个法术获得等同基础费用的 Echo；既有 `SFD·209/221` 遗忘丰碑在玩家第三回合前阻止战场来源得分、`UNL-214/219` 鬼影湾单位返回手牌后支付 1 叫符文、`UNL-213/219` 蜕变花园授予战场单位 `ACTIVATE_ABILITY`、`OGN·289/298` 征服后选择两枚符文并在回合结束时重置、`SFD·207/221` 征服后支付 1 返回己方战场单位并打出黄沙士兵、`UNL-215/219` 首次战场单位打出后移动另一名己方单位到基地、`UNL-218/219` 战场单位打出后支付 1 给予增益、`UNL-219/219` 据守后本回合非指示物单位费用 +1、`OGN·296/298` 目标法术/技能伤害 +1、`UNL-211/219` 高费法术洞察回收、`UNL-205/219` 法术强化战场单位、`OGN·292/298` 友方法术指定抽牌、`SFD·213/221` 首个友方装备费用减少 1、`SFD·211/221` 友方回响费用减少 1、`SFD·216/221` 战场单位打出限制，以及 `UNL` / `OGN` / `SFD` 多个战场代表效果均已由后端结算。当前实现 `807/811` 个功能单元，manual deferred 剩余 `4/811`，全部集中在战场。详见 `docs/CURRENT_P7_9_STATUS.md`。
