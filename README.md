...
- 2026-05-06 已追加本地产品 UI polish：单屏三栏对战桌面、中文化主界面、操作标签页、全卡图鉴弹窗和当前代码 Browser smoke。
...
访问 `http://127.0.0.1:5173` 后，使用默认 `server URL = http://127.0.0.1:5088`。P7/P7.9 Web UI 已支持产品级房间入口、双玩家 ready、断线重连、单屏三栏对战桌面、结构化 ActionPrompt 操作、点击式出牌/目标/费用/战斗/传奇/战场能力、中文化事件日志、战报/回放边界、全卡图鉴弹窗和卡牌详情。UI 只显示和转发服务端 `Snapshot`、`Prompt`、`Events`、错误和命令日志，不做规则裁定。
