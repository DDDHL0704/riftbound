...
rg -q '_matchScreen!\.CardActivated \+= HandleMatchCardActivated' "$main_script"
rg -U -q 'HandleMatchCardActivated\([\s\S]*?TrySelectObject\(objectId\)[\s\S]*?ApplyCardPreview\(card\)' "$main_script"
...
rg -q 'ScheduleAutoSmokePrompt\(_lastAppliedPromptView\)' "$main_script"
snapshot_method="$(sed -n '/public void ApplySnapshotSections(/,/^    }/p' "$main_script")"
table_render_line="$(printf '%s\n' "$snapshot_method" | rg -n '_matchScreen\.RenderSections\(sections\)' | head -1 | cut -d: -f1)"
render_gate_line="$(printf '%s\n' "$snapshot_method" | rg -n '_battleTableRendered = true' | head -1 | cut -d: -f1)"
test -n "$table_render_line"
test -n "$render_gate_line"
test "$table_render_line" -lt "$render_gate_line"
