...
STUB
    ...
    cat >"${destination}/clients/godot/tools/check-official-card-table-screenshot.sh" <<'STUB'
#!/usr/bin/env bash
set -euo pipefail
echo "official-card table guard ran: $*"
STUB
    chmod +x "${destination}/clients/godot/tools/check-official-card-table-screenshot.sh"
    cat >"${destination}/clients/godot/tools/check-centered-result-overlay-screenshot.sh" <<'STUB'
#!/usr/bin/env bash
set -euo pipefail
echo "centered result overlay guard ran: $*"
STUB
    chmod +x "${destination}/clients/godot/tools/check-centered-result-overlay-screenshot.sh"
    exit 0
    ;;
...
rg -q "official-card table guard ran: .*player-a-result\\.png.*player-b-result\\.png" "${output}" \
  || fail "clean-main simulated wrapper did not run the official-card table guard on both result screenshots"
rg -q "centered result overlay guard ran: .*player-a-result\\.png.*player-b-result\\.png" "${output}" \
  || fail "clean-main simulated wrapper did not run the centered result overlay guard on both result screenshots"
